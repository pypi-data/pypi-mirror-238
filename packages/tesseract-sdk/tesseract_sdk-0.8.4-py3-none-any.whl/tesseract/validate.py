    ...
    def __init__(
        ...
        if isinstance(test_data, str):
            ...
            try:
                ...
                raise ValueError("test_data must be a valid tesseract job id or a gs:// url")
            ...
            if info["shape"] is not None and len(info["shape"]) > 0:
                if self.test_data is not None:
                    ...
                    arr = zf[info["name"]]["tesseract"][
                        0 : info["shape"][0], :, 0 : info["shape"][2], 0 : info["shape"][3]
                    ]
                    ...
                    end_date = np.datetime64("2020-01-01") + (np.timedelta64(info["shape"][0], "D"))
                ...
                with open(self.data_path / "in" / f"{info['name']}.dat", "wb") as fp:
                    fp.write(arr_bts)
                with open(self.data_path / "in" / f"{info['name']}[grid-x].dat", "wb") as fp:
                    fp.write(x_bts)
                with open(self.data_path / "in" / f"{info['name']}[grid-y].dat", "wb") as fp:
                    fp.write(y_bts)
                with open(self.data_path / "in" / f"{info['name']}[grid-t].dat", "wb") as fp:
                    fp.write(times_bts)
            else:
    ...
    def test_tensor_response(self, resp, expected_asset, result, n_tests) -> int:
        ...
        for output_asset in self.output_asset_bands:
            ...
            try:
                ...
                self.gutenberger.task(f"validation image written out to {img_path}/{img_name}")
            except Exception as e:
                self.gutenberger.test_status(
                    f"({resp.name}) Could not write out validation image, {e}", "warning"
                )
