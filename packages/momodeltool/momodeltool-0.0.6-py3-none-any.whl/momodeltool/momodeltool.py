    ...
    def layer_show(input, module_name='ShowImage', show=False, save=None, norm=False, pseudo=False, hist=False):
        ...
        if show or save:
            if len(input.shape) == 4:
                ...
                write_count = 0
                while pair_factor[1] == input.shape[1] and pair_factor[1] > 10:
                    write_count += 1
                    ...
                    for i in range(pair_factor[1]):
                        ...
                        for mid in range(i * pair_factor[0], pair_factor[0] * (i + 1)):
                            try:
                                ...
                                mid_slice = np.zeros(
                                    mid_slices[0].shape[0], mid_slices[0].shape[1], dtype=np.uint8)
                            if hist:
                ...
                for d in range(input.shape[1]):
                    ...
                    while pair_factor[1] == input.shape[2] and pair_factor[1] > 10:
                        write_count += 1
                        pair_factor = _closest_factors(
