...
from simba.utils.enums import TagNames, ConfigKey, Options, Methods, Dtypes
...
class TrainMultiClassRandomForestClassifier(ConfigReader, TrainModelMixin):
    def __init__(self,
        ...
        self.under_sample_setting = read_config_entry(self.config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.UNDERSAMPLE_SETTING.value, data_type=Dtypes.STR.value, default_value=Dtypes.NONE.value).lower().strip()
        self.under_sample_ratio = ast.literal_eval(read_config_entry(self.config, ConfigKey.CREATE_ENSEMBLE_SETTINGS.value, ConfigKey.UNDERSAMPLE_RATIO.value, data_type=Dtypes.STR.value))
    ...
    def _perform_sampling(self):
        ...
        if self.under_sample_setting != None:
            ...
            if self.under_sample_setting == 'random undersample multiclass bouts':
