...
def get_db_host(host):
    ns = get_env('ENV_QIANQIUYUN_NS', '')
    if host == "mysql" or host == "influxdb":
       if ns != "":
           return host + "." + ns
...
def create_pooleddb_by(datasource):
    # 获取 root:qianqiu@#20222@tcp(mysql:3306)/oadb_cvxa3663
    part = datasource.split('?')[0] 
    items = part.split('@tcp')

    # 从 root:qianqiu@#20222 获取用户名和密码
    i = items[0].index(':')
    dbuser = items[0][0:i]
    dbpwd = items[0][i + 1:]
    # dbpwd = 'qian@123'

    # 从 (mysql:3306)/oadb_cvxa3663 获取端口，host和数据库名
    i = items[1].index('/')
    dbname = items[1][i + 1:]
