...
class S3_Bucket(AWS_Session):
    ...
    def download_file(
        ...
        if self._non_blank_key(key):
            ...
            try:
                ...
                if self._boto3_error_match(ex, ERROR_S3_NOT_FOUND):
                    raise ConfigError(f"{self} does not exist")
                else:
                    raise ConfigError(f"{self} with ID {self.user_id} get info yielded error {ex}")
            except Exception as ex2:
                raise ConfigError(f"{self} get info yielded error {ex2} {repr(ex2)}")
            ...
            if do_download:
                ...
                try:
                    ...
                    if self._boto3_error_match(ex, ERROR_S3_NOT_FOUND):
                        ...
                        raise ConfigError(f"{self} with ID {self.user_id} download yielded error {ex}")
    ...
    def list_objects(self, key: Optional[Union[str, PurePosixPath]] = None, ) -> 'BucketObjectsCollection':
        ...
        try:
            ...
            if self._boto3_error_match(ex, ERROR_S3_NOT_FOUND):
                raise ConfigError(f"{self} with key={key} does not exist")
            else:
                raise ConfigError(f"{self} with key={key} list_objects yielded error {ex}")
        return collection
