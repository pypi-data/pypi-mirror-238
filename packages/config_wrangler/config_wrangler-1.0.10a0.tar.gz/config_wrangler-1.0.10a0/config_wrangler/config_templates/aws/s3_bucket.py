...
class S3_Bucket(AWS_Session):
        ...
        if self._non_blank_key(key):
            ...
            try:
                ...
                if self._boto3_error_match(ex, ERROR_S3_NOT_FOUND):
                    ...
                    raise ValueError(f"{self} get info yielded error {ex}")
            ...
            if do_download:
                ...
                try:
                    ...
                    if self._boto3_error_match(ex, ERROR_S3_NOT_FOUND):
                        ...
                        raise ValueError(f"{self} download yielded error {ex}")
    ...
    def list_objects(self, key: Optional[Union[str, PurePosixPath]] = None, ) -> 'BucketObjectsCollection':
        ...
        try:
            ...
            if self._boto3_error_match(ex, ERROR_S3_NOT_FOUND):
                raise ValueError(f"{self} with key={key} does not exist")
            else:
                raise ValueError(f"{self} with key={key} list_objects yielded error {ex}")
        return collection
