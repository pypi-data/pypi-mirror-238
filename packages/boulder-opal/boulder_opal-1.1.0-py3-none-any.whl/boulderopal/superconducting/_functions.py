...
def _create_qho_operators(
    graph: Graph, transmons: list[Transmon], cavities: list[Cavity]
) -> dict[str, _QHOOps]:
    """
...
def _convert_to_pwc(
    ...
    raise TypeError(
        f"{name} has an invalid type.", {f"type({name})": type(coefficient)}
    )
...
def simulate(
    ...
    """
    gate_duration = ScalarDType.REAL("gate_duration").gt(0)(gate_duration)
    sample_count = ScalarDType.INT("sample_count").gt(0)(sample_count)
    cutoff_frequency = nullable(ScalarDType.REAL("cutoff_frequency"), cutoff_frequency)

    system_dimension = np.prod(
        [system.dimension for system in transmons + cavities], dtype=int
    )
    initial_state = nullable(
        ArrayDType.COMPLEX("initial_state").ndim(1).shape((system_dimension,)),
        initial_state,
    )
...
def optimize(
    ...
    **optimization_kwargs : dict
    ...
    initial_state = nullable(
        ArrayDType.COMPLEX("initial_state").ndim(1).shape((dim,)), initial_state
    )
    target_state = nullable(
        ArrayDType.COMPLEX("target_state").ndim(1).shape((dim,)), target_state
    )
    target_operation = nullable(
        ArrayDType.COMPLEX("target_operation").ndim(2).shape((dim, dim)),
        target_operation,
    )
