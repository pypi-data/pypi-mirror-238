...
def _is_strict_complex(val: Any) -> bool:
    return isinstance(val, ScalarDType.COMPLEX.value.types)
...
class _ScalarValidator:
    def __init__(self, type_: "ScalarDType", name: str):
        self._type = type_
        self._name = name
        self._validator_pipe: list[Callable[[_SCALAR], None]] = []

    def _compare(
        self, operator: _Compare, rhs: float | int, name: str, value: _SCALAR
    ) -> None:
        Checker.VALUE(
    ...
    def lt(self, bound: float | int, rhs_name: str = "") -> "_ScalarValidator":
        """
        Scalar < bound.
        """
        self._validator_pipe.append(
            partial(self._compare, _Compare.LESS, bound, rhs_name)
        )
        return self

    def le(self, bound: float | int, rhs_name: str = "") -> "_ScalarValidator":
        """
        Scalar <= bound.
        """
        self._validator_pipe.append(
            partial(self._compare, _Compare.LESS_EQUAL, bound, rhs_name)
        )
        return self

    def gt(self, bound: float | int, rhs_name: str = "") -> "_ScalarValidator":
        """
        Scalar > bound.
        """
        self._validator_pipe.append(
            partial(self._compare, _Compare.GREATER, bound, rhs_name)
        )
        return self

    def ge(self, bound: float | int, rhs_name: str = "") -> "_ScalarValidator":
        """
        Scalar >= bound.
        """
        self._validator_pipe.append(
            partial(self._compare, _Compare.GREATER_EQUAL, bound, rhs_name)
        )
        return self

    def __call__(self, value: _SCALAR) -> _SCALAR:
        Checker.TYPE(
...
class _ArrayValidator:
    def __init__(self, type_: "ArrayDType", name: str):
        self._type = type_
        self._name = name
        self._validator_pipe: list[
            Callable[[np.ndarray], tuple[bool | np.bool_, str]]
        ] = []

    def ndim(self, dim_: int) -> "_ArrayValidator":
        """
    ...
    def shape(self, shape_: tuple, extra: str = "") -> "_ArrayValidator":
        """
    ...
    def _compare(
        self, operator: _Compare, rhs: float | int, name: str, value: np.ndarray
    ) -> tuple[np.bool_, str]:
        if value.dtype.kind == "c":
            raise RuntimeError(
                f"Complex array {self._name} doesn't support comparison."
            )
        _name_val = str(rhs) if len(name) == 0 else f"{name} {rhs}"
        return (
    ...
    def lt(self, bound: float | int, rhs_name: str = "") -> "_ArrayValidator":
        """
        np.all(array < bound).
        """
        self._validator_pipe.append(
    ...
    def le(self, bound: float | int, rhs_name: str = "") -> "_ArrayValidator":
        """
        np.all(array <= bound).
        """
        self._validator_pipe.append(
    ...
    def gt(self, bound: float | int, rhs_name: str = "") -> "_ArrayValidator":
        """
        np.all(array > bound).
        """
        self._validator_pipe.append(
    ...
    def ge(self, bound: float | int, rhs_name: str = "") -> "_ArrayValidator":
        """
        np.all(array >= bound).
        """
        self._validator_pipe.append(
    ...
    def __call__(self, value: Any) -> np.ndarray:
        ...
        Checker.VALUE(
            _converted.size > 0, f"The {self._name} must not be an empty array."
        )
        Checker.TYPE(
        ...
        if self._type not in (ArrayDType.INT, ArrayDType.NUMERIC):
            try:
