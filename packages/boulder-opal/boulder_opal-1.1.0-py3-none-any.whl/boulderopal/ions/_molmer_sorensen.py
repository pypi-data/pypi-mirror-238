...
def _get_ion_drives(
    pwc_addressing_pairs: list[tuple[Pwc, tuple[int, ...]]], ion_count: int
...
def ms_simulate(
    ...
    duration = ScalarDType.REAL("duration").gt(0)(duration)

    lamb_dicke_parameters = ArrayDType.REAL("lamb_dicke_parameters").ndim(3)(
        lamb_dicke_parameters
    )
    relative_detunings = ArrayDType.REAL("relative_detunings").ndim(2)(
        relative_detunings
    )
    target_phases = nullable(ArrayDType.REAL("target_phases").ndim(2), target_phases)
    ...
    if len(addressed_ions) < ion_count:
        graph.matmul(
            displacements, np.eye(ion_count)[addressed_ions], name=_MS_NODE_NAMES[2]
        )
    else:
...
def ms_optimize(
    ...
    **optimization_kwargs
    ...
    target_phases = ArrayDType.REAL("target_phases").ndim(2)(target_phases)
    ion_count = _validate_system_parameters(
        lamb_dicke_parameters, relative_detunings, target_phases
    )
