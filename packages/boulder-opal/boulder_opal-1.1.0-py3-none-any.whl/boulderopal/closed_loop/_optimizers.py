...
class Cmaes(ClosedLoopOptimizer):
    ...
    def __post_init__(self) -> None:
        self.initial_mean = nullable(
            ArrayDType.REAL("initial_mean").ndim(1), self.initial_mean
        )
        self.initial_step_size = nullable(
            ScalarDType.REAL("initial_step_size").gt(0), self.initial_step_size
        )
        self.population_size = nullable(
            ScalarDType.INT("population_size").gt(0), self.population_size
        )
        self.seed = nullable(ScalarDType.INT("seed").ge(0), self.seed)
...
class SimulatedAnnealing(ClosedLoopOptimizer):
    ...
    def __post_init__(self) -> None:
        self.temperatures = (
            ArrayDType.REAL("temperatures").ndim(1).gt(0)(self.temperatures)
        )
        self.temperature_cost = ScalarDType.REAL("temperature_cost").gt(0)(
...
def _validate_bounds(bounds: np.ndarray, name: str = "bounds") -> np.ndarray:
    ...
    """
    bounds = ArrayDType.REAL(name).ndim(2)(bounds)
    Checker.VALUE(
        bounds.shape[1] == 2,
