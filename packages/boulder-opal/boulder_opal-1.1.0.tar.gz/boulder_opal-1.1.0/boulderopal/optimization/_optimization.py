...
def run_optimization(
    ...
    result = run_optimization_workflow(
        graph=graph,
        optimization_count=ScalarDType.INT("optimization_count").gt(0)(
            optimization_count
        ),
        cost_node_name=cost_node_name,
        output_node_names=output_node_names,
        target_cost=nullable(ScalarDType.REAL("target_cost"), target_cost),
        max_iteration_count=nullable(
            ScalarDType.INT("max_iteration_count").gt(0), max_iteration_count
        ),
        cost_tolerance=nullable(
            ScalarDType.REAL("cost_tolerance").gt(0), cost_tolerance
        ),
        cost_history_scope=validate_enum(HistoryScope, cost_history_scope),
        seed=nullable(ScalarDType.INT("seed").ge(0), seed),
    )
    return result
