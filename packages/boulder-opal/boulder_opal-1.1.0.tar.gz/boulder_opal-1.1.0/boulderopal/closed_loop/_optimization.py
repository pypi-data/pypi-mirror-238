...
class Results:
    ...
    def __post_init__(self) -> None:
        self.parameters = ArrayDType.REAL("parameters").ndim(2)(self.parameters)
        self.costs = (
            ArrayDType.REAL("costs")
            .ndim(1)
            .shape((self.parameters.shape[0],), "one item per measurement")(self.costs)
        )
        self.cost_uncertainties = nullable(
            ArrayDType.REAL("cost_uncertainties")
            .ndim(1)
            .shape(self.costs.shape, "same as the costs"),
...
def step(
    ...
    test_point_count = nullable(
        ScalarDType.INT("test_point_count").gt(0), test_point_count
    )
...
def optimize(
    ...
    initial_parameters = nullable(
        ArrayDType.REAL("initial_parameters").ndim(2), initial_parameters
    )
    ...
    max_iteration_count = ScalarDType.INT("max_iteration_count").gt(0)(
        max_iteration_count
    )
