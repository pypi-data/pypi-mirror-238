...
class FilterFunction:
    ...
    def __post_init__(self) -> None:
        self.frequencies = ArrayDType.REAL("frequencies").ndim(1)(self.frequencies)
        self.inverse_powers = (
            ArrayDType.REAL("inverse_powers")
            .ndim(1)
            .ge(0)
            .shape(self.frequencies.shape, "same as the frequencies")(
                self.inverse_powers
            )
        )
        self.uncertainties = nullable(
            ArrayDType.REAL("uncertainties")
            .ndim(1)
            .ge(0)
...
class ConvexOptimization(NoiseReconstructionMethod):
    ...
    def __post_init__(self) -> None:
        ...
        ).ge(0)(self.power_density_lower_bound)

        self.power_density_upper_bound = ScalarDType.REAL(
            "power_density_upper_bound"
        ).gt(self.power_density_lower_bound, "power_density_lower_bound")(
            self.power_density_upper_bound
        )

        self.regularization_hyperparameter = ScalarDType.REAL(
            "regularization_hyperparameter"
        ).ge(0)(self.regularization_hyperparameter)
...
class SVDEntropyTruncation(NoiseReconstructionMethod):
    ...
    def __post_init__(self) -> None:
        self.rounding_threshold = (
            ScalarDType.REAL("rounding_threshold").ge(0).le(1)(self.rounding_threshold)
        )
...
class SVDFixedLengthTruncation(NoiseReconstructionMethod):
    ...
    def __post_init__(self) -> None:
        self.singular_value_count = nullable(
            ScalarDType.INT("singular_value_count").ge(1), self.singular_value_count
        )
