...
def obtain_ion_chain_properties(
    ...
    center_of_mass_frequencies = (
        ArrayDType.REAL("center_of_mass_frequencies")
        .ndim(1)
        .shape((3,))
        .gt(0)(center_of_mass_frequencies)
    )
    wavevector = ArrayDType.REAL("wavevector").ndim(1).shape((3,))(wavevector)
    Checker.VALUE(
    ...
    return obtain_ion_chain_properties_workflow(
        atomic_mass=ScalarDType.REAL("atomic_mass").gt(0)(atomic_mass),
        ion_count=ScalarDType.INT("ion_count").gt(0)(ion_count),
