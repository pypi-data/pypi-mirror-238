...
def property_value_diff(stream: io.StringIO, ref_value: Any, value: Any, include_vis: bool, force_instantiation: bool = False, prefer_oneliner: bool = False, no_direct_assignments: bool = False) -> List[str]:
    ...
    if (ref_value is None or isinstance(ref_value, (RefTarget, HasTraits))) and (value is None or isinstance(value, (RefTarget, HasTraits))):
        result = []
...
def generate_object_instantiation(stream: io.StringIO, variable_name: str, ref_obj: Any, obj: Any, include_vis: bool = False, prefer_oneliner: bool = False) -> List[str]:
...
def get_object_modified_properties(stream: io.StringIO, ref_obj: Union[RefTarget, HasTraits, None], obj: Union[RefTarget, HasTraits], include_vis: bool = False) -> Dict[str, str]:
...
def codegen_modifier(stream: io.StringIO, modifier: Modifier, include_vis: bool, group: Any) -> None:
    ...
    if isinstance(modifier, ovito.modifiers.PythonModifier):
        ...
        if modifier.delegate is not None:
            ...
            if not type(modifier.delegate).__module__.startswith("ovito._extensions.scripts."):
            ...
            codegen_modifier_builtin(stream, modifier.delegate, include_vis, group, modifier.object_title)
        else:
        ...
        codegen_modifier_builtin(stream, modifier, include_vis, group, modifier.object_title)

def codegen_modifier_builtin(stream: io.StringIO, modifier: Union[Modifier, ModifierInterface], include_vis: bool, group: Any, title: str) -> None:
    """ Generates code lines for setting up a modifier and its parameters. """
...
def codegen_user_modify_func(stream: io.StringIO, modifier: ovito.modifiers.PythonModifier, include_vis: bool, group: Any) -> None:
...
def find_object_prefix_recursive(obj, needle, make_mutable: bool = False) -> Optional[str]:
...
def codegen_pipeline(pipeline: Pipeline, include_vis: bool) -> str:
    ...
    if isinstance(pipeline_source, ovito.pipeline.FileSource):
        ...
        if pipeline_source.delegate is not None:
            ...
            # code into the generated script.
            if not type(pipeline_source.delegate).__module__.startswith("ovito._extensions.scripts."):
            ...
            statements = generate_object_instantiation(stream, "src", default_source, pipeline_source.delegate, include_vis)
            if isinstance(statements, str):
                # Generate in-place class instantiation:
                source_statements = f"pipeline = Pipeline(source=PythonSource(delegate={statements}))"
            else:
                # Generate code with a temporary variable holding the source object.
                assignments = [statement for statement in statements if is_property_assignment(statement)]
                method_calls = [statement for statement in statements if not is_property_assignment(statement)]
                all_statements = assignments + ['pipeline = Pipeline(source=PythonSource(delegate=src))'] + method_calls
                source_statements = "\n".join(all_statements)
            ...
            if not pipeline_source.kwargs:
                ...
                stream.write("\npipeline = Pipeline(source = PythonSource(function = functools.partial(create, {}))".format(", ".join(kwargs_list)))
    else:
        stream.write("\n# The currently selected data pipeline '{}' has a data source of type {}.\n# This program version is not able to generate code for this pipeline source type.".format(pipeline.object_title, type(pipeline_source)))
    ...
    if include_vis and ovito.scene.viewports.active_vp:
        ...
        if rs.background_color != (1.0, 1.0, 1.0):
            args.append(f"background={rs.background_color!r}")
        if has_renderer:
