...
class PluginMultiprocessManager():
    ...
    def immediate_showoff_when_possible(self, fp):
        ...
        if file_type.lower() in ['png', 'jpg']:
            ...
            self.chatbot.append(['Detected new image:', f'Local file preview: <br/><div align="center"><img src="file={image_path}"></div>'])
    ...
    def main_process_ui_control(self, txt, create_or_resume) -> str:
        ...
        while True:
            ...
            if self.parent_conn.poll(): 
                ...
                if msg.cmd == "done":
                    self.chatbot.append([f"End", msg.content]); self.cnt += 1
                    yield from update_ui(chatbot=self.chatbot, history=self.history)
                    self.terminate(); break
                if msg.cmd == "show":
                    yield from self.overwatch_workdir_file_change()
                    self.chatbot.append([f"Running phase-{self.cnt}", msg.content]); self.cnt += 1
                    yield from update_ui(chatbot=self.chatbot, history=self.history)
                if msg.cmd == "interact":
                    yield from self.overwatch_workdir_file_change()
