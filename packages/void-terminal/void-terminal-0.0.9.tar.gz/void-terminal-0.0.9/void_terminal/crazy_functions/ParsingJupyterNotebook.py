...
from void_terminal.toolbox import CatchException, report_execption
...
def IpynbExplanation(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt):
    ...
    #  <-------- Splitting overly long IPynb files ---------->
    ...
    inputs_show_user_array = [f"{f}analysis is as follows" for f in pfg.sp_file_tag]
    ...
    chatbot.append(("The parsing result is as follows", block_result))
    history.extend(["The parsing result is as follows", block_result])
    yield from update_ui(chatbot=chatbot, history=history)  # Refresh the page

    #  <-------- Writing to file，Exit ---------->
...
def ParsingIpynbFiles(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    chatbot.append([
        ...
        "Parse the IPynb file。Contributor: codycjy."])
    yield from update_ui(chatbot=chatbot, history=history)  # Refresh the page
    ...
    if len(file_manifest) == 0:
        report_execption(chatbot, history,
                         a=f"Parsing project: {txt}", b=f"Cannot find any .ipynb files: {txt}")
        yield from update_ui(chatbot=chatbot, history=history)  # Refresh the page
        return
    yield from IpynbExplanation(file_manifest, project_folder, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, )
