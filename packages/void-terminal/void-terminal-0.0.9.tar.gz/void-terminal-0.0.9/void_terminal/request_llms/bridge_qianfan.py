...
from void_terminal.toolbox import get_conf, update_ui, is_any_api_key, select_api_key, what_keys, clip_history, trimmed_format_exc, get_conf

model_name = 'Qianfan Big Model Platform'
...
def get_access_token():
    """
    Using AK，SK generates authentication signature（Access Token）
    :return: access_token，Or None(If there is an error)
    """
    ...
    if len(BAIDU_CLOUD_SECRET_KEY) == 0: raise RuntimeError("No BAIDU_CLOUD_SECRET_KEY configured")
    if len(BAIDU_CLOUD_API_KEY) == 0: raise RuntimeError("No configuration for BAIDU_CLOUD_API_KEY")
    ...
    for line in response.iter_lines():
        ...
        try:
            ...
            if ('error_code' in dec) and ("max length" in dec['error_msg']):
                raise ConnectionAbortedError(dec['error_msg'])  # The context is too long, causing token overflow
...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    try:
        ...
        yield from update_ui(chatbot=chatbot, history=history, msg="Exception") # Refresh the page
        return
