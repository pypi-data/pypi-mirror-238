...
from void_terminal.toolbox import update_ui, trimmed_format_exc, gen_time_str, disable_auto_promotion
from void_terminal.toolbox import CatchException, report_execption, get_log_folder
...
@CatchException
def TranslateMarkdownFromEnglishToChinese(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    ...
    if len(file_manifest) == 0:
        report_execption(chatbot, history, a = f"Parsing project: {txt}", b = f"Cannot find any .md files: {txt}")
        yield from update_ui(chatbot=chatbot, history=history) # Refresh the page
        return
...
@CatchException
def MarkdownChineseToEnglish(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    # Basic information：Function, contributor
    chatbot.append([
        "Function plugin feature？",
        "Translate the entire Markdown project。Function plugin contributor: Binary-Husky"])
...
@CatchException
def TranslateMarkdownToSpecifiedLanguage(txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, web_port):
    # Basic information：Function, contributor
    chatbot.append([
        "Function plugin feature？",
        "Translate the entire Markdown project。Function plugin contributor: Binary-Husky"])
