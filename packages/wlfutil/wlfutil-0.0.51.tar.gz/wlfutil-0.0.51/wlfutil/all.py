...
class DtUtil:
    ...
    def format_seconds(secs: int):
        ...
        if secs < 60:
            ...
            res = f'{int(hou)}h{DtUtil.format_seconds(secs)}'
        elif secs >= 24 * 60 * 60:
            day, secs = divmod(secs, 24 * 60 * 60)
            res = f'{int(day)}d{DtUtil.format_seconds(secs)}'
        else:
            res = None
        return res
...
class MinioUtil:
    ...
    @classmethod
    def download(cls, conf: dict, bucket: str, srcfilepath: str, dstfilepath: str = None):
        """下载保存文件保存本地
        :param bucket: 桶分区名称
        :param srcfilepath: minio桶分区下的路径
        :param dstfilepath: 下载后保存的本地路径
        """
        cls._init(conf)
        if dstfilepath:
            with open(dstfilepath, 'wb') as fw:
                for data in cls.CONN.get_object(bucket, srcfilepath):
                    fw.write(data)
        else:
            cls.CONN.fget_object(bucket, srcfilepath, srcfilepath)
...
class KafkaUtil:
    ...
    @classmethod
    def _init_producer(cls, conf: dict):
        if cls.PFID == UniUtil.get_uuid(conf):
            pass
        else:
            cls.connect(cls.PRODUCER, conf)

    @classmethod
    def _init_consumer(cls, conf: dict):
        if cls.CFID == UniUtil.get_uuid(conf):
            pass
        else:
            cls.connect(cls.CONSUMER, conf)

    @classmethod
    def connect(cls, client, conf: dict):
        if client == cls.PRODUCER:
            pass
            # cls.PFID = UniUtil.get_uuid(conf)
        else:
            pass
