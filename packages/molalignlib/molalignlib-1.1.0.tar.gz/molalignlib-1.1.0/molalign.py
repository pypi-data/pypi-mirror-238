...
def molalign():
    ...
    parser.add_argument('-sort', action='store_true', default=SUPPRESS)
    ...
    if args.rec is None:
        ...
        legend = 'Map={0} RMSD={1:.4f}'

    if 'sort' in args:
        raise SystemExit('Option -sort is deprecated, use -reorder instead')
    ...
    if args.reorder:
        ...
        for i, assignment in enumerate(assignments, start=1):
            ...
            if not args.stats:
                print(legend.format(i, rmsd))
            write(args.out, atoms2, append=True, comment=legend.format(i, rmsd))
    else:
        rmsd = atoms1.align_to(atoms0)
        legend='RMSD={:.4f}'.format(rmsd)
        print(legend)
        write(args.out, atoms1, append=True, comment=legend)
