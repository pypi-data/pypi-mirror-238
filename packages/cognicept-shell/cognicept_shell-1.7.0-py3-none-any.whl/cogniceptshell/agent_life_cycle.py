...
class AgentLifeCycle:
    ...
    def populate_config_files(object, args):
        ...
        output_folder = os.path.expanduser(args.path)
        ...
        try:
            ...
            # Raise FileNotFoundError if no template files found
            if len(template_files) == 0:
                print("No template files found in the templates folder.")
                return None

            # Process each template file
            for template_file_name in template_files:
                template_file_path = os.path.join(templates_folder, template_file_name)
                output_file_path = os.path.join(output_folder, template_file_name)
    ...
    def update_agents(object, args):
        ...
        for image_name in images:
            ...
            try:
                if args.space:
                    ...
                    if disk_space >= 3:
                        if('412284733352.dkr.ecr.ap-southeast-1.amazonaws.com/' in image_name):
                            ...
                            object.pull_image('412284733352.dkr.ecr.ap-southeast-1.amazonaws.com/' + image_name, N=N, i=i)
                        print(f"Remaining disk space: {disk_space} GB \n")
                    else:
                        raise OSError
                ...
                print(f"Remaining disk space is too small! Current disk space is: {disk_space} GB")
                success_flag = False
                break
    ...
    def _update_event_log(object, result, args):
        """
        ...
        if result:
            payload["message"] = "Update is successful!"
            payload["compounding"] = False
            payload["level"] = "2"
        else:
            payload["message"] = "Update has failed due to lack of disk space or error in updating images."
            payload["compounding"] = False
            payload['create_ticket'] = True
            payload["level"] = "16"

        # Get current version of all images
        data = {}
        images = object._docker_images

        for x in images:
            image_data = images[x].split(':')
            image_uri = image_data[0]
            if 'ecr' in image_uri:
                version_tag = object.get_version_tag_from_latest(image_data)
                if version_tag is not None:
                    data[image_data[0].split('/')[1]] = [image_data[1], version_tag]
                else:
                    data[image_data[0].split('/')[1]] = [image_data[1]]
                    
        formatted_string = ' ; '.join([f"{key}: '{':'.join(value)}'" for key, value in data.items()])

        #Update payload message based on current version of images
        payload["message"] = payload["message"] + f' Current images are: {formatted_string}'
        
        #Post update log to event pipeline
        object.post_event_log(payload, args)

    def post_event_log(object, payload, args):
        env_val = args.config.config
        agent_post_api = env_val['AGENT_POST_API']
        post_destination = agent_post_api + "/agentstream/put-record"
        ...
        try:
            r = requests.post(post_destination, json=payload, timeout=5)
    ...
    def get_version_tag_from_latest(object, image_data):
        """
    ...
    def display_version(object, args):
        ...
        for x in data['Container Name']:
            if x in object._docker_images:
                ...
                if len(image_data) > 1:
                    latest_version_data = object.get_version_tag_from_latest(image_data)
