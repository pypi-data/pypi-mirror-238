...
@pytest.mark.parametrize(
    ...
    [
        ...
        # default features with dev-dependencies
        ("rand_core-0.6.4.json", FeatureFlags(), True, set()),
        # default features without dev-dependencies
...
def test_rpm_buildrequires(filename: str, flags: FeatureFlags, with_dev_deps: bool, expected: set[str]):
    metadata = load_metadata_from_resource(filename)
    assert buildrequires(metadata.packages[0], flags, with_dev_deps) == expected
...
def test_rpm_workspace_buildrequires(filename: str, flags: FeatureFlags, with_dev_deps: bool, expected: set[str]):
    metadata = load_metadata_from_resource(filename)
    assert workspace_buildrequires(metadata, flags, with_dev_deps) == expected
