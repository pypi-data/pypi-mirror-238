...
SETTINGS = [
    ('enableCompletionAndCreationDates', 'false'),
...
class Tasks:
    ...
    def add(self, new_task):
        # Initialize an empty string to hold the final task text
        final_task = ""

        # Check if the setting for creation dates is enabled
        if setting_enabled('enableCompletionAndCreationDates'):
            creation_date = datetime.now().date().strftime('%Y-%m-%d')
            final_task += creation_date + " "

        # Append the normalized task text
        final_task += new_task
    ...
    def complete(self, task_text):
        ...
        for i, task in enumerate(tasks):
            ...
            # Check if the task text matches the provided task_text
            if stripped_task == task_text and not task_toggled:
                task_toggled = True  # Set the flag to True

                # Toggle the task's completed state
                if originally_completed:
                    # Always remove 'x '
                    modified_task = re.sub(r'^x ', '', stripped_task)
                    # Remove completion date if setting is enabled
                    if setting_enabled('enableCompletionAndCreationDates'):
                        modified_task = re.sub(r'^(\d{4}-\d{2}-\d{2} )', '', modified_task)
                else:
                    # Add 'x ' and completion date if setting is enabled
                    if setting_enabled('enableCompletionAndCreationDates'):
                        parts = stripped_task.split(' ', 1)
                        modified_task = f'x {completion_date} {parts[0]} {parts[1]}'
                    else:
                        modified_task = f'x {stripped_task}'
    ...
    def sync(self, loop, user_data):
        ...
        if isinstance(tasklist_instance.main_frame.body, urwid.Overlay):
            ...
            loop.set_alarm_in(__sync_refresh_interval__, self.sync, user_data)
            return
        ...
        loop.set_alarm_in(__sync_refresh_interval__, self.sync, (txt_file, tasklist_instance, last_mod_time))
...
class TaskUI:
    ...
    def render_and_display_tasks(tasks, palette):
        ...
        for task in tasks:
            ...
            # Initialize word position counter
            word_position = 0

            # Initialize variables to identify if a priority and/or completion date is present
            priority_present = False
            completion_date_present = is_task_complete  # True if the task is complete

            # Loop through each word to apply color-coding logic
            for word in task_words:
                word_position += 1  # Increment the word position counter

                # Check if the word is a priority tag
                if re.match(r'\([A-Z]\)', word):
                    priority_present = True

                # Define the positions where dates could appear based on task completion and priority
                date_offset = 0
                if priority_present:
                    date_offset += 1
                if completion_date_present:
                    date_offset += 1

                # If the word is a date and should be hidden, skip it
                if re.match(r'\d{4}-\d{2}-\d{2}', word) and word_position <= (1 + date_offset):
                    continue
...
def main():
    ...
    tasklist.loop.set_alarm_in(__sync_refresh_interval__, tasks.sync, (txt_file, tasklist, last_mod_time))
