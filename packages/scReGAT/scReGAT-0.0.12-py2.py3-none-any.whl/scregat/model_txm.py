...
def explain_model_gene(dataset_atac: ATACDataset, model: Module, use_device: device,
                       explain_dataset: ATACGraphDataset, file_weight: str,genes: list,
                       method: str = 'ixg', batch_size: int = 32, print_process: bool = False):
    test_loader_explain = DataLoader(explain_dataset, batch_size=batch_size, shuffle=False)
    if method == 'ixg':
        ixg = attr.InputXGradient(model_forward)
    ...
    list_pairs = []
    # idx_gene = 305
    for idx_gene in range(len(genes)):
        list_edge_array = []
        gene = genes[idx_gene]
        for data in test_loader_explain:
            data = data.to(use_device)
