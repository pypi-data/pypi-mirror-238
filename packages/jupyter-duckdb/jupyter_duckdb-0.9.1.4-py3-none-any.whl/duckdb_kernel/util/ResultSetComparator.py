...
class ResultSetComparator:
    ...
    def __only(left: List[Tuple], right: List[Tuple]) -> List[Tuple]:
        ...
        for row in left:
            right_index = ResultSetComparator.row_in(right, row)
    ...
    def __ordered_only(left: List[Tuple], right: List[Tuple]) -> List[Tuple]:
        ...
        for row in left:
            position = ResultSetComparator.row_in(right[i:], row)
    ...
    @staticmethod
    def row_in(rows: List[Tuple], row: Tuple) -> Optional[int]:
        for i, sr in enumerate(rows):
            if ResultSetComparator.rows_equal(sr, row):
                return i
