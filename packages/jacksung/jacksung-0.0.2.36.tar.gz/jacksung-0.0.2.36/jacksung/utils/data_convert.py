...
def save_np2tif(np_data, output_dir, out_name, coordinate=None, resolution=None):
    h, w = np_data.shape
    ...
    if coordinate:
        ...
        with rasterio.open(save_path, "w", driver="GTiff", width=w, height=h, count=1, dtype=np_data.dtype,
...
def np2tif(input_data, save_path, out_name='out', left=None, top=None, x_res=None, y_res=None):
    ...
    for idx, single_np in enumerate(np_data):
        ...
        out_name = name + out_name + '.tif'
        save_np2tif(single_np, save_path, out_name, coordinate=coordinate, resolution=(x_res, y_res))
