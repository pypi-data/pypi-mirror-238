...
def write_observe_frames_to_task(
    ...
    num_sub_repeats: int,
    arm_id: str,
    change_translated_headers: Callable,
...
def write_non_linearized_frames(
    ...
    change_translated_headers: callable = dict,
    tags: list[str] | None = None,
):
...
def make_linearized_test_frames(
    ...
    num_modstates: int,
    num_steps: int,
    change_translated_headers: Callable | None = None,
    num_map_scans: int = 1,
...
def make_non_linearized_test_frames(
    task,
    change_translated_headers: dict | None = None,
):
    arm_id = "SP"
...
def test_parse_cryonirsp_non_linearized_data(parse_non_linearized_task):
    ...
    def fix_ramp_frame_headers(translated_header: fits.Header):
        num_mod_states = translated_header["CRSP_041"]
        num_scan_steps = translated_header["CRSP_006"]
        num_frames = num_mod_states * num_scan_steps
        translated_header["XPOSURE"] = 100 * num_frames * 0.01
        translated_header["CNCNDR"] = num_frames + 1
...
def test_parse_cryonirsp_linearized_incomplete_final_map(parse_linearized_task, arm_id):
    ...
    for map_scan in range(num_map_scans, num_map_scans + 1):  # Only make final map
        for mod_state in range(1, num_modstates + 1):
            for step in range(1, num_steps):  # One step is missing in the last map
                for repeat in range(1, num_sub_repeats + 1):
                    frame_generator = ModulatedObserveHeaders(
                        num_modstates=num_modstates,
                        modstate=mod_state,
    ...
    task()
    assert task.constants._db_dict[CryonirspBudName.num_scan_steps.value] == num_steps
...
def test_parse_cryonirsp_linearized_incomplete_raster_scan(parse_linearized_task, arm_id):
    ...
    def make_incomplete_raster_scan_headers(translated_header):
        # translated_header["CNNUMSCN"] = num_steps % 3
        translated_header["CNCURSCN"] = translated_header["CNCURSCN"] % (num_steps - 1) + 1
        return translated_header
    ...
    # assert task.constants._db_dict[CryonirspBudName.num_scan_steps.value] == num_steps
    assert task.constants._db_dict[CryonirspBudName.num_scan_steps.value] == num_steps - 1
...
def test_parse_cryonirsp_linearized_data_constants(parse_linearized_task, arm_id):
    ...
    assert task.constants._db_dict[CryonirspBudName.wavelength.value] == 1080.2
    assert task.constants._db_dict[CryonirspBudName.modulator_spin_mode.value] == "Continuous"

    # TODO: is there a better way to access these constants (i.e. through a bud?)
    assert task.constants._db_dict["DARK_EXPOSURE_TIMES"] == [
        5.0,
        6.0,
        7.0,
        10.0,
