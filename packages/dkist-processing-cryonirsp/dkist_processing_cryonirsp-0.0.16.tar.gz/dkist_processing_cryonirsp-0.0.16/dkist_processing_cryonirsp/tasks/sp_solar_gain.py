            ...
            for exposure_time in target_exposure_times:
                for beam in range(1, self.constants.num_beams + 1):
                    ...
                    with self.apm_processing_step(
                        ...
                        array_with_solar_signal_removed = self._remove_solar_signal(
                            char_solar_spectra=distorted_char_spectrum,
                            beam=beam,
                            exposure_time=exposure_time,
                        )
                    with self.apm_processing_step(
                        f"Normalizing the solar gain for {beam = } and {exposure_time = }"
                    ):
                        final_gain = array_with_solar_signal_removed / np.nanmean(
                            array_with_solar_signal_removed
                        )
