...
class _RampSet:
    ...
    last_frame_name: str
    last_frame_header: fits.header
    frames_to_process: np.ndarray
    index_offset_to_first_frame: int

    @property
    def is_valid(self):
        return self.num_frames_in_ramp > 1
...
class LinearityCorrection(WorkflowTaskBase, InputDatasetMixin):
    ...
    def run(self):
        ...
        for ramp_set in self._identify_ramp_sets():
            ...
            hdul = fits.HDUList(
                [fits.PrimaryHDU(header=ramp_set.last_frame_header, data=output_array)]
            )
            self.write(
                data=hdul,
                tags=tags,
    ...
    def _populate_ramp_set(self, time_obs: str, idx: int) -> _RampSet | None:
        ...
        """
        num_frames_in_ramp = len(list(self.read(CryonirspTag.time_obs(time_obs))))
        exp_times = self._get_ordered_exposure_time_list(time_obs, num_frames_in_ramp)
        last_frame = self._read_single_ramp_frame(time_obs, num_frames_in_ramp)
        ramp_set_num = idx + 1
        # The default list of curr_frame_in_ramp tag numbers to use, which may be altered later on
        frames_to_process = np.array(range(1, num_frames_in_ramp + 1), dtype=int)
        ramp_set = _RampSet(
            current_ramp_set_num=ramp_set_num,
            time_obs=time_obs,
            num_frames_in_ramp=num_frames_in_ramp,
            exposure_times=exp_times,
            frame_shape=last_frame.data.shape,
            last_frame_name=last_frame.name,
            last_frame_header=last_frame.header,
    ...
    def _reduce_ramp_set(
        ...
        lin_curve
            TODO: Get good definition from Tom S
    ...
    def _reduce_ramp_set_for_lookup_table_and_fast_up_the_ramp(
        ...
        ramp_set.exposure_times = ramp_set.exposure_times[1:]
    ...
    def _linearize_fast_up_the_ramp_with_lookup_table(
        ...
        for chunk in range(1, num_chunks + 1):
            ...
            linearized_frame[current_slice] = self._linearize_chunk(
                chunk_stack, lin_curve, thresholds_flattened[current_slice], ramp_set.exposure_times
            )
