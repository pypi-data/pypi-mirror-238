...
def dissect_array(mat, *blk_labs, unique=(), ordered=False, axes=None):
    ndim = len(mat.shape)
    if len(unique) != 0:  # define order
        unique_labels = unique[0]
    elif ordered:  # preserve order
        unique_labels = list(dict.fromkeys(blk_labs[0]))
    else:  # determine canonical order
        unique_labels = np.unique(blk_labs[0], axis=0)
    ...
    for ulab in unique_labels:
        ...
        if len(blk_labs) > 1:
            yield dissect_array(_mat, *blk_labs[1:], unique=unique[1:],
                                ordered=ordered, axes=axes)
        else:
            yield _mat


def extract_blocks(mat, *blk_labs, unique=None):
    for ulab in np.unique(blk_labs[0], axis=0) if unique is None else unique:
