...
auto set_blinds(std::vector< player > &players, int32_t sb_bet, int32_t bb_bet) -> void {
    for (size_t i = 0; i < players.size(); i++) {
        players[i].behind = players[i].stack;
        players[i].front = 0;

        if (i < 2) {
            players[i].state = enums::state_t::alive;

            players[i].front = i == 0 ? (players.size() == constants::MIN_PLAYERS ? sb_bet : bb_bet) :
                                        (players.size() == constants::MIN_PLAYERS ? bb_bet : sb_bet);
            if (players[i].front > players[i].behind) {
                players[i].front = players[i].behind;
            }

            players[i].behind -= players[i].front;
            players[i].round_bet = players[i].front;
        }

        if (players[i].behind == 0) {
            players[i].state = enums::state_t::allin;
        }
    }
}
} // namespace v1
