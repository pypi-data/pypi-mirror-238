...
auto get_side_pot_redistribution(
    ...
    auto chips_adjusted = adjust_side_pot(ps.get_players(), upper_bound, lower_bound);
...
public:
    ...
    [[maybe_unused]] auto pay(const cards &cards, bool flop_dealt)
                    -> std::vector< std::pair< std::pair< result, enums::position_t >, int32_t > > {
        auto pots = actual::get_all_pots(value_.get_players(), get_highest_bet());
        auto chips = std::accumulate(
                        pots.cbegin(), pots.cend(), std::vector< int32_t >{}, [&](auto val, const auto &e) {
                            return val +
        ...
        std::vector< std::pair< std::pair< result, enums::position_t >, int32_t > > result;
        for (uint8_t index = 0; index < chips.size(); index++) {
            auto evaluation_result = get_evaluation_result_one(cards, index);
            result.emplace_back(
                            std::make_pair(evaluation_result, static_cast< enums::position_t >(index)),
                            chips[index]);
        }
