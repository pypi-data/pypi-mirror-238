        ...
        while True:
            try:
                ...
                cpu_task = asyncio.create_task(
                    asyncio.to_thread(self.cpu_percent)
                )
                ram_task = asyncio.create_task(
                    asyncio.to_thread(self.get_virtual_memory)
                )
                memory_info_task = asyncio.create_task(
                    asyncio.to_thread(self.get_memory_info)
                )
                # Wait for all tasks to complete
                await asyncio.gather(cpu_task, ram_task, memory_info_task)

                self._cpu_usage_data.append(cpu_task.result())
                self._ram_usage_data.append(ram_task.result())
                self._memory_info.append(memory_info_task.result())
