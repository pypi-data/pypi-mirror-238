...
class TaskService(BaseView):
    ...
    async def check_task(self):
        ...
        try:
            ...
            raise TaskDefinition(
                'Resource Error: We need a Program Name'
            ) from ex
        try:
            ...
            # we need a Task ID
            raise TaskDefinition(
                'Resource Error: We need a Task Name'
            ) from ex
    ...
    async def get_task(self, task: str, program: str, **kwargs) -> Task:
        ...
        try:
            user = await get_session(self.request)
