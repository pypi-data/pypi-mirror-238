...
from .client import DbtClient, DbtRun  # type: ignore

_DBT_CLIENT_PATH = "source.packages.extractor.castor_extractor.transformation.dbt.client.client.DbtClient"
...
def test_DbtClient_last_run():
    ...
    with patch(infer_path, return_value=40), patch(call_path) as mocked_call:
        dbt_client = DbtClient(token="some-token", job_id=default_job_id)
        ...
        mocked_call.assert_called_with(*_expected_args(default_job_id))
        ...
        mocked_call.assert_called_with(*_expected_args(42))
        ...
        mocked_call.assert_called_with(
            *_expected_args(default_job_id, (two_days_ago, now))
        )
...
def test_DbtClient_list_job_identifiers():
    ...
    with patch(infer_path, return_value=40), patch(call_path) as mocked_call:
        ...
        dbt_client = DbtClient(token="some-token", job_id=1)
