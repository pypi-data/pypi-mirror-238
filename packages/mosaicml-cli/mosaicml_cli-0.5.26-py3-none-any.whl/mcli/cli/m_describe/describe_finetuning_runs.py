    ...
    def __iter__(self) -> Generator[MCLIDisplayItem, None, None]:
        for model in self.models:
            ...
            if model.estimated_end_time:
                display_config['ETA'] = format_timestamp(model.estimated_end_time)

            item = MCLIDisplayItem(display_config)
            yield item


class MCLIConfigDisplay(MCLIGetDisplay):
    """ Vertical table view of finetune config """
...
def format_event_log(run_events: List[FormattedRunEvent]) -> Table:
    ...
    for event in run_events:
        ...
        if len(text) > 0:
            ...
            grid.add_row(format_timestamp(event_time), text)
    return grid
...
def describe_finetuning_run(
    ...
    **kwargs,
    ...
    if finetune.submitted_config:
        ...
        config_display = MCLIConfigDisplay(finetune.submitted_config)
        config_display.print(output)
