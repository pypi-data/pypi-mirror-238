...
from opengsl.module.model.ggsl import GGSL
import torch
import time
from .solver import Solver


class GGSLSolver(Solver):
    '''
    ...
    def learn(self, debug=False):
        ...
        for epoch in range(self.conf.training['n_epochs']):
            ...
            output, _, output_mlp= self.model(self.feats, self.adj)
            loss_train = self.loss_fn(output[self.train_mask], self.labels[self.train_mask])
            loss_train += self.conf.training['lambda'] * self.loss_fn(output_mlp[self.train_mask], self.labels[self.train_mask])
    ...
    def set_method(self):
        ...
        '''
        self.model = GGSL(self.n_nodes, self.dim_feats, self.num_targets, self.device, self.conf).to(self.device)
