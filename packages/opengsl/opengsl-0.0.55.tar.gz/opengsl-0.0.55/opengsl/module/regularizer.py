...
def connectivity_regularizer(adj):
    '''
    Loss function to control the connectivity of learned structure. Lower loss means the structure are more connected.
...
def smoothness_regularizer(x, adj, style=None, symmetric=False):
    '''
    Loss function to control the smoothness of learned structure on given node features.
    Lower loss means the structure are more smoothed.
    ...
    adj : torch.tensor
        Learned stcuture.
    style : str
        adj will be normalized first to calculate its normalized Lagrange。
        If set as `row`, matrix will be row-wise normalized.
        If set as `symmetric`, matrix will be symmetricly normalized.
        If set as `None`, matrix will not be normalized.
    symmetric : bool
        Whether to symmetry the adj first. Note that this function is only meaningful to symmetric adj.
...
def smoothness_regularizer_direct(x, adj):
    ...
    Loss function to control the smoothness of learned structure on given node features.
    Lower loss means the structure are more smoothed.
    This function is tailored for big sparse adj.
...
def norm_regularizer(adj, p='fro'):
    '''
    Loss function as the norm of learned structure.
