...
def submit_control(api_inputs: ApiInputs, installation_id: Union[uuid.UUID, str], df: pandas.DataFrame, has_priority: bool, session_id: uuid.UUID = uuid.uuid4(), timeout: int = WS_MQTT_CONNECTION_TIMEOUT):
    ...
    logger.info(
        "Renaming columns to match Control Request Payload for MQTT Broker.")
