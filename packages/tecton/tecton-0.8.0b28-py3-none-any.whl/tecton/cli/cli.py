...
def dump(ctx) -> None:
    """Print debug info."""
    top_level_objects, _, _ = get_tecton_objects(cli_utils.get_debug(ctx))
...
def web(workspace, print_) -> None:
    ...
    if workspace_name:
        web_url = urllib.parse.urljoin(cli_utils.cluster_url(), f"app/repo/{workspace_name}/")
    else:
        web_url = urllib.parse.urljoin(cli_utils.cluster_url(), "app")
