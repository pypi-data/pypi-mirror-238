...
{%- for feature in aggregation.features %}
    {{ feature.output_feature_name|upper }}_TABLE AS (
        ...
        WHERE pat._ANCHOR_TIME <= _SPINE._ANCHOR_TIME + {{ feature.time_window.window_end.ToSeconds() }}
        AND  pat._ANCHOR_TIME > _SPINE._ANCHOR_TIME + {{ feature.time_window.window_start.ToSeconds() }}
