...
class DataQuality:
    ...
    def main(self):
        ...
        if self.choice['AnomalyDetection']==1:
            ...
            column_types_str = self.column_types.replace("'", "\"")
            column_contamination_str = self.columns_and_contamination.replace("'", "\"")
            column_null_handling_str = self.columns_and_null_handling.replace("'", "\"")
            column_dbscan_params_str = self.columns_and_dbscan_params.replace("'", "\"")

            # Parse the modified strings as dictionaries
            column_types = json.loads(column_types_str)
            column_contamination = json.loads(column_contamination_str)
            column_null_handling = json.loads(column_null_handling_str)
            column_dbscan_params = json.loads(column_dbscan_params_str)


            result=AnomalyDetection( column_types, column_contamination,
                 column_null_handling, column_dbscan_params).run_anomaly_detection(df)
