...
from arthurai.common.exceptions import UserTypeError, UserValueError, UnexpectedValueError
...
def format_timestamps(inferences: List[Dict[str, Any]],
                      ...
                      time_series_attributes: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    if len(inferences) > 0:
        for inference in inferences:
            ...
            if time_series_attributes is not None:
                inference["inference_data"] = format_time_series_attr_timestamps(inference, time_series_attributes, tz)

    return inferences


def format_timestamp(timestamp: Union[datetime, str], tz: Optional[Union[timezone, str]] = None,
                     col_name: str="") -> str:
    ...
    if isinstance(timestamp, datetime):
        ...
        if is_timezone_aware:
            ...
            if tz and isinstance(tz, timezone):
                ...
                raise UserValueError(f"Timestamps{col_name} should be timezone aware. "
                                     f"Please specify a pytz tz or set the timezone in the datetime object")
    elif isinstance(timestamp, str):
        if is_valid_datetime_string(timestamp):
            return timestamp
        else:
            raise UserValueError(f"Timestamp '{timestamp}'{col_name} is not in ISO8601 format.")
    else:
        raise UserTypeError(f"Timestamps{col_name} must be of type datetime or string. Provided type: {str(type(timestamp))}")


def format_time_series_attr_timestamps(inference: Dict[str, Any], time_series_attributes: List[str],
                                       tz: Optional[Union[timezone, str]] = None) -> Dict[str, Any]:
    ...
    :param inference: Single inference from input to `send_inferences()` function (this enforces requirement that
        the inference has an `"inference_data"` field)
    :param time_series_attributes: List of names of attributes of the model of time series type
    :param tz: datetime timezone object or timezone string
    :returns: updated json inference_data field
    :raises TypeError: timestamp is not of type `datetime.datetime`
    ...
    for attr in time_series_attributes:
        if inference.get("inference_data") is None or not isinstance(inference.get("inference_data"), dict):
            raise UserValueError("inference does not have the expected inference_data field")
        time_series_data = inference["inference_data"].get(attr)
        if time_series_data is not None and core_util.is_list_like(time_series_data):
            for i in range(len(time_series_data)):
                ...
                if time_series_timestamp is not None:
                    inference["inference_data"][attr][i]["timestamp"] = format_timestamp(time_series_timestamp, tz,
                                                                                         col_name=attr)
...
def normal_random_ints_fixed_sum(num_values: int, total_sum: int, relative_std_dev: float = 0.5) -> np.ndarray:
    ...
    if values.sum() != total_sum:
        raise UnexpectedValueError("Computed random integers do not sum to expected total. Please report to Arthur with "
