...
from arthurai.common.constants import API_PREFIX, API_PREFIX_V4, AccuracyMetric, Enrichment, IMAGE_FILE_EXTENSION_MAP, \
    ...
    Stage, TextDelimiter, ValueType, PARQUET_INGESTIBLE_INPUT_TYPES, PARQUET_INGESTIBLE_OUTPUT_TYPES
...
from arthurai.core.dataset_validation_utils import valid_rec_obj, validate_attr_names, validate_series_data_type
...
from arthurai.core.model_utils import _image_attr, _text_attr, _token_likelihoods_attr, _tokens_attr, _get_json_data, \
    _get_parquet_file
...
class ArthurModel(ArthurBaseJsonDataclass):
    ...
    def _get_attribute_data(value_type: ValueType, attribute_data_series: Series) -> Dict:
        ...
        if value_type == ValueType.TimeSeries:  # time series attrs should always be marked unique
            unique = True
            cnt_distinct = 0
        else:
    ...
    def _to_arthur_dtype(self, dtype: str, series: Series, stage: Stage) -> Optional[ValueType]:
        ...
        if dtype == "object" and (is_list_like(example_val) or example_val is None) and \
                self.input_type == InputType.TimeSeries and stage == Stage.ModelPipelineInput:
            return ValueType.TimeSeries
        elif dtype in ["string", "object"] and (isinstance(example_val, str) or example_val is None):
            ...
            if isinstance(cat_dtype, CategoricalDtype):
                ...
                raise TypeError(f"Pandas dtype was 'categorical' but data type was not a pd.Categorical")
    ...
    def build_token_sequence_model(self,
        ...
        if self.input_type not in [InputType.Tabular, InputType.TimeSeries]:
            if isinstance(input_column, list):
                raise ValueError("Only tabular or time series models may specify multiple input columns")
            if data is not None:
                logger.warning("Data was supplied by model with an input type that is not tabular or time series. "
    ...
    def _validate_time_series_model_schema(self) -> None:
        ...
        """
        if self.input_type != InputType.TimeSeries:
            raise MethodNotApplicableError("model is not a time series input type model")

        found_time_series = False
        if self.attributes is not None:
            for attr in self.attributes:
                if attr.stage == Stage.ModelPipelineInput and attr.value_type == ValueType.TimeSeries:
                    if attr.categorical:
                        raise UserValueError("Time Series value type attributes cannot be categorical.")
                    found_time_series = True
                    break
        if not found_time_series:
            raise UserValueError("Time Series input type models must have at least one time series type PIPELINE_INPUT "
    ...
    def _send_files(self,
                    ...
                    files: Optional[List[Union[str, Path, PathLike, BufferedReader, BufferedRandom]]] = None,
        ...
        if data is not None:
            if isinstance(data, DataFrame):
                ...
                directory_path = self.convert_dataframe(stage=None, df=data, is_ref_data=is_ref_data)
            else:
    ...
    def convert_dataframe(self, stage: Optional[Stage], df: DataFrame, is_ref_data: bool,
                          max_rows_per_file=DatasetService.MAX_ROWS_PER_FILE) -> str:
        """Convert a dataframe to file named {model.id}-{stage}.{extension} in the system tempdir

        `extension` is "parquet" if model types are parquet ingestible, "json" otherwise
        ...
        :param max_rows_per_file: the maximum number of rows per file
        :returns: the name of the directory containing the parquet or json files that were created
        """
        # some model types can't ingest parquet data
        to_parquet = self.input_type in PARQUET_INGESTIBLE_INPUT_TYPES and \
            self.output_type in PARQUET_INGESTIBLE_OUTPUT_TYPES
        ...
        for chunk in range(num_chunks):
            ...
            if to_parquet:
                df.iloc[start_idx:end_idx].to_parquet(filename, index=False, allow_truncated_timestamps=True,
                                                      row_group_size=DatasetService.ROW_GROUP_SIZE, engine="pyarrow")
            else:  # write data to json files instead of parquet
                if is_ref_data:
                    data = self._ref_df_to_json(data=df.iloc[start_idx:end_idx])
                else:
                    data_list = self._format_inferences(inferences=df.iloc[start_idx:end_idx], format_timestamps=False)
                    data = json.dumps(data_list, indent=4)
                with open(filename, "w") as outfile:
                    outfile.write(data)
        return temp_dir

    def _transform_json_reference_data(self, directory_path: Optional[str] = None,
                                       files: Optional[List[Union[str, Path, PathLike, BufferedReader,
                                                            BufferedRandom]]] = None) \
        ...
        if directory_path is None and (files is None or len(files) == 0):
            raise UserValueError("directory_path and files cannot both be None or empty lists")
        ...
        for i in range(len(json_files)):  # transform json files if needed
            ...
            if not isinstance(reference_data, list):
                ...
                raise UserValueError("json data is not in a valid format (list of dicts with reference data)")
            for j in range(len(reference_data)):
                if not isinstance(reference_data[j], dict):
                    shutil.rmtree(temp_dir)
                    raise UserValueError("json data is not in a valid format (list of dicts with reference data)")

            transformed_reference_data = inferences_util.nest_reference_data(reference_data, self.attributes)

            # write transformed json file to temp directory
            with open(os.path.join(temp_dir, f"reference-data-transformed-{i}.json"), "w") as open_file:
                ref_data_json = json.dumps(transformed_reference_data, indent=4)
                open_file.write(ref_data_json)
            all_files.append(os.path.join(temp_dir, f"reference-data-transformed-{i}.json"))
    ...
    def set_reference_data(self, directory_path: Optional[str] = None, data: Optional[Union[DataFrame, Series]] = None,
        ...
        if directory_path is None and data is None and (files is None or len(files) == 0):
            raise MissingParameterError("Either directory_path, data, or files must be provided")
        elif self.input_type == InputType.TimeSeries:  # TODO: Should be removed on completion of OBS-699: https://arthurai.atlassian.net/browse/OBS-699
            logger.warning("Sending reference data for time series input models is not supported. "
                           "Proceeding without reference data upload.")
            return
        ...
        if res[DatasetService.COUNTS][DatasetService.FAILURE] != 0:
            ...
            if data is None:
                if directory_path is not None:
                    files = core_util.retrieve_json_files(directory_path) + \
                            core_util.retrieve_parquet_files(directory_path)
                num_rows = self._count_json_num_rows(files) + self._count_parquet_num_rows(files)
            else:
                num_rows = len(data)
    ...
    def _ref_df_to_json(self, data: DataFrame) -> str:
        ...
        for i in range(len(references)):
            references[i]["reference_data"] = self._replace_nans_and_infinities_in_dict(references[i]["reference_data"])
            references[i]['reference_data'] = self._convert_numpy_to_native(references[i]["reference_data"])
        return json.dumps(references, indent=4)

    def _format_inferences(self,
                           inferences: Union[List[Dict[str, Any]], Dict[str, List[Any]], DataFrame],
                           ...
                           ground_truth_timestamps: Optional[Sequence[Union[datetime, str]]] = None,
                           tz: Optional[str] = None,
                           partner_inference_ids: Optional[Sequence[str]] = None) -> List[Dict[str, Any]]:
        ...
        :param format_timestamps: True if timestamp columns still need to be reformatted as ISO-8601 strings from
         datetime objects, False otherwise
        :param predictions: the model predictions, in a table-like format for one or more columns or a list-like format
        ...
        timestamp_attributes: Optional[List[str]] = [] if format_timestamps else None
        time_series_attributes: List[str] = []
        if self.attributes is not None:
            for attr in self.attributes:
                if attr.value_type == ValueType.Timestamp and timestamp_attributes is not None:
                    timestamp_attributes.append(attr.name)
                elif attr.value_type == ValueType.TimeSeries:
                    time_series_attributes.append(attr.name)
    ...
    def send_bulk_ground_truths(self, directory_path: Optional[str] = None,
        ...
        :param directory_path: file path to a directory of parquet or json files containing ground truth data. Required
    ...
    def _count_parquet_num_rows(self, file_paths: Union[Optional[List[Union[str, Path, PathLike, BufferedReader,
                                                                      BufferedRandom]]], List[Path]]) -> int:
    ...
    def _count_json_num_rows(self, file_paths: Union[Optional[List[Union[str, Path, PathLike, BufferedReader,
                                                                      BufferedRandom]]], List[Path]]) -> int:
