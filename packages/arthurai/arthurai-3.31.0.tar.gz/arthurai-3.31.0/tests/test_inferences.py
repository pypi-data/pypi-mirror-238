...
class TestInferences:
    ...
    @pytest.mark.parametrize('inference, expected_inf, expected_err', [
        ({"inference_data": {"bill_pay_history": [{"timestamp": datetime(2000, 10, 29, 0, 0, 0, 0, pytz.UTC),
                                                  "value": 1},
        ...
        ({"bill_pay_history": [{"timestamp": datetime(2000, 10, 29, 0, 0, 0, 0, pytz.UTC), "value": 1}]}, None,
         UserValueError),  # test missing inference_data field
        ({"inference_data": {"bill_pay_history": [{"value": 1}]}},
         {"bill_pay_history": [{"value": 1}]}, None),  # test missing timestamp field doesn't error
        ({"inference_data": {"value": 1}}, {"value": 1}, None),  # test missing attribute data doesn't error
    ])
    def test_format_time_series_attr_timestamps(self, inference, expected_inf, expected_err):
        time_series_attributes = ["bill_pay_history"]
        if expected_err is None:
            assert arthur_util.format_time_series_attr_timestamps(inference, time_series_attributes) == expected_inf
        else:
