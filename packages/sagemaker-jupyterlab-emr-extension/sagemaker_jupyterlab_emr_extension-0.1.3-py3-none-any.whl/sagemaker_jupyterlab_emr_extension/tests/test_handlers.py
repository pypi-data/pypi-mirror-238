...
async def test_describe_cluster_success(emr_client_mock, jp_fetch):
...
@pytest.mark.aysncio
@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emr_client")
async def test_describe_cluster_bad_request(emr_client_mock, jp_fetch):
    emr_client_mock.return_value = EmrClientMock(
        TEST_DESCRIBE_CLUSTER_RESPONSE, TEST_LIST_CLUSTER_RESPONSE
    )
...
@pytest.mark.aysncio
@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emr_client")
async def test_describe_cluster_cluster_cofiguration_empty(emr_client_mock, jp_fetch):
    emr_client_mock.return_value = EmrClientMock(
...
@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emr_client")
async def test_list_cluster_success(emr_client_mock, jp_fetch):
    emr_client_mock.return_value = EmrClientMock(
        TEST_DESCRIBE_CLUSTER_RESPONSE, TEST_LIST_CLUSTER_RESPONSE
    )
    kwargs = {}
...
async def test_create_persistent_app_ui_success(emrprivate_client_mock, jp_fetch):
    ...
    resp = json.loads(response.body.decode("utf-8"))
    assert resp == {"persistentAppUIId": "p-a123456789"}


@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emrprivate_client")
async def test_get_persistent_app_ui_presigned_url_success(
    emrprivate_client_mock, jp_fetch
):
    emrprivate_client_mock.return_value = PrivateEmrClientMock()
...
@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emrprivate_client")
async def test_describe_persistent_app_ui_success(emrprivate_client_mock, jp_fetch):
    emrprivate_client_mock.return_value = PrivateEmrClientMock()
    kwargs = {"PersistentAppUIId": "p-a123456789"}
    response = await jp_fetch(
...
@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emr_client")
async def test_list_instance_groups_success(emr_client_mock, jp_fetch):
    emr_client_mock.return_value = EmrClientMock(
        TEST_DESCRIBE_CLUSTER_RESPONSE, TEST_LIST_CLUSTER_RESPONSE
    )
    kwargs = {"ClusterId": "c12345688"}
...
@pytest.mark.aysncio
@patch("sagemaker_jupyterlab_emr_extension.handlers.get_emr_client")
async def test_list_instance_groups_bad_request(emr_client_mock, jp_fetch):
    emr_client_mock.return_value = EmrClientMock(
        TEST_DESCRIBE_CLUSTER_RESPONSE, TEST_LIST_CLUSTER_RESPONSE
    )
    kwargs = {}
