...
const EmrPresignedURL: React.FC<EmrPresignedURLProps> = ({
  ...
  const getActiveClusterPresignedURL = useCallback(
    (clusterId: string, accountId: string | undefined, applicationId: string | undefined) => {
      getOnClusterAppUIPresignedURL(clusterId, applicationId)
  ...
  const getInactiveClusterPresignedURL = useCallback(
    (
      ...
      createPersistentAppUI(cluster.clusterArn)
        .then((res) =>
          pollUntilPersistentAppUIReady(res?.persistentAppUIId, MAX_DESCRIBE_POLL_WAIT, DESCRIBE_POLL_INTERVAL_MS),
        )
        .then((res) => getPersistentAppUIPresignedURL(res?.persistentAppUI.persistentAppUIId, persistentAppUIType))
        .then((res) => openPresignedURLInNewTab(res?.presignedURL))
        .catch((error) => handleError(error))
        .finally(() => setLoading(false));
    },
  ...
  const handleGetPresignedURL = useCallback(
    (
      ...
      async () => {
        ...
        const res = await describeCluster(clusterId).catch((error) => handleError(error));
        if (!res || !res?.cluster) {
