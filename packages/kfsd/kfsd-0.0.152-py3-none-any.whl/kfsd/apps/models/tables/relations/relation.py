...
def gen_relation_id(name, sourceId, targetId):
    identifierData = [
        "NAME={}".format(name),
...
class Relation(BaseModel):
    ...
    source = models.ForeignKey(
        "HRel", on_delete=models.CASCADE, related_name="relations"
    )
    target = models.ForeignKey(
        "HRel", on_delete=models.CASCADE, related_name="relations_from"
    )

    def save(self, *args, **kwargs):
        self.identifier = gen_relation_id(
            self.name, self.source.identifier, self.target.identifier
        )
