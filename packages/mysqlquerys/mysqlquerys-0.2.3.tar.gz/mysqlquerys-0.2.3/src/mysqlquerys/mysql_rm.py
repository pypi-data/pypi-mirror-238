    ...
    def createTableFromFile(self, file):
        ...
        try:
            ...
            for line in f.read().splitlines():
                ...
                command += line
                if ';' in line:
                    if re.search("^CREATE TABLE", command) or re.search("^ALTER TABLE", command) or re.search(
                            "^INSERT INTO", command):
            ...
            for i, tab in enumerate(tablelist):
                if self.connectionType == 'mysql':
                    ...
                    if not exists:
                        ...
                        tablelList.pop(i)
                        continue
                    query = "SELECT table_name FROM information_schema.KEY COLUMN USAGE WHERE table_schema = %s AND referenced table_name = %s"
                    cur = self.db.cursor()
                    ...
                    if not children:
                        ...
                        for child in children:
                            if child[0] not in tablelist:
                                ...
                                break
                    cur.close()
                elif self.connectionType == 'postgresqgl':
