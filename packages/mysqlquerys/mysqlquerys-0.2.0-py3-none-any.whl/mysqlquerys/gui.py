...
class guiSqlRM(QMainWindow, CheckRequiredFiles):
    def __init__(self, connection):
        ...
        Ui_MysglWindow, QtBaseClass = uic.loadUiType(self.mysql_gui)
        self.ui = Ui_MysglWindow()
        ...
        self.GB_not_callable = self.ui.GB_not_callable
        self.GB_callable_without_args = self.ui.GB_callable_without_args
        self.GB_callable_with_args = self.ui.GB_callable_with_args
        ...
        self.ui.CheckB_hide_tables_list.toggled.connect(lambda: self.populate_list(self.db.allAvailableTablesInSchema, self.ui.LW_tables_available))
        ...
        if self.db_type == 'mysql':
            ...
            self.ui.LW_tables_available.itemDoubleClicked.connect(self.show_full_table)
    ...
    def populate_class_methods(self):
        ...
        if 'DataBase' in self.sender().text():
            ...
            GB_not_callable = self.GB_not_callable
            GB_callable_without_args = self.GB_callable_without_args
            GB_callable_with_args =self.GB_callable_with_args
    ...
    def start_noncallable_method(self):
        ...
        if self.sender().isChecked():
            ...
            if not callable(getattr(self.sender().module_class, method)):
                ...
                self.ul.TE_result.setPlainText(str(text))
            else:
                text = getattr(self.sender().module_instance, self.sender().text())
    ...
    def setFilter(self, logical_index):
        ...
        if self.table.columnsProperties[colName] == 'datetime' or \
            ...
            filt = FilterWindow.getDateTimeIn(colName)
            if not filt:
                return
            ...
            filt = FilterWindow.getTimeInt(colName)
            if not filt:
                return
            if isinstance(filt, tuple):
            ...
            item.show()
            item.setFocus()
            item.editingFinished.connect(lambda: (self.applyFilter(colName, item.text()), item.clear(), item.hide(), item.deleteLater()))
...
class FilterWindow(QDialog, CheckRequiredFiles):
    def __init__(self, colType, colName):
        ...
        if colType == 'dateTime':
            self.gui.GBDateTime.setVisi(True)
            self.gui.dateTimeEdit_MaxVal.setEnabled(False)
            self.gui.label_dateTime.setText('= {}'.format(self.colName))
            self.gui.dateTimeEdit_MinVal.setDateTime(QDateTime(datetime.now().year, datetime.now().month, 1, 0, 0, 0))
            self.gui.dateTimeEdit_MaxVal.setDateTime(
                QDateTime(datetime.now().year, datetime.now().month, datetime.now().day, datetime.now().hour,
    ...
    def openDateTimeInterval(self):
        if self.gui.checkBoxDateTime.isChecked() :
            ...
            self.gui.dateTimeEdit_MaxVal.setEnabled(False)
            self.gui.label_dateTime.setTéxt('= {}'.format (self.colName))
    ...
    def openTimeInterval(self):
        if self.gui.checkBoxTime.isChecked() :
            ...
            self.gui.timeEditMax.setEnabled(False)
            self.gui.label_Time.setTéxt('= {}'.format (self.colName))
    ...
    def openValInterval(self):
        if self.gui.checkBoxMaxVal.isChecked() :
            ...
            self.gui.lineEditMax.setEnabled(False)
            self.gui.label_Val.setTéxt('= {}'.format (self.colName))
