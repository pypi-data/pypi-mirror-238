...
class TrainerClasses:
    ...
    def _is_finetuning_supported(self):
        ...
        # models with launguage info are not supported
        lang_model_name = config.get(MmDetectionModelLiterals.LANG_MODEL_NAME, None)
        if lang_model_name:
            error_str = "Language models are not yet supported for image finetuning tasks"
            logger.error(error_str)
            raise ACFTValidationException._with_error(
