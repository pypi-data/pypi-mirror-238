...
class ObjectDetectionModelWrapper(nn.Module):
    ...
    def forward(self, **data) -> Union[Dict[str, Any], Tuple[Tensor, Tensor]]:
        ...
        inputs, batch_data_samples = mlflow_obj_det_wrapper.prepare_inputs_for_model_forward(data, is_train=is_train)
        if is_train:
    ...
    def save_pretrained(self, output_dir: os.PathLike, state_dict: OrderedDict) -> None:
        ...
        if model_type == MmDetectionConfigLiterals.DEFORMABLE_DETR_MODEL and hasattr(self.config.model, "bbox_head"):
