...
from ...direct.user_items.data_set import (
    ...
    _get_user_data_set_data_points,
)
...
class DataPoints:
    ...
    def get_data_points(
        ...
        if self.data_point_ids:
            ...
            valid_data_point_id_set = set(map(lambda x: x["datapointId"], data_point_list))
            ...
            if data_point_id_set == invalid_data_point_id_set:  # All invalid data points
                ...
                raise BadRequestException(f"The specified data point IDs do not exist: {invalid_data_points} or the datapoint settings specified are not valid.")
    ...
    def _handle_special_data_point_columns(self, settings_data_frame: pd.DataFrame) -> None:
        ...
        settings_data_frame[convert_exist_columns] = settings_data_frame[convert_exist_columns].replace(r"^\s*$", None, regex=True)
        settings_data_frame[convert_exist_columns] = settings_data_frame[convert_exist_columns].astype(float).astype(pd.Int64Dtype()).fillna(-1)
    ...
    def _get_data_set_data_points(self, data_point_alias: Optional[list] = None) -> List[Any]:
        if self.data_set_id is not None and re.match(r"^\d{4}-\d{4}$", self.data_set_id, re.M) is not None:  # morningstar data view
            ...
            sub_data_set_content = [x for x in data_set_content if x.get("alias", "") in data_point_alias] if data_point_alias else data_set_content
