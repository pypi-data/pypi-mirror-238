...
from .._exceptions import ApiResponseException, BadRequestException, ValueErrorException, ResourceNotFoundError, NetworkExceptionError
from .._utils import _reduce_list_data
from .._base_api import APIBackend
from .._error_messages import RESOURCE_NOT_FOUND_ERROR_INVESTMENT_List, BAD_REQUEST_ERROR_INVALID_INVESTMENT_LIST_ID
...
def get_investment_list(list_id: str) -> DataFrame:
    ...
    try:
        ...
        list_df["masterportfolioid"] = list_df["masterportfolioid"].fillna(-1).astype(int).astype(str).replace({"-1": None, "None": None})

        if list_df.empty:
            _logger.debug(f"Returning empty DataFrame.")
...
def get_investment_lists() -> DataFrame:
    ...
    """
    try:
        _logger.debug(f"Getting All Direct Lists")
...
def save_investment_list(list_name: str, investment_ids: List[str], overwrite_if_exists: bool) -> DataFrame:
    ...
    * Modified Date
    ...
    try:
        ...
        data: Dict[str, Any]
        if (list_name in get_item.values) and overwrite_if_exists == False:
