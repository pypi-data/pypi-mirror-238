    ...
    def parse_profile_item(self, item):
        ...
        if len(items) < 2:
            if item.endswith('.*'):
                ...
                _log('warning', 'invalid profile format', item)
                return
        else:
    ...
    def gather_hook_info(self, target_hooking_func_dict):
        ...
        for user_func in user_func_list:
            ...
            if is_instance is False:
                is_module_function = self.text.endswith('.*')

            if is_instance is False:
                if isinstance(user_func, staticmethod) is False and is_module_function is False:
                    continue
            ...
            if self.profile_func_key is not None and func_name != self.profile_func_key:
                continue
        ...
        try:
            ...
            if is_instance:
                ...
                if isinstance(target_func, staticmethod):
                    ...
                    _log('diagnostics', 'hook_func.static_method', target_func_text, target_func.__func__)
                else:
