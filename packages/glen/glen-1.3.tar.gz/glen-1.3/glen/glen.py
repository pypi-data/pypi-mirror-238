...
def trigger_identification(sentence_list, params, tokenizer, device):
...
def type_ranking(predict_set, params, device):
...
def type_classification(predict_set, params, tokenizer, device):
...
def event_detection(sentence_list, path_to_ckpt, bs_TI=64, bs_TC=64, bs_TR=4, k=10):
    params = parse_arguments()
    params['path_to_ckpt'] = path_to_ckpt
    params['bs_TI'] = bs_TI
    params['bs_TC'] = bs_TC
    params['bs_TR'] = bs_TR
    params['k'] = k
    ...
    predict_set = trigger_identification(sentence_list, params, tokenizer, device)
    
    predict_set = type_ranking(predict_set, params, device)

    predicted_items = type_classification(predict_set, params, tokenizer, device)
...
if __name__ == '__main__':    
    sen_list = ['One Air Force technician died and 21 others were injured .', 'One Air Force technician died and 21 others were injured.']
    print(f"start predicting {len(sen_list)} sentence")
    print(event_detection(sen_list, '../ckpts'))
