...
from config.utils import default_config
from data_factory.data import AnomalyData
from data_factory.dataloader import get_dataloader
from data_factory.dataset.base import AnomalyTransformerDataset
...
class DBAnomDector:
    ...
    def infer(self, data: pd.DataFrame) -> Tuple[List[float], List[bool], List[int]]:
        ...
        if self.stats["stats_weight"] and False:
            # Calculate distance
...
def main(config: omegaconf.DictConfig) -> None:
    ...
    # Load model
    detector = DBAnomDector(config=config)
    detector.train()
