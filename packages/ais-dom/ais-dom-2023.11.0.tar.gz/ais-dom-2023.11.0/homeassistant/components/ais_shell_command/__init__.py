...
async def _change_remote_access(hass, call):
    ...
    if access == "on":
        ...
        if not os.path.isfile(
            ...
            await _run("mkdir -p /data/data/com.termux/files/home/.cloudflared")
            ...
            # create named tunnel
            await _run(
                "/data/data/com.termux/files/usr/bin/cloudflared --origincert "
                "/data/data/com.termux/files/home/.cloudflared/cert.pem tunnel delete -f "
                + gate_id
            )
            await _run(
                "/data/data/com.termux/files/usr/bin/cloudflared --origincert "
                "/data/data/com.termux/files/home/.cloudflared/cert.pem tunnel create "
                + gate_id
            )
            # rename credentials file
            await _run(
                "mv /data/data/com.termux/files/home/.cloudflared/*.json "
                "/data/data/com.termux/files/home/.cloudflared/key.json"
            )

            # create config.yaml
            f = open(
                "/data/data/com.termux/files/home/.cloudflared/config.yaml", mode="w"
            )
            ...
            await _run(
                "/data/data/com.termux/files/usr/bin/cloudflared --origincert "
                "/data/data/com.termux/files/home/.cloudflared/cert.pem tunnel route dns -f "
                + gate_id
                + " "
                + gate_id
                + ".paczka.pro"
            ...
            # delete cert file
            await _run("rm /data/data/com.termux/files/home/.cloudflared/cert.pem")

            #
        await _run(
            "pm2 start /data/data/com.termux/files/usr/bin/cloudflared "
            "--name tunnel "
            "--restart-delay=150000 -- --config /data/data/com.termux/files/home/.cloudflared/config.yaml "
            "tunnel run"
        )
        await _run("pm2 save")
        ...
        # OLD WAY
        # await _run(
        #     "pm2 restart tunnel || pm2 start /data/data/com.termux/files/usr/bin/cloudflared"
        ...
        await _run("rm /data/data/com.termux/files/home/.cloudflared/*")
...
async def _ssh_remote_access(hass, call):
    ...
    if access == "on":
        await _run(
            "pm2 restart ssh-tunnel || pm2 start /data/data/com.termux/files/usr/bin/cloudflared"
...
async def _restart_pm2_service(hass, call):
    ...
    if service == "zwave":
        ...
        cmd_to_run = (
            "pm2 restart zwave || pm2 start /data/data/com.termux/files/home/zwavejs2mqtt/server/bin/www.js "
            "--name zwave --output /dev/null --error /dev/null --restart-delay=120000"
        )
    elif service == "zigbee":
        await _run("su -c 'chmod 777 /dev/ttyACM*'")
        cmd_to_run = (
            "pm2 restart zigbee || cd /data/data/com.termux/files/home/zigbee2mqtt; pm2 start index.js "
...
async def _flush_logs(hass, call):
    ...
    await _run("rm /data/data/com.termux/files/home/.pm2/logs/*.log")

    # pip cache
    await _run("rm -rf /data/data/com.termux/files/home/.cache/pip")
