...
async def prepare_usb_device(hass, device_info):
    ...
    if device_info["id"] in G_ZIGBEE_DEVICES_ID:
        ...
        if G_CONBEE_STARTED and device_info["id"] == G_CONBEE_ID:
            ...
            # add info in app
            if not os.path.isdir("/data/data/com.termux/files/home/zigbee2mqtt"):
                await say_it(
                    hass,
    ...
    if device_info["id"] == G_ZWAVE_ID:
        ...
        if ais_global.has_root():
            ...
            await _run(hass, "su -c 'chmod 777 /dev/ttyACM*'")
        # zwavejs2mqtt exists?
        if not os.path.isdir("/data/data/com.termux/files/home/zwavejs2mqtt"):
            await say_it(
                hass,
            ...
            try:
                with open(
                    "/data/data/com.termux/files/home/zwavejs2mqtt/store/settings.json"
                ) as file_r:
