        ...
        if access == "on":
            try:
                ...
                if r.status_code == 404:
                    pass
                    # command = "pm2 restart tunnel || pm2 start /data/data/com.termux/files/usr/bin/cloudflared" \
...
async def _async_process(
    hass, text, calling_client_id=None, hot_word_on=False, conversation_id=None
):
    ...
    if CURR_ENTITIE_ENTERED and CURR_ENTITIE is not None:
        if CURR_ENTITIE.startswith("input_text."):
            ...
            ir = intent.IntentResponse(language="PL-pl")
            ir.async_set_speech("wpisano w pole tekst: " + text)
            CURR_VIRTUAL_KEYBOARD_VALUE = text
            result = conversation.ConversationResult(
                response=ir, conversation_id=conversation_id
            )
            return result
    ...
    if ais_global.G_AUTOMATION_CONFIGS is not None:
        automations = {
    ...
    for key, value in automations.items():
        if value.lower().startswith("jolka"):
            # get aliases
            all_commands = []
            for auto_config in ais_global.G_AUTOMATION_CONFIGS:
    ...
    # return response to the conversation dialog
    intent_resp = intent.IntentResponse(language="PL-pl")
    intent_resp.async_set_speech(speech=m)
    result = conversation.ConversationResult(
        response=intent_resp, conversation_id=conversation_id
    )
    return result
...
def _match_entity(hass, name, domain=None):
    ...
    try:
        entity_id = fuzzy_extract.extractOne(name, entities, score_cutoff=88)[2]
    except Exception as e:
        entity_id = None
