...
async def _async_remove_file(hass, path):
    if "api/image/serve" in path:
        ...
        path = path.replace("/local/", "/data/data/com.termux/files/home/AIS/www/")
        path = path.replace(
            "/media/galeria/", "/data/data/com.termux/files/home/AIS/www/img/"
        ).split("?authSig")[0]
        os.remove(path)
...
class FileReadView(HomeAssistantView):
    ...
    async def post(self, request):
        ...
        if file_path == "/data/data/com.termux/files/home/AIS/ais_welcome.txt":
    ...
    async def post(self, request):
        ...
        if error_info == "":
            if (
                ...
                db_connection["dbUrl"] = (
                    "sqlite://///data/data/com.termux/files/home/dom/dyski-wymienne/"
                    + db_connection["dbDrive"]
                    + "/ais.db"
                )
                db_connection["dbPassword"] = ""
