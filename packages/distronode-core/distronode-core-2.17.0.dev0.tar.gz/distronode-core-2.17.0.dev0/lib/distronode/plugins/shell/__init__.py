...
class ShellBase(DistronodePlugin):
    def __init__(self):
    ...
    def _normalize_system_tmpdirs(self):
        ...
        if not all(os.path.isabs(d) for d in normalized_paths):
            raise DistronodeError('The configured system_tmpdirs contains a relative path: {0}. All'
    ...
    def _generate_temp_dir_name():
        return 'distronode-tmp-%s-%s-%s' % (time.time(), os.getpid(), random.randint(0, 2**48))
