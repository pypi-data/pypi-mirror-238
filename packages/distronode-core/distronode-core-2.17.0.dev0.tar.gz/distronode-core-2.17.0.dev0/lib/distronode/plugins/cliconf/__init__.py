...
def enable_mode(func):
    ...
    def wrapped(self, *args, **kwargs):
        ...
        if not to_text(prompt, errors='surrogate_or_strict').strip().endswith('#'):
            raise DistronodeError('operation requires privilege escalation')
        return func(self, *args, **kwargs)
    return wrapped


class CliconfBase(DistronodePlugin):
    """
    ...
    def copy_file(self, source=None, destination=None, proto='scp', timeout=30):
        ...
        if proto == 'scp':
            if not HAS_SCP:
                raise DistronodeError("Required library scp is not installed.  Please install it using `pip install scp`")
    ...
    def _update_cli_prompt_context(self, config_context=None, exit_command='exit'):
        ...
        if out is None:
            raise DistronodeConnectionFailure(message=u'cli prompt is not identified from the last received'
