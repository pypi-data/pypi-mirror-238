...
class ActionModule(ActionBase):
    ...
    # HACK: list of unqualified service manager names that are/were built-in, we'll prefix these with `distronode.legacy` to
        ...
        if module == 'auto':
            try:
                if self._task.delegate_to:  # if we delegate, we should use delegated host's facts
                    module = self._templar.template("{{hostvars['%s']['distronode_facts']['service_mgr']}}" % self._task.delegate_to)
                else:
                    module = self._templar.template('{{distronode_facts.service_mgr}}')
            except Exception:
        ...
        try:
            if module == 'auto':
                facts = self._execute_module(
                    ...
                    module_args=dict(gather_subset='!all', filter='distronode_service_mgr'), task_vars=task_vars)
                self._display.debug("Facts %s" % facts)
                module = facts.get('distronode_facts', {}).get('distronode_service_mgr', 'auto')

            if not module or module == 'auto' or not self._shared_loader_obj.module_loader.has_plugin(module):
                module = 'distronode.legacy.service'
            ...
            if module != 'auto':
                ...
                raise DistronodeActionFail('Could not detect which service manager to use. Try gathering facts or setting the "use" option.')
