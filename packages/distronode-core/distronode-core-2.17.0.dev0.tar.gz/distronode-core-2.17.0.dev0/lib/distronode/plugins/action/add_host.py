...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if new_name is None:
            raise DistronodeActionFail('name, host or hostname needs to be provided')
        ...
        if groups:
            if isinstance(groups, list):
                ...
                raise DistronodeActionFail("Groups must be specified as a list.", obj=self._task)
