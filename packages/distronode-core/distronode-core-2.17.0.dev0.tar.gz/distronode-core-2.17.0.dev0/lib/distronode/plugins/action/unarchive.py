...
from distronode.errors import DistronodeError, DistronodeAction, DistronodeActionFail, DistronodeActionSkip
        ...
        try:
            ...
            if 'copy' in self._task.args:
                ...
                if 'remote_src' in self._task.args:
                    raise DistronodeActionFail("parameters are mutually exclusive: ('copy', 'remote_src')")
            ...
            if source is None or dest is None:
                raise DistronodeActionFail("src (or content) and dest are required")
            ...
            if creates:
                ...
                if self._remote_file_exists(creates):
                    raise DistronodeActionSkip("skipped, since %s exists" % creates)
            ...
            if not remote_stat['exists'] or not remote_stat['isdir']:
                raise DistronodeActionFail("dest '%s' must be an existing dir" % dest)
            ...
            # execute the unarchive module now, with the updated args (using distronode.legacy prefix to eliminate collections
            # collisions with local override
            result.update(self._execute_module(module_name='distronode.legacy.unarchive', module_args=new_module_args, task_vars=task_vars))
