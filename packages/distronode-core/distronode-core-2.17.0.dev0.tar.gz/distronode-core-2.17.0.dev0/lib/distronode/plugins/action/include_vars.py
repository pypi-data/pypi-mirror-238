...
class ActionModule(ActionBase):
    ...
    def _set_args(self):
        ...
        if not isinstance(self.valid_extensions, list):
            raise DistronodeError('Invalid type for "extensions" option, it must be a list')
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        for arg in self._task.args:
            if arg in self.VALID_DIR_ARGUMENTS:
                ...
                raise DistronodeError('{0} is not a valid option in include_vars'.format(to_native(arg)))

        if dirs and files:
            raise DistronodeError("You are mixing file only and dir only arguments, these are incompatible")
        ...
        result['distronode_included_var_files'] = self.included_files
        result['distronode_facts'] = results
        result['_distronode_no_log'] = not self.show_content
