...
from distronode.module_utils.common.collections import Mapping, MutableMapping
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            if remote_src:
                ...
                # call with distronode.legacy prefix to prevent collections collisions while allowing local override
                raise _DistronodeActionDone(result=self._execute_module(module_name='distronode.legacy.uri',
            ...
            # call with distronode.legacy prefix to prevent collections collisions while allowing local override
            result.update(self._execute_module('distronode.legacy.uri', module_args=new_module_args, task_vars=task_vars, wrap_async=self._task.async_val))
