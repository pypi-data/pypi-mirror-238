...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if "expand_argument_vars" in self._task.args:
            raise DistronodeActionFail(f"Unsupported parameters for ({self._task.action}) module: expand_argument_vars")

        command_action = self._shared_loader_obj.action_loader.get('distronode.legacy.command',
                                                                   task=self._task,
