...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'that' not in self._task.args:
            raise DistronodeError('conditional required in "that" string')
        ...
        if fail_msg is None:
            ...
            if not all(isinstance(x, string_types) for x in fail_msg):
                raise DistronodeError('Type of one of the elements in fail_msg or msg list is not string type')
        elif not isinstance(fail_msg, (string_types, list)):
            raise DistronodeError('Incorrect type for fail_msg or msg, expected a string or list and got %s' % type(fail_msg))
        ...
        if success_msg is None:
            ...
            if not all(isinstance(x, string_types) for x in success_msg):
                raise DistronodeError('Type of one of the elements in success_msg list is not string type')
        elif not isinstance(success_msg, (string_types, list)):
            raise DistronodeError('Incorrect type for success_msg, expected a string or list and got %s' % type(success_msg))
