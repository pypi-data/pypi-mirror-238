...
DOCUMENTATION = '''
    ...
    options:
        plugin:
            ...
            required: True
            choices: ['distronode.builtin.constructed', 'constructed']
        use_vars_plugins:
            description:
...
EXAMPLES = r'''
    ...
    plugin: distronode.builtin.constructed
    strict: False
    compose:
        var_sum: var1 + var2
        ...
        server_type: "distronode_hostname | regex_replace ('(.{6})(.{2}).*', '\\2')"
    groups:
...
from distronode.errors import DistronodeParserError, DistronodeOptionsError
from distronode.inventory.helpers import get_group_vars
from distronode.plugins.inventory import BaseInventoryPlugin, Constructable
...
class InventoryModule(BaseInventoryPlugin, Constructable):
    ...
    def parse(self, inventory, loader, path, cache=False):
        ...
        try:
            ...
            if self.get_option('use_vars_plugins'):
                raise DistronodeOptionsError("The option use_vars_plugins requires distronode >= 2.11.")
        ...
        try:
            ...
            raise DistronodeParserError("failed to parse %s: %s " % (to_native(path), to_native(e)), orig_exc=e)
