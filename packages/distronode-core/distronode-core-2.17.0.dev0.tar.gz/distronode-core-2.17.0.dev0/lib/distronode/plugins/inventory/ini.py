...
DOCUMENTATION = '''
    name: ini
    version_added: "2.4"
    short_description: Uses an Distronode INI file as inventory source.
    description:
...
[web]
host1
host2 distronode_port=222 # defined inline, interpreted as an integer
...
from distronode.inventory.group import to_safe_group_name
...
from distronode.utils.shlex import shlex_split
...
class InventoryModule(BaseFileInventoryPlugin):
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        try:
            ...
            try:
                ...
                # Handle non-utf8 in comment lines: https://github.com/distronode/distronode/issues/17593
                data = []
    ...
    def _raise_error(self, message):
        raise DistronodeError("%s:%d: " % (self._filename, self.lineno) + message)
    ...
    def _parse(self, path, lines):
        ...
        for g in pending_declarations:
            ...
            if decl['state'] == 'vars':
                raise DistronodeError("%s:%d: Section [%s:vars] not valid for undefined group: %s" % (path, decl['line'], decl['name'], decl['name']))
            elif decl['state'] == 'children':
                raise DistronodeError("%s:%d: Section [%s:children] includes undefined group: %s" % (path, decl['line'], decl['parents'].pop(), decl['name']))
    ...
    def _expand_hostpattern(self, hostpattern):
        ...
        if hostpattern.strip().endswith(':') and port is None:
            raise DistronodeParserError("Invalid host pattern '%s' supplied, ending in ':' is not allowed, this character is reserved to provide a port." %
                                     hostpattern)
        for pattern in hostnames:
            # some YAML parsing prevention checks
            if pattern.strip() == '---':
                raise DistronodeParserError("Invalid host pattern '%s' supplied, '---' is normally a sign this is a YAML file." % hostpattern)
