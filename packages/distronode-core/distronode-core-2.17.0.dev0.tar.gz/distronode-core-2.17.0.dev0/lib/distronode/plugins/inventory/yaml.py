...
DOCUMENTATION = '''
    ...
    options:
      yaml_extensions:
        ...
        env:
          - name: DISTRONODE_YAML_FILENAME_EXT
          - name: DISTRONODE_INVENTORY_PLUGIN_EXTS
        ini:
...
class InventoryModule(BaseFileInventoryPlugin):
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if not data:
            raise DistronodeParserError('Parsed empty YAML file')
        elif not isinstance(data, MutableMapping):
            raise DistronodeParserError('YAML inventory has invalid structure, it should be a dictionary, got: %s' % type(data))
    ...
    def _parse_group(self, group, group_data):
        ...
        if isinstance(group_data, (MutableMapping, NoneType)):  # type: ignore[misc]
            ...
            try:
                ...
                raise DistronodeParserError("Unable to add group %s: %s" % (group, to_text(e)))
                ...
                for section in ['vars', 'children', 'hosts']:
                    if section in group_data:
                        ...
                        if not isinstance(group_data[section], (MutableMapping, NoneType)):  # type: ignore[misc]
                            raise DistronodeParserError('Invalid "%s" entry for "%s" group, requires a dictionary, found "%s" instead.' %
