...
DOCUMENTATION = '''
    ...
    options:
      always_show_stderr:
        ...
        env:
           - name: DISTRONODE_INVENTORY_PLUGIN_SCRIPT_STDERR
    description:
        - The source provided must be an executable that returns Distronode inventory JSON
    ...
    notes:
        ...
        - To find the scripts that used to be part of the code release, go to U(https://github.com/distronode-community/contrib-scripts/).
'''
...
from distronode.module_utils.basic import json_dict_bytes_to_unicode
...
class InventoryModule(BaseInventoryPlugin):
    ''' Host inventory parser for distronode using external inventory scripts. '''
    ...
    def parse(self, inventory, loader, path, cache=None):
        ...
        try:
            try:
                ...
                raise DistronodeParserError("problem running %s (%s)" % (' '.join(cmd), to_native(e)))
            ...
            if sp.returncode != 0:
                raise DistronodeError("Inventory script (%s) had an execution error: %s " % (path, err))
            ...
            try:
                ...
                raise DistronodeError("Inventory {0} contained characters that cannot be interpreted as UTF-8: {1}".format(path, to_native(e)))

            try:
                processed = self.loader.load(data, json_only=True)
            except Exception as e:
                raise DistronodeError("failed to parse executable inventory script results from {0}: {1}\n{2}".format(path, to_native(e), err))
            ...
            if not isinstance(processed, Mapping):
                raise DistronodeError("failed to parse executable inventory script results from {0}: needs to be a json dict\n{1}".format(path, err))
            ...
            for host in self._hosts:
                ...
                if data_from_meta is None:
                    ...
                    try:
                        ...
                        raise DistronodeError("Improperly formatted host information for %s: %s" % (host, to_native(e)), orig_exc=e)
    ...
    def _parse_group(self, group, data):
        ...
        if 'hosts' in data:
            if not isinstance(data['hosts'], list):
                raise DistronodeError("You defined a group '%s' with bad data for the host list:\n %s" % (group, data))
        ...
        if 'vars' in data:
            if not isinstance(data['vars'], dict):
                raise DistronodeError("You defined a group '%s' with bad data for variables:\n %s" % (group, data))
    ...
    def get_host_variables(self, path, host):
        ...
        try:
            ...
            raise DistronodeError("problem running %s (%s)" % (' '.join(cmd), e))
        (out, stderr) = sp.communicate()

        if sp.returncode != 0:
            raise DistronodeError("Inventory script (%s) had an execution error: %s" % (path, to_native(stderr)))
        ...
        try:
            ...
            raise DistronodeError("could not parse post variable response: %s, %s" % (cmd, out))
