...
class BecomeBase(DistronodePlugin):
    ...
    def get_option(self, option, hostvars=None, playcontext=None):
        ...
        implement the base become options required in Distronode. """
    ...
    def _build_success_command(self, cmd, shell, noexe=False):
        ...
        try:
            ...
            raise DistronodeError('The %s shell family is incompatible with the %s become plugin' % (shell.SHELL_FAMILY, self.name))
