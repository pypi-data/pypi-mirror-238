...
options:
  mode:
    ...
    - For those used to I(/usr/bin/chmod) remember that modes are actually octal numbers.
      You must give Distronode enough information to parse them correctly.
      For consistent results, quote octal numbers (for example, V('644') or V('1777')) so Distronode receives
      a string and can do its own conversion from string into number.
      Adding a leading zero (for example, V(0755)) works sometimes, but can fail in loops and some other circumstances.
    - Giving Distronode a number without following either of these rules will end up with a decimal
      number which will have unexpected results.
    - As of Distronode 1.8, the mode may be specified as a symbolic mode (for example, V(u+rwx) or
      V(u=rw,g=r,o=r)).
  ...
  unsafe_writes:
    ...
    - This option allows Distronode to fall back to unsafe methods of updating filesystem objects when atomic operations fail
      (however, it doesn't force Distronode to perform unsafe writes).
