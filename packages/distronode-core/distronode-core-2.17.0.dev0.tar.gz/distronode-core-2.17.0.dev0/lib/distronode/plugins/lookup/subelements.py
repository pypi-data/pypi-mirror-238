...
- name: show var structure as it is needed for example to make sense
  ...
  tasks:
    - name: Set authorized ssh key, extracting just that data from 'users'
      distronode.posix.authorized_key:
        user: "{{ item.0.name }}"
    ...
    - name: list groups for users that have them, don't error if groups key is missing
      ...
      loop: "{{ q('distronode.builtin.subelements', users, 'groups', {'skip_missing': True}) }}"
"""
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for item0 in elementlist:
            if not isinstance(item0, dict):
                raise DistronodeError("subelements lookup expects a dictionary, got '%s'" % item0)
            ...
            for subkey in subelements:
                ...
                if subkey not in subvalue:
                    if skip_missing:
                        ...
                        raise DistronodeError("could not find '%s' key in iterated item '%s'" % (subkey, subvalue))
                if not lastsubkey:
                    if not isinstance(subvalue[subkey], dict):
                        if skip_missing:
                            continue
                        else:
                            raise DistronodeError("the key %s should point to a dictionary, got '%s'" % (subkey, subvalue[subkey]))
                    else:
                        subvalue = subvalue[subkey]
                else:  # lastsubkey
                    if not isinstance(subvalue[subkey], list):
                        raise DistronodeError("the key %s should point to a list, got '%s'" % (subkey, subvalue[subkey]))
                    else:
