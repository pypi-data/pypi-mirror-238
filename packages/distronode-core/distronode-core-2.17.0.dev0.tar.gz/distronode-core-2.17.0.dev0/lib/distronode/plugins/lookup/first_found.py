...
- name: Set _found_file to the first existing file, raising an error if a file is not found
  distronode.builtin.set_fact:
    _found_file: "{{ lookup('distronode.builtin.first_found', findme) }}"
  vars:
    findme:
      - /path/to/foo.txt
...
- name: Set _found_file to the first existing file, or an empty list if no files found
  distronode.builtin.set_fact:
    _found_file: "{{ lookup('distronode.builtin.first_found', files, paths=['/extra/path'], skip=True) }}"
  vars:
    files:
      - /path/to/foo.txt
      - /path/to/bar.txt
...
- name: Include tasks only if one of the files exists, otherwise skip
  distronode.builtin.include_tasks: '{{ tasks_file }}'
  when: tasks_file != ""
  vars:
    tasks_file: "{{ lookup('distronode.builtin.first_found', files=['tasks.yaml', 'other_tasks.yaml'], errors='ignore') }}"
...
- name: |
  ...
  distronode.builtin.copy:
    src: "{{ lookup('distronode.builtin.first_found', findme) }}"
    dest: /some/file
  vars:
    findme:
      - foo
...
- name: same copy but specific paths
  distronode.builtin.copy:
    src: "{{ lookup('distronode.builtin.first_found', params) }}"
    dest: /some/file
  vars:
    params:
      files:
...
- name: INTERFACES | Create Distronode header for /etc/network/interfaces
  distronode.builtin.template:
    src: "{{ lookup('distronode.builtin.first_found', findme)}}"
    dest: "/etc/foo.conf"
  vars:
    findme:
      - "{{ distronode_virtualization_type }}_foo.conf"
      - "default_foo.conf"

- name: read vars from first file found, use 'vars/' relative subdir
  distronode.builtin.include_vars: "{{lookup('distronode.builtin.first_found', params)}}"
  vars:
    params:
      files:
        - '{{ distronode_distribution }}.yml'
...
from distronode.errors import DistronodeLookupError, DistronodeUndefinedVariable
...
class LookupModule(LookupBase):
    ...
    def _process_terms(self, terms, variables, kwargs):
        ...
        for term in terms:
            if isinstance(term, Mapping):
                ...
                raise DistronodeLookupError("Invalid term supplied, can handle string, mapping or list of strings but got: %s for %s" % (type(term), term))
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for fn in total_search:
            ...
            except (DistronodeUndefinedVariable, UndefinedError):
        ...
        if skip:
            ...
            return []
        raise DistronodeLookupError("No file was found when using first_found.")
