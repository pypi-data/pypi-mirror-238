...
DOCUMENTATION = """
    ...
    notes:
      ...
      - See R(Distronode task paths,playbook_task_paths) to understand how file lookup occurs with paths.
      - Matching is against local system files on the Distronode controller.
        To iterate a list of files on a remote node, use the M(distronode.builtin.find) module.
...
- name: Display paths of all .txt files in dir
  distronode.builtin.debug: msg={{ lookup('distronode.builtin.fileglob', '/my/path/*.txt') }}
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        for term in terms:
            ...
            if term_file != term:
                ...
                if 'distronode_search_path' in variables:
                    paths = variables['distronode_search_path']
                else:
