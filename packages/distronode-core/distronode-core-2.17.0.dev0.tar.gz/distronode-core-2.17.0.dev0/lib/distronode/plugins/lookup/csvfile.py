...
- name:  Match 'Li' on the first column, return the second column (0 based index)
  distronode.builtin.debug: msg="The atomic number of Lithium is {{ lookup('distronode.builtin.csvfile', 'Li file=elements.csv delimiter=,') }}"

- name: msg="Match 'Li' on the first column, but return the 3rd column (columns start counting after the match)"
  distronode.builtin.debug: msg="The atomic mass of Lithium is {{ lookup('distronode.builtin.csvfile', 'Li file=elements.csv delimiter=, col=2') }}"
...
- name: Define Values From CSV File, this reads file in one go, but you could also use col= to read each in it's own lookup.
  ...
  vars:
    csvline: "{{ lookup('distronode.builtin.csvfile', bgp_neighbor_ip, file='bgp_neighbors.csv', delimiter=',') }}"
...
class LookupModule(LookupBase):
    ...
    def read_csv(self, filename, key, delimiter, encoding='utf-8', dflt=None, col=1):
        ...
        try:
            ...
            raise DistronodeError("csvfile: %s" % to_native(e))
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        for term in terms:
            ...
            if '_raw_params' not in kv:
                raise DistronodeError('Search key is required but was not found')
            ...
            try:
                for name, value in kv.items():
                    ...
                    if name not in paramvals:
                        raise DistronodeAssertionError('%s is not a valid option' % name)
