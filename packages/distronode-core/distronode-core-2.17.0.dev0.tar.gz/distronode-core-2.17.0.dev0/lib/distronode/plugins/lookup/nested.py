...
class LookupModule(LookupBase):
    ...
    def _lookup_variables(self, terms, variables):
        ...
        for x in terms:
            try:
                ...
                raise DistronodeUndefinedVariable("One of the nested variables was undefined. The error was: %s" % e)
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        result = []
        if len(my_list) == 0:
            raise DistronodeError("with_nested requires at least one element in the nested list")
        result = my_list.pop()
        while len(my_list) > 0:
