...
from distronode.errors import DistronodeError, DistronodeFileNotFound, DistronodeUndefinedVariable, DistronodeParserError
from distronode.executor import action_write_locks
from distronode.executor.play_iterator import IteratingStates, PlayIterator
from distronode.executor.process.worker import WorkerProcess
from distronode.executor.task_result import TaskResult
from distronode.executor.task_queue_manager import CallbackSend, DisplaySend, PromptSend
from distronode.module_utils.six import string_types
from distronode.module_utils.common.text.converters import to_text
from distronode.module_utils.connection import Connection, ConnectionError
from distronode.playbook.conditional import Conditional
from distronode.playbook.handler import Handler
from distronode.playbook.helpers import load_list_of_blocks
from distronode.playbook.task import Task
from distronode.playbook.task_include import TaskInclude
from distronode.plugins import loader as plugin_loader
...
from distronode.vars.clean import strip_internal_keys, module_response_deepcopy
...
def _get_item_vars(result, task):
    ...
    if task.loop or task.loop_with:
        loop_var = result.get('distronode_loop_var', 'item')
        index_var = result.get('distronode_index_var')
        ...
        if '_distronode_item_label' in result:
            item_vars['_distronode_item_label'] = result['_distronode_item_label']
        if 'distronode_loop' in result:
            item_vars['distronode_loop'] = result['distronode_loop']
    return item_vars
        ...
        try:
            ...
            if isinstance(result, StrategySentinel):
                ...
                try:
                    ...
                    try:
                        raise DistronodeError(f"{e}") from e
                    except DistronodeError as e:
                        value = e
        ...
        for result in results:
            ...
            while result.needs_debugger(globally_enabled=self.debugger_active):
                ...
                if next_action.result == NextAction.REDO:
                    ...
                    # Matches KeyboardInterrupt from bin/distronode
                    sys.exit(99)
            else:
                _processed_results.append(result)
...
class StrategyBase:
    ...
    def add_tqm_variables(self, vars, play):
        ...
        '''
        vars['distronode_current_hosts'] = self.get_hosts_remaining(play)
        vars['distronode_failed_hosts'] = self.get_failed_hosts(play)
    ...
    def _queue_task(self, host, task, task_vars, play_context):
        ...
        try:
            ...
            raise DistronodeError("Failed to convert the throttle value to an integer.", obj=task._ds, orig_exc=e)
    ...
    def get_delegated_hosts(self, result, task):
        host_name = result.get('_distronode_delegated_vars', {}).get('distronode_delegated_host', None)
    ...
    def _set_always_delegated_facts(self, result, task):
        ...
        if always_keys:
            _pop = facts.pop
            always_facts = {
                'distronode_facts': dict((k, _pop(k)) for k in list(facts) if k in always_keys)
            }
        ...
        while True:
            ...
            if task_result.is_failed():
                ...
                if not ignore_errors:
                    ...
                    if iterator.is_any_block_rescuing(state_when_failed):
                        ...
                        self._variable_manager.set_nonpersistent_facts(
                            original_host.name,
                            dict(
                                distronode_failed_task=wrap_var(original_task.serialize()),
                                distronode_failed_result=task_result._result,
                            ),
                        )
                    else:
                ...
                for result_item in result_items:
                    if '_distronode_notify' in result_item and task_result.is_changed():
                        ...
                        for notification in result_item['_distronode_notify']:
                    ...
                    if 'distronode_facts' in result_item and original_task.action not in C._ACTION_DEBUG:
                        ...
                        if original_task.action in C._ACTION_INCLUDE_VARS:
                            for (var_name, var_value) in result_item['distronode_facts'].items():
                                # find the host we're actually referring too here, which may
                                # be a host that is not really in inventory at all
                                for target_host in host_list:
                                    self._variable_manager.set_host_variable(target_host, var_name, var_value)
                        else:
                            cacheable = result_item.pop('_distronode_facts_cacheable', False)
                            for target_host in host_list:
                                ...
                                if not is_set_fact or cacheable:
                                    self._variable_manager.set_host_facts(target_host, result_item['distronode_facts'].copy())
                                if is_set_fact:
                                    self._variable_manager.set_nonpersistent_facts(target_host, result_item['distronode_facts'].copy())

                    if 'distronode_stats' in result_item and 'data' in result_item['distronode_stats'] and result_item['distronode_stats']['data']:

                        if 'per_host' not in result_item['distronode_stats'] or result_item['distronode_stats']['per_host']:
                            host_list = self.get_task_hosts(iterator, original_host, original_task)
                        else:
                            host_list = [None]

                        data = result_item['distronode_stats']['data']
                        aggregate = 'aggregate' in result_item['distronode_stats'] and result_item['distronode_stats']['aggregate']
            ...
            if original_task.register:
                ...
                if not isidentifier(original_task.register):
                    raise DistronodeError("Invalid variable name in 'register' specified: '%s'" % original_task.register)
    ...
    def _wait_on_pending_results(self, iterator):
        ...
        while self._pending_results > 0 and not self._tqm._terminated:
            ...
            if self._tqm.has_dead_workers():
                raise DistronodeError("A worker was found in a dead state")
    ...
    def _load_included_file(self, included_file, iterator, is_handler=False):
        ...
        Raises DistronodeError exception in case of a failure during including a file,
        ...
        try:
            ...
            if data is None:
                ...
                raise DistronodeError("included task files must contain a list of tasks")
            ...
            if isinstance(e, DistronodeFileNotFound):
                reason = "Could not find or access '%s' on the Distronode Controller." % to_text(e.file_name)
            else:
                reason = to_text(e)
            ...
            raise DistronodeError(reason) from e
    ...
    def _execute_meta(self, task, play_context, iterator, target_host):
        ...
        if meta_action == 'noop':
            ...
            if _evaluate_conditional(target_host):
                ...
                if host_state.run_state == IteratingStates.HANDLERS:
                    raise DistronodeError('flush_handlers cannot be used as a handler')
            ...
            # Allow users to use this in a play as reported in https://github.com/distronode/distronode/issues/22286?
            ...
            raise DistronodeError("invalid meta action requested: %s" % meta_action, obj=task._ds)
    ...
    def _get_cached_role(self, task, play):
        ...
        try:
            ...
            raise DistronodeError(f'Cannot locate {task._role.get_name()} in role cache')
    ...
    def update_active_connections(self, results):
        ...
        for r in results:
            if 'args' in r._task_fields:
                socket_path = r._task_fields['args'].get('_distronode_socket')
                if socket_path:
