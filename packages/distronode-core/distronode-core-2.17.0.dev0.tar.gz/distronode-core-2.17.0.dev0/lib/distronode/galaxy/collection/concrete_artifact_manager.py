...
if t.TYPE_CHECKING:
    ...
    from distronode.galaxy.token import GalaxyToken

from distronode.errors import DistronodeError
from distronode.galaxy import get_collections_galaxy_meta_info
from distronode.galaxy.api import should_retry_error
from distronode.galaxy.dependency_resolution.dataclasses import _GALAXY_YAML
...
class ConcreteArtifactsManager:
    ...
    def get_galaxy_artifact_path(self, collection):
        ...
        display.vvvv(
            ...
            'Distronode Galaxy'.format(collection=collection),
        )
...
def _extract_collection_from_git(repo_url, coll_ver, b_path):
    ...
    try:
        ...
        raise DistronodeError(  # should probably be LookupError
...
def _download_file(url, b_path, expected_hash, validate_certs, token=None, timeout=60):
    ...
    try:
        ...
        raise DistronodeError(to_native(err), orig_exc=err)
    ...
    if expected_hash:
        ...
        if expected_hash != actual_hash:
            raise DistronodeError('Mismatch artifact hash with downloaded file')
...
def _get_meta_from_src_dir(
    ...
    if not isinstance(manifest, dict):
        if require_build_metadata:
            raise DistronodeError(f"The collection galaxy.yml at '{to_native(galaxy_yml)}' is incorrectly formatted.")
        # Valid build metadata is not required by distronode-galaxy list. Raise ValueError to fall back to implicit metadata.
