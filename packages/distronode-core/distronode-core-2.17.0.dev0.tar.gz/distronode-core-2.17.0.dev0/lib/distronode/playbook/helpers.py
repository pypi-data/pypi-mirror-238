...
from distronode.errors import DistronodeParserError, DistronodeUndefinedVariable, DistronodeAssertionError
from distronode.module_utils.common.text.converters import to_native
from distronode.parsing.mod_args import ModuleArgsParser
...
def load_list_of_blocks(ds, play, parent_block=None, role=None, task_include=None, use_handlers=False, variable_manager=None, loader=None):
    ...
    if not isinstance(ds, (list, type(None))):
        raise DistronodeAssertionError('%s should be a list or None but is %s' % (ds, type(ds)))
...
def load_list_of_tasks(ds, play, block=None, role=None, task_include=None, use_handlers=False, variable_manager=None, loader=None):
    ...
    from distronode.playbook.role_include import IncludeRole
    from distronode.playbook.handler_task_include import HandlerTaskInclude
    from distronode.template import Templar
    from distronode.utils.plugin_docs import get_versioned_doclink

    if not isinstance(ds, list):
        raise DistronodeAssertionError('The ds (%s) should be a list but was a %s' % (ds, type(ds)))

    task_list = []
    for task_ds in ds:
        if not isinstance(task_ds, dict):
            raise DistronodeAssertionError('The ds (%s) should be a dict but was a %s' % (ds, type(ds)))

        if 'block' in task_ds:
            if use_handlers:
                raise DistronodeParserError("Using a block as a handler is not supported.", obj=task_ds)
            t = Block.load(
                task_ds,
                play=play,
                parent_block=block,
            ...
            try:
                ...
                raise DistronodeParserError(to_native(e), obj=task_ds, orig_exc=e)
            ...
            if action in C._ACTION_ALL_INCLUDE_IMPORT_TASKS:
                ...
                if action in C._ACTION_IMPORT_TASKS:
                    if t.loop is not None:
                        raise DistronodeParserError("You cannot use loops on 'import_tasks' statements. You should use 'include_tasks' instead.", obj=task_ds)
                    ...
                    if not data:
                        ...
                        raise DistronodeParserError("included task files must contain a list of tasks", obj=data)
                ...
                if use_handlers:
                    raise DistronodeParserError(f"Using '{action}' as a handler is not supported.", obj=task_ds)
                ...
                if action in C._ACTION_IMPORT_ROLE:
                    if ir.loop is not None:
                        raise DistronodeParserError("You cannot use loops on 'import_role' statements. You should use 'include_role' instead.", obj=task_ds)
...
def load_list_of_roles(ds, play, current_role_path=None, variable_manager=None, loader=None, collection_search_list=None):
    ...
    if not isinstance(ds, list):
        raise DistronodeAssertionError('ds (%s) should be a list but was a %s' % (ds, type(ds)))
