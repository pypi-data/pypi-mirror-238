...
RESET_VARS = (
    ...
    # TODO: ???
    'distronode_docker_extra_args',
    'distronode_ssh_host',
    'distronode_ssh_pass',
    'distronode_ssh_port',
    'distronode_ssh_user',
    'distronode_ssh_private_key_file',
    'distronode_ssh_pipelining',
    'distronode_ssh_executable',
)
...
class PlayContext(Base):
    ...
    pipelining = FieldAttribute(isa='bool', default=C.DISTRONODE_PIPELINING)
    ...
    def verbosity(self):
        display.deprecated(
            "PlayContext.verbosity is deprecated, use distronode.utils.display.Display.verbosity instead.",
            version="2.18"
        )
    ...
    def set_task_and_variable_override(self, task, variables, templar):
        ...
        # If the value 'distronode_delegated_vars' is in the variables, it means
        ...
        if task.delegate_to is not None:
            ...
            delegated_vars = variables.get('distronode_delegated_vars', dict()).get(delegated_host_name, dict())
            ...
            for address_var in ('distronode_%s_host' % delegated_transport,) + C.MAGIC_VARIABLE_MAPPING.get('remote_addr'):
                if address_var in delegated_vars:
                    break
            else:
                display.debug("no remote address found for delegated host %s\nusing its name, so success depends on DNS resolution" % delegated_host_name)
                delegated_vars['distronode_host'] = delegated_host_name

            # reset the port back to the default if none was specified, to prevent
            # the delegated host from inheriting the original host's setting
            for port_var in ('distronode_%s_port' % delegated_transport,) + C.MAGIC_VARIABLE_MAPPING.get('port'):
                if port_var in delegated_vars:
                    break
            else:
                if delegated_transport == 'winrm':
                    delegated_vars['distronode_port'] = 5986
                else:
                    delegated_vars['distronode_port'] = C.DEFAULT_REMOTE_PORT

            # and likewise for the remote user
            for user_var in ('distronode_%s_user' % delegated_transport,) + C.MAGIC_VARIABLE_MAPPING.get('remote_user'):
                if user_var in delegated_vars and delegated_vars[user_var]:
                    break
            else:
                delegated_vars['distronode_user'] = task.remote_user or self.remote_user
        else:
            delegated_vars = dict()
