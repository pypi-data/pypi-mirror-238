...
def module_response_deepcopy(v):
    ...
    Designed to be used within the Distronode "engine" to improve performance
    ...
    * ``distronode.executor.task_result.TaskResult.clean_copy``
    * ``distronode.vars.clean.clean_facts``
    * ``distronode.vars.namespace_facts``
    """
...
def strip_internal_keys(dirty, exceptions=None):
    # All keys starting with _distronode_ are internal, so change the 'dirty' mapping and remove them.
    ...
    if isinstance(dirty, MutableSequence):
        ...
        for k in list(dirty.keys()):
            if isinstance(k, six.string_types):
                if k.startswith('_distronode_') and k not in exceptions:
                    del dirty[k]
                    continue
        ...
        raise DistronodeError("Cannot strip invalid keys from %s" % type(dirty))
...
def remove_internal_keys(data):
    ...
    for key in list(data.keys()):
        if (key.startswith('_distronode_') and key != '_distronode_parsed') or key in C.INTERNAL_RESULT_KEYS:
    ...
    for key in list(data.get('distronode_facts', {}).keys()):
        if key.startswith('discovered_interpreter_') or key.startswith('distronode_discovered_interpreter_'):
            del data['distronode_facts'][key]
...
def clean_facts(facts):
    ...
    for conn_path in connection_loader.all(path_only=True):
        ...
        re_key = re.compile('^distronode_%s_' % re.escape(conn_name))
        for fact_key in fact_keys:
            # most lightweight VM or container tech creates devices with this pattern, this avoids filtering them out
            if (re_key.match(fact_key) and not fact_key.endswith(('_bridge', '_gwbridge'))) or fact_key.startswith('distronode_become_'):
    ...
    re_interp = re.compile('^distronode_.*_interpreter$')
    for fact_key in fact_keys:
        if re_interp.match(fact_key):
            remove_keys.add(fact_key)
    # then we remove them (except for ssh host keys)
    for r_key in remove_keys:
        if not r_key.startswith('distronode_ssh_host_key_'):
...
def namespace_facts(facts):
    ''' return all facts inside 'distronode_facts' w/o an distronode_ prefix '''
    deprefixed = {}
    for k in facts:
        if k.startswith('distronode_') and k not in ('distronode_local',):
            deprefixed[k[8:]] = module_response_deepcopy(facts[k])
        else:
            deprefixed[k] = module_response_deepcopy(facts[k])

    return {'distronode_facts': deprefixed}
