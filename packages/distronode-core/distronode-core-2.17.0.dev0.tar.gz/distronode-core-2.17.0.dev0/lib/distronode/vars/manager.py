...
from distronode.errors import DistronodeError, DistronodeParserError, DistronodeUndefinedVariable, DistronodeFileNotFound, DistronodeAssertionError, DistronodeTemplateError
from distronode.inventory.host import Host
from distronode.inventory.helpers import sort_groups, get_group_vars
from distronode.module_utils.common.text.converters import to_text
from distronode.module_utils.six import text_type, string_types
from distronode.plugins.loader import lookup_loader
from distronode.vars.fact_cache import FactCache
...
from distronode.utils.vars import combine_vars, load_extra_vars, load_options_vars
...
def preprocess_vars(a):
    ...
    for item in data:
        if not isinstance(item, MutableMapping):
            raise DistronodeError("variable files must contain either a dictionary of variables, or a list of dictionaries. Got: %s (%s)" % (a, type(a)))
...
class VariableManager:
    ...
    def get_vars(self, play=None, host=None, task=None, include_hostvars=True, include_delegate_to=False, use_cache=True,
        ...
        if task:
            ...
            if C.PLAYBOOK_VARS_ROOT == 'all':  # should be default
                ...
                raise DistronodeError('Unknown playbook vars logic: %s' % C.PLAYBOOK_VARS_ROOT)
        ...
        if host:
            ...
            try:
                ...
                if C.INJECT_FACTS_AS_VARS:
                    ...
                    # always 'promote' distronode_local
                    all_vars = _combine_and_track(all_vars, wrap_var({'distronode_local': facts.get('distronode_local', {})}), "facts")
            except KeyError:
                pass
        ...
        if play:
            ...
            try:
                for vars_file_item in vars_files:
                    ...
                    #       distronode_facts['os_distribution'] can be used, ala include_vars.
                    ...
                    try:
                        for vars_file in vars_file_list:
                            ...
                            if include_delegate_to and host:
                                raise DistronodeFileNotFound("vars file %s was not found" % vars_file_item)
                    except (UndefinedError, DistronodeUndefinedVariable):
                        if host is not None and self._fact_cache.get(host.name, dict()).get('module_setup') and task is not None:
                            raise DistronodeUndefinedVariable("an undefined variable was found when attempting to template the vars_files item '%s'"
                ...
                raise DistronodeParserError("Error while reading vars files - please supply a list of file names. "
        ...
        if task and host and task.delegate_to is not None and include_delegate_to:
            all_vars['distronode_delegated_vars'], all_vars['_distronode_loop_cache'] = self._get_delegated_vars(play, task, all_vars)
    ...
    def _get_magic_variables(self, play, host, task, include_hostvars, _hosts=None, _hosts_all=None):
        '''
        Returns a dictionary of so-called "magic" variables in Distronode,
        which are special variables we set internally for use.
        '''

        variables = {}
        variables['playbook_dir'] = os.path.abspath(self._loader.get_basedir())
        variables['distronode_playbook_python'] = sys.executable
        variables['distronode_config_file'] = C.CONFIG_FILE
        ...
        if play:
            ...
            # distronode_role_names includes all role names, dependent or directly referenced by the play
            variables['distronode_role_names'] = list(set(dependency_role_names + play_role_names))
            # distronode_play_role_names includes the names of all roles directly referenced by this play
            # roles that are implicitly referenced via dependencies are not listed.
            variables['distronode_play_role_names'] = play_role_names
            # distronode_dependent_role_names includes the names of all roles that are referenced via dependencies
            # dependencies that are also explicitly named as roles are included in this list
            variables['distronode_dependent_role_names'] = dependency_role_names

            # DEPRECATED: role_names should be deprecated in favor of distronode_role_names or distronode_play_role_names
            variables['role_names'] = variables['distronode_play_role_names']

            variables['distronode_play_name'] = play.get_name()
        ...
        if task:
            if task._role:
                ...
                variables['distronode_collection_name'] = task._role._role_collection
                variables['distronode_role_name'] = task._role.get_name()
        ...
        if self._inventory is not None:
            ...
            if play:
                ...
                variables['distronode_play_hosts_all'] = _hosts_all[:]
                variables['distronode_play_hosts'] = [x for x in variables['distronode_play_hosts_all'] if x not in play._removed_hosts]
                variables['distronode_play_batch'] = [x for x in _hosts if x not in play._removed_hosts]

                # DEPRECATED: play_hosts should be deprecated in favor of distronode_play_batch,
                # however this would take work in the templating engine, so for now we'll add both
                variables['play_hosts'] = variables['distronode_play_batch']
    ...
    def get_delegated_vars_and_hostname(self, templar, task, variables):
        ...
        if task.delegate_to:
            ...
            delegated_vars['distronode_delegated_vars'] = {
            ...
            delegated_vars['distronode_delegated_vars'][delegated_host_name]['inventory_hostname'] = variables.get('inventory_hostname')
    ...
    def _get_delegated_vars(self, play, task, existing_variables):
        ...
        vars_copy['distronode_search_path'] = task.get_search_path()

        # ensure basedir is always in (dwim already searches here but we need to display it)
        if self._loader.get_basedir() not in vars_copy['distronode_search_path']:
            vars_copy['distronode_search_path'].append(self._loader.get_basedir())
        ...
        if task.loop_with is not None:
            if task.loop_with in lookup_loader:
                ...
                except DistronodeTemplateError:
                    # This task will be skipped later due to this, so we just setup
                    # a dummy array for the later code so it doesn't fail
                    items = [None]
            else:
                raise DistronodeError("Failed to find the lookup named '%s' in the available lookup plugins" % task.loop_with)
        ...
        for item in items:
            ...
            if delegated_host_name is None:
                raise DistronodeError(message="Undefined delegate_to host for task:", obj=task._ds)
            if not isinstance(delegated_host_name, string_types):
                raise DistronodeError(message="the field 'delegate_to' has an invalid type (%s), and could not be"
        ...
        if has_loop and cache_items:
            ...
            _distronode_loop_cache = items

        return delegated_host_vars, _distronode_loop_cache
    ...
    def set_host_facts(self, host, facts):
        ...
        if not isinstance(facts, Mapping):
            raise DistronodeAssertionError("the type of 'facts' to set for host_facts should be a Mapping but is a %s" % type(facts))
    ...
    def set_nonpersistent_facts(self, host, facts):
        ...
        if not isinstance(facts, Mapping):
            raise DistronodeAssertionError("the type of 'facts' to set for nonpersistent_facts should be a Mapping but is a %s" % type(facts))
