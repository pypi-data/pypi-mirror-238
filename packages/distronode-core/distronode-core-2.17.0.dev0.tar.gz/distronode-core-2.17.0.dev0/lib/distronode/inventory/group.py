...
def to_safe_group_name(name, replacer="_", force=False, silent=False):
    # Converts 'bad' characters in a string to underscores (or provided replacer) so they can be used as Distronode hosts or groups
...
class Group:
    ...
    def add_child_group(self, group):
        ...
        if group not in self.child_groups:
            ...
            if group in new_ancestors:
                raise DistronodeError("Adding group '%s' as child to '%s' creates a recursive dependency loop." % (to_native(group.name), to_native(self.name)))
    ...
    def _check_children_depth(self):
        ...
        while unprocessed:
            ...
            if depth - start_depth > len(seen):
                raise DistronodeError("The group named '%s' has a recursive dependency loop." % to_native(self.name))
    ...
    def set_variable(self, key, value):
        ...
        if key == 'distronode_group_priority':
