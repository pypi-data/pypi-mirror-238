...
def _validate_mutable_mappings(a, b):
    ...
    :raises DistronodeError: if one of the arguments is not a MutableMapping
    """
    ...
    if not (isinstance(a, MutableMapping) and isinstance(b, MutableMapping)):
        ...
        raise DistronodeError("failed to combine variables, expected dicts but got a '{0}' and a '{1}': \n{2}\n{3}".format(
...
def merge_hash(x, y, recursive=True, list_merge='replace'):
    ...
    if list_merge not in ('replace', 'keep', 'append', 'prepend', 'append_rp', 'prepend_rp'):
        raise DistronodeError("merge_hash: 'list_merge' argument can only be equal to 'replace', 'keep', 'append', 'prepend', 'append_rp' or 'prepend_rp'")
    ...
    if not getattr(load_extra_vars, 'extra_vars', None):
        ...
        for extra_vars_opt in context.CLIARGS.get('extra_vars', tuple()):
            ...
            if extra_vars_opt.startswith(u"@"):
                ...
                raise DistronodeOptionsError("Please prepend extra_vars filename '%s' with '@'" % extra_vars_opt)
            elif extra_vars_opt[0] in [u'[', u'{']:
                # Arguments as YAML
            ...
            if isinstance(data, MutableMapping):
                ...
                raise DistronodeOptionsError("Invalid extra vars data supplied. '%s' could not be made into a dictionary" % extra_vars_opt)
...
def load_options_vars(version):
    ...
    if not getattr(load_options_vars, 'options_vars', None):
        if version is None:
            version = 'Unknown'
        options_vars = {'distronode_version': version}
        ...
        for attr, alias in attrs.items():
            ...
            if opt is not None:
                options_vars['distronode_%s' % alias] = opt
