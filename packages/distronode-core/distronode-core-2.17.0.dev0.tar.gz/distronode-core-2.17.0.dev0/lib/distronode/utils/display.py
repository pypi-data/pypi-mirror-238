...
from distronode.errors import DistronodeError, DistronodeAssertionError, DistronodePromptInterrupt, DistronodePromptNoninteractive
...
from distronode.utils.multiprocessing import context as multiprocessing_context
from distronode.utils.singleton import Singleton
from distronode.utils.unsafe_proxy import wrap_var

if t.TYPE_CHECKING:
    # avoid circular import at runtime
    from distronode.executor.task_queue_manager import FinalQueue
...
if getattr(C, 'DEFAULT_LOG_PATH'):
    ...
    if path and (os.path.exists(path) and os.access(path, os.W_OK)) or os.access(os.path.dirname(path), os.W_OK):
        ...
        logger = logging.getLogger('distronode')
...
class Display(metaclass=Singleton):
    ...
    def __init__(self, verbosity: int = 0) -> None:
        ...
        self.noncow = C.DISTRONODE_COW_SELECTION
        ...
        if self.b_cowsay:
            try:
                ...
                if C.DISTRONODE_COW_ACCEPTLIST and any(C.DISTRONODE_COW_ACCEPTLIST):
                    self.cows_available = set(C.DISTRONODE_COW_ACCEPTLIST).intersection(self.cows_available)
            except Exception:
    ...
    def set_queue(self, queue: FinalQueue) -> None:
        ...
        This is only needed in distronode.executor.process.worker:WorkerProcess._run
        """
    ...
    def set_cowsay_info(self) -> None:
        if C.DISTRONODE_NOCOWS:
            return

        if C.DISTRONODE_COW_PATH:
            self.b_cowsay = C.DISTRONODE_COW_PATH
        else:
    ...
    def display(
        ...
        if not log_only:
            ...
            # code in 'bin/distronode-connection' (and other relevant places).
            if not stderr:
                fileobj = sys.stdout
            else:
                fileobj = sys.stderr
        ...
        if logger and not screen_only:
            ...
            if color:
                ...
                try:
                    ...
                    raise DistronodeAssertionError('Invalid color supplied to display: %s' % color)
            # actually log
            logger.log(lvl, msg2)
    ...
    def get_deprecation_message(
        ...
        if collection_name == 'distronode.builtin':
            collection_name = 'distronode-core'
        ...
        if removed:
            ...
            help_text = 'Deprecation warnings can be disabled by setting deprecation_warnings=False in distronode.cfg.'
    ...
    def deprecated(
        ...
        if removed:
            raise DistronodeError(message_text)
    ...
    def do_var_prompt(
        ...
        if encrypt:
            ...
            from distronode.utils.encrypt import do_encrypt
    ...
    def prompt_until(
        ...
        if self._final_q:
            from distronode.executor.process.worker import current_worker
        ...
        if (
            ...
            raise DistronodePromptNoninteractive('stdin is not interactive')
        ...
        #         raise DistronodePromptInterrupt('user interrupt')
