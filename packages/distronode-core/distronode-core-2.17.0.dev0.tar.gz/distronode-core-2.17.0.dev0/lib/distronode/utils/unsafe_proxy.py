...
__all__ = ['DistronodeUnsafe', 'wrap_var']


class DistronodeUnsafe(object):
    __UNSAFE__ = True


class DistronodeUnsafeBytes(binary_type, DistronodeUnsafe):
    def decode(self, *args, **kwargs):
        """Wrapper method to ensure type conversions maintain unsafe context"""
        return DistronodeUnsafeText(super(DistronodeUnsafeBytes, self).decode(*args, **kwargs))


class DistronodeUnsafeText(text_type, DistronodeUnsafe):
    def encode(self, *args, **kwargs):
        """Wrapper method to ensure type conversions maintain unsafe context"""
        return DistronodeUnsafeBytes(super(DistronodeUnsafeText, self).encode(*args, **kwargs))


class NativeJinjaUnsafeText(NativeJinjaText, DistronodeUnsafeText):
    pass
...
def wrap_var(v):
    if v is None or isinstance(v, DistronodeUnsafe):
        return v
    ...
    if isinstance(v, Mapping):
        ...
        v = DistronodeUnsafeBytes(v)
    elif isinstance(v, text_type):
        v = DistronodeUnsafeText(v)
