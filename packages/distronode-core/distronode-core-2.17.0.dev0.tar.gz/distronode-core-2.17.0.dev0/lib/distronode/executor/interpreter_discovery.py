...
from distronode.module_utils.facts.system.distribution import Distribution
...
def discover_interpreter(action, interpreter_name, discovery_mode, task_vars):
    ...
    try:
        ...
        platform_script = pkgutil.get_data('distronode.executor.discovery', 'python_target.py')
            ...
            if platform_interpreter != u'/usr/bin/python' and u'/usr/bin/python' in found_interpreters:
                if not is_silent:
                    action._discovery_warnings.append(
                        ...
                        u"/usr/bin/python for backward compatibility with prior Distronode releases. "
