...
class PSModuleDepFinder(object):
    ...
    def __init__(self):
        ...
        self._re_cs_module = [
            ...
            # 'using distronode_collections.{namespace}.{collection}.plugins.module_utils.{name}'
            re.compile(to_bytes(r'(?i)^using\s((Distronode\..+)|'
                                r'(distronode_collections\.\w+\.\w+\.plugins\.module_utils\.[\w\.]+));\s*$')),
        ]

        self._re_cs_in_ps_module = [
            # Reference C# module_util in a PowerShell module
            # '#DistronodeRequires -CSharpUtil Distronode.{name}'
            # '#DistronodeRequires -CSharpUtil distronode_collections.{namespace}.{collection}.plugins.module_utils.{name}'
            # '#DistronodeRequires -CSharpUtil ..module_utils.{name}'
            # Can have '-Optional' at the end to denote the util is optional
            re.compile(to_bytes(r'(?i)^#\s*distronoderequires\s+-csharputil\s+((Distronode\.[\w\.]+)|'
                                r'(distronode_collections\.\w+\.\w+\.plugins\.module_utils\.[\w\.]+)|'
                                r'(\.[\w\.]+))(?P<optional>\s+-Optional){0,1}')),
        ]

        self._re_ps_module = [
            # Original way of referencing a builtin module_util
            # '#Requires -Module Distronode.ModuleUtils.{name}
            re.compile(to_bytes(r'(?i)^#\s*requires\s+\-module(?:s?)\s*(Distronode\.ModuleUtils\..+)')),
            # New way of referencing a builtin and collection module_util
            # '#DistronodeRequires -PowerShell Distronode.ModuleUtils.{name}'
            # '#DistronodeRequires -PowerShell distronode_collections.{namespace}.{collection}.plugins.module_utils.{name}'
            # '#DistronodeRequires -PowerShell ..module_utils.{name}'
            # Can have '-Optional' at the end to denote the util is optional
            re.compile(to_bytes(r'(?i)^#\s*distronoderequires\s+-powershell\s+((Distronode\.ModuleUtils\.[\w\.]+)|'
                                r'(distronode_collections\.\w+\.\w+\.plugins\.module_utils\.[\w\.]+)|'
                                r'(\.[\w\.]+))(?P<optional>\s+-Optional){0,1}')),
        ]

        self._re_wrapper = re.compile(to_bytes(r'(?i)^#\s*distronoderequires\s+-wrapper\s+(\w*)'))
        self._re_ps_version = re.compile(to_bytes(r'(?i)^#requires\s+\-version\s+([0-9]+(\.[0-9]+){0,3})$'))
        self._re_os_version = re.compile(to_bytes(r'(?i)^#distronoderequires\s+\-osversion\s+([0-9]+(\.[0-9]+){0,3})$'))
        self._re_become = re.compile(to_bytes(r'(?i)^#distronoderequires\s+\-become$'))
    ...
    def scan_module(self, module_data, fqn=None, wrapper=False, powershell=True):
        ...
        if powershell:
            checks = [
                # PS module contains '#Requires -Module Distronode.ModuleUtils.*'
                # PS module contains '#DistronodeRequires -Powershell Distronode.*' (or collections module_utils ref)
                (self._re_ps_module, self.ps_modules, ".psm1"),
                # PS module contains '#DistronodeRequires -CSharpUtil Distronode.*' (or collections module_utils ref)
                (self._re_cs_in_ps_module, cs_utils, ".cs"),
            ]
        else:
            checks = [
                # CS module contains 'using Distronode.*;' or 'using distronode_collections.ns.coll.plugins.module_utils.*;'
    ...
    def scan_exec_script(self, name):
        # scans lib/distronode/executor/powershell for scripts used in the module
        # exec side. It also scans these scripts for any dependencies
        name = to_text(name)
        if name in self.exec_scripts.keys():
            return

        data = pkgutil.get_data("distronode.executor.powershell", to_native(name + ".ps1"))
        if data is None:
            raise DistronodeError("Could not find executor powershell script "
                               "for '%s'" % name)
    ...
    def _add_module(self, name, ext, fqn, optional, wrapper=False):
        ...
        if m.startswith("Distronode."):
            ...
            if not mu_path:
                ...
                raise DistronodeError('Could not find imported module support code '
                                   'for \'%s\'' % m)
            ...
            try:
                ...
                if len(resource_paths) != 1:
                    ...
                    raise DistronodeError("Internal error: Referenced module_util package '%s' contains 0 or multiple "
                ...
                if getattr(err, "errno", errno.ENOENT) == errno.ENOENT:
                    ...
                    raise DistronodeError('Could not find collection imported module support code for \'%s\''
                                       % to_native(m))
...
def _slurp(path):
    if not os.path.exists(path):
        raise DistronodeError("imported module support code does not exist at %s"
...
def _create_powershell_wrapper(b_module_data, module_path, module_args,
    ...
    if substyle != 'script':
        # don't scan the module for util dependencies and other Distronode related
    ...
    # make sure Distronode.ModuleUtils.AddType is added if any C# utils are used
    if len(finder.cs_utils_wrapper) > 0 or len(finder.cs_utils_module) > 0:
        finder._add_module(b"Distronode.ModuleUtils.AddType", ".psm1", None, False,
                           wrapper=False)
