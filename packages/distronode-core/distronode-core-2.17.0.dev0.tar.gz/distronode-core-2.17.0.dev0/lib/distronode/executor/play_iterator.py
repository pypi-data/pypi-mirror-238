...
from distronode.errors import DistronodeAssertionError
...
class PlayIterator:
    ...
    def __init__(self, inventory, play, play_context, variable_manager, all_vars, start_at_done=False):
        ...
        setup_task.resolved_action = 'distronode.builtin.gather_facts'
            ...
            if state.run_state == IteratingStates.SETUP:
                ...
                if not state.pending_setup:
                    ...
                    if (gathering == 'implicit' and implied) or \
                       ...
                       (gathering == 'smart' and implied and not (self._variable_manager._fact_cache.get(host.name, {}).get('_distronode_facts_gathered', False))):
    ...
    def set_state_for_host(self, hostname: str, state: HostState) -> None:
        if not isinstance(state, HostState):
            raise DistronodeAssertionError('Expected state to be a HostState but was a %s' % type(state))
    ...
    def set_run_state_for_host(self, hostname: str, run_state: IteratingStates) -> None:
        if not isinstance(run_state, IteratingStates):
            raise DistronodeAssertionError('Expected run_state to be a IteratingStates but was %s' % (type(run_state)))
    ...
    def set_fail_state_for_host(self, hostname: str, fail_state: FailedStates) -> None:
        if not isinstance(fail_state, FailedStates):
            raise DistronodeAssertionError('Expected fail_state to be a FailedStates but was %s' % (type(fail_state)))
