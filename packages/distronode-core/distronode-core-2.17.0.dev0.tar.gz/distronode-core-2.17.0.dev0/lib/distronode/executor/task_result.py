...
from distronode.vars.clean import module_response_deepcopy, strip_internal_keys

_IGNORE = ('failed', 'skipped')
_PRESERVE = ('attempts', 'changed', 'retries')
_SUB_PRESERVE = {'_distronode_delegated_vars': ('distronode_host', 'distronode_port', 'distronode_user', 'distronode_connection')}

# stuff callbacks need
CLEAN_EXCEPTIONS = (
    '_distronode_verbose_always',  # for debug and other actions, to always expand data (pretty jsonification)
    '_distronode_item_label',  # to know actual 'item' variable
    '_distronode_no_log',  # jic we didnt clean up well enough, DON'T LOG
    '_distronode_verbose_override',  # controls display of distronode_facts, gathering would be very noise with -v otherwise
)
...
class TaskResult:
    ...
    def clean_copy(self):
        ...
        if isinstance(self._task.no_log, bool) and self._task.no_log or self._result.get('_distronode_no_log', False):
