...
# installed beneath this one will be masked from the Distronode loader, and available from the front of sys.path.
# It is expected that the vendored packages will be loaded very early, so a warning will be fired on import of
# the top-level distronode package if any packages beneath this are already loaded at that point.
#
...
# mask vendored content below this package from being accessed as an distronode subpackage
__path__ = []
...
def _ensure_vendored_path_entry():
    ...
    if vendored_module_names:
        ...
        if already_loaded_vendored_modules:
            warnings.warn('One or more Python packages bundled by this distronode-core distribution were already '
