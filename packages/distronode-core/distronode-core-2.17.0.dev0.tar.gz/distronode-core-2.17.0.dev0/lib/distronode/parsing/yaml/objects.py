...
class DistronodeBaseYAMLObject(object):
    '''
    ...
    def _get_distronode_position(self):
        return (self._data_source, self._line_number, self._column_number)

    def _set_distronode_position(self, obj):
        try:
            (src, line, col) = obj
        except (TypeError, ValueError):
            raise AssertionError(
                'distronode_pos can only be set with a tuple/list '
    ...
    distronode_pos = property(_get_distronode_position, _set_distronode_position)


class DistronodeMapping(DistronodeBaseYAMLObject, dict):
    ''' sub class for dictionaries '''
    pass


class DistronodeUnicode(DistronodeBaseYAMLObject, text_type):
    ''' sub class for unicode objects '''
    pass


class DistronodeSequence(DistronodeBaseYAMLObject, list):
    ''' sub class for lists '''
    pass


class DistronodeVaultEncryptedUnicode(Sequence, DistronodeBaseYAMLObject):
    ...
    def from_plaintext(cls, seq, vault, secret):
        if not vault:
            raise vault.DistronodeVaultError('Error creating DistronodeVaultEncryptedUnicode, invalid vault (%s) provided' % vault)
    ...
    def __init__(self, ciphertext):
        '''A DistronodeUnicode with a Vault attribute that can decrypt it.
        ...
        '''
        super(DistronodeVaultEncryptedUnicode, self).__init__()
    ...
    def __lt__(self, string):
        if isinstance(string, DistronodeVaultEncryptedUnicode):
    ...
    def __contains__(self, char):
        if isinstance(char, DistronodeVaultEncryptedUnicode):
            char = char.data
    ...
    def __add__(self, other):
        if isinstance(other, DistronodeVaultEncryptedUnicode):
    ...
    def count(self, sub, start=0, end=_sys.maxsize):
        if isinstance(sub, DistronodeVaultEncryptedUnicode):
            sub = sub.data
    ...
    def replace(self, old, new, maxsplit=-1):
        if isinstance(old, DistronodeVaultEncryptedUnicode):
            old = old.data
        if isinstance(new, DistronodeVaultEncryptedUnicode):
            new = new.data
