...
string_to_vars = {
    ...
    'RETURN': 'returndocs',
    'DISTRONODE_METADATA': 'metadata',  # NOTE: now unused, but kept for backwards compat
}
...
def read_docstring_from_yaml_file(filename, verbose=True, ignore_errors=True):
    ...
    try:
        with open(filename, 'rb') as yamlfile:
            file_data = DistronodeLoader(yamlfile.read(), file_name=filename).get_single_data()
    except Exception as e:
        msg = "Unable to parse yaml file '%s': %s" % (filename, to_native(e))
        if not ignore_errors:
            raise DistronodeParserError(msg, orig_exc=e)
        elif verbose:
            display.error(msg)
        ...
        for token in tokens:
            ...
            if next_string is not None and token.type == tokenize.STRING:
                ...
                if next_string == 'plainexamples':
                    ...
                    # yaml load the data
                    try:
                        data[next_string] = DistronodeLoader(value, file_name=filename).get_single_data()
                    except Exception as e:
                    ...
                    if theid in string_to_vars:
                        ...
                        if isinstance(child.value, ast.Dict):
                            ...
                            if theid == 'EXAMPLES':
                                ...
                                data[varkey] = DistronodeLoader(child.value.value, file_name=filename).get_single_data()
...
def read_docstring(filename, verbose=True, ignore_errors=True):
    ''' returns a documentation dictionary from Distronode plugin docstrings '''
    ...
    if filename.endswith(C.YAML_DOC_EXTENSIONS):
        ...
        raise DistronodeError("Unknown documentation format: %s" % to_native(filename))

    if not docstring and not ignore_errors:
        raise DistronodeError("Unable to parse documentation for: %s" % to_native(filename))
...
def read_docstub(filename):
    ...
    operations like distronode-doc -l.
    """
    ...
    data = DistronodeLoader(short_description, file_name=filename).get_single_data()
