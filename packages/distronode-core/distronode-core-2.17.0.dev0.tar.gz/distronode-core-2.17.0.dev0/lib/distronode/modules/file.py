...
- Many other modules support the same options as the M(distronode.builtin.file) module - including M(distronode.builtin.copy),
  M(distronode.builtin.template), and M(distronode.builtin.assemble).
- For Windows targets, use the M(distronode.windows.win_file) module instead.
options:
  path:
    description:
  ...
  state:
    ...
    - If V(absent), directories will be recursively deleted, and files or symlinks will
      ...
      under C(path_contents). Note that V(absent) will not cause M(distronode.builtin.file) to fail if the O(path) does
      not exist as the state did not change.
    - If V(directory), all intermediate subdirectories will be created if they
      do not exist. Since Distronode 1.7 they will be created with the supplied permissions.
    ...
    - If V(file), even with other options (such as O(mode)), the file will be modified if it exists but will NOT be created if it does not exist.
      Set to V(touch) or use the M(distronode.builtin.copy) or M(distronode.builtin.template) module if you want to create the file if it does not exist.
  ...
  access_time_format:
    ...
    version_added: '2.7'
seealso:
- module: distronode.builtin.assemble
- module: distronode.builtin.copy
- module: distronode.builtin.stat
- module: distronode.builtin.template
- module: distronode.windows.win_file
attributes:
    check_mode:
        support: full
    diff_mode:
...
# There will only be a single DistronodeModule object per module
module = None


class DistronodeModuleError(Exception):
    def __init__(self, results):
        self.results = results

    def __repr__(self):
        return 'DistronodeModuleError(results={0})'.format(self.results)


class ParameterError(DistronodeModuleError):
    pass
...
def _distronode_excepthook(exc_type, exc_value, tb):
    # Using an exception allows us to catch it if the calling code knows it can recover
    if issubclass(exc_type, DistronodeModuleError):
...
def get_timestamp_for_time(formatted_time, time_format):
    if formatted_time == 'preserve':
        ...
        try:
            ...
            raise DistronodeModuleError(results={'msg': 'Error while obtaining timestamp for time %s using format %s: %s'
...
def update_timestamp_for_file(path, mtime, atime, diff=None):
    ...
    try:
        ...
        # https://github.com/distronode/distronode/issues/50943
        ...
        raise DistronodeModuleError(results={'msg': 'Error while updating modification or access time: %s'
        ...
        if not module.check_mode:
            if prev_state == 'directory':
                try:
                    ...
                    raise DistronodeModuleError(results={'msg': "rmtree failed: %s" % to_native(e)})
            else:
                try:
                    os.unlink(b_path)
                except OSError as e:
                    if e.errno != errno.ENOENT:  # It may already have been removed
                        raise DistronodeModuleError(results={'msg': "unlinking failed: %s " % to_native(e),
                                                          'path': path})
...
def execute_touch(path, follow, timestamps):
    ...
    if prev_state == 'absent':
        ...
        try:
            ...
            raise DistronodeModuleError(results={'msg': 'Error, could not touch target: %s'
...
def ensure_file_attributes(path, follow, timestamps):
    ...
    if prev_state not in ('file', 'hard'):
        ...
        raise DistronodeModuleError(results={'msg': 'file (%s) is %s, cannot continue' % (path, prev_state),
...
def ensure_directory(path, follow, recurse, timestamps):
    ...
    if prev_state == 'absent':
        ...
        try:
            ...
            raise DistronodeModuleError(results={'msg': 'There was an issue creating %s as requested:'
        ...
        raise DistronodeModuleError(results={'msg': '%s already exists as a %s' % (path, prev_state),
                                          'path': path})
...
def ensure_symlink(path, src, follow, force, timestamps):
    ...
    if not force and src is not None and not os.path.exists(b_absrc):
        raise DistronodeModuleError(results={'msg': 'src file does not exist, use "force=yes" if you'
                                                 ' really want to create the link: %s' % absrc,
                                          'path': path, 'src': src})

    if prev_state == 'directory':
        if not force:
            raise DistronodeModuleError(results={'msg': 'refusing to convert from %s to symlink for %s'
                                                     % (prev_state, path),
                                              'path': path})
        elif os.listdir(b_path):
            # refuse to replace a directory that has files in it
            raise DistronodeModuleError(results={'msg': 'the directory %s is not empty, refusing to'
                                                     ' convert it' % path,
                                              'path': path})
    ...
    if prev_state in ('hard', 'file', 'directory', 'absent'):
        if src is None:
            raise DistronodeModuleError(results={'msg': 'src is required for creating new symlinks'})
        changed = True
        ...
        raise DistronodeModuleError(results={'msg': 'unexpected position reached', 'dest': path, 'src': src})
        ...
        if prev_state != 'absent':
            ...
            try:
                ...
                if os.path.exists(b_tmppath):
                    os.unlink(b_tmppath)
                raise DistronodeModuleError(results={'msg': 'Error while replacing: %s'
                                                         % to_native(e, nonstring='simplerepr'),
                                                  'path': path})
        else:
            try:
                ...
                raise DistronodeModuleError(results={'msg': 'Error while linking: %s'
                                                         % to_native(e, nonstring='simplerepr'),
                                                  'path': path})
...
def ensure_hardlink(path, src, follow, force, timestamps):
    ...
    if prev_state != 'hard' and src is None:
        raise DistronodeModuleError(results={'msg': 'src is required for creating new hardlinks'})
    ...
    if src is not None and not os.path.exists(b_src):
        raise DistronodeModuleError(results={'msg': 'src does not exist', 'dest': path, 'src': src})
    ...
    if prev_state == 'absent':
        ...
        if src is not None and not os.stat(b_path).st_ino == os.stat(b_src).st_ino:
            changed = True
            if not force:
                raise DistronodeModuleError(results={'msg': 'Cannot link, different hard link exists at destination',
                                                  'dest': path, 'src': src})
    elif prev_state == 'file':
        changed = True
        if not force:
            raise DistronodeModuleError(results={'msg': 'Cannot link, %s exists at destination' % prev_state,
                                              'dest': path, 'src': src})
    elif prev_state == 'directory':
        changed = True
...
def main():
    ...
    # When we rewrite basic.py, we will do something similar to this on instantiating an DistronodeModule
    sys.excepthook = _distronode_excepthook
