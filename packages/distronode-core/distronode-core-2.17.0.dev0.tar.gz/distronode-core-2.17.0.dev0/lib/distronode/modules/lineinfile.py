...
description:
  ...
  - See the M(distronode.builtin.replace) module if you want to change multiple, similar lines
    or check M(distronode.builtin.blockinfile) if you want to insert/update/remove a block of lines in a file.
    For other cases, see the M(distronode.builtin.copy) or M(distronode.builtin.template) modules.
version_added: "0.7"
options:
  path:
    description:
...
seealso:
- module: distronode.builtin.blockinfile
- module: distronode.builtin.copy
- module: distronode.builtin.file
- module: distronode.builtin.replace
