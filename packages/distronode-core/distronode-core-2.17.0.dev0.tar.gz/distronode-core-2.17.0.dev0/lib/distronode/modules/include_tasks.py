...
options:
  ...
  free-form:
    description:
      ...
      - Most keywords, including loop, with_items, and conditionals, apply to this statement unlike M(distronode.builtin.import_tasks).
...
- hosts: all
  tasks:
    ...
    - name: Include task list in play only if the condition is true
      distronode.builtin.include_tasks: "{{ hostvar }}.yaml"
...
- name: Apply tags to tasks within included file when using free-form
  distronode.builtin.include_tasks: install.yml
  args:
    apply:
      tags:
        - install
