...
description:
  ...
  - May be used only where Distronode tasks are allowed - inside C(pre_tasks), C(tasks), or C(post_tasks) play objects, or as a task inside a role.
  - Task-level keywords, loops, and conditionals apply only to the C(include_role) statement itself.
  - To apply keywords to the tasks within the role, pass them using the O(apply) option or use M(distronode.builtin.import_role) instead.
  ...
  public:
    description:
      - This option dictates whether the role's C(vars) and C(defaults) are exposed to the play. If set to V(true)
        ...
        standard variable exposure for roles listed under the C(roles) header or M(distronode.builtin.import_role) as they are exposed
...
notes:
  ...
  - After Distronode 2.4, you can use M(distronode.builtin.import_role) for B(static) behaviour and this action for B(dynamic) one.
seealso:
