...
- name: Checkout a github repo and use refspec to fetch all pull requests
  distronode.builtin.git:
    repo: https://github.com/distronode/distronode-examples.git
    dest: /src/distronode-examples
...
- name: Create git archive from repo
  distronode.builtin.git:
    repo: https://github.com/distronode/distronode-examples.git
    dest: /src/distronode-examples
    archive: /tmp/distronode-examples.zip

- name: Clone a repo with separate git directory
  distronode.builtin.git:
    repo: https://github.com/distronode/distronode-examples.git
    dest: /src/distronode-examples
    separate_git_dir: /src/distronode-examples.git
...
- name: Avoid hanging when http(s) password is missing
  distronode.builtin.git:
    repo: https://github.com/distronode/could-be-a-private-repo
...
from distronode.module_utils.six import b, string_types
...
def head_splitter(headfile, remote, module=None, fail_on_error=False):
    ...
    # https://github.com/distronode/distronode-modules-core/pull/907
...
def unfrackgitpath(path):
    ...
    # copied from distronode.utils.path
...
def get_head_branch(git_path, module, dest, remote, bare=False):
    ...
    taken from lib/distronode/utils/__init__.py.  It finds the correct
