...
options:
  follow:
    ...
    - Previous to Distronode 2.4, this was hardcoded as V(true).
    type: bool
    default: no
    version_added: '2.4'
notes:
- For Windows you can use M(distronode.windows.win_template) which uses V(\\r\\n) as O(newline_sequence) by default.
- The C(jinja2_native) setting has no effect. Native types are never used in the M(distronode.builtin.template) module
  which is by design used for generating text files. For working with templates and utilizing Jinja2 native types see
  the O(distronode.builtin.template#lookup:jinja2_native) parameter of the P(distronode.builtin.template#lookup) lookup.
seealso:
...
- name: Copy a version of named.conf that is dependent on the OS. setype obtained by doing ls -Z /etc/named.conf on original file
  distronode.builtin.template:
    src: named.conf_{{ distronode_os_family }}.j2
    dest: /etc/named.conf
    group: named
    setype: named_conf_t
    mode: '0640'
