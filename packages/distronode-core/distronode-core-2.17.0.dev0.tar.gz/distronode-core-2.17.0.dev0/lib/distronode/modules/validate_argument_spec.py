...
options:
  argument_spec:
    description:
        - A dictionary like DistronodeModule argument_spec. See R(argument spec definition,argument_spec)
    required: true
  provided_arguments:
    description:
...
- name: verify vars needed for this task file are present when included
  distronode.builtin.validate_argument_spec:
...
- name: verify vars needed for this task file are present when included, with spec from a spec file
  distronode.builtin.validate_argument_spec:
        argument_spec: "{{(lookup('distronode.builtin.file', 'myargspec.yml') | from_yaml )['specname']['options']}}"


- name: verify vars needed for next include and not from inside it, also with params i'll only define there
  block:
    - distronode.builtin.validate_argument_spec:
        argument_spec: "{{lookup('distronode.builtin.file', 'nakedoptions.yml'}}"
        provided_arguments:
