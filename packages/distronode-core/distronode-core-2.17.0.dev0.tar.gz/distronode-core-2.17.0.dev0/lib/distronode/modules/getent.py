...
- name: Get root user info
  distronode.builtin.getent:
    database: passwd
    key: root
- distronode.builtin.debug:
    var: distronode_facts.getent_passwd
...
- name: Get all groups
  distronode.builtin.getent:
    ...
    split: ':'
- distronode.builtin.debug:
    var: distronode_facts.getent_group

- name: Get all hosts, split by tab
  distronode.builtin.getent:
    database: hosts
- distronode.builtin.debug:
    var: distronode_facts.getent_hosts
...
- name: Get http service info, no error if missing
  distronode.builtin.getent:
    ...
    fail_key: False
- distronode.builtin.debug:
    var: distronode_facts.getent_services
...
- name: Get user password hash (requires sudo/root)
  distronode.builtin.getent:
    ...
    split: ':'
- distronode.builtin.debug:
    var: distronode_facts.getent_shadow
...
def main():
    ...
    if rc == 0:
        ...
        module.exit_json(distronode_facts=results)
        ...
        if not fail_key:
            ...
            module.exit_json(distronode_facts=results, msg=msg)
    elif rc == 3:
