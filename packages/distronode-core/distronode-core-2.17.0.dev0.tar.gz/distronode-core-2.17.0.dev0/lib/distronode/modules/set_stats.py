...
# Copyright: (c) 2016, Distronode RedHat, Inc
...
---
module: set_stats
short_description: Define and display stats for the current distronode run
description:
     - This module allows setting/accumulating stats on the current distronode run, either per host or for all hosts in the run.
...
attributes:
    ...
    diff_mode:
        support: none
notes:
    - In order for custom stats to be displayed, you must set C(show_custom_stats) in section C([defaults]) in C(distronode.cfg)
      or by defining environment variable C(DISTRONODE_SHOW_CUSTOM_STATS) to V(true). See the P(distronode.builtin.default#callback) callback plugin for details.
version_added: "2.3"
'''

EXAMPLES = r'''
- name: Aggregating packages_installed stat per host
  distronode.builtin.set_stats:
    data:
...
- name: Aggregating random stats for all hosts using complex arguments
  distronode.builtin.set_stats:
    data:
      ...
      another_stat: "{{ some_registered_var.results | map(attribute='distronode_facts.some_fact') | list }}"
    per_host: no
