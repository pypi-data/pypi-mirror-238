...
description:
     ...
     - In Distronode 1.6 and later, this module can also be used to wait for a file to be available or
       absent on the filesystem.
     - In Distronode 1.8 and later, this module can also be used to wait for active connections to be closed before continuing, useful if a node
       is being rotated out of a load balancer pool.
     - For Windows targets, use the M(distronode.windows.win_wait_for) module instead.
version_added: "0.7"
options:
  host:
    description:
  ...
  sleep:
    description:
      ...
      - Before Distronode 2.3 this was hardcoded to 1 second.
    type: int
    default: 1
    version_added: "2.3"
  msg:
...
attributes:
    ...
    platform:
        platforms: posix
notes:
  - The ability to use search_regex with a port connection was added in Distronode 1.7.
  - Prior to Distronode 2.4, testing for the absence of a directory or UNIX socket did not work correctly.
  - Prior to Distronode 2.4, testing for the presence of a file did not work correctly if the remote user did not have read access to that file.
...
seealso:
- module: distronode.builtin.wait_for_connection
- module: distronode.windows.win_wait_for
...
- name: Wait 300 seconds for port 22 to become open and contain "OpenSSH"
  distronode.builtin.wait_for:
    port: 22
    host: '{{ (distronode_ssh_host|default(distronode_host))|default(inventory_hostname) }}'
    search_regex: OpenSSH
    delay: 10
  connection: local

# Same as above but you normally have distronode_connection set in inventory, which overrides 'connection'
- name: Wait 300 seconds for port 22 to become open and contain "OpenSSH"
  distronode.builtin.wait_for:
    port: 22
    host: '{{ (distronode_ssh_host|default(distronode_host))|default(inventory_hostname) }}'
...
from distronode.module_utils.compat.datetime import utcnow
