...
options:
  ...
  marker:
    ...
    type: str
    default: '# {mark} DISTRONODE MANAGED BLOCK'
  block:
    description:
  ...
  insertafter:
    ...
    - The presence of the multiline flag (?m) in the regular expression controls whether the match is done line by line or with multiple lines.
      This behaviour was added in distronode-core 2.14.
    type: str
    choices: [ EOF, '*regex*' ]
    default: EOF
  insertbefore:
  ...
  marker_begin:
    description:
    - This will be inserted at C({mark}) in the opening distronode block O(marker).
    type: str
    default: BEGIN
    version_added: '2.5'
  marker_end:
    required: false
    description:
    - This will be inserted at C({mark}) in the closing distronode block O(marker).
    type: str
    default: END
    version_added: '2.5'
  append_newline:
...
- name: Insert/Update configuration using a local file and validate it
  distronode.builtin.blockinfile:
    block: "{{ lookup('distronode.builtin.file', './local/sshd_config') }}"
    path: /etc/ssh/sshd_config
    backup: yes
...
- name: Insert/Update HTML surrounded by custom markers after <body> line
  distronode.builtin.blockinfile:
    ...
    marker: "<!-- {mark} DISTRONODE MANAGED BLOCK -->"
    insertafter: "<body>"
    block: |
      <h1>Welcome to {{ distronode_hostname }}</h1>
      <p>Last updated on {{ distronode_date_time.iso8601 }}</p>
...
- name: Add mappings to /etc/hosts
  distronode.builtin.blockinfile:
    ...
    block: |
      {{ item.ip }} {{ item.name }}
    marker: "# {mark} DISTRONODE MANAGED BLOCK {{ item.name }}"
  loop:
...
import os
import tempfile
from distronode.module_utils.six import b
...
def main():
    module = DistronodeModule(
        argument_spec=dict(
            ...
            marker=dict(type='str', default='# {mark} DISTRONODE MANAGED BLOCK'),
