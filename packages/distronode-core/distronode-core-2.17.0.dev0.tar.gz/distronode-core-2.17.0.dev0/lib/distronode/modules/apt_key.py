...
notes:
    ...
    - Adding a new key requires an apt cache update (e.g. using the M(distronode.builtin.apt) module's update_cache option).
requirements:
    - gpg
seealso:
  - module: distronode.builtin.deb822_repository
options:
    id:
        description:
...
- name: One way to avoid apt_key once it is removed from your distro, armored keys should use .asc extension, binary should use .gpg
  block:
    ...
    - name: somerepo | apt source
      distronode.builtin.apt_repository:
        repo: "deb [arch=amd64 signed-by=/etc/apt/keyrings/myrepo.asc] https://download.example.com/linux/ubuntu {{ distronode_distribution_release }} stable"
        state: present
...
- name: Add a key from a file on the Distronode server
  distronode.builtin.apt_key:
    data: "{{ lookup('distronode.builtin.file', 'apt.asc') }}"
    state: present
...
from distronode.module_utils.urls import fetch_url
