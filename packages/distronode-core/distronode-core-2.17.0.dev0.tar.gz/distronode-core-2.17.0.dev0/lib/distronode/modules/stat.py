...
description:
     ...
     - For Windows targets, use the M(distronode.windows.win_stat) module instead.
options:
  path:
    description:
  ...
  get_mime:
    description:
      ...
      - In Distronode 2.3 this option changed from O(mime) to O(get_mime) and the default changed to V(true).
    type: bool
    default: yes
...
- module: distronode.windows.win_stat
