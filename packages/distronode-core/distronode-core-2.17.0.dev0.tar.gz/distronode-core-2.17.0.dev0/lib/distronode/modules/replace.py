...
options:
  path:
    description:
      - The file to modify.
      - Before Distronode 2.3 this option was only usable as O(dest), O(destfile) and O(name).
    type: path
    required: true
  ...
  regexp:
    description:
      ...
      - Note that, as of Distronode 2.0, short form tasks should have any escape
  ...
  others:
    description:
      - All arguments accepted by the M(distronode.builtin.file) module also work here.
    type: str
  encoding:
    description:
...
notes:
  ...
  - As of Distronode 2.7.10, the combined use of O(before) and O(after) works properly. If you were relying on the
    previous incorrect behavior, you may be need to adjust your tasks.
    See U(https://github.com/distronode/distronode/issues/31354) for details.
...
EXAMPLES = r'''
- name: Replace old hostname with new hostname (requires Distronode >= 2.4)
  distronode.builtin.replace:
    path: /etc/hosts
    regexp: '(\s+)old\.host\.name(\s+.*)?$'
    replace: '\1new.host.name\2'

- name: Replace after the expression till the end of the file (requires Distronode >= 2.4)
  distronode.builtin.replace:
...
- name: Replace before the expression till the begin of the file (requires Distronode >= 2.4)
  distronode.builtin.replace:
    path: /etc/apache2/sites-available/default.conf
...
# Prior to Distronode 2.7.10, using before and after in combination did the opposite of what was intended.
# see https://github.com/distronode/distronode/issues/31354 for details.
- name: Replace between the expressions (requires Distronode >= 2.4)
  distronode.builtin.replace:
    path: /etc/hosts
    after: '<VirtualHost [*]>'
    before: '</VirtualHost>'
...
- name: Short form task (in distronode 2+) necessitates backslash-escaped sequences
  distronode.builtin.replace: path=/etc/hosts regexp='\\b(localhost)(\\d*)\\b' replace='\\1\\2.localdomain\\2 \\1\\2'
