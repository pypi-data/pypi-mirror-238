...
- name: Gather the package facts
  distronode.builtin.package_facts:
    manager: auto

- name: Print the package facts
  distronode.builtin.debug:
    var: distronode_facts.packages

- name: Check whether a package called foobar is installed
  distronode.builtin.debug:
    msg: "{{ distronode_facts.packages['foobar'] | length }} versions of foobar are installed!"
  when: "'foobar' in distronode_facts.packages"

'''

RETURN = '''
distronode_facts:
  description: Facts to add to distronode_facts.
  returned: always
  type: complex
  contains:
    packages:
...
from distronode.module_utils.facts.packages import LibMgr, CLIMgr, get_all_pkg_managers
...
def main():
    ...
    # start work
    global module
    module = DistronodeModule(argument_spec=dict(manager={'type': 'list', 'elements': 'str', 'default': ['auto']},
                                              strategy={'choices': ['first', 'all'], 'default': 'first'}),
                           supports_check_mode=True)
    packages = {}
    results = {'distronode_facts': {}}
    ...
    # Set the facts, this will override the facts in distronode_facts that might exist from previous runs
    # when using operating system level or distribution package managers
    results['distronode_facts']['packages'] = packages
