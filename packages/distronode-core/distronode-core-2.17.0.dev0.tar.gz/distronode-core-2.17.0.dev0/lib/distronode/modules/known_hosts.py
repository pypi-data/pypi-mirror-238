...
description:
   - The M(distronode.builtin.known_hosts) module lets you add or remove a host keys from the C(known_hosts) file.
   - Starting at Distronode 2.2, multiple entries per host are allowed, but only one for each key type supported by ssh.
     This is useful if you're going to want to use the M(distronode.builtin.git) module over ssh, for example.
   - If you have a very large number of host keys to manage, you will find the M(distronode.builtin.template) module more useful.
version_added: "1.9"
options:
  name:
    aliases: [ 'host' ]
...
- name: Tell the host about our servers it might want to ssh to
  distronode.builtin.known_hosts:
    path: /etc/ssh/ssh_known_hosts
    name: foo.com.invalid
    key: "{{ lookup('distronode.builtin.file', 'pubkeys/foo.com.invalid') }}"
