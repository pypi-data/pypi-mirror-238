...
notes:
    - This module does B(NOT) modify C(/etc/hosts). You need to modify it yourself using other modules such as M(distronode.builtin.template)
      or M(distronode.builtin.replace).
...
from distronode.module_utils.facts.utils import get_file_lines, get_file_content
from distronode.module_utils.common.text.converters import to_native, to_text
from distronode.module_utils.six import PY3, text_type
...
def main():
    ...
    kw = dict(changed=changed, name=name,
              distronode_facts=dict(distronode_hostname=name.split('.')[0],
                                 distronode_nodename=name,
                                 distronode_fqdn=socket.getfqdn(),
                                 distronode_domain='.'.join(socket.getfqdn().split('.')[1:])))
