...
from distronode.config.manager import ConfigManager, Setting
...
from distronode.parsing.quoting import is_quoted
...
def yaml_dump(data, default_flow_style=False, default_style=None):
    return yaml.dump(data, Dumper=DistronodeDumper, default_flow_style=default_flow_style, default_style=default_style)
...
class ConfigCLI(CLI):
    ...
    def init_parser(self):
        ...
        super(ConfigCLI, self).init_parser(
            desc="View distronode configuration.",
        )
    ...
    def run(self):
        ...
        if context.CLIARGS['config_file']:
            ...
            if os.path.exists(b_config) and os.access(b_config, os.R_OK):
                ...
                raise DistronodeOptionsError('The provided configuration file is missing or not accessible: %s' % to_native(self.config_file))
        else:
        ...
        if self.config_file:
            try:
                if not os.path.exists(self.config_file):
                    raise DistronodeOptionsError("%s does not exist or is not accessible" % (self.config_file))
                elif not os.path.isfile(self.config_file):
                    raise DistronodeOptionsError("%s is not a valid file" % (self.config_file))

                os.environ['DISTRONODE_CONFIG'] = to_native(self.config_file)
            except Exception:
            ...
            raise DistronodeError('Invalid or no config file was supplied')
    ...
    def execute_update(self):
        '''
        Updates a single setting in the specified distronode.cfg
        '''
        raise DistronodeError("Option not implemented yet")

        # pylint: disable=unreachable
        if context.CLIARGS['setting'] is None:
            raise DistronodeOptionsError("update option requires a setting to update")
    ...
    def execute_view(self):
        ...
        try:
            ...
            raise DistronodeError("Failed to open config file: %s" % to_native(e))

    def execute_edit(self):
        '''
        Opens distronode.cfg in the default EDITOR
        '''
        raise DistronodeError("Option not implemented yet")
        ...
        try:
            ...
            raise DistronodeError("Failed to open editor: %s" % to_native(e))
    ...
    def _get_settings_vars(self, settings, subkey):
        ...
        for setting in settings:
            ...
            if subkey in settings[setting] and settings[setting][subkey]:
                ...
                if subkey == 'env':
                    if entry.startswith('_DISTRONODE_'):
                        continue
    ...
    def execute_dump(self):
        '''
        Shows the current settings, merges distronode.cfg if specified
        '''
        if context.CLIARGS['type'] == 'base':
            # deal with base
