...
from distronode.module_utils.common.warnings import warn
from distronode.module_utils.errors import (
    AliasError,
    DistronodeFallbackNotFound,
...
from distronode.module_utils.parsing.convert_bool import BOOLEANS_FALSE, BOOLEANS_TRUE
...
PASS_VARS = {
    ...
    'version': ('distronode_version', '0.0'),
}
...
def _validate_elements(wanted_type, parameter, values, options_context=None, errors=None):
    ...
    if errors is None:
        errors = DistronodeValidationErrorMultiple()
...
def env_fallback(*args, **kwargs):
    ...
    raise DistronodeFallbackNotFound
...
def set_fallbacks(argument_spec, parameters):
    ...
    for param, value in argument_spec.items():
        ...
        if param not in parameters and fallback_strategy is not None:
            ...
            except DistronodeFallbackNotFound:
                continue
            else:
    ...
    while deferred_removals:
        ...
        if isinstance(new_data, Mapping):
            for old_key, old_elem in old_data.items():
                if old_key in ignore_keys or old_key.startswith('_distronode'):
...
def remove_values(value, no_log_strings):
    ...
    large amounts of data (see `issue #24560 <https://github.com/distronode/distronode/issues/24560>`_).
    """
