...
Function Get-DistronodeWebRequest {
    <#
    .SYNOPSIS
    Creates a System.Net.WebRequest object based on common URL module options in Distronode.

    .DESCRIPTION
    Will create a WebRequest based on common input options within Distronode. This can be used manually or with
    ...
    .PARAMETER Module
    The DistronodeBasic module that can be used as a backup parameter source or a way to return warnings back to the
    Distronode controller.

    .EXAMPLE
    $spec = @{
        options = @{}
    }
    $module = Distronode.Basic.DistronodeModule]::Create($args, $spec, @(Get-DistronodeWebRequestSpec))

    $web_request = Get-DistronodeWebRequest -Module $module
    #>
    [CmdletBinding()]
    ...
    Param (
        ...
        [ValidateScript({ $_.GetType().FullName -eq 'Distronode.Basic.DistronodeModule' })]
        [System.Object]
        $Module
    )
    ...
    if ($web_request -is [System.Net.HttpWebRequest]) {
        ...
        if ($Headers -and $Headers.ContainsKey("User-Agent")) {
            if ($HttpAgent -eq $distronode_web_request_options.http_agent.default) {
...
Function Invoke-WithWebRequest {
    ...
    .PARAMETER Module
    The Distronode.Basic module to set the return values for. This will set the following return values;
    ...
    .PARAMETER Request
    The System.Net.WebRequest to call. This can either be manually crafted or created with Get-DistronodeWebRequest.
...
Function Get-DistronodeWebRequestSpec {
    <#
    .SYNOPSIS
    Used by modules to get the argument spec fragment for DistronodeModule.
    ...
    $module = [Distronode.Basic.DistronodeModule]::Create($args, $spec, @(Get-DistronodeWebRequestSpec))
    #>
    @{ options = $distronode_web_request_options }
}

# See lib/distronode/plugins/doc_fragments/url_windows.py
# Kept here for backwards compat as this variable was added in Distronode 2.9. Ultimately this util should be removed
# once the deprecation period has been added.
$distronode_web_request_options = @{
    method = @{ type = "str" }
    follow_redirects = @{ type = "str"; choices = @("all", "none", "safe"); default = "safe" }
    headers = @{ type = "dict" }
    http_agent = @{ type = "str"; default = "distronode-httpget" }
...
$export_members = @{
    Function = "Get-DistronodeWebRequest", "Get-DistronodeWebRequestSpec", "Invoke-WithWebRequest"
    Variable = "distronode_web_request_options"
}
