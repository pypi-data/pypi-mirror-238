...
Function Exit-Module {
    ...
    [Distronode.Basic.DistronodeModule]::Exit = { param([Int32]$rc) exit $rc }
...
[Distronode.Basic.DistronodeModule]::Exit = {
    param([Int32]$rc)
...
[Distronode.Basic.DistronodeModule]::WriteLine = {
    param([String]$line)
...
$tests = @{
    "Empty spec and no options - args file" = {
        ...
        [System.IO.File]::WriteAllText($args_file, '{ "DISTRONODE_MODULE_ARGS": {} }')
        $m = [Distronode.Basic.DistronodeModule]::Create(@($args_file), @{})
        ...
        $m.DistronodeVersion | Assert-Equal -Expected $null
    }

    "Empty spec and no options - complex_args" = {
        Set-Variable -Name complex_args -Scope Global -Value @{}
        $m = [Distronode.Basic.DistronodeModule]::Create(@(), @{})
...
{
    "DISTRONODE_MODULE_ARGS": {
        "_distronode_check_mode": true,
        "_distronode_debug": true,
        "_distronode_diff": true,
        "_distronode_keep_remote_files": true,
        "_distronode_module_name": "distronode_basic_tests",
        "_distronode_no_log": true,
        "_distronode_remote_tmp": "%TEMP%",
        "_distronode_selinux_special_fs": "ignored",
        "_distronode_shell_executable": "ignored",
        "_distronode_socket": "ignored",
        "_distronode_syslog_facility": "ignored",
        "_distronode_tmpdir": "$($m_tmpdir -replace "\\", "\\")",
        "_distronode_verbosity": 3,
        "_distronode_version": "2.8.0"
    }
}
"@)
        $m = [Distronode.Basic.DistronodeModule]::Create(@($args_file), @{supports_check_mode = $true })
        ...
        $m.ModuleName | Assert-Equal -Expected "distronode_basic_tests"
        $m.NoLog | Assert-Equal -Expected $true
        $m.Verbosity | Assert-Equal -Expected 3
        $m.DistronodeVersion | Assert-Equal -Expected "2.8.0"
    ...
    "Internal param changes - complex_args" = {
        ...
        Set-Variable -Name complex_args -Scope Global -Value @{
            ...
            _distronode_socket = "ignored"
            _distronode_syslog_facility = "ignored"
            _distronode_tmpdir = $m_tmpdir.ToString()
            _distronode_verbosity = 3
            _distronode_version = "2.8.0"
        }
        $spec = @{
        ...
        $m = [Distronode.Basic.DistronodeModule]::Create(@(), $spec)
        $m.CheckMode | Assert-Equal -Expected $true
        $m.DebugMode | Assert-Equal -Expected $true
        $m.DiffMode | Assert-Equal -Expected $true
        $m.KeepRemoteFiles | Assert-Equal -Expected $true
    ...
    "Parse complex module options" = {
        ...
        $jsonValue = ([Distronode.Basic.DistronodeModule]::FromJson('{"a":"a","b":1,"c":["a","b"]}'))
        [Distronode.Basic.DistronodeModule]::FromJson($m.Params.json_type_dict) | Assert-DictionaryEqual -Expected $jsonValue
        ...
        try {
            ...
            $actual = [Distronode.Basic.DistronodeModule]::FromJson($_.Exception.InnerException.Output)
        }
    ...
    "Parse module args with case insensitive input" = {
        ...
        Set-Variable -Name complex_args -Scope Global -Value @{
            _distronode_module_name = "win_test"
            Option1 = "1"
        }
        ...
        $expected_warnings += "Module options will become case sensitive in a future Distronode release. "
    ...
    "No log values" = {
        ...
        Set-Variable -Name complex_args -Scope Global -Value @{
            _distronode_module_name = "test_no_log"
...
test_no_log - Invoked with:
  ...
  password: VALUE_SPECIFIED_IN_NO_LOG_PARAMETER
'@
        $actual_event = (Get-EventLog -LogName Application -Source Distronode -Newest 1).Message
    ...
    "Removed in version" = {
        $spec = @{
            options = @{
                ...
                removed3 = @{removed_in_version = "2.3"; removed_from_collection = "distronode.builtin" }
            }
        }
        Set-Variable -Name complex_args -Scope Global -Value @{
            removed1 = "value"
    ...
    "Removed at date" = {
        $spec = @{
            options = @{
                ...
                removed3 = @{removed_at_date = [DateTime]"2020-06-07"; removed_from_collection = "distronode.builtin" }
            }
        }
        Set-Variable -Name complex_args -Scope Global -Value @{
            removed1 = "value"
        ...
        $spec = @{
            options = @{
                ...
                option3 = @{
                    ...
                    options = @{
                        ...
                        option3 = @{
                            ...
                            aliases = "alias3"
                            deprecated_aliases = @(
                                @{name = "alias3"; version = "2.12"; collection_name = "distronode.builtin" }
                            )
                        }
                        ...
                        option6 = @{
                            ...
                            aliases = "alias6"
                            deprecated_aliases = @(
                                @{name = "alias6"; date = [DateTime]"2020-06-01"; collection_name = "distronode.builtin" }
                            )
                        }
                    }
                }
                ...
                option6 = @{
                    ...
                    aliases = "alias6"
                    deprecated_aliases = @(
                        @{name = "alias6"; version = "2.12"; collection_name = "distronode.builtin" }
                    )
                }
                option7 = @{
                    type = "str"
                    aliases = "alias7"
                    deprecated_aliases = @(
                        @{name = "alias7"; date = [DateTime]"2020-06-07"; collection_name = "distronode.builtin" }
                    )
                }
            }
        }
    ...
    "Required by failed - single value" = {
        ...
        $failed = $false
        try {
            $null = [Distronode.Basic.DistronodeModule]::Create(@(), $spec)
        }
        catch [System.Management.Automation.RuntimeException] {
            $failed = $true
    ...
    "Deprecate and warn with version" = {
        ...
        $actual_deprecate_event_1 = Get-EventLog -LogName Application -Source Distronode -Newest 1
        $m.Deprecate("message w collection", "2.8", "distronode.builtin")
        $actual_deprecate_event_2 = Get-EventLog -LogName Application -Source Distronode -Newest 1
        $m.Warn("warning")
        $actual_warn_event = Get-EventLog -LogName Application -Source Distronode -Newest 1
        ...
        $expected = @{
            ...
            deprecations = @(
                ...
                @{msg = "message w collection"; version = "2.8"; collection_name = "distronode.builtin" }
            )
        }
        $actual | Assert-DictionaryEqual -Expected $expected
    }
    ...
    "Deprecate and warn with date" = {
        ...
        $actual_deprecate_event_1 = Get-EventLog -LogName Application -Source Distronode -Newest 1
        $m.Deprecate("message w collection", [DateTime]"2020-01-02", "distronode.builtin")
        $actual_deprecate_event_2 = Get-EventLog -LogName Application -Source Distronode -Newest 1
        $m.Warn("warning")
        $actual_warn_event = Get-EventLog -LogName Application -Source Distronode -Newest 1
        ...
        $expected = @{
            ...
            deprecations = @(
                ...
                @{msg = "message w collection"; date = "2020-01-02"; collection_name = "distronode.builtin" }
            )
        }
        $actual | Assert-DictionaryEqual -Expected $expected
    }
    ...
    "ParseBool tests" = {
        ...
        foreach ($map in $mapping.GetEnumerator()) {
            $expected = $map.Value
            $actual = [Distronode.Basic.DistronodeModule]::ParseBool($map.Key)
        ...
        foreach ($fail_bool in $fail_bools) {
            $failed = $false
            try {
                [Distronode.Basic.DistronodeModule]::ParseBool($fail_bool)
            }
            catch {
                $failed = $true
    ...
    "Unknown internal key" = {
        Set-Variable -Name complex_args -Scope Global -Value @{
            _distronode_invalid = "invalid"
        }
        $failed = $false
        try {
            $null = [Distronode.Basic.DistronodeModule]::Create(@(), @{})
        }
        catch [System.Management.Automation.RuntimeException] {
            $failed = $true
            $_.Exception.Message | Assert-Equal -Expected "exit: 1"
            ...
            $expected = @{
                ...
                changed = $false
                failed = $true
                msg = "Unsupported parameters for (undefined win module) module: _distronode_invalid. Supported parameters include: "
            }
            $actual = [Distronode.Basic.DistronodeModule]::FromJson($_.Exception.InnerException.Output)
            $actual | Assert-DictionaryEqual -Expected $expected
        }
    ...
    "Module tmpdir with present remote tmp" = {
        ...
        Set-Variable -Name complex_args -Scope Global -Value @{
            _distronode_remote_tmp = $remote_tmp.ToString()
        }
        $m = [Distronode.Basic.DistronodeModule]::Create(@(), @{})
        ...
        $tmpdir_name.StartSwith("distronode-moduletmp-") | Assert-Equal -Expected $true
        ...
        try {
            ...
            $output = [Distronode.Basic.DistronodeModule]::FromJson($_.Exception.InnerException.Output)
        }
    ...
    "Unsupported options with ignore" = {
        ...
        Set-Variable -Name complex_args -Scope Global -Value @{
            ...
            invalid_key = "def"
            another_key = "ghi"
            _distronode_ignore_unknown_opts = $true
        }
        ...
        $spec = @{
            options = @{
                sub_options = @{
                    ...
                    # Distronode.Basic automatically sets that to $false and we want it to ignore it for a nested check
                    type = "dict"
                    options = @{
    ...
    "Case insensitive choice" = {
        ...
        $expected_warning += "Checking of choices will be case sensitive in a future Distronode release. "
    ...
    "String json array to object" = {
        $input_json = '["abc", "def"]'
        $actual = [Distronode.Basic.DistronodeModule]::FromJson($input_json)
    ...
    "Spec with fragments" = {
        ...
        $m = [Distronode.Basic.DistronodeModule]::Create(@(), $spec, @($fragment1))
        ...
        $spec = @{
            options = @{
                ...
                option2 = @{
                    aliases = @("alias2_spec")
                    deprecated_aliases = @(
                        @{name = "alias2_spec"; version = "2.0"; collection_name = "distronode.builtin" }
                    )
                }
            }
        }
    ...
    "Fragment spec with mutual args" = {
        ...
        $failed = $false
        try {
            [Distronode.Basic.DistronodeModule]::Create(@(), $spec, @($fragment1, $fragment2))
        }
        catch [System.Management.Automation.RuntimeException] {
            $failed = $true
            $_.Exception.Message | Assert-Equal -Expected "exit: 1"
    ...
    "Catch invalid fragment spec format" = {
        ...
        $failed = $false
        try {
            [Distronode.Basic.DistronodeModule]::Create(@(), $spec, @($fragment))
        }
        catch [System.Management.Automation.RuntimeException] {
            $failed = $true
            $_.Exception.Message | Assert-Equal -Expected "exit: 1"
...
try {
    ...
    if ($_.Exception.Message.StartSwith("exit: ")) {
        ...
        $module.Result.msg = "Uncaught DistronodeModule exit in tests, see output"
    }
    else {
        # Unrelated exception
