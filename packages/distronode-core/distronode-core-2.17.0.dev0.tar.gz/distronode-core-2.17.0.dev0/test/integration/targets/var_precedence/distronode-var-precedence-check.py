...
class VarTestMaker(object):
    ...
    def run(self):
        '''
        if self.dynamic_inventory:
            cmd = 'distronode-playbook -c local -i inventory/hosts site.yml'
        else:
            cmd = 'distronode-playbook -c local -i inventory site.yml'
        '''
        cmd = 'distronode-playbook -c local -i inventory site.yml'
        if 'extra_vars' in self.features:
            cmd += ' --extra-vars="findme=extra_vars"'
        cmd = cmd + ' -vvvvv'
        self.distronode_command = cmd
