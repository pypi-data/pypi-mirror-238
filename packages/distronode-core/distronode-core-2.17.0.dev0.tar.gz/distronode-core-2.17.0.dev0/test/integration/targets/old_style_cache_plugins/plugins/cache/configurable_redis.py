...
from distronode.plugins.cache import BaseCacheModule
from distronode.utils.display import Display

try:
    from redis import StrictRedis, VERSION
except ImportError:
    raise DistronodeError("The 'redis' python module (version 2.4.5 or newer) is required for the redis fact cache, 'pip install redis'")
...
class CacheModule(BaseCacheModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._keys_set = 'distronode_cache_keys'
    ...
    def get(self, key):
        ...
        if key not in self._cache:
            ...
            if value is None:
                self.delete(key)
                raise KeyError
            self._cache[key] = json.loads(value, cls=DistronodeJSONDecoder)

        return self._cache.get(key)

    def set(self, key, value):

        value2 = json.dumps(value, cls=DistronodeJSONEncoder, sort_keys=True, indent=4)
