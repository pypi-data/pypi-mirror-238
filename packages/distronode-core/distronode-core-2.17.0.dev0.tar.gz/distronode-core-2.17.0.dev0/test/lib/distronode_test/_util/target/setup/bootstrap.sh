...
install_pip() {
    if ! "${python_interpreter}" -m pip.__main__ --version --disable-pip-version-check 2>/dev/null; then
        case "${python_version}" in
            *)
                pip_bootstrap_url="https://ci-files.testing.distronode.github.io/distronode-test/get-pip-23.1.2.py"
                ;;
        esac
...
[global]
extra-index-url = https://spare-tire.testing.distronode.github.io/simple/
prefer-binary = yes
" > /etc/pip.conf
