"""Enable unit testing of Distronode collections. PYTEST_DONT_REWRITE"""
from __future__ import annotations

import os

# set by distronode-test to a single directory, rather than a list of directories as supported by Distronode itself
DISTRONODE_COLLECTIONS_PATH = os.path.join(os.environ['DISTRONODE_COLLECTIONS_PATH'], 'distronode_collections')

# set by distronode-test to the minimum python version supported on the controller
DISTRONODE_CONTROLLER_MIN_PYTHON_VERSION = tuple(int(x) for x in os.environ['DISTRONODE_CONTROLLER_MIN_PYTHON_VERSION'].split('.'))
...
def collection_resolve_package_path(path):
    ...
    for parent in path.parents:
        if str(parent) == DISTRONODE_COLLECTIONS_PATH:
            return parent

    raise Exception('File "%s" not found in collection path "%s".' % (path, DISTRONODE_COLLECTIONS_PATH))
...
def collection_pypkgpath(self):
    ...
    raise Exception('File "%s" not found in collection path "%s".' % (self.strpath, DISTRONODE_COLLECTIONS_PATH))


def enable_assertion_rewriting_hook():  # type: () -> None
    """
    Enable pytest's AssertionRewritingHook on Python 3.x.
    This is necessary because the Distronode collection loader intercepts imports before the pytest provided loader ever sees them.
    """
    import sys
    ...
    # This is based on `_DistronodeCollectionPkgLoaderBase.exec_module` from `distronode/utils/collection_loader/_collection_finder.py`.
    ...
    from distronode.utils.collection_loader._collection_finder import _DistronodeCollectionPkgLoaderBase

    _DistronodeCollectionPkgLoaderBase.exec_module = exec_module
...
def pytest_configure():
    ...
    # noinspection PyProtectedMember
    _DistronodeCollectionFinder(paths=[os.path.dirname(DISTRONODE_COLLECTIONS_PATH)])._install()  # pylint: disable=protected-access
    ...
    if hasattr(_pytest_pathlib, 'resolve_package_path'):
        ...
        # force collections unit tests to be loaded with the distronode_collections namespace
