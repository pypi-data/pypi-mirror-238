...
class PosixCoverageHandler(CoverageHandler[PosixConfig]):
    ...
    def __init__(self, args: IntegrationConfig, host_state: HostState, inventory_path: str) -> None:
        ...
        self.common_temp_path = f'/tmp/distronode-test-{generate_name()}'
    ...
    def teardown_target(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(prefix='distronode-test-coverage-', suffix='.tgz') as coverage_tgz:
            try:
    ...
    def get_environment(self, target_name: str, aliases: tuple[str, ...]) -> dict[str, str]:
        ...
        variables = dict(
            _DISTRONODE_COVERAGE_CONFIG=config_file,
            _DISTRONODE_COVERAGE_OUTPUT=coverage_file,
        )
...
class WindowsCoverageHandler(CoverageHandler[WindowsConfig]):
    ...
    def __init__(self, args: IntegrationConfig, host_state: HostState, inventory_path: str) -> None:
        ...
        self.remote_temp_path = f'C:\\distronode_test_coverage_{generate_name()}'
    ...
    def get_environment(self, target_name: str, aliases: tuple[str, ...]) -> dict[str, str]:
        ...
        variables = dict(
            _DISTRONODE_COVERAGE_REMOTE_OUTPUT=os.path.join(self.remote_temp_path, coverage_name),
            _DISTRONODE_COVERAGE_REMOTE_PATH_FILTER=os.path.join(data_context().content.root, '*'),
        )
