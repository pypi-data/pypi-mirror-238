...
def ssh_options_to_str(options: t.Union[dict[str, t.Union[int, str]], dict[str, str]]) -> str:
    """Format a dictionary of SSH options as a string suitable for passing as `distronode_ssh_extra_args` in inventory."""
    ...
    inventory = dict(
        all=dict(
            hosts=dict((ssh.name, exclude_none_values(dict(
                distronode_host=ssh.host,
                distronode_port=ssh.port,
                distronode_user=ssh.user,
                distronode_ssh_private_key_file=os.path.abspath(ssh.identity_file),
                distronode_connection='ssh',
                distronode_pipelining='yes',
                distronode_python_interpreter=ssh.python_interpreter,
                distronode_shell_type=ssh.shell_type,
                distronode_ssh_extra_args=ssh_options_to_str(dict(UserKnownHostsFile='/dev/null', **ssh.options)),  # avoid changing the test environment
                distronode_ssh_host_key_checking='no',
