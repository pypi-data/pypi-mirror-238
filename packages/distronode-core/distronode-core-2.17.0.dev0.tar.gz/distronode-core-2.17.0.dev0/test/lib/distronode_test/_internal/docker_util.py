...
UTILITY_IMAGE = 'quay.io/distronode/distronode-test-utility-container:2.0.0'
...
def detect_host_properties(args: CommonConfig) -> ContainerHostProperties:
    ...
    The information collected is:
      ...
      - The max number of open files supported by the container host to run containers.
        This value may be capped to the maximum value used by distronode-test.
    ...
    stdout = run_utility_container(args, 'distronode-test-probe', cmd, options)[0]
    ...
    if hard_limit < MAX_NUM_OPEN_FILES and hard_limit < system_limit and require_docker().command == 'docker':
        ...
        try:
            stdout = run_utility_container(args, 'distronode-test-ulimit', cmd, options)[0]
...
def run_utility_container(
    ...
    """Run the specified command using the distronode-test utility container, returning stdout and stderr."""
...
class DockerCommand:
    ...
    def detect() -> t.Optional[DockerCommand]:
        ...
        if os.environ.get('DISTRONODE_TEST_PREFER_PODMAN'):
