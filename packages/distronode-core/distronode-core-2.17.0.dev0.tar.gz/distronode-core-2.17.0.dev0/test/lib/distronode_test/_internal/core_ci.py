"""Access Distronode Core CI remote services."""
...
class Resource(metaclass=abc.ABCMeta):
    """Base class for Distronode Core CI resources."""
...
class VmResource(Resource):
    """Details needed to request a VM from Distronode Core CI."""
...
class CloudResource(Resource):
    """Details needed to request cloud credentials from Distronode Core CI."""
...
class DistronodeCoreCI:
    ...
    def __init__(
        ...
        self.path = os.path.expanduser(f'~/.distronode/test/instances/{self.name}')
    ...
    def available(self) -> bool:
        """Return True if Distronode Core CI is supported."""
...
class SshKey:
    ...
    def get_relative_in_tree_private_key_path() -> str:
        """Return the distronode-test SSH private key path relative to the content tree."""
    ...
    def get_in_tree_key_pair_paths(self) -> t.Optional[tuple[str, str]]:
        """Return the distronode-test SSH key pair paths from the content tree."""
    ...
    def get_source_key_pair_paths(self) -> t.Optional[tuple[str, str]]:
        """Return the distronode-test SSH key pair paths for the current user."""
        base_dir = os.path.expanduser('~/.distronode/test/')
    ...
    def get_key_pair(self) -> t.Optional[tuple[str, str]]:
        """Return the distronode-test SSH key pair paths if present, otherwise return None."""
    ...
    def generate_key_pair(self, args: EnvironmentConfig) -> tuple[str, str]:
        """Generate an SSH key pair for use by all distronode-test invocations for the current user."""
        key, pub = self.get_source_key_pair_paths()
