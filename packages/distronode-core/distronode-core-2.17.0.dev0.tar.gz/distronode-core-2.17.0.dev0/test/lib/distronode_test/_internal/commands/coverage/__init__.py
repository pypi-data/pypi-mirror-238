...
COVERAGE_CONFIG_PATH = os.path.join(DISTRONODE_TEST_DATA_ROOT, 'coveragerc')
...
def get_python_modules() -> dict[str, str]:
    """Return a dictionary of Distronode module names and their paths."""
...
def enumerate_powershell_lines(
    ...
    for filename, hits in coverage_run.items():
        ...
        if isinstance(hits, dict) and not hits.get('Line'):
            # Input data was previously aggregated and thus uses the standard distronode-test output format for PowerShell coverage.
...
def sanitize_filename(
    ...
    distronode_path = os.path.abspath('lib/distronode/') + '/'
    ...
    if '/distronode_modlib.zip/distronode/' in filename:
        # Rewrite the module_utils path from the remote host to match the controller. Distronode 2.6 and earlier.
        new_name = re.sub('^.*/distronode_modlib.zip/distronode/', distronode_path, filename)
        ...
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif re.search(r'/distronode_[^/]+_payload\.zip/distronode/', filename):
        # Rewrite the module_utils path from the remote host to match the controller. Distronode 2.7 and later.
        new_name = re.sub(r'^.*/distronode_[^/]+_payload\.zip/distronode/', distronode_path, filename)
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif '/distronode_module_' in filename:
        # Rewrite the module path from the remote host to match the controller. Distronode 2.6 and earlier.
        module_name = re.sub('^.*/distronode_module_(?P<module>.*).py$', '\\g<module>', filename)
        ...
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
    elif re.search('^(/.*?)?/root/distronode/', filename):
        # Rewrite the path of code running on a remote host or in a docker container as root.
        new_name = re.sub('^(/.*?)?/root/distronode/', root_path, filename)
        display.info('%s -> %s' % (filename, new_name), verbosity=3)
        filename = new_name
