...
from distronode.config.manager import ConfigManager, ensure_type, resolve_path, get_config_type
...
class TestConfigManager:
    ...
    def test_config_types(self):
        assert get_config_type('/tmp/distronode.ini') == 'ini'
        assert get_config_type('/tmp/distronode.cfg') == 'ini'
        assert get_config_type('/tmp/distronode.yaml') == 'yaml'
        assert get_config_type('/tmp/distronode.yml') == 'yaml'

    def test_config_types_negative(self):
        with pytest.raises(DistronodeOptionsError) as exec_info:
            get_config_type('/tmp/distronode.txt')
    ...
    def test_read_config_yaml_file_negative(self):
        with pytest.raises(DistronodeError) as exec_info:
    ...
    def test_entry_as_vault_var(self):
        ...
        vault_var = DistronodeVaultEncryptedUnicode(b"vault text")
