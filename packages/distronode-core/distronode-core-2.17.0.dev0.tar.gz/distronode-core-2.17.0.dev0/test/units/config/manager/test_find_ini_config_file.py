...
from distronode.config.manager import find_ini_config_file
...
cfg_in_cwd = os.path.join(working_dir, 'distronode.cfg')

cfg_dir = os.path.join(working_dir, 'data')
cfg_file = os.path.join(cfg_dir, 'distronode.cfg')
alt_cfg_file = os.path.join(cfg_dir, 'test.cfg')
cfg_in_homedir = os.path.expanduser('~/.distronode.cfg')


@pytest.fixture
def setup_env(request, monkeypatch):
    cur_config = os.environ.get('DISTRONODE_CONFIG', None)
    cfg_path = request.param[0]

    if cfg_path is None and cur_config:
        monkeypatch.delenv('DISTRONODE_CONFIG')
    else:
        monkeypatch.setenv('DISTRONODE_CONFIG', request.param[0])
...
class TestFindIniFile:
    ...
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/distronode/distronode.cfg', cfg_in_homedir, cfg_in_cwd, alt_cfg_file, cfg_file)]],
                             indirect=['setup_existing_files'])
    def test_env_has_cfg_file(self, setup_env, setup_existing_files, expected):
        """DISTRONODE_CONFIG is specified, use it"""
        warnings = set()
    ...
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/distronode/distronode.cfg', cfg_in_homedir, cfg_in_cwd)]],
                             indirect=['setup_existing_files'])
    def test_env_has_no_cfg_file(self, setup_env, setup_existing_files):
        """DISTRONODE_CONFIG is specified but the file does not exist"""

        warnings = set()
        # since the cfg file specified by DISTRONODE_CONFIG doesn't exist, the one at cwd that does
        # exist should be returned
        assert find_ini_config_file(warnings) == cfg_in_cwd
        assert warnings == set()

    # DISTRONODE_CONFIG not specified
    @pytest.mark.parametrize('setup_env', [[None]], indirect=['setup_env'])
    # All config files are present
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/distronode/distronode.cfg', cfg_in_homedir, cfg_in_cwd, cfg_file, alt_cfg_file)]],
                             indirect=['setup_existing_files'])
    def test_ini_in_cwd(self, setup_env, setup_existing_files):
        """DISTRONODE_CONFIG not specified.  Use the cwd cfg"""
        warnings = set()
        assert find_ini_config_file(warnings) == cfg_in_cwd
        assert warnings == set()
    ...
    # No config in cwd
    @pytest.mark.parametrize('setup_existing_files',
                             [[('/etc/distronode/distronode.cfg', cfg_in_homedir, cfg_file, alt_cfg_file)]],
                             indirect=['setup_existing_files'])
    ...
    @pytest.mark.parametrize('setup_env', [[None]], indirect=['setup_env'])
    # No config in cwd
    @pytest.mark.parametrize('setup_existing_files', [[('/etc/distronode/distronode.cfg', cfg_file, alt_cfg_file)]], indirect=['setup_existing_files'])
    def test_ini_in_systemdir(self, setup_env, setup_existing_files):
        """First config found is the system config"""
        warnings = set()
        assert find_ini_config_file(warnings) == '/etc/distronode/distronode.cfg'
        assert warnings == set()
    ...
    def test_cwd_warning_on_writable(self, setup_env, setup_existing_files, monkeypatch):
        ...
        assert u'Distronode is being run in a world writable directory' in warning
        assert u'ignoring it as an distronode.cfg source' in warning

    # DISTRONODE_CONFIG is sepcified
    ...
    def test_no_warning_on_writable_if_env_used(self, setup_env, setup_existing_files, monkeypatch, expected):
        """If the cwd is writable but DISTRONODE_CONFIG was used, no warning should be issued"""
        real_stat = os.stat
