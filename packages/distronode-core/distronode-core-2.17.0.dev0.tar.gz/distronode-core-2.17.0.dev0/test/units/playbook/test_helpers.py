...
from distronode.playbook import helpers
...
class MixinForMocks(object):
    def _setup(self):
        ...
        self.fake_role_loader = DictDataLoader({os.path.join(os.path.realpath("/etc"), "distronode/roles/bogus_role/tasks/main.yml"): """
...
class TestLoadListOfTasks(unittest.TestCase, MixinForMocks):
    ...
    def test_empty_task(self):
        ds = [{}]
        self.assertRaisesRegex(errors.DistronodeParserError,
...
#        print(res)
    ...
    def test_one_bogus_include_role_use_handlers(self):
        ...
        self.assertRaises(errors.DistronodeError, helpers.load_list_of_tasks,
                          ds,
                          self.mock_play,
                          True,  # use_handlers
                          self.mock_block,
