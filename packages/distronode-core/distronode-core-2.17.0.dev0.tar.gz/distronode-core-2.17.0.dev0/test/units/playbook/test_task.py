...
class TestTask(unittest.TestCase):
    ...
    @patch.object(errors.DistronodeError, '_get_error_lines_from_file')
    def test_load_task_kv_form_error_36848(self, mock_get_err_lines):
        init_plugin_loader()
        ds = objects.DistronodeMapping(kv_bad_args_ds)
        ds.distronode_pos = ('test_task_faux_playbook.yml', 1, 1)
        mock_get_err_lines.return_value = (kv_bad_args_str, '')

        with self.assertRaises(errors.DistronodeParserError) as cm:
            Task.load(ds)

        self.assertIsInstance(cm.exception, errors.DistronodeParserError)
