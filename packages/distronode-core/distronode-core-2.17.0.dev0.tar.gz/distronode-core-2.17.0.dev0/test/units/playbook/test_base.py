...
from distronode.playbook.attribute import FieldAttribute, NonInheritableFieldAttribute
from distronode.template import Templar
from distronode.playbook import base
...
class TestBase(unittest.TestCase):
    ...
    def test_load_data_invalid_attr(self):
        ...
        self.assertRaises(DistronodeParserError, self.b.load_data, ds)
...
class TestBaseSubClass(TestBase):
    ...
    def test_attr_dict_string(self):
        ...
        self.assertRaisesRegex(DistronodeParserError, 'is not a dictionary', self._base_validate, ds)
    ...
    def test_attr_class_post_validate_class_not_instance(self):
        ...
        self.assertRaisesRegex(DistronodeParserError, "is not a valid.*got a <class 'type'> instead",
                               self._base_validate, ds)

    def test_attr_class_post_validate_wrong_class(self):
        not_a_esc = 37
        ds = {'test_attr_class_post_validate': not_a_esc}
        self.assertRaisesRegex(DistronodeParserError, 'is not a valid.*got a.*int.*instead',
                               self._base_validate, ds)
    ...
    def test_attr_example_undefined(self):
        ...
        self.assertRaises(DistronodeParserError)
    ...
    def test_attr_string_invalid_list(self):
        ...
        self.assertRaises(DistronodeParserError, self._base_validate, ds)
    ...
    def test_attr_list_required_empty_string(self):
        ...
        fake_loader = DictDataLoader({})
        templar = Templar(loader=fake_loader)
        self.assertRaisesRegex(DistronodeParserError, 'cannot have empty values',
                               bsc.post_validate, templar)
    ...
    def test_get_validated_value_string_rewrap_unsafe(self):
        ...
        value = DistronodeUnsafeText(u'bar')
        templar = Templar(None)
        bsc = self.ClassUnderTest()
        result = bsc.get_validated_value('foo', attribute, value, templar)
        self.assertIsInstance(result, DistronodeUnsafeText)
        self.assertEqual(result, DistronodeUnsafeText(u'bar'))
