...
from distronode.template import Templar, DistronodeContext, DistronodeEnvironment, DistronodeUndefined
from distronode.utils.unsafe_proxy import DistronodeUnsafe, wrap_var
...
class BaseTemplar(object):
    def setUp(self):
        ...
        self._distronode_context = DistronodeContext(self.templar.environment, {}, {}, {})

    def is_unsafe(self, obj):
        return self._distronode_context._is_unsafe(obj)


class SomeUnsafeClass(DistronodeUnsafe):
    def __init__(self):
...
class TestTemplarTemplate(BaseTemplar, unittest.TestCase):
    ...
    def test_is_possible_template(self):
        ...
        self.assertRaises(DistronodeError, self.templar.template, '{{ foo|default(False)) }}')
    ...
    def test_template_convert_bare_unsafe(self):
        ...
        # self.assertIsInstance(res, DistronodeUnsafe)
    ...
    def test_weird(self):
        ...
        self.assertRaisesRegex(DistronodeError,
                               'template error while templating string',
                               self.templar.template,
                               data)

    def test_template_with_error(self):
        """Check that DistronodeError is raised, fail if an unhandled exception is raised"""
        self.assertRaises(DistronodeError, self.templar.template, "{{ str_with_error }}")
...
class TestTemplarMisc(BaseTemplar, unittest.TestCase):
    def test_templar_simple(self):
        ...
        # force errors
        self.assertRaises(DistronodeUndefinedVariable, templar.template, "{{bad_var}}")
        self.assertRaises(DistronodeUndefinedVariable, templar.template, "{{lookup('file', bad_var)}}")
        self.assertRaises(DistronodeError, templar.template, "{{lookup('bad_lookup')}}")
        self.assertRaises(DistronodeError, templar.template, "{{recursive}}")
        self.assertRaises(DistronodeUndefinedVariable, templar.template, "{{foo-bar}}")
...
class TestTemplarLookup(BaseTemplar, unittest.TestCase):
    ...
    def test_lookup_jinja_undefined(self):
        self.assertRaisesRegex(DistronodeUndefinedVariable,
    ...
    def test_lookup_jinja_list_wantlist_unsafe(self):
        ...
        for lookup_result in res:
            ...
            # self.assertIsInstance(lookup_result, DistronodeUnsafe)

        # Should this be an DistronodeUnsafe
        # self.assertIsInstance(res, DistronodeUnsafe)
    ...
    def test_lookup_jinja_dict(self):
        ...
        # self.assertIsInstance(res['{{ a_keyword }}'], DistronodeUnsafe)
        # self.assertIsInstance(res, DistronodeUnsafe)
    ...
    def test_lookup_jinja_dict_unsafe(self):
        ...
        # self.assertIsInstance(res['{{ some_unsafe_key }}'], DistronodeUnsafe)
        # TODO: Should this be an DistronodeUnsafe
        # self.assertIsInstance(res, DistronodeUnsafe)
...
class TestDistronodeContext(BaseTemplar, unittest.TestCase):
    def _context(self, variables=None):
        variables = variables or {}

        env = DistronodeEnvironment()
        context = DistronodeContext(env, parent={}, name='some_context',
                                 blocks={})
    ...
    def test(self):
        ...
        self.assertIsInstance(context, DistronodeContext)
    ...
    def test_resolve_unsafe(self):
        ...
        # self.assertIsInstance(res, DistronodeUnsafe)
        self.assertTrue(self.is_unsafe(res),
                        'return of DistronodeContext.resolve (%s) was expected to be marked unsafe but was not' % res)

    def test_resolve_unsafe_list(self):
        context = self._context(variables={'some_unsafe_key': [wrap_var('some unsafe string 1')]})
        res = context.resolve('some_unsafe_key')
        # self.assertIsInstance(res[0], DistronodeUnsafe)
        self.assertTrue(self.is_unsafe(res),
                        'return of DistronodeContext.resolve (%s) was expected to be marked unsafe but was not' % res)
    ...
    def test_resolve_unsafe_dict(self):
        ...
        self.assertTrue(self.is_unsafe(res['an_unsafe_dict']),
                        'return of DistronodeContext.resolve (%s) was expected to be marked unsafe but was not' % res['an_unsafe_dict'])

    def test_resolve(self):
        context = self._context(variables={'some_key': 'some_string'})
        res = context.resolve('some_key')
        self.assertEqual(res, 'some_string')
        # self.assertNotIsInstance(res, DistronodeUnsafe)
        self.assertFalse(self.is_unsafe(res),
                         'return of DistronodeContext.resolve (%s) was not expected to be marked unsafe but was' % res)
    ...
    def test_is_unsafe(self):
        context = self._context()
        self.assertFalse(context._is_unsafe(DistronodeUndefined()))
