...
from distronode.plugins.filter.core import get_encrypted_password
from distronode.utils import encrypt
...
def assert_hash(expected, secret, algorithm, **settings):
    ...
    if encrypt.PASSLIB_AVAILABLE:
        ...
        with pytest.raises(DistronodeError) as excinfo:
...
def test_password_hash_filter_no_passlib():
    with passlib_off():
        ...
        with pytest.raises(DistronodeFilterError):
