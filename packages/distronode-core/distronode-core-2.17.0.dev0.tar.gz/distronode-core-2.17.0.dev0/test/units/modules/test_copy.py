...
from distronode.modules.copy import DistronodeModuleError, split_pre_existing_dir
...
def test_split_pre_existing_dir_root_does_not_exist(directory, mocker):
    ...
    with pytest.raises(DistronodeModuleError) as excinfo:
...
def test_good_symbolic_modes(mocker, stat_info, mode_string, expected):
    ...
    assert DistronodeModule._symbolic_mode_to_octal(mock_stat, mode_string) == expected
...
def test_invalid_symbolic_modes(mocker, stat_info, mode_string, expected):
    ...
    with pytest.raises(ValueError) as exc:
        assert DistronodeModule._symbolic_mode_to_octal(mock_stat, mode_string) == 'blah'
