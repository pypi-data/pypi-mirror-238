...
class TestHostWithPort(TestHost):
    distronode_port = 8822

    def setUp(self):
        self.hostA = Host(name='a', port=self.distronode_port)
        self.hostB = Host(name='b', port=self.distronode_port)

    def test_get_vars_distronode_port(self):
        host_vars = self.hostA.get_vars()
        self.assertEqual(host_vars['distronode_port'], self.distronode_port)
