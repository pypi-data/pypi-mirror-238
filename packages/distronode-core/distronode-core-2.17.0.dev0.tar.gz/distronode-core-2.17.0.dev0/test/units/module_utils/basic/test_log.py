...
class TestDistronodeModuleLogSmokeTest:
    ...
    def test_smoketest_syslog(self, am, mocker, msg):
        ...
        mocker.patch('distronode.module_utils.basic.has_journal', False)
    ...
    @pytest.mark.skipif(not distronode.module_utils.basic.has_journal, reason='python systemd bindings not installed')
    # pylint bug: https://github.com/PyCQA/pylint/issues/511
    @pytest.mark.parametrize('msg, stdin', ((m, {}) for m in DATA), indirect=['stdin'])  # pylint: disable=undefined-variable
    def test_smoketest_journal(self, am, mocker, msg):
        ...
        # show that what we send doesn't cause an issue once it gets to the
        # daemon.  These are just smoketests to test that we don't fail.
        mocker.patch('distronode.module_utils.basic.has_journal', True)
...
class TestDistronodeModuleLogSyslog:
    """Test the DistronodeModule Log Method"""
...
@pytest.mark.skipif(not distronode.module_utils.basic.has_journal, reason='python systemd bindings not installed')
class TestDistronodeModuleLogJournal:
    """Test the DistronodeModule Log Method"""
