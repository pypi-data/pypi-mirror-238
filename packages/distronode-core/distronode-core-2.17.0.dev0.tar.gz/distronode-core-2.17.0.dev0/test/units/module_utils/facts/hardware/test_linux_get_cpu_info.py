...
def test_get_cpu_info(mocker):
    ...
    for test in CPU_INFO_TEST_SCENARIOS:
        mocker.patch('distronode.module_utils.facts.hardware.linux.get_file_lines', side_effect=[[], test['cpuinfo']])
        ...
        collected_facts = {'distronode_architecture': test['architecture']}
...
def test_get_cpu_info_nproc(mocker):
    ...
    for test in CPU_INFO_TEST_SCENARIOS:
        ...
        mocker.patch('distronode.module_utils.facts.hardware.linux.get_bin_path', return_value='/usr/bin/nproc')
        module.run_command.return_value = (0, test['nproc_out'], '')
        collected_facts = {'distronode_architecture': test['architecture']}
