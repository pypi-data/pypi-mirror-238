...
def test_distribution_version(am, mocker, testcase):
    ...
    * the output of distronode.module_utils.distro.linux_distribution() [called platform.dist() for historical reasons]
    * results for the distronode variables distribution* and os_family
    ...
    mocker.patch('distronode.module_utils.facts.system.distribution.get_file_content', mock_get_file_content)
    mocker.patch('distronode.module_utils.facts.system.distribution.get_file_lines', mock_get_file_lines)
    mocker.patch('distronode.module_utils.facts.system.distribution.get_uname', mock_get_uname)
    mocker.patch('distronode.module_utils.facts.system.distribution._file_exists', mock_file_exists)
    mocker.patch('distronode.module_utils.distro.name', mock_distro_name)
    mocker.patch('distronode.module_utils.distro.id', mock_distro_id)
    mocker.patch('distronode.module_utils.distro.version', mock_distro_version)
    mocker.patch('distronode.module_utils.distro.codename', mock_distro_codename)
    mocker.patch(
        'distronode.module_utils.common.sys_info.distro.os_release_info',
        mock_distro_os_release_info)
    mocker.patch(
        'distronode.module_utils.common.sys_info.distro.lsb_release_info',
    ...
    mocker.patch('distronode.module_utils.basic.DistronodeModule.run_command', mock_run_command_output)
