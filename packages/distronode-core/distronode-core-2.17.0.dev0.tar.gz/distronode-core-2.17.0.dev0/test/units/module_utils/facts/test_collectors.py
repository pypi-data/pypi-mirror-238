# unit tests for distronode fact collectors
# -*- coding: utf-8 -*-
#
...
from distronode.module_utils.facts import collector
...
from distronode.module_utils.facts.system.pkg_mgr import PkgMgrFactCollector, OpenBSDPkgMgrFactCollector
...
class TestApparmorFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_apparmor'
...
class TestCapsFacts(BaseFactsTest):
    ...
    valid_subsets = ['caps']
    fact_namespace = 'distronode_system_capabilities'
...
class TestCmdLineFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_cmdline'
...
class TestDistributionFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_distribution'
...
class TestFipsFacts(BaseFactsTest):
    ...
    valid_subsets = ['fips']
    fact_namespace = 'distronode_fips'
...
class TestHardwareCollector(BaseFactsTest):
    ...
    fact_namespace = 'distronode_hardware'
    collector_class = HardwareCollector
    collected_facts = {'distronode_architecture': 'x86_64'}
...
class TestNetworkCollector(BaseFactsTest):
    ...
    fact_namespace = 'distronode_network'
...
class TestPkgMgrFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_pkgmgr'
    collector_class = PkgMgrFactCollector
    collected_facts = {
        "distronode_distribution": "Fedora",
        "distronode_distribution_major_version": "28",
        "distronode_os_family": "RedHat"
    }
...
class TestMacOSXPkgMgrFacts(BaseFactsTest):
    ...
    collector_class = PkgMgrFactCollector
    collected_facts = {
        "distronode_distribution": "MacOSX",
        "distronode_distribution_major_version": "11",
        "distronode_os_family": "Darwin"
    }

    @patch('distronode.module_utils.facts.system.pkg_mgr.os.path.exists', side_effect=lambda x: x == '/opt/homebrew/bin/brew')
    ...
    @patch('distronode.module_utils.facts.system.pkg_mgr.os.path.exists', side_effect=lambda x: x == '/usr/local/bin/brew')
    ...
    @patch('distronode.module_utils.facts.system.pkg_mgr.os.path.exists', side_effect=lambda x: x == '/opt/local/bin/port')
...
class TestPkgMgrFactsAptFedora(BaseFactsTest):
    ...
    collected_facts = {
        ...
        "distronode_distribution_major_version": "28",
        "distronode_os_family": "RedHat",
        "distronode_pkg_mgr": "apt"
    }

    @patch('distronode.module_utils.facts.system.pkg_mgr.os.path.exists', side_effect=_sanitize_os_path_apt_get)
...
class TestPlatformFactCollector(BaseFactsTest):
    ...
    fact_namespace = 'distronode_platform'
...
class TestPythonFactCollector(BaseFactsTest):
    ...
    fact_namespace = 'distronode_python'
...
class TestSelinuxFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_selinux'
    collector_class = SelinuxFactCollector

    def test_no_selinux(self):
        with patch('distronode.module_utils.facts.system.selinux.HAVE_SELINUX', False):
            module = self._mock_module()
            fact_collector = self.collector_class()
...
class TestServiceMgrFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_service_mgr'
    collector_class = ServiceMgrFactCollector

    # TODO: dedupe some of this test code

    @patch('distronode.module_utils.facts.system.service_mgr.get_file_content', return_value=None)
    @patch('distronode.module_utils.facts.system.service_mgr.ServiceMgrFactCollector.is_systemd_managed', return_value=False)
    @patch('distronode.module_utils.facts.system.service_mgr.ServiceMgrFactCollector.is_systemd_managed_offline', return_value=False)
    @patch('distronode.module_utils.facts.system.service_mgr.os.path.exists', return_value=False)
    ...
    @patch('distronode.module_utils.facts.system.service_mgr.get_file_content', return_value='runit-init')
    @patch('distronode.module_utils.facts.system.service_mgr.os.path.islink', side_effect=lambda x: x == '/sbin/init')
    @patch('distronode.module_utils.facts.system.service_mgr.os.readlink', side_effect=lambda x: '/sbin/runit-init' if x == '/sbin/init' else '/bin/false')
    def test_service_mgr_runit(self, mock_gfc, mock_opl, mock_orl):
        ...
        module = self._mock_module()
        module.run_command = Mock(return_value=(1, '', ''))
        collected_facts = {'distronode_system': 'Linux'}
        fact_collector = self.collector_class()
        facts_dict = fact_collector.collect(module=module,
                                            collected_facts=collected_facts)
        self.assertIsInstance(facts_dict, dict)
...
class TestSshPubKeyFactCollector(BaseFactsTest):
    ...
    fact_namespace = 'distronode_ssh_pub_leys'
...
class TestUserFactCollector(BaseFactsTest):
    ...
    valid_subsets = ['user']
    fact_namespace = 'distronode_user'
...
class TestVirtualFacts(BaseFactsTest):
    ...
    fact_namespace = 'distronode_virtual'
