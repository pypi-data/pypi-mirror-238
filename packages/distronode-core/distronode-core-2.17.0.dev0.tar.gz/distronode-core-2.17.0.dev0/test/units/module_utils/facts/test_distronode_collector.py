...
from distronode.module_utils.facts import namespace
...
def _collectors(module,
    ...
    collector_meta_data_collector = \
        distronode_collector.CollectorMetaDataCollector(gather_subset=gather_subset,
                                                     module_setup=True)
...
ns = namespace.PrefixFactNamespace('distronode_facts', 'distronode_')
...
class TestInPlace(unittest.TestCase):
    ...
    def test(self):
        ...
        fact_collector = \
            distronode_collector.DistronodeFactCollector(collectors=collectors,
                                                   namespace=ns)
...
class TestCollectedFacts(unittest.TestCase):
    ...
    # TODO: add distronode_cmdline, distronode_*_pubkey* back when TempFactCollector goes away
    ...
    @patch('distronode.module_utils.facts.system.service_mgr.get_file_content', return_value='systemd')
    ...
    # everything starts with distronode_ namespace
    def _assert_distronode_namespace(self, facts):
        ...
        for fact_key in facts:
            self.assertTrue(fact_key.startswith('distronode_'),
                            'The fact name "%s" does not startwith "distronode_"' % fact_key)
...
class TestCollectorDepsWithFilter(unittest.TestCase):
    ...
    def test_with_filter_no_match(self):
        _mock_module = mock_module(gather_subset=['all', '!facter', '!ohai'],
                                   filter='distronode_this_doesnt_exist')
    ...
    def test_concat_collector(self):
        ...
        fact_collector = \
            distronode_collector.DistronodeFactCollector(collectors=_collectors,
                                                   namespace=ns,
...
class TestPkgMgrOSTreeFacts(TestPkgMgrFacts):
    @patch(
        'distronode.module_utils.facts.system.pkg_mgr.os.path.exists',
        side_effect=lambda x: x == '/run/ostree-booted')
    def _recollect_facts(self, distribution, version, mock_exists):
        self.collected_facts['distronode_distribution'] = distribution
        self.collected_facts['distronode_distribution_major_version'] = \
            str(version)
        # Recollect facts
        self.setUp()
