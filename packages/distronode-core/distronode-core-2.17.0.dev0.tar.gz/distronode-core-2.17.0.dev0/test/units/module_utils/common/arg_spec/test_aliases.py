...
from distronode.module_utils.errors import DistronodeValidationError, DistronodeValidationErrorMultiple
...
def test_aliases_invalid(arg_spec, parameters, expected, error):
    ...
    assert isinstance(result.errors.errors[0], DistronodeValidationError)
    assert isinstance(result.errors, DistronodeValidationErrorMultiple)
