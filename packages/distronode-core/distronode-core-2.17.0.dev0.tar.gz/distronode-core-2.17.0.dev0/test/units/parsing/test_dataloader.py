...
from distronode.errors import DistronodeParserError, yaml_strings, DistronodeFileNotFound
from distronode.parsing.vault import DistronodeVaultError
...
class TestDataLoader(unittest.TestCase):
    ...
    def test_parse_fail_from_file(self, mock_def):
        ...
        TEXT:
            ***
               NOT VALID
        """, True)
        self.assertRaises(DistronodeParserError, self._loader.load_from_file, 'dummy_yaml_bad.txt')

    @patch('distronode.errors.DistronodeError._get_error_lines_from_file')
    @patch.object(DataLoader, '_get_file_contents')
    def test_tab_error(self, mock_def, mock_get_error_lines):
        ...
        with self.assertRaises(DistronodeParserError) as cm:
    ...
    @patch('distronode.parsing.dataloader.unfrackpath', mock_unfrackpath_noop)
    ...
    def test_get_file_contents_none_path(self):
        self.assertRaisesRegex(DistronodeParserError, 'Invalid filename',
                               self._loader._get_file_contents, None)

    def test_get_file_contents_non_existent_path(self):
        self.assertRaises(DistronodeFileNotFound, self._loader._get_file_contents, '/non_existent_file')
...
class TestPathDwimRelativeStackDataLoader(unittest.TestCase):
    ...
    def test_none(self):
        self.assertRaisesRegex(DistronodeFileNotFound, 'on the Distronode Controller', self._loader.path_dwim_relative_stack, None, None, None)
    ...
    def test_path_endswith_role_source_main_yml(self):
        self.assertRaises(DistronodeFileNotFound, self._loader.path_dwim_relative_stack, ['foo/bar/tasks/'], '/', 'main.yml')

    def test_path_endswith_role_source_main_yml_source_in_dirname(self):
        self.assertRaises(DistronodeFileNotFound, self._loader.path_dwim_relative_stack, 'foo/bar/tasks/', 'tasks', 'tasks/main.yml')
...
class TestDataLoaderWithVault(unittest.TestCase):
    ...
    def setUp(self):
        self._loader = DataLoader()
        vault_secrets = [('default', TextVaultSecret('distronode'))]
    ...
    def test_get_real_file_vault_no_vault(self):
        ...
        self.assertRaises(DistronodeParserError, self._loader.get_real_file, self.test_vault_data_path)
    ...
    def test_get_real_file_vault_wrong_password(self):
        ...
        self.assertRaises(DistronodeVaultError, self._loader.get_real_file, self.test_vault_data_path)

    def test_get_real_file_not_a_path(self):
        self.assertRaisesRegex(DistronodeParserError, 'Invalid filename', self._loader.get_real_file, None)

    @patch.multiple(DataLoader, path_exists=lambda s, x: True, is_file=lambda s, x: True)
    def test_parse_from_vault_1_1_file(self):
        vaulted_data = """$DISTRONODE_VAULT;1.1;AES256
