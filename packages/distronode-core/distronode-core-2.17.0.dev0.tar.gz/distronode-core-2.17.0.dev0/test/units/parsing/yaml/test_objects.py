...
# module under test
from distronode.parsing.yaml import objects

from units.mock.yaml_helper import YamlTestUtils
from units.mock.vault_helper import TextVaultSecret


class TestDistronodeVaultUnicodeNoVault(unittest.TestCase, YamlTestUtils):
    def test_empty_init(self):
        self.assertRaises(TypeError, objects.DistronodeVaultEncryptedUnicode)
    ...
    def _assert_values(self, avu, seq):
        self.assertIsInstance(avu, objects.DistronodeVaultEncryptedUnicode)
        self.assertTrue(avu.vault is None)
        # DistronodeVaultEncryptedUnicode without a vault should never == any string
        self.assertNotEqual(avu, seq)

    def assert_values(self, seq):
        avu = objects.DistronodeVaultEncryptedUnicode(seq)
...
class TestDistronodeVaultEncryptedUnicode(unittest.TestCase, YamlTestUtils):
    def setUp(self):
    ...
    def _loader(self, stream):
        return DistronodeLoader(stream, vault_secrets=self.vault_secrets)

    def test_dump_load_cycle(self):
        aveu = self._from_plaintext('the test string for TestDistronodeVaultEncryptedUnicode.test_dump_load_cycle')
    ...
    def _from_plaintext(self, seq):
        ...
        return objects.DistronodeVaultEncryptedUnicode.from_plaintext(seq, vault=self.vault, secret=id_secret[1])
    ...
    def test_empty_string_wrong_password(self):
        ...
        self.assertRaises(DistronodeError, compare, avu, seq)
