...
from distronode import context, errors
from distronode.cli.vault import VaultCLI
...
class TestVaultCli(unittest.TestCase):
    def setUp(self):
        self.tty_patcher = patch('distronode.cli.sys.stdin.isatty', return_value=False)
    ...
    def test_parse_view_file(self):
        cli = VaultCLI(args=['distronode-vault', 'view', '/dev/null/foo'])
        cli.parse()

    @patch('distronode.cli.vault.VaultCLI.setup_vault_secrets')
    def test_view_missing_file_no_secret(self, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = []
        cli = VaultCLI(args=['distronode-vault', 'view', '/dev/null/foo'])
        cli.parse()
        self.assertRaisesRegex(errors.DistronodeOptionsError,
                               "A vault password is required to use Distronode's Vault",
                               cli.run)

    @patch('distronode.cli.vault.VaultCLI.setup_vault_secrets')
    def test_encrypt_missing_file_no_secret(self, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = []
        cli = VaultCLI(args=['distronode-vault', 'encrypt', '/dev/null/foo'])
        cli.parse()
        self.assertRaisesRegex(errors.DistronodeOptionsError,
                               "A vault password is required to use Distronode's Vault",
                               cli.run)
    ...
    @patch('distronode.cli.vault.VaultCLI.setup_vault_secrets')
    @patch('distronode.cli.vault.VaultEditor')
    ...
    def test_encrypt_string(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault', 'encrypt_string',
    ...
    @patch('distronode.cli.vault.VaultCLI.setup_vault_secrets')
    @patch('distronode.cli.vault.VaultEditor')
    @patch('distronode.cli.vault.display.prompt', return_value='a_prompt')
    def test_encrypt_string_prompt(self, mock_display, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault',
                             'encrypt_string',
                             '--prompt',
                             '--show-input',
                             'some string to encrypt'])
    ...
    @patch('distronode.cli.vault.VaultCLI.setup_vault_secrets')
    @patch('distronode.cli.vault.VaultEditor')
    @patch('distronode.cli.vault.sys.stdin.read', return_value='This is data from stdin')
    ...
    def test_create(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault', 'create', '/dev/null/foo'])
        cli.parse()
        self.assertRaisesRegex(errors.DistronodeOptionsError,
    ...
    def test_create_skip_tty_check(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault', 'create', '--skip-tty-check', '/dev/null/foo'])
        cli.parse()
        cli.run()
    ...
    def test_create_with_tty(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        self.tty_stdout_patcher = patch('distronode.cli.sys.stdout.isatty', return_value=True)
    ...
    def test_edit(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault', 'edit', '/dev/null/foo'])
        cli.parse()
        cli.run()
    ...
    def test_decrypt(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault', 'decrypt', '/dev/null/foo'])
        cli.parse()
        cli.run()
    ...
    def test_rekey(self, mock_vault_editor, mock_setup_vault_secrets):
        mock_setup_vault_secrets.return_value = [('default', TextVaultSecret('password'))]
        cli = VaultCLI(args=['distronode-vault', 'rekey', '/dev/null/foo'])
        cli.parse()
        cli.run()


@pytest.mark.parametrize('cli_args, expected', [
    (['distronode-vault', 'view', 'vault.txt'], 0),
    (['distronode-vault', 'view', 'vault.txt', '-vvv'], 3),
    (['distronode-vault', 'view', 'vault.txt', '-vv'], 2),
])
def test_verbosity_arguments(cli_args, expected, tmp_path_factory, monkeypatch):
    ...
    test_dir = to_text(tmp_path_factory.mktemp('test-distronode-vault'))
