...
def cliargs(collections_paths=None, collection_name=None):
    if collections_paths is None:
        collections_paths = ['/root/.distronode/collections', '/usr/share/distronode/collections']
...
def mock_from_path(mocker, monkeypatch):
    collection_args = {
        '/usr/share/distronode/collections/distronode_collections/sandwiches/pbj': (
            'sandwiches.pbj',
            '1.0.0',
            '/usr/share/distronode/collections/distronode_collections/sandwiches/pbj',
            'dir',
        ...
        '/usr/share/distronode/collections/distronode_collections/sandwiches/ham': (
            'sandwiches.ham',
            '1.0.0',
            '/usr/share/distronode/collections/distronode_collections/sandwiches/ham',
            'dir',
        ...
        '/root/.distronode/collections/distronode_collections/sandwiches/pbj': (
            'sandwiches.pbj',
            '1.5.0',
            '/root/.distronode/collections/distronode_collections/sandwiches/pbj',
            'dir',
        ...
        '/root/.distronode/collections/distronode_collections/sandwiches/reuben': (
            'sandwiches.reuben',
            '2.5.0',
            '/root/.distronode/collections/distronode_collections/sandwiches/reuben',
            'dir',
    ...
    mocker.patch('distronode.galaxy.collection.files', return_value=files_mock)
    ...
    mocker.patch('distronode.galaxy.collection.Candidate.from_dir_path_as_unknown', side_effect=dispatch_requirement)

    monkeypatch.setattr(C, 'COLLECTIONS_PATHS', ['/root/.distronode/collections', '/usr/share/distronode/collections'])
...
def test_execute_list_collection_all(mocker, capsys, mock_from_path, tmp_path_factory):
    ...
    gc = GalaxyCLI(['distronode-galaxy', 'collection', 'list'])
    ...
    assert out_lines[0] == ''
    assert out_lines[1] == '# /root/.distronode/collections/distronode_collections'
    assert out_lines[2] == 'Collection        Version'
    assert out_lines[3] == '----------------- -------'
    assert out_lines[4] == 'sandwiches.pbj    1.5.0  '
    assert out_lines[5] == 'sandwiches.reuben 2.5.0  '
    assert out_lines[6] == ''
    assert out_lines[7] == '# /usr/share/distronode/collections/distronode_collections'
...
def test_execute_list_collection_specific(mocker, capsys, mock_from_path, tmp_path_factory):
    ...
    mocker.patch('distronode.galaxy.collection.validate_collection_name', collection_name)
    mocker.patch('distronode.cli.galaxy._get_collection_widths', return_value=(14, 5))

    gc = GalaxyCLI(['distronode-galaxy', 'collection', 'list', collection_name])
    tmp_path = tmp_path_factory.mktemp('test-ÅÑŚÌβŁÈ Collections')
    concrete_artifact_cm = collection.concrete_artifact_manager.ConcreteArtifactsManager(tmp_path, validate_certs=False)
    gc.execute_list_collection(artifacts_manager=concrete_artifact_cm)
    ...
    assert out_lines[0] == ''
    assert out_lines[1] == '# /usr/share/distronode/collections/distronode_collections'
    assert out_lines[2] == 'Collection     Version'
    assert out_lines[3] == '-------------- -------'
...
def test_execute_list_collection_specific_duplicate(mocker, capsys, mock_from_path, tmp_path_factory):
    ...
    assert out_lines[4] == 'sandwiches.pbj 1.5.0  '
    assert out_lines[5] == ''
    assert out_lines[6] == '# /usr/share/distronode/collections/distronode_collections'
...
def test_execute_list_collection_specific_invalid_fqcn(mocker, tmp_path_factory):
    ...
    tmp_path = tmp_path_factory.mktemp('test-ÅÑŚÌβŁÈ Collections')
    concrete_artifact_cm = collection.concrete_artifact_manager.ConcreteArtifactsManager(tmp_path, validate_certs=False)
    with pytest.raises(DistronodeError, match='Invalid collection name'):
        gc.execute_list_collection(artifacts_manager=concrete_artifact_cm)
...
def test_execute_list_collection_no_valid_paths(mocker, capsys, tmp_path_factory):
    ...
    mocker.patch('distronode.utils.color.DISTRONODE_COLOR', False)
    mocker.patch('distronode.cli.galaxy.display.columns', 79)
    gc = GalaxyCLI(['distronode-galaxy', 'collection', 'list'])

    tmp_path = tmp_path_factory.mktemp('test-ÅÑŚÌβŁÈ Collections')
    concrete_artifact_cm = collection.concrete_artifact_manager.ConcreteArtifactsManager(tmp_path, validate_certs=False)

    with pytest.raises(DistronodeOptionsError, match=r'None of the provided paths were usable.'):
        gc.execute_list_collection(artifacts_manager=concrete_artifact_cm)
...
def test_execute_list_collection_one_invalid_path(mocker, capsys, mock_from_path, tmp_path_factory):
    ...
    gc = GalaxyCLI(['distronode-galaxy', 'collection', 'list', '-p', 'nope'])
    tmp_path = tmp_path_factory.mktemp('test-ÅÑŚÌβŁÈ Collections')
    concrete_artifact_cm = collection.concrete_artifact_manager.ConcreteArtifactsManager(tmp_path, validate_certs=False)
    gc.execute_list_collection(artifacts_manager=concrete_artifact_cm)
