...
from distronode.galaxy.collection import _extract_tar_dir


@pytest.fixture
def fake_tar_obj(mocker):
    m_tarfile = mocker.Mock()
    m_tarfile._distronode_normalized_cache = {'/some/dir': mocker.Mock()}
...
def test_extract_tar_member_trailing_sep(mocker):
    m_tarfile = mocker.Mock()
    m_tarfile._distronode_normalized_cache = {}

    with pytest.raises(DistronodeError, match='Unable to extract'):
