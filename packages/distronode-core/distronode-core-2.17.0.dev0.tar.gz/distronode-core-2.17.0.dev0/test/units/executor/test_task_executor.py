...
from distronode.executor.task_executor import TaskExecutor, remove_omit
from distronode.plugins.loader import action_loader, lookup_loader
from distronode.parsing.yaml.objects import DistronodeUnicode
from distronode.utils.unsafe_proxy import DistronodeUnsafeText, DistronodeUnsafeBytes
...
class TestTaskExecutor(unittest.TestCase):
    ...
    def test_task_executor_run(self):
        ...
        te._get_loop_items = MagicMock(side_effect=DistronodeError(""))
        res = te.run()
    ...
    def test_task_executor_run_clean_res(self):
        ...
        te._run_loop = MagicMock(
            return_value=[
                {
                    'unsafe_bytes': DistronodeUnsafeBytes(b'{{ $bar }}'),
                    'unsafe_text': DistronodeUnsafeText(u'{{ $bar }}'),
                    'bytes': b'bytes',
                    'text': u'text',
                    'int': 1,
                }
        ...
        res = te.run()
        data = res['results'][0]
        self.assertIsInstance(data['unsafe_bytes'], DistronodeUnsafeText)
        self.assertIsInstance(data['unsafe_text'], DistronodeUnsafeText)
    ...
    def test_task_executor_get_action_handler(self):
        ...
        with patch('distronode.executor.task_executor.start_connection'):
    ...
    def test_task_executor_get_handler_normal(self):
        ...
        action_loader.get.assert_called_with(
            'distronode.legacy.normal', task=te._task, connection=te._connection,
            play_context=te._play_context, loader=te._loader,
            templar=mock_templar, shared_loader_obj=te._shared_loader_obj,
    ...
    def test_task_executor_execute(self):
        ...
        mock_action.run.return_value = dict(distronode_facts=dict())
        res = te._execute()

        mock_task.changed_when = MagicMock(return_value=DistronodeUnicode("1 == 1"))
        res = te._execute()

        mock_task.changed_when = None
        mock_task.failed_when = MagicMock(return_value=DistronodeUnicode("1 == 1"))
        res = te._execute()
    ...
    def test_task_executor_poll_async_result(self):
        ...
        with patch.object(action_loader, 'get', _get):
            ...
            self.assertIn('failed', res)
            res = te._poll_async_result(result=dict(distronode_job_id=1), templar=mock_templar)
            self.assertIn('failed', res)
