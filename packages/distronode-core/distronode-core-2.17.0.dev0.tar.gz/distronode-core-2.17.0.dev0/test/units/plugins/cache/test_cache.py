...
from distronode.plugins.cache import CachePluginAdjudicator
from distronode.plugins.cache.memory import CacheModule as MemoryCache
from distronode.plugins.loader import cache_loader, init_plugin_loader
...
class TestJsonFileCache(TestCachePluginAdjudicator):
    ...
    def setUp(self):
        self.cache_dir = tempfile.mkdtemp(prefix='distronode-plugins-cache-')
        self.cache = CachePluginAdjudicator(
...
class TestFactCache(unittest.TestCase):
    def setUp(self):
        with mock.patch('distronode.constants.CACHE_PLUGIN', 'memory'):
    ...
    def test_plugin_load_failure(self):
        init_plugin_loader()
        # See https://github.com/distronode/distronode/issues/18751
        # Note no fact_connection config set, so this will fail
        with mock.patch('distronode.constants.CACHE_PLUGIN', 'json'):
