...
from distronode.errors import DistronodeError, DistronodeAuthenticationFailure
from distronode.module_utils.six import text_type
from distronode.module_utils.six.moves import shlex_quote, builtins
...
from distronode.vars.clean import clean_facts
...
#MODULE_COMPLEX_ARGS = "<<INCLUDE_DISTRONODE_MODULE_COMPLEX_ARGS>>"
...
WINDOWS_ARGS = "<<INCLUDE_DISTRONODE_MODULE_JSON_ARGS>>"
# POWERSHELL_COMMON
"""
...
class TestActionBase(unittest.TestCase):
    ...
    def test_action_base__configure_module(self):
        init_plugin_loader()
        # Pre-populate the distronode.builtin collection
        # so reading the distronode_builtin_runtime.yml happens
        # before the mock_open below
        import_module('distronode_collections.distronode.builtin')
        fake_loader = DictDataLoader({
        })
        ...
        with patch.object(builtins, 'open', mock_open(read_data=to_bytes(python_module_replacers.strip(), encoding='utf-8'))):
            with patch.object(os, 'rename'):
                ...
                (style, shebang, data, path) = action_base._configure_module(mock_task.action, mock_task.args,
                                                                             task_vars=dict(distronode_python_interpreter='/usr/bin/python',
                                                                                            distronode_playbook_python='/usr/bin/python'))
                self.assertEqual(style, "new")
                self.assertEqual(shebang, u"#!/usr/bin/python")

                # test module not found
                self.assertRaises(DistronodeError, action_base._configure_module, 'badmodule', mock_task.args, {})
    ...
    def test_action_base__compute_environment_string(self):
        ...
        self.assertRaises(DistronodeError, action_base._compute_environment_string)
    ...
    def test_action_base__make_tmp_path(self):
        ...
        self.assertRaises(DistronodeError, action_base._make_tmp_path, 'root')
    ...
    def test_action_base__fixup_perms2(self):
        ...
        def raise_if_plus_a(definitely_not_underscore, mode):
            if mode == '+a':
                raise DistronodeAuthenticationFailure()
        ...
        # We threw DistronodeAuthenticationFailure above, try Solaris fallback.
        ...
        action_base.get_shell_option.side_effect = get_shell_option_for_arg(
            {},
            None)
        assertThrowRegex('on the temporary files Distronode needs to create')
    ...
    def test_action_base__remove_tmp_path(self):
        ...
        action_base._remove_tmp_path('/good/path/to/distronode-tmp-thing')
    ...
    def test_action_base__transfer_data(self, mock_mkstemp, mock_fdopen, mock_unlink):
        ...
        self.assertRaises(DistronodeError, action_base._transfer_data, '/path/to/remote/file', '')
    ...
    def test_action_base__execute_remote_stat(self):
        ...
        self.assertRaises(DistronodeError, action_base._execute_remote_stat, path='/path/to/file', all_vars=dict(), follow=False)
    ...
    def test_action_base__execute_module(self):
        ...
        self.assertEqual(action_base._execute_module(module_name=None, module_args=None), dict(_distronode_parsed=True, rc=0, stdout="ok", stdout_lines=['ok']))
        self.assertEqual(
        ...
        action_base._is_pipelining_enabled.return_value = False
        action_base._make_tmp_path.return_value = '/the/tmp/path'
        self.assertEqual(action_base._execute_module(), dict(_distronode_parsed=True, rc=0, stdout="ok", stdout_lines=['ok']))
        ...
        action_base._is_pipelining_enabled.return_value = False
        action_base._make_tmp_path.return_value = '/the/tmp/path'
        self.assertRaises(DistronodeError, action_base._execute_module)
    ...
    def test__remote_expand_user_relative_pathing(self):
        ...
        with self.assertRaises(DistronodeError) as cm:
...
class TestActionBaseCleanReturnedData(unittest.TestCase):
    def test(self):
        data = {'distronode_playbook_python': '/usr/bin/python',
                'distronode_python_interpreter': '/usr/bin/python',
                'distronode_ssh_some_var': 'whatever',
                'distronode_ssh_host_key_somehost': 'some key here',
                'some_other_var': 'foo bar'}
        data = clean_facts(data)
        self.assertNotIn('distronode_playbook_python', data)
        self.assertNotIn('distronode_python_interpreter', data)
        self.assertIn('distronode_ssh_host_key_somehost', data)
...
class TestActionBaseParseReturnedData(unittest.TestCase):
    ...
    def test_fail_no_json(self):
        ...
        self.assertFalse(res['_distronode_parsed'])
    ...
    def test_json_empty(self):
        ...
        returned_data = {'rc': rc,
                         ...
                         'stderr': err}
        res = action_base._parse_returned_data(returned_data)
        del res['_distronode_parsed']  # we always have _distronode_parsed
    ...
    def test_json_facts(self):
        action_base = _action_base()
        rc = 0
        stdout = '{"distronode_facts": {"foo": "bar", "distronode_blip": "blip_value"}}\n'
        err = ''
        ...
        returned_data = {'rc': rc,
                         ...
                         'stderr': err}
        res = action_base._parse_returned_data(returned_data)
        self.assertTrue(res['distronode_facts'])
        self.assertIn('distronode_blip', res['distronode_facts'])
        # TODO: Should this be an DistronodeUnsafe?
        # self.assertIsInstance(res['distronode_facts'], DistronodeUnsafe)
