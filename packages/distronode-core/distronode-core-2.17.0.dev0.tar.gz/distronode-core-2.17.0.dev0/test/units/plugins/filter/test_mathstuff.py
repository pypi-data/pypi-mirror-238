...
import distronode.plugins.filter.mathstuff as ms
...
class TestLogarithm:
    def test_log_non_number(self):
        ...
        with pytest.raises(DistronodeFilterTypeError, match='log\\(\\) can only be used on numbers: (a float is required|must be real number, not str)'):
            ms.logarithm('a')
        with pytest.raises(DistronodeFilterTypeError, match='log\\(\\) can only be used on numbers: (a float is required|must be real number, not str)'):
...
class TestPower:
    def test_power_non_number(self):
        # Message changed in python3.6
        with pytest.raises(DistronodeFilterTypeError, match='pow\\(\\) can only be used on numbers: (a float is required|must be real number, not str)'):
            ms.power('a', 10)
...
class TestInversePower:
    def test_root_non_number(self):
        ...
        with pytest.raises(DistronodeFilterTypeError, match="root\\(\\) can only be used on numbers:"
                           " (invalid literal for float\\(\\): a"
                           "|could not convert string to float: a"
                           "|could not convert string to float: 'a')"):
            ms.inversepower(10, 'a')

        with pytest.raises(DistronodeFilterTypeError, match="root\\(\\) can only be used on numbers: (a float is required|must be real number, not str)"):
...
class TestRekeyOnMember():
    ...
    INVALID_ENTRIES = (
        ...
        (DistronodeFilterError, [{"proto": "eigrp", "state": "enabled"}], 'invalid_key', "Key invalid_key was not found"),
        (DistronodeFilterError, {"eigrp": {"proto": "eigrp", "state": "enabled"}}, 'invalid_key', "Key invalid_key was not found"),
        # Fail when key is duplicated
        (DistronodeFilterError, [{"proto": "eigrp"}, {"proto": "ospf"}, {"proto": "ospf"}],
         'proto', 'Key ospf is not unique, cannot correctly turn into dict'),
        # Fail when value is not a dict
        (DistronodeFilterTypeError, ["string"], 'proto', "List item is not a valid dict"),
        (DistronodeFilterTypeError, [123], 'proto', "List item is not a valid dict"),
        (DistronodeFilterTypeError, [[{'proto': 1}]], 'proto', "List item is not a valid dict"),
        # Fail when we do not send a dict or list
        (DistronodeFilterTypeError, "string", 'proto', "Type is not a valid list, set, or dict"),
        (DistronodeFilterTypeError, 123, 'proto', "Type is not a valid list, set, or dict"),
    )
