...
from distronode.plugins.connection import winrm
...
class TestConnectionWinRM(object):
    ...
    OPTIONS_DATA = (
        ...
        (
            {'_extras': {}, 'distronode_user': 'user@domain.com'},
            {},
            {
                '_kerb_managed': False,
                '_kinit_cmd': 'kinit',
        ...
        (
            {'_extras': {}, 'distronode_user': 'user@domain.com',
             'distronode_winrm_kinit_mode': 'managed'},
            {'password': 'pass'},
            {
                '_kerb_managed': True,
            },
        ...
        (
            {'_extras': {}, 'distronode_user': 'user@domain.com',
             'distronode_winrm_kinit_mode': 'manual'},
            {'password': 'pass'},
            {
                '_kerb_managed': False,
            },
        ...
        (
            {'_extras': {}, 'distronode_user': 'user@domain.com',
             'distronode_winrm_transport': ['ntlm', 'certificate']},
            {},
            {
                '_winrm_kwargs': {'username': 'user@domain.com',
                                  'password': None},
        ...
        # winrm extras
        (
            {'_extras': {'distronode_winrm_server_cert_validation': 'ignore',
                         'distronode_winrm_service': 'WSMAN'}},
            {},
            {
                '_winrm_kwargs': {'username': None, 'password': None,
        ...
        # direct override
        (
            {'_extras': {}, 'distronode_winrm_connection_timeout': 5},
        ...
        # password as distronode_password
        (
            {'_extras': {}, 'distronode_password': 'pass'},
            {},
            {
                '_winrm_pass': 'pass',
        ...
        # password as distronode_winrm_pass
        (
            {'_extras': {}, 'distronode_winrm_pass': 'pass'},
            {},
            {
                '_winrm_pass': 'pass',
                '_winrm_kwargs': {'username': None, 'password': 'pass'}
        ...
        # password as distronode_winrm_password
        (
            {'_extras': {}, 'distronode_winrm_password': 'pass'},
            {},
            {
                '_winrm_pass': 'pass',
                '_winrm_kwargs': {'username': None, 'password': 'pass'}
...
class TestWinRMKerbAuth(object):
    ...
    @pytest.mark.parametrize('options, expected', [
        [{"_extras": {}},
         (["kinit", "user@domain"],)],
        [{"_extras": {}, 'distronode_winrm_kinit_cmd': 'kinit2'},
         (["kinit2", "user@domain"],)],
        [{"_extras": {'distronode_winrm_kerberos_delegation': True}},
         (["kinit", "-f", "user@domain"],)],
        [{"_extras": {}, 'distronode_winrm_kinit_args': '-f -p'},
         (["kinit", "-f", "-p", "user@domain"],)],
        [{"_extras": {}, 'distronode_winrm_kerberos_delegation': True, 'distronode_winrm_kinit_args': '-p'},
         (["kinit", "-p", "user@domain"],)]
    ])
    ...
    def test_kinit_with_missing_executable_subprocess(self, monkeypatch):
        ...
        new_stdin = StringIO()
        conn = connection_loader.get('winrm', pc, new_stdin)
        options = {"_extras": {}, "distronode_winrm_kinit_cmd": "/fake/kinit"}
        conn.set_options(var_options=options)
        conn._build_winrm_kwargs()

        with pytest.raises(DistronodeConnectionFailure) as err:
            conn._kerb_auth("user@domain", "pass")
    ...
    def test_exec_command_with_timeout(self, monkeypatch):
        ...
        with pytest.raises(DistronodeConnectionFailure) as e:
    ...
    def test_connect_failure_auth_401(self, monkeypatch):
        ...
        new_stdin = StringIO()
        conn = connection_loader.get('winrm', pc, new_stdin)
        conn.set_options(var_options={"distronode_winrm_transport": "basic", "_extras": {}})
        ...
        with pytest.raises(DistronodeConnectionFailure, match="the specified credentials were rejected by the server"):
            conn.exec_command('cmd', in_data=None, sudoable=True)
    ...
    def test_connect_failure_other_exception(self, monkeypatch):
        ...
        with pytest.raises(DistronodeConnectionFailure, match="basic: Custom exc"):
            conn.exec_command('cmd', in_data=None, sudoable=True)
    ...
    def test_connect_failure_operation_timed_out(self, monkeypatch):
        ...
        with pytest.raises(DistronodeError, match="the connection attempt timed out"):
            conn.exec_command('cmd', in_data=None, sudoable=True)
    ...
    def test_connect_no_transport(self):
        ...
        with pytest.raises(DistronodeError, match="No transport found for WinRM connection"):
            conn._winrm_connect()
