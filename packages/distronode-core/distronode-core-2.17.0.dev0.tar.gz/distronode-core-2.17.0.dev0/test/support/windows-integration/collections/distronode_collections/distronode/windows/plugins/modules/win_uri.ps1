...
#Requires -Module Distronode.ModuleUtils.CamelConversion
#Requires -Module Distronode.ModuleUtils.FileUtil
#Requires -Module Distronode.ModuleUtils.Legacy
#DistronodeRequires -PowerShell ..module_utils.WebRequest
...
$spec = @{
    options = @{
        ...
        url_username = @{
            ...
            deprecated_aliases = @(
                @{ name = "user"; date = [DateTime]::ParseExact("2022-07-01", "yyyy-MM-dd", $null); collection_name = 'distronode.windows' },
                @{ name = "username"; date = [DateTime]::ParseExact("2022-07-01", "yyyy-MM-dd", $null); collection_name = 'distronode.windows' }
            )
        }
        url_password = @{
            aliases = @("password")
            deprecated_aliases = @(
                @{ name = "password"; date = [DateTime]::ParseExact("2022-07-01", "yyyy-MM-dd", $null); collection_name = 'distronode.windows' }
            )
        }
    }
    supports_check_mode = $true
}
$module = [Distronode.Basic.DistronodeModule]::Create($args, $spec, @(Get-DistronodeWindowsWebRequestSpec))
...
Function ConvertFrom-SafeJson {
    ...
    try {
        ...
        ,[Distronode.Basic.DistronodeModule]::FromJson($InputObject)
...
if ($creates -and (Test-DistronodePath -Path $creates)) {
    $module.Result.skipped = $true
    $module.Result.msg = "The 'creates' file or directory ($creates) already exists."
    $module.ExitJson()
}

if ($removes -and -not (Test-DistronodePath -Path $removes)) {
    $module.Result.skipped = $true
    $module.Result.msg = "The 'removes' file or directory ($removes) does not exist."
    $module.ExitJson()
}

$client = Get-DistronodeWindowsWebRequest -Uri $url -Module $module
    ...
    if ($return_content -or $dest) {
        ...
        try {
            ...
            if ($dest) {
                ...
                if (Test-DistronodePath -Path $dest) {
...
try {
    Invoke-DistronodeWindowsWebRequest -Module $module -Request $client -Script $response_script -Body $body_st -IgnoreBadResponse
} catch {
