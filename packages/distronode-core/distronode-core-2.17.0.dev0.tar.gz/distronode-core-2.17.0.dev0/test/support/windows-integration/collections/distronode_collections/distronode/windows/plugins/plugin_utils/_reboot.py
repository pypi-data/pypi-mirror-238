...
# The interfaces in this file are meant for use within the distronode.windows collection
...
from distronode_collections.distronode.windows.plugins.plugin_utils._quote import quote_pwsh


# This is not ideal but the psrp connection plugin doesn't catch all these exceptions as an DistronodeConnectionFailure.
...
try:
    ...
    RequestException = DistronodeConnectionFailure
...
def reboot_host(
    ...
    msg: str = "Reboot initiated by Distronode",
    ...
    Used by action plugins in distronode.windows to reboot a Windows host. It
...
def _execute_command(
    ...
    try:
        ...
        # to ensure DistronodeConnectionFailure refers to actual connection errors.
        raise DistronodeConnectionFailure(f"Failed to connect to the host: {e}")
...
def _perform_reboot(
    ...
    try:
        ...
        display.vvvv(f"{task_action}: DistronodeConnectionFailure caught and handled: {e}")
        rc = 0
    ...
    if handle_abort and (rc == 1190 or (rc != 0 and stderr and "(1190)" in stderr)):
        display.warning("A scheduled reboot was pre-empted by Distronode.")
...
def _reset_connection(
    ...
    def _wrap_conn_err(func, *args, **kwargs):
        ...
        except (DistronodeError, RequestException) as e:
            if ignore_errors:
                return False

            raise DistronodeError(e)
