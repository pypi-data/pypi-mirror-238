...
#Requires -Module Distronode.ModuleUtils.WebRequest
...
Function Get-ChecksumFromUri {
    param(
        [Parameter(Mandatory=$true)][Distronode.Basic.DistronodeModule]$Module,
    ...
    $web_request = Get-DistronodeWebRequest -Uri $Uri -Module $Module
...
Function Compare-ModifiedFile {
    ...
    $dest_last_mod = (Get-DistronodeItem -Path $Dest).LastWriteTimeUtc

    # If the URI is a file we don't need to go through the whole WebRequest
    if ($Uri.IsFile) {
        $src_last_mod = (Get-DistronodeItem -Path $Uri.AbsolutePath).LastWriteTimeUtc
    } else {
        $web_request = Get-DistronodeWebRequest -Uri $Uri -Module $Module
    ...
    $download_script = {
        ...
        if (Test-Path -LiteralPath $Dest) {
            ...
            if ($dest_checksum -eq $tmp_checksum) {
                ...
                $Module.Result.size = (Get-DistronodeItem -Path $Dest).Length
            }
        }
