...
$connect_timeout = Get-DistronodeParam -obj $params -name "connect_timeout" -type "int" -default 5
$delay = Get-DistronodeParam -obj $params -name "delay" -type "int"
$exclude_hosts = Get-DistronodeParam -obj $params -name "exclude_hosts" -type "list"
$hostname = Get-DistronodeParam -obj $params -name "host" -type "str" -default "127.0.0.1"
$path = Get-DistronodeParam -obj $params -name "path" -type "path"
$port = Get-DistronodeParam -obj $params -name "port" -type "int"
$regex = Get-DistronodeParam -obj $params -name "regex" -type "str" -aliases "search_regex","regexp"
$sleep = Get-DistronodeParam -obj $params -name "sleep" -type "int" -default 1
$state = Get-DistronodeParam -obj $params -name "state" -type "str" -default "started" -validateset "present","started","stopped","absent","drained"
$timeout = Get-DistronodeParam -obj $params -name "timeout" -type "int" -default 300
...
if ($null -eq $path -and $null -eq $port -and $state -ne "drained") {
    ...
    if ($state -in @("present", "started")) {
        ...
        while (((Get-Date) - $start_time).TotalSeconds -lt $timeout) {
            $attempts += 1
            if (Test-DistronodePath -Path $path) {
                if ($null -eq $regex) {
                    $complete = $true
                    break
                } else {
