...
$check_mode = Get-DistronodeParam -obj $params -name "_distronode_check_mode" -type "bool" -default $false
$diff_mode = Get-DistronodeParam -obj $params -name "_distronode_diff" -type "bool" -default $false
$_remote_tmp = Get-DistronodeParam $params "_distronode_remote_tmp" -type "path" -default $env:TMP

$path = Get-DistronodeParam -obj $params -name "path" -type "str" -failifempty $true -aliases "key"
$name = Get-DistronodeParam -obj $params -name "name" -type "str" -aliases "entry","value"
$data = Get-DistronodeParam -obj $params -name "data"
$type = Get-DistronodeParam -obj $params -name "type" -type "str" -default "string" -validateset "none","binary","dword","expandstring","multistring","string","qword" -aliases "datatype"
$state = Get-DistronodeParam -obj $params -name "state" -type "str" -default "present" -validateset "present","absent"
$delete_key = Get-DistronodeParam -obj $params -name "delete_key" -type "bool" -default $true
$hive = Get-DistronodeParam -obj $params -name "hive" -type "path"
...
namespace Distronode.WinRegedit
{
...
if ($registry_path -ne $registry_leaf -and -not $registry_path.Contains('\')) {
    $msg = "path is not using '\' as a separator, support for '/' as a separator will be removed in a future Distronode version"
...
Function Set-StateAbsent {
    param(
        # Used for diffs and exception messages to match up against Distronode input
...
if ($type -in @("binary", "none")) {
    ...
    if ($data -is [String]) {
        ...
        # it needs to be unsigned as Distronode passes in an unsigned value while
...
try {
    if ($hive) {
        ...
        try {
            Set-DistronodePrivilege -Name SeBackupPrivilege -Value $true
            Set-DistronodePrivilege -Name SeRestorePrivilege -Value $true
        ...
        if (Test-Path -Path HKLM:\DISTRONODE) {
            Add-Warning -obj $result -message "hive already loaded at HKLM:\DISTRONODE, had to unload hive for win_regedit to continue"
            try {
                [Distronode.WinRegedit.Hive]::UnloadHive("DISTRONODE")
            } catch [System.ComponentModel.Win32Exception] {
                Fail-Json -obj $result -message "failed to unload registry hive HKLM:\DISTRONODE from $($hive): $($_.Exception.Message)"
            }
        }

        try {
            $loaded_hive = New-Object -TypeName Distronode.WinRegedit.Hive -ArgumentList "DISTRONODE", $hive
        } catch [System.ComponentModel.Win32Exception] {
            Fail-Json -obj $result -message "failed to load registry hive from '$hive' to HKLM:\DISTRONODE: $($_.Exception.Message)"
        }
    }
