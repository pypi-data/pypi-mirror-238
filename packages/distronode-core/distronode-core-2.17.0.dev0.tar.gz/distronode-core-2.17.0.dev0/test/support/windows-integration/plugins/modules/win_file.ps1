...
$check_mode = Get-DistronodeParam -obj $params -name "_distronode_check_mode" -default $false
$_remote_tmp = Get-DistronodeParam $params "_distronode_remote_tmp" -type "path" -default $env:TMP

$path = Get-DistronodeParam -obj $params -name "path" -type "path" -failifempty $true -aliases "dest","name"
$state = Get-DistronodeParam -obj $params -name "state" -type "str" -validateset "absent","directory","file","touch"
