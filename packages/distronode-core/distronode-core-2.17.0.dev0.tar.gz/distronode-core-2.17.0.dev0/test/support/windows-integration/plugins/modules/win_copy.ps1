...
$copy_mode = Get-DistronodeParam -obj $params -name "_copy_mode" -type "str" -default "single" -validateset "explode","query","remote","single"

# used in explode, remote and single mode
$src = Get-DistronodeParam -obj $params -name "src" -type "path" -failifempty ($copy_mode -in @("explode","process","single"))
$dest = Get-DistronodeParam -obj $params -name "dest" -type "path" -failifempty $true
$backup = Get-DistronodeParam -obj $params -name "backup" -type "bool" -default $false

# used in single mode
$original_basename = Get-DistronodeParam -obj $params -name "_original_basename" -type "str"

# used in query and remote mode
$force = Get-DistronodeParam -obj $params -name "force" -type "bool" -default $true

# used in query mode, contains the local files/directories/symlinks that are to be copied
$files = Get-DistronodeParam -obj $params -name "files" -type "list"
$directories = Get-DistronodeParam -obj $params -name "directories" -type "list"
...
if ($copy_mode -eq "query") {
    ...
    # always result in a change as the calculation is done on the Distronode side
