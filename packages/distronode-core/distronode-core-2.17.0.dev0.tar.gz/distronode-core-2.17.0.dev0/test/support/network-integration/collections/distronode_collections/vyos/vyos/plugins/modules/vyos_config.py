...
options:
  ...
  backup:
    ...
    - The C(backup) argument will backup the current devices active configuration
      to the Distronode control host prior to making any changes. If the C(backup_options)
      ...
      distronode role. If the directory does not exist, it is created.
    type: bool
    default: 'no'
  comment:
    description:
...
backup_path:
  ...
  type: str
  sample: /playbooks/distronode/backup/vyos_config.2016-07-16@22:28:34
filename:
...
shortname:
  ...
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: /playbooks/distronode/backup/vyos_config
date:
