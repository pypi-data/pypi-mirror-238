...
options:
  ...
  src:
    ...
    - Specifies the source path to the file that contains the configuration or configuration
      ...
      the Distronode control host or a relative path from the playbook or role root directory.  This
  ...
  backup:
    ...
    - This argument will cause the module to create a full backup of the current C(running-config)
      ...
      playbook root directory or role root directory, if playbook is part of an distronode
  ...
  save_when:
    ...
    - When changes are made to the device running-configuration, the changes are not
      ...
      has made a change. I(changed) was added in Distronode 2.5.
    default: never
    choices:
    - always
    - never
  ...
  diff_against:
    description:
    - When using the C(distronode-playbook --diff) command line argument the module can
...
- name: SETTING BOOT IMAGE
  ...
  when: distronode_net_version != version
...
backup_path:
  ...
  type: str
  sample: /playbooks/distronode/backup/ios_config.2016-07-16@22:28:34
filename:
...
shortname:
  ...
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: /playbooks/distronode/backup/ios_config
date:
...
from distronode.module_utils.connection import ConnectionError
...
from distronode_collections.distronode.netcommon.plugins.module_utils.network.common.config import (
    NetworkConfig,
    dumps,
)
