...
from distronode.plugins.action.normal import ActionModule as _ActionModule
...
class ActionModule(_ActionModule):
    ...
    def _handle_backup_option(self, result, task_vars):
        ...
        try:
            ...
            raise DistronodeError("Failed while reading configuration backup")
    ...
    def _handle_src_option(self, convert_data=True):
        ...
        if not os.path.exists(source):
            raise DistronodeError("path specified in src not found")
    ...
    def _get_network_os(self, task_vars):
        if "network_os" in self._task.args and self._task.args["network_os"]:
            ...
            "network_os" in task_vars.get("distronode_facts", {})
            and task_vars["distronode_facts"]["network_os"]
        ):
            display.vvvv("Getting network OS from fact")
            network_os = task_vars["distronode_facts"]["network_os"]
        else:
            raise DistronodeError(
                "distronode_network_os must be specified on this host"
            )
