...
DOCUMENTATION = """
author:
 - Distronode Networking Team (@distronode-network)
name: network_cli
...
version_added: 1.0.0
requirements:
- distronode-pylibssh if using I(ssh_type=libssh)
extends_documentation_fragment:
- distronode.netcommon.connection_persistent
options:
  host:
    description:
  ...
  become_errors:
    ...
    vars:
    - name: distronode_network_become_errors
    default: fail
  ...
  terminal_errors:
    ...
    - When set to C(ignore), the errors are silently ignored.
      ...
      The default option C(fail), triggers a failure and halts execution.
    vars:
    - name: distronode_network_terminal_errors
    default: fail
    choices: ["ignore", "warn", "fail"]
    version_added: 3.1.0
  become_method:
    ...
    - section: privilege_escalation
      key: become_method
    env:
    - name: DISTRONODE_BECOME_METHOD
    vars:
    - name: distronode_become_method
  host_key_auto_add:
    type: boolean
    description:
    - By default, Distronode will prompt the user before adding SSH keys to the known
    ...
    env:
    - name: DISTRONODE_HOST_KEY_AUTO_ADD
  persistent_buffer_read_timeout:
    ...
    env:
    - name: DISTRONODE_PERSISTENT_BUFFER_READ_TIMEOUT
    vars:
    - name: distronode_buffer_read_timeout
  terminal_stdout_re:
    type: list
    elements: dict
    description:
    ...
    vars:
    - name: distronode_terminal_stdout_re
  terminal_stderr_re:
    type: list
    elements: dict
    description:
    ...
    vars:
    - name: distronode_terminal_stderr_re
  terminal_initial_prompt:
    ...
    vars:
    - name: distronode_terminal_initial_prompt
  terminal_initial_answer:
    ...
    - The answer to reply with if the C(terminal_initial_prompt) is matched. The value
      ...
      matched.
    vars:
    - name: distronode_terminal_initial_answer
  terminal_initial_prompt_checkall:
    ...
    default: false
    vars:
    - name: distronode_terminal_initial_prompt_checkall
  terminal_inital_prompt_newline:
    ...
    default: true
    vars:
    - name: distronode_terminal_initial_prompt_newline
  network_cli_retries:
    description:
    ...
    type: integer
    env:
    - name: DISTRONODE_NETWORK_CLI_RETRIES
    ini:
    - section: persistent_connection
  ...
  ssh_type:
    description:
      ...
      - I(libssh) will use the distronode-pylibssh package, which needs to be installed in order to work.
      - I(paramiko) will instead use the paramiko package to manage the SSH connection.
      - I(auto) will use distronode-pylibssh if that package is installed, otherwise will fallback to paramiko.
    default: auto
    choices: ["libssh", "paramiko", "auto"]
    env:
        - name: DISTRONODE_NETWORK_CLI_SSH_TYPE
    ini:
        - section: persistent_connection
          key: ssh_type
    vars:
  ...
  single_user_mode:
    ...
    env:
    - name: DISTRONODE_NETWORK_SINGLE_USER_MODE
    vars:
    - name: distronode_network_single_user_mode
"""
...
from distronode.module_utils.six.moves import cPickle
from distronode.playbook.play_context import PlayContext
from distronode.plugins.loader import (
    cache_loader,
    cliconf_loader,
    connection_loader,
    terminal_loader,
...
from distronode_collections.distronode.netcommon.plugins.plugin_utils.connection_base import (
...
class DistronodeCmdRespRecv(Exception):
    pass
...
class Connection(NetworkConnectionBase):
    ...
    transport = "distronode.netcommon.network_cli"
    has_pipelining = True
    ...
    def __init__(self, play_context, new_stdin, *args, **kwargs):
        ...
        if self._network_os:
            ...
            raise DistronodeConnectionFailure(
                ...
                "manually configure distronode_network_os value for this host"
            )
        ...
        if self._ssh_type is None:
            ...
            if self._ssh_type == "auto":
                ...
                if HAS_PYLIBSSH:
                    ...
                    self.queue_message(
                        "warning",
                        "distronode-pylibssh not installed, falling back to paramiko",
                    )
    ...
    def ssh_type_conn(self):
        if self._ssh_type_conn is None:
            if self.ssh_type == "libssh":
                connection_plugin = "distronode.netcommon.libssh"
    ...
    def set_options(self, task_keys=None, var_options=None, direct=None):
        ...
        if not any(
            ...
            if not look_for_keys:
                # This actually can't be overridden yet without changes in distronode-core
    ...
    def receive_paramiko(
        ...
        while True:
            ...
            if self._find_prompt(window):
                if errored_response:
                    raise DistronodeConnectionFailure(errored_response)
    ...
    def copy_file(
        ...
        if self.ssh_type == "libssh":
            ...
            if proto == "scp":
                if not HAS_SCP:
                    raise DistronodeError(missing_required_lib("scp"))
                with SCPClient(
    ...
    def get_cache(self):
        if not self._cache:
            ...
            self._cache = cache_loader.get("distronode.netcommon.memory")
        return self._cache
    ...
    def _needs_cache_invalidation(self, command):
        ...
        cfg_cmds = []
        try:
            # DistronodePlugin base class in Distronode 2.9 does not have has_option() method.
