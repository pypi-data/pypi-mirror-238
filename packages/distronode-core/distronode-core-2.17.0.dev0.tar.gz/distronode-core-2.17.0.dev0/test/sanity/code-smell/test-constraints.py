...
def main():
    constraints_path = 'test/lib/distronode_test/_data/requirements/constraints.txt'

    requirements = {}

    for path in sys.argv[1:] or sys.stdin.read().splitlines():
        if path == 'test/lib/distronode_test/_data/requirements/distronode.txt':
            # This file is an exact copy of the distronode requirements.txt and should not conflict with other constraints.
            continue

        with open(path, 'r') as path_fd:
            requirements[path] = parse_requirements(path_fd.read().splitlines())

        if path == 'test/lib/distronode_test/_data/requirements/distronode-test.txt':
            # Special handling is required for distronode-test's requirements file.
            check_distronode_test(path, requirements.pop(path))
            continue
