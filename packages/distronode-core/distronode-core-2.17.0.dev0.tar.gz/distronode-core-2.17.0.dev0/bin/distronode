...
class AdHocCLI(CLI):
    ...
    def init_parser(self):
        ...
        # options unique to distronode ad-hoc
    ...
    def post_process_args(self, options):
        '''Post process and validate options for bin/distronode '''
    ...
    def run(self):
        ...
        if context.CLIARGS['module_name'] in C.MODULE_REQUIRE_ARGS and not context.CLIARGS['module_args']:
            ...
            if pattern.endswith(".yml"):
                err = err + ' (did you mean to run distronode-playbook?)'
            raise DistronodeOptionsError(err)

        # Avoid modules that don't work with ad-hoc
        if context.CLIARGS['module_name'] in C._ACTION_IMPORT_PLAYBOOK:
            raise DistronodeOptionsError("'%s' is not a valid action for ad-hoc commands"
        ...
        if self.callback:
            ...
            cb = 'oneline'
        # Respect custom 'stdout_callback' only with enabled 'bin_distronode_callbacks'
