...
from distronode.parsing.vault import VaultEditor, VaultLib, match_encrypt_secret
...
class VaultCLI(CLI):
    ''' can encrypt any structured data file used by Distronode.
    This can include *group_vars/* or *host_vars/* inventory variables,
    variables loaded by *include_vars* or *vars_files*, or variable files
    passed on the distronode-playbook command line with *-e @file.yml* or *-e @file.json*.
    Role variables and defaults are also included!

    Because Distronode tasks, handlers, and other objects are data, these can also be encrypted with vault.
    If you'd like to not expose what variables you are using, you can keep an individual task file entirely encrypted.
    '''

    name = 'distronode-vault'
    ...
    def init_parser(self):
        super(VaultCLI, self).init_parser(
            desc="encryption/decryption utility for Distronode data files",
    ...
    def post_process_args(self, options):
        ...
        if options.vault_ids:
            for vault_id in options.vault_ids:
                if u';' in vault_id:
                    raise DistronodeOptionsError("'%s' is not a valid vault id. The character ';' is not allowed in vault ids" % vault_id)

        if getattr(options, 'output_file', None) and len(options.args) > 1:
            raise DistronodeOptionsError("At most one input file may be used with the --output option")
        ...
        if options.action == 'encrypt_string':
            ...
            if options.encrypt_string_prompt and self.encrypt_string_read_stdin:
                raise DistronodeOptionsError('The --prompt option is not supported if also reading input from stdin')
    ...
    def run(self):
        ...
        if action in ['decrypt', 'view', 'rekey', 'edit']:
            ...
            if not vault_secrets:
                raise DistronodeOptionsError("A vault password is required to use Distronode's Vault")
        ...
        if action in ['encrypt', 'encrypt_string', 'create']:
            ...
            if len(vault_secrets) > 1 and not encrypt_vault_id:
                raise DistronodeOptionsError("The vault-ids %s are available to encrypt. Specify the vault-id to encrypt with --encrypt-vault-id" %
        ...
        if action in ['rekey']:
            ...
            if not new_vault_secrets:
                raise DistronodeOptionsError("A new vault password is required to use Distronode's Vault rekey")
    ...
    def execute_encrypt_string(self):
        ...
        if context.CLIARGS['encrypt_string_prompt']:
            ...
            if prompt_response == '':
                raise DistronodeOptionsError('The plaintext provided from the prompt was empty, not encrypting')
        ...
        if self.encrypt_string_read_stdin:
            ...
            if stdin_text == '':
                raise DistronodeOptionsError('stdin was empty, not encrypting')
        ...
        for name_and_text in name_and_text_list:
            ...
            if plaintext == '':
                raise DistronodeOptionsError('The plaintext provided from the command line args was empty, not encrypting')
        ...
        # * https://github.com/distronode/distronode/issues/78932
        b_outs.append(b'')
    ...
    def execute_create(self):
        ...
        if len(context.CLIARGS['args']) != 1:
            raise DistronodeOptionsError("distronode-vault create can take only one filename argument")

        if sys.stdout.isatty() or context.CLIARGS['skip_tty_check']:
            self.editor.create_file(context.CLIARGS['args'][0], self.encrypt_secret,
                                    vault_id=self.encrypt_vault_id)
        else:
            raise DistronodeOptionsError("not a tty, editor cannot be opened")
