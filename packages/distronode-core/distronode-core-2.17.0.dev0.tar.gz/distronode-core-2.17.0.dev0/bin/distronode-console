...
from distronode.plugins.loader import module_loader, fragment_loader
from distronode.utils import plugin_docs
...
class ConsoleCLI(CLI, cmd.Cmd):
    '''
       ...
       - ``!``: forces shell module instead of the distronode module
         (``!yum update -y``)
       ...
       - ``exit``: exit ``distronode-console``
    '''

    name = 'distronode-console'
    ...
    def __init__(self, args):
        ...
        self.intro = 'Welcome to the distronode console. Type help or ? to list commands.\n'
    ...
    def init_parser(self):
        super(ConsoleCLI, self).init_parser(
            desc="REPL console for executing Distronode tasks.",
    ...
    def cmdloop(self):
        try:
            ...
            self.display("[Distronode-console was exited]")
            self.do_exit(self)
    ...
    def run(self):
        ...
        histfile = os.path.join(os.path.expanduser("~"), ".distronode-console_history")
        try:
