...
from distronode.module_utils.compat.version import StrictVersion
...
from distronode.constants import DOCUMENTABLE_PLUGINS
...
def _add_distronode_error_code(exception, error_code):
    setattr(exception, 'distronode_error_code', error_code)
    return exception
...
def isodate(v, error_code=None):
    try:
        ...
        raise _add_distronode_error_code(Invalid(str(e)), error_code or 'distronode-invalid-date')
    return v
...
def collection_name(v, error_code=None):
    if not isinstance(v, string_types):
        raise _add_distronode_error_code(
...
def _check_url(directive, content):
    try:
        ...
        return [_add_distronode_error_code(
    ...
    for par in parse(v, Context(), errors='message', strict=True, add_source=True):
        for part in par:
            if part.type == dom.PartType.ERROR:
                errors.append(_add_distronode_error_code(Invalid(part.message), 'invalid-documentation-markup'))
            ...
            if part.type == dom.PartType.MODULE:
                if not FULLY_QUALIFIED_COLLECTION_RESOURCE_RE.match(part.fqcn):
                    errors.append(_add_distronode_error_code(Invalid(
...
def check_removal_version(v, version_field, collection_name_field, error_code='invalid-removal-version'):
    ...
    if collection_name == 'distronode.builtin':
        try:
            ...
            raise _add_distronode_error_code(
                Invalid('%s (%r) is not a valid distronode-core version: %s' % (version_field, version, exc)),
...
def distronode_module_kwargs_schema(module_name, for_collection):
    schema = {
...
def version_added(v, error_code='version-added-invalid', accept_historical=False):
    if 'version_added' in v:
        ...
        if isinstance(version_added, string_types):
            ...
            # - or we have a float and we are in distronode/distronode, in which case we're
            # also happy.
            if v.get('version_added_collection') == 'distronode.builtin':
                ...
                try:
                    ...
                    raise _add_distronode_error_code(
                        Invalid('version_added (%r) is not a valid distronode-core version: '
                                '%s' % (version_added, exc)),
                        error_code=error_code)
            else:
                try:
