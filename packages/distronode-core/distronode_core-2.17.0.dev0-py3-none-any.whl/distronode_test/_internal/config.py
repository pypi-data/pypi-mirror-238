...
class IntegrationConfig(TestConfig):
    ...
    def get_distronode_config(self) -> str:
        ...
        distronode_config_relative_path = os.path.join(data_context().content.integration_path, '%s.cfg' % self.command)
        distronode_config_path = os.path.join(data_context().content.root, distronode_config_relative_path)

        if not os.path.exists(distronode_config_path):
            # use the default empty configuration unless one has been provided
            distronode_config_path = super().get_distronode_config()

        return distronode_config_path
