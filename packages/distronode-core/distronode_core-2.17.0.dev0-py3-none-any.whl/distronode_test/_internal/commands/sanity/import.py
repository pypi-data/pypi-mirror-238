        ...
        for import_type, test in (
            ...
            if data_context().content.collection:
                ...
                env.update(
                    ...
                    SANITY_YAML_TO_JSON=os.path.join(DISTRONODE_TEST_TOOLS_ROOT, 'yaml_to_json.py'),
                    DISTRONODE_CONTROLLER_MIN_PYTHON_VERSION=CONTROLLER_MIN_PYTHON_VERSION,
                    PYTHONPATH=':'.join((get_distronode_test_python_path(), env["PYTHONPATH"])),
                )
...
@cache
def get_distronode_test_python_path() -> str:
    """
    Return a directory usable for PYTHONPATH, containing only the distronode-test collection loader.
