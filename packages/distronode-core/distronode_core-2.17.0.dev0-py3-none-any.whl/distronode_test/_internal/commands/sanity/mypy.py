...
class MypyTest(SanityMultipleVersion):
    ...
    vendored_paths = (
        'lib/distronode/module_utils/six/__init__.py',
        'lib/distronode/module_utils/distro/_distro.py',
    )
    ...
    def filter_targets(self, targets: list[TestTarget]) -> list[TestTarget]:
        ...
        return [target for target in targets if os.path.splitext(target.path)[1] == '.py' and target.path not in self.vendored_paths and (
                target.path.startswith('lib/distronode/') or target.path.startswith('test/lib/distronode_test/_internal/')
                or target.path.startswith('packaging/')
                or target.path.startswith('test/lib/distronode_test/_util/target/sanity/import/'))]
    ...
    def test(self, args: SanityConfig, targets: SanityTargets, python: PythonConfig) -> TestResult:
        ...
        contexts = (
            MyPyContext('distronode-test', ['test/lib/distronode_test/_util/target/sanity/import/'], controller_python_versions),
            MyPyContext('distronode-test', ['test/lib/distronode_test/_internal/'], controller_python_versions),
            MyPyContext('distronode-core', ['lib/distronode/'], controller_python_versions),
            MyPyContext('modules', ['lib/distronode/modules/', 'lib/distronode/module_utils/'], remote_only_python_versions),
    ...
    def test_context(
        ...
        config_path = os.path.join(DISTRONODE_TEST_CONTROLLER_ROOT, 'sanity', 'mypy', f'{context.name}.ini')
