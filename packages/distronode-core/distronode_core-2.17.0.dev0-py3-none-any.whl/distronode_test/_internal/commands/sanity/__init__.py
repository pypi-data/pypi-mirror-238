"""Execute Distronode sanity tests."""
...
from ...util import (
    ...
    DISTRONODE_TEST_CONTROLLER_ROOT,
...
COMMAND = 'sanity'
SANITY_ROOT = os.path.join(DISTRONODE_TEST_CONTROLLER_ROOT, 'sanity')
TARGET_SANITY_ROOT = os.path.join(DISTRONODE_TEST_TARGET_ROOT, 'sanity')

# NOTE: must match distronode.constants.DOCUMENTABLE_PLUGINS, but with 'module' replaced by 'modules'!
...
def collect_code_smell_tests() -> tuple[SanityTest, ...]:
    ...
    if data_context().content.is_distronode:
        # include Distronode specific code-smell tests which are not configured to be skipped
        distronode_code_smell_root = os.path.join(data_context().content.root, 'test', 'sanity', 'code-smell')
        skip_tests = read_lines_without_comments(os.path.join(distronode_code_smell_root, 'skip.txt'), remove_blank_lines=True, optional=True)
        paths.extend(path for path in glob.glob(os.path.join(distronode_code_smell_root, '*.py')) if os.path.basename(path) not in skip_tests)
...
class SanityIgnoreParser:
    ...
    def __init__(self, args: SanityConfig) -> None:
        if data_context().content.collection:
            distronode_version = '%s.%s' % tuple(get_distronode_version().split('.')[:2])

            distronode_label = 'Distronode %s' % distronode_version
            file_name = 'ignore-%s.txt' % distronode_version
        else:
            distronode_label = 'Distronode'
        ...
        display.info('Read %d sanity test ignore line(s) for %s from: %s' % (len(lines), distronode_label, self.relative_path), verbosity=1)
...
class SanityTest(metaclass=abc.ABCMeta):
    ...
    def error_code(self) -> t.Optional[str]:
        """Error code for distronode-test matching the format used by the underlying test program, or None if the program does not use error codes."""
        return None
    ...
    def filter_remote_targets(targets: list[TestTarget]) -> list[TestTarget]:
        ...
        targets = [target for target in targets if (
            ...
            # special handling for content in distronode-core
            (data_context().content.is_distronode and (
                # utility code that runs in target environments and requires support for remote-only Python versions
                is_subdir(target.path, 'test/lib/distronode_test/_util/target/') or
                ...
                re.search('^lib/distronode/utils/collection_loader/', target.path)
            ))
        )]
...
class SanityCodeSmellTest(SanitySingleVersion):
    ...
    def filter_targets(self, targets: list[TestTarget]) -> list[TestTarget]:
        ...
        if self.ignore_self and data_context().content.is_distronode:
    ...
    def test(self, args: SanityConfig, targets: SanityTargets, python: PythonConfig) -> TestResult:
        ...
        env = distronode_environment(args, color=False)
...
def sanity_get_tests() -> tuple[SanityTest, ...]:
    ...
    sanity_tests = tuple(plugin() for plugin in sanity_plugins.values() if data_context().content.is_distronode or not plugin.distronode_only)
...
def create_sanity_virtualenv(
    ...
    virtualenv_cache = os.path.join(os.path.expanduser('~/.distronode/test/venv'))
...
def omit_pre_build_from_requirement(path: str, requirements: str) -> tuple[str, str]:
    ...
    #          test/lib/distronode_test/_util/target/setup/requirements.py
