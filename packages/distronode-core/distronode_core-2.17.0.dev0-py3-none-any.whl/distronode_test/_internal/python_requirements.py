...
def install_requirements(
    ...
    if distronode:
        try:
            distronode_cache = install_requirements.distronode_cache  # type: ignore[attr-defined]
        except AttributeError:
            distronode_cache = install_requirements.distronode_cache = {}  # type: ignore[attr-defined]

        distronode_installed = distronode_cache.get(python.path)

        if distronode_installed:
            distronode = False
        else:
...
def run_pip(
    ...
    # It can be seen by running distronode-test with increased verbosity (showing all commands executed).
...
def collect_general_install(
    ...
    if distronode:
        path = os.path.join(DISTRONODE_TEST_DATA_ROOT, 'requirements', 'distronode.txt')
        requirements_paths.append((DISTRONODE_TEST_DATA_ROOT, path))

    if command:
        path = os.path.join(DISTRONODE_TEST_DATA_ROOT, 'requirements', f'{command}.txt')
        requirements_paths.append((DISTRONODE_TEST_DATA_ROOT, path))
...
def collect_sanity_install(sanity: str) -> list[PipInstall]:
    ...
    path = os.path.join(DISTRONODE_TEST_DATA_ROOT, 'requirements', f'sanity.{sanity}.txt')
    requirements_paths.append((DISTRONODE_TEST_DATA_ROOT, path))
...
def collect_integration_install(command: str, controller: bool) -> list[PipInstall]:
    ...
    # Support for prefixed files was added to distronode-test in distronode-core 2.12 when split controller/target testing was implemented.
    # Previous versions of distronode-test only recognize non-prefixed files.
...
def collect_install(
    ...
    # listing content constraints first gives them priority over constraints provided by distronode-test
    constraints_paths = list(constraints_paths)

    if constraints:
        constraints_paths.append((DISTRONODE_TEST_DATA_ROOT, os.path.join(DISTRONODE_TEST_DATA_ROOT, 'requirements', 'constraints.txt')))
...
def get_venv_packages(python: PythonConfig) -> dict[str, str]:
    ...
    #       See: https://github.com/distronode/base-test-container/blob/main/files/installer.py
...
def requirements_allowed(args: EnvironmentConfig, controller: bool) -> bool:
    ...
    The install will occur on the controller and the controller or controller Python is managed by distronode-test.
    The install will occur on the target and the target or target Python is managed by distronode-test.
    """
