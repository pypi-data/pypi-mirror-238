...
def makedirs_safe(path, mode=None):
    ...
    :raises DistronodeError: If the directory cannot be created and does not already exist.
    ...
    if not os.path.exists(b_rpath):
        try:
            ...
            if e.errno != EEXIST:
                raise DistronodeError("Unable to create local directories(%s): %s" % (to_native(rpath), to_native(e)))
