...
def random_password(length=DEFAULT_PASSWORD_LENGTH, chars=C.DEFAULT_PASSWORD_CHARS, seed=None):
    ...
    if not isinstance(chars, text_type):
        raise DistronodeAssertionError('%s (%s) is not a text_type' % (chars, type(chars)))
...
class CryptHash(BaseHash):
    def __init__(self, algorithm):
        ...
        if not HAS_CRYPT:
            raise DistronodeError("crypt.crypt cannot be used as the 'crypt' python library is not installed or is unusable.", orig_exc=CRYPT_E)

        if sys.platform.startswith('darwin'):
            raise DistronodeError("crypt.crypt not supported on Mac OS X/Darwin, install passlib python module")

        if algorithm not in self.algorithms:
            raise DistronodeError("crypt.crypt does not support '%s' algorithm" % self.algorithm)
    ...
    def _salt(self, salt, salt_size):
        ...
        if re.search(r'[^./0-9A-Za-z]', ret):
            raise DistronodeError("invalid characters in salt")
        if self.algo_data.salt_exact and len(ret) != self.algo_data.salt_size:
            raise DistronodeError("invalid salt size")
...
class PasslibHash(BaseHash):
    def __init__(self, algorithm):
        ...
        if not PASSLIB_AVAILABLE:
            raise DistronodeError("passlib must be installed and usable to hash with '%s'" % algorithm, orig_exc=PASSLIB_E)
        display.vv("Using passlib to hash input with '%s'" % algorithm)

        try:
            self.crypt_algo = getattr(passlib.hash, algorithm)
        except Exception:
            raise DistronodeError("passlib does not support '%s' algorithm" % algorithm)
    ...
    def _hash(self, secret, salt, salt_size, rounds, ident):
        ...
        try:
            if hasattr(self.crypt_algo, 'hash'):
                ...
                raise DistronodeError("installed passlib version %s not supported" % passlib.__version__)
        except ValueError as e:
            raise DistronodeError("Could not hash the secret.", orig_exc=e)
        ...
        if not result:
            raise DistronodeError("failed to hash with algorithm '%s'" % self.algorithm)
...
def do_encrypt(result, encrypt, salt_size=None, salt=None, ident=None, rounds=None):
    ...
    raise DistronodeError("Unable to encrypt nor hash, either crypt or passlib must be installed.", orig_exc=CRYPT_E)
