...
class _DistronodeCollectionConfig(type):
    ...
    def collection_finder(cls, value):
        if cls._collection_finder:
            raise ValueError('an DistronodeCollectionFinder has already been configured')
    ...
    def _require_finder(cls):
        if not cls._collection_finder:
            raise NotImplementedError('an DistronodeCollectionFinder has not been installed in this process')


# concrete class of our metaclass type that defines the class properties we want
@add_metaclass(_DistronodeCollectionConfig)
class DistronodeCollectionConfig(object):
    pass
