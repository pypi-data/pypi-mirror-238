...
# DO NOT add new non-stdlib import deps here, this loader is used by external tools (eg distronode-test import sanity)
# that only allow stdlib and module_utils
from distronode.module_utils.common.text.converters import to_native, to_text, to_bytes
from distronode.module_utils.six import string_types, PY3
from ._collection_config import DistronodeCollectionConfig
...
SYNTHETIC_PACKAGE_NAME = '<distronode_synthetic_collection_package>'


class _DistronodeNSTraversable:
    """Class that implements the ``importlib.resources.abc.Traversable``
    interface for the following ``distronode_collections`` namespace packages::

    * ``distronode_collections``
    * ``distronode_collections.<namespace>``
    ...
    * ``/usr/share/distronode/collections/distronode_collections/distronode/posix/``
    * ``/home/user/.distronode/collections/distronode_collections/distronode/windows/``
    ...
    def __repr__(self):
        return "_DistronodeNSTraversable('%s')" % "', '".join(map(to_text, self._paths))
...
class _DistronodeTraversableResources(TraversableResources):
    ...
    ``distronode_collections`` or a particular namespace. For a collection and
    its subpackages, a ``pathlib.Path`` instance will be returned, whereas
    for the higher level namespace packages, ``_DistronodeNSTraversable``
    will be returned.
    """
    ...
    def _is_distronode_ns_package(self, package):
    ...
    def _ensure_package(self, package):
        if self._is_distronode_ns_package(package):
    ...
    def files(self):
        ...
        parts = package.split('.')
        is_ns = parts[0] == 'distronode_collections' and len(parts) < 3
        ...
        if is_ns:
            return _DistronodeNSTraversable(*package.submodule_search_locations)
        return pathlib.Path(self._get_path(package)).parent


class _DistronodeCollectionFinder:
    def __init__(self, paths=None, scan_sys_paths=True):
        # TODO: accept metadata loader override
        self._distronode_pkg_path = to_native(os.path.dirname(to_bytes(sys.modules['distronode'].__file__)))
        ...
        for p in paths:
            ...
            # ensure we always have distronode_collections
            if os.path.basename(p) == 'distronode_collections':
                p = os.path.dirname(p)

            if p not in good_paths and os.path.isdir(to_bytes(os.path.join(p, 'distronode_collections'), errors='surrogate_or_strict')):
                good_paths.append(p)
    ...
    def _remove(cls):
        for mps in sys.meta_path:
            if isinstance(mps, _DistronodeCollectionFinder):
                sys.meta_path.remove(mps)

        # remove any path hooks that look like ours
        for ph in sys.path_hooks:
            if hasattr(ph, '__self__') and isinstance(ph.__self__, _DistronodeCollectionFinder):
                sys.path_hooks.remove(ph)

        # zap any cached path importer cache entries that might refer to us
        sys.path_importer_cache.clear()

        DistronodeCollectionConfig._collection_finder = None

        # validate via the public property that we really killed it
        if DistronodeCollectionConfig.collection_finder is not None:
            raise AssertionError('_DistronodeCollectionFinder remove did not reset DistronodeCollectionConfig.collection_finder')

    def _install(self):
        self._remove()
        sys.meta_path.insert(0, self)

        sys.path_hooks.insert(0, self._distronode_collection_path_hook)

        DistronodeCollectionConfig.collection_finder = self

    def _distronode_collection_path_hook(self, path):
        path = to_native(path)
    ...
    def set_playbook_paths(self, playbook_paths):
        ...
        # NB: this should NOT be used for late additions; ideally we'd fix the playbook dir setup earlier in Distronode init
        # to prevent this from occurring
        for pkg in ['distronode_collections', 'distronode_collections.distronode']:
    ...
    def _get_loader(self, fullname, path=None):
        ...
        if toplevel_pkg not in ['distronode', 'distronode_collections']:
            # not interested in anything other than distronode_collections (and limited cases under distronode)
            return None
        ...
        if toplevel_pkg == 'distronode':
            # something under the distronode package, delegate to our internal loader in case of redirections
            initialize_loader = _DistronodeInternalRedirectLoader
        elif part_count == 1:
            initialize_loader = _DistronodeCollectionRootPkgLoader
        elif part_count == 2:  # ns pkg eg, distronode_collections, distronode_collections.somens
            initialize_loader = _DistronodeCollectionNSPkgLoader
        elif part_count == 3:  # collection pkg eg, distronode_collections.somens.somecoll
            initialize_loader = _DistronodeCollectionPkgLoader
        else:
            # anything below the collection
            initialize_loader = _DistronodeCollectionLoader
...
class _DistronodePathHookFinder:
    ...
    def _get_finder(self, fullname):
        ...
        if toplevel_pkg == 'distronode_collections':
            # collections content? delegate to the collection finder
            return self._collection_finder
        else:
            # Something else; we'd normally restrict this to `distronode` descendent modules so that any weird loader
            # behavior that arbitrary Python modules have can be serviced by those loaders. In some dev/test
            # scenarios (eg a venv under a collection) our path_hook signs us up to load non-Distronode things, and
            ...
            if PY3:
                ...
                if not self._file_finder:
                    try:
                        self._file_finder = _DistronodePathHookFinder._filefinder_path_hook(self._pathctx)
                    except ImportError:
    ...
    def find_spec(self, fullname, target=None):
        ...
        if finder is None:
            return None
        elif toplevel_pkg == 'distronode_collections':
...
class _DistronodeCollectionPkgLoaderBase:
    ...
    def __init__(self, fullname, path_list=None):
        ...
        self._parent_package_name = self._rpart_name[0]  # eg distronode_collections for distronode_collections.somens, '' for toplevel
        self._package_to_load = self._rpart_name[2]  # eg somens for distronode_collections.somens
    ...
    def _validate_args(self):
        if self._split_name[0] != 'distronode_collections':
            raise ImportError('this loader can only load packages from the distronode_collections package, not {0}'.format(self._fullname))
    ...
    def get_resource_reader(self, fullname):
        return _DistronodeTraversableResources(fullname, self)
...
class _DistronodeCollectionRootPkgLoader(_DistronodeCollectionPkgLoaderBase):
    def _validate_args(self):
        super(_DistronodeCollectionRootPkgLoader, self)._validate_args()
        if len(self._split_name) != 1:
            raise ImportError('this loader can only load the distronode_collections toplevel package, not {0}'.format(self._fullname))


# Implements Distronode's custom namespace package support.
# The distronode_collections package and one level down (collections namespaces) are Python namespace packages
...
# by this loader.
class _DistronodeCollectionNSPkgLoader(_DistronodeCollectionPkgLoaderBase):
    def _validate_args(self):
        super(_DistronodeCollectionNSPkgLoader, self)._validate_args()
    ...
    def _validate_final(self):
        # special-case the `distronode` namespace, since `distronode.builtin` is magical
        if not self._subpackage_search_paths and self._package_to_load != 'distronode':
...
class _DistronodeCollectionPkgLoader(_DistronodeCollectionPkgLoaderBase):
    def _validate_args(self):
        super(_DistronodeCollectionPkgLoader, self)._validate_args()
        if len(self._split_name) != 3:
            raise ImportError('this loader can only load collection packages, not {0}'.format(self._fullname))

    def _validate_final(self):
        if self._split_name[1:3] == ['distronode', 'builtin']:
    ...
    def _load_module(self, module):
        if not _meta_yml_to_dict:
            raise ValueError('distronode.utils.collection_loader._meta_yml_to_dict is not set')
        ...
        if collection_name == 'distronode.builtin':
            # distronode.builtin is a synthetic collection, get its routing config from the Distronode distro
            distronode_pkg_path = os.path.dirname(import_module('distronode').__file__)
            metadata_path = os.path.join(distronode_pkg_path, 'config/distronode_builtin_runtime.yml')
        ...
        DistronodeCollectionConfig.on_collection_load.fire(collection_name=collection_name, collection_path=os.path.dirname(module.__file__))

        return module

    def exec_module(self, module):
        super(_DistronodeCollectionPkgLoader, self).exec_module(module)
    ...
    def load_module(self, fullname):
        module = super(_DistronodeCollectionPkgLoader, self).load_module(fullname)
...
class _DistronodeCollectionLoader(_DistronodeCollectionPkgLoaderBase):
    # HACK: stash this in a better place
    _redirected_package_map = {}  # type: dict[str, str]
    _allows_package_code = True

    def _validate_args(self):
        super(_DistronodeCollectionLoader, self)._validate_args()
    ...
    def _get_candidate_paths(self, path_list):
        if len(path_list) != 1 and self._split_name[1:3] != ['distronode', 'builtin']:
...
# This loader only answers for intercepted Distronode Python modules. Normal imports will fail here and be picked up later
# by our path_hook importer (which proxies the built-in import mechanisms, allowing normal caching etc to occur)
class _DistronodeInternalRedirectLoader:
    def __init__(self, fullname, path_list):
        ...
        builtin_meta = _get_collection_metadata('distronode.builtin')
...
class DistronodeCollectionRef:
    ...
    def __init__(self, collection_name, subdirs, resource, ref_type):
        """
        Create an DistronodeCollectionRef from components
        ...
        package_components = [u'distronode_collections', self.collection]
    ...
    def __repr__(self):
        return 'DistronodeCollectionRef(collection={0!r}, subdirs={1!r}, resource={2!r})'.format(self.collection, self.subdirs, self.resource)
    ...
    def from_fqcr(ref, ref_type):
        ...
        :param ref_type: the type of the reference, eg 'module', 'role', 'doc_fragment'
        :return: a populated DistronodeCollectionRef object
        """
        ...
        # ns.coll.resource -> distronode_collections.ns.coll.plugins.(plugintype).resource
        # ns.coll.subdir1.resource -> distronode_collections.ns.coll.plugins.subdir1.(plugintype).resource
        # ns.coll.rolename -> distronode_collections.ns.coll.roles.rolename
        if not DistronodeCollectionRef.is_valid_fqcr(ref):
        ...
        return DistronodeCollectionRef(collection_name, subdirs, resource + ext, ref_type)
    ...
    def try_parse_fqcr(ref, ref_type):
        ...
        :param ref: collection reference to parse (a valid ref is of the form 'ns.coll.resource' or 'ns.coll.subdir1.subdir2.resource')
        :param ref_type: the type of the reference, eg 'module', 'role', 'doc_fragment'
        :return: a populated DistronodeCollectionRef object on successful parsing, else None
        """
        try:
            return DistronodeCollectionRef.from_fqcr(ref, ref_type)
        except ValueError:
            pass
    ...
    def legacy_plugin_dir_to_plugin_type(legacy_plugin_dir_name):
        ...
        if plugin_type not in DistronodeCollectionRef.VALID_REF_TYPES:
    ...
    def is_valid_fqcr(ref, ref_type=None):
        ...
        if not ref_type:
            return bool(re.match(DistronodeCollectionRef.VALID_FQCR_RE, ref))

        return bool(DistronodeCollectionRef.try_parse_fqcr(ref, ref_type))
...
def _get_collection_path(collection_name):
    ...
    try:
        collection_pkg = import_module('distronode_collections.' + collection_name)
    except ImportError:
...
def _get_collection_playbook_path(playbook):
    ...
    acr = DistronodeCollectionRef.try_parse_fqcr(playbook, u'playbook')
    if acr:
        try:
            # get_collection_path
...
def _get_collection_resource_path(name, ref_type, collection_list=None):
    ...
    acr = DistronodeCollectionRef.try_parse_fqcr(name, ref_type)
    if acr:
    ...
    for collection_name in collection_list:
        try:
            acr = DistronodeCollectionRef(collection_name=collection_name, subdirs=subdirs, resource=resource, ref_type=ref_type)
...
def _get_collection_name_from_path(path):
    ...
    if path_parts.count('distronode_collections') != 1:
        return None

    ac_pos = path_parts.index('distronode_collections')
    ...
    try:
        ...
        imported_pkg_path = to_native(os.path.dirname(to_bytes(import_module('distronode_collections.' + candidate_collection_name).__file__)))
    except ImportError:
        return None
