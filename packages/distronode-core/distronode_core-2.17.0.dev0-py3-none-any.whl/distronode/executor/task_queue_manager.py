...
from distronode.executor.play_iterator import PlayIterator
from distronode.executor.stats import AggregateStats
...
from distronode.plugins.loader import callback_loader, strategy_loader, module_loader
...
from distronode.vars.reserved import warn_if_reserved
from distronode.utils.display import Display
from distronode.utils.lock import lock_decorator
...
class DistronodeEndPlay(Exception):
...
class TaskQueueManager:
    ...
    '''
    This class handles the multiprocessing requirements of Distronode by
    ...
    def __init__(self, inventory, variable_manager, loader, passwords, stdout_callback=None, run_additional_callbacks=True, run_tree=False, forks=None):
        ...
        try:
            ...
            raise DistronodeError("Unable to use multiprocessing, this is normally caused by lack of access to /dev/shm: %s" % to_native(e))
    ...
    def load_callbacks(self):
        ...
        if isinstance(self._stdout_callback, CallbackBase):
            ...
            if self._stdout_callback not in callback_loader:
                raise DistronodeError("Invalid callback for stdout specified: %s" % self._stdout_callback)
            else:
                self._stdout_callback = callback_loader.get(self._stdout_callback)
                self._stdout_callback.set_options()
                stdout_callback_loaded = True
        else:
            raise DistronodeError("callback must be an instance of CallbackBase or the name of a callback plugin")
        ...
        for callback_plugin in callback_list:
            ...
            if callback_type == 'stdout':
                ...
                # 2.x plugins shipped with distronode should require enabling, older or non shipped should load automatically
                continue
    ...
    def run(self, play):
        ...
        operates like classic Distronode by keeping all hosts in lock-step with
        ...
        if strategy is None:
            raise DistronodeError("Invalid play strategy specified: %s" % new_play.strategy, obj=play._ds)
        ...
        if iterator.end_play:
            raise DistronodeEndPlay(play_return)
