...
from distronode.playbook.loop_control import LoopControl
from distronode.playbook.notifiable import Notifiable
...
class Task(Base, Conditional, Taggable, CollectionSearch, Notifiable, Delegatable):
    ...
    def _preprocess_with_loop(self, ds, new_ds, k, v):
        ...
        if new_ds.get('loop') is not None or new_ds.get('loop_with') is not None:
            raise DistronodeError("duplicate loop in task: %s" % loop_name, obj=ds)
        if v is None:
            raise DistronodeError("you must specify a value when using %s" % k, obj=ds)
        new_ds['loop_with'] = loop_name
        new_ds['loop'] = v
        # display.deprecated("with_ type loops are being phased out, use the 'loop' keyword instead",
        #                    version="2.10", collection_name='distronode.builtin')
    ...
    def preprocess_data(self, ds):
        ...
        if collections_list and 'distronode.builtin' not in collections_list and 'distronode.legacy' not in collections_list:
            collections_list.append('distronode.legacy')
        ...
        try:
            ...
            raise DistronodeParserError(to_native(e), obj=ds, orig_exc=e)
        else:
        ...
        if action in C._ACTION_HAS_CMD:
            if 'cmd' in args:
                if args.get('_raw_params', '') != '':
                    raise DistronodeError("The 'cmd' argument cannot be used when other raw parameters are specified."
    ...
    def post_validate(self, templar):
        ...
        if DistronodeCollectionConfig.default_collection:
            pass
        ...
        if value is not None:
            ...
            def _parse_env_kv(k, v):
                try:
                    ...
                    error = to_native(e)
                    if self.action in C._ACTION_FACT_GATHERING and 'distronode_facts.env' in error or 'distronode_env' in error:
