...
from distronode.errors import DistronodeError, DistronodeParserError, DistronodeAssertionError
...
from distronode.playbook.role.metadata import RoleMetadata
...
from distronode.utils.path import is_subpath
...
#       strategies (distronode/plugins/strategy/__init__.py)
...
class Role(Base, Conditional, Taggable, CollectionSearch, Delegatable):
    ...
    def load(role_include, play, parent_role=None, from_files=None, from_include=False, validate=True, public=True):
        ...
        try:
            ...
            #  that role?)
            # see https://github.com/distronode/distronode/issues/61527
            ...
            raise DistronodeError("A recursion loop was detected with the roles specified. Make sure child roles do not have dependencies on parent roles",
    ...
    def _load_role_data(self, role_include, parent_role=None):
        ...
        if self._role_vars is None:
            ...
            raise DistronodeParserError("The vars/main.yml file for role '%s' must contain a dictionary of variables" % self._role_name)
        ...
        if self._default_vars is None:
            ...
            raise DistronodeParserError("The defaults/main.yml file for role '%s' must contain a dictionary of variables" % self._role_name)
        ...
        # FIXME: need exception for explicit distronode.legacy?
        if self._role_collection:  # this is a collection-hosted role
            ...
            default_collection = DistronodeCollectionConfig.default_collection
        ...
        if self.collections:
            ...
            default_append_collection = 'distronode.builtin' if self._role_collection else 'distronode.legacy'
            if 'distronode.builtin' not in self.collections and 'distronode.legacy' not in self.collections:
        ...
        if task_data:
            try:
                ...
                raise DistronodeParserError("The tasks/main.yml file for role '%s' must contain a list of tasks" % self._role_name,
        ...
        if handler_data:
            try:
                ...
                raise DistronodeParserError("The handlers/main.yml file for role '%s' must contain a list of tasks" % self._role_name,
    ...
    def _get_role_argspecs(self):
        ...
        # dict from the role meta data, if it exists. Distronode 2.11 and later will
    ...
    def _create_validation_task(self, argument_spec, entrypoint_name):
        ...
        return {
            'action': {
                'module': 'distronode.builtin.validate_argument_spec',
            ...
            if found_files:
                for found in found_files:
                    ...
                    if not is_subpath(found, file_path):
                        raise DistronodeParserError("Failed loading '%s' for role (%s) as it is not inside the expected role path: '%s'" %
                ...
                raise DistronodeParserError("Could not find specified file in role: %s/%s" % (subdir, main))
