...
from distronode.utils.galaxy import scm_archive_resource
...
class RoleRequirement(RoleDefinition):
    ...
    def role_yaml_parse(role):
        ...
        if isinstance(role, string_types):
            ...
            if ',' in role:
                if role.count(',') == 1:
                    ...
                    raise DistronodeError("Invalid role line (%s). Proper format is 'role_name[,version[,name]]'" % role)
            else:
                src = role
        ...
        if 'role' in role:
            name = role['role']
            if ',' in name:
                raise DistronodeError("Invalid old style role requirement: %s" % name)
            else:
                del role['role']
                role['name'] = name
        else:
