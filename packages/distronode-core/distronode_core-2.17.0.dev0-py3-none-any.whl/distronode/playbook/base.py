...
from distronode.playbook.attribute import Attribute, FieldAttribute, ConnectionFieldAttribute, NonInheritableFieldAttribute
from distronode.plugins.loader import module_loader, action_loader
from distronode.utils.collection_loader._collection_finder import _get_collection_metadata, DistronodeCollectionRef
from distronode.utils.display import Display
from distronode.utils.sentinel import Sentinel
from distronode.utils.vars import combine_vars, isidentifier, get_unique_id
...
class FieldAttributeBase:
    ...
    def load_data(self, ds, variable_manager=None, loader=None):
        ...
        if ds is None:
            raise DistronodeAssertionError('ds (%s) should not be None but it is.' % ds)
    ...
    def _post_validate_debugger(self, attr, value, templar):
        ...
        if value and isinstance(value, string_types) and value not in valid_values:
            raise DistronodeParserError("'%s' is not a valid value for debugger. Must be one of %s" % (value, ', '.join(valid_values)), obj=self.get_ds())
        return value
    ...
    def _validate_attributes(self, ds):
        ...
        for key in ds:
            if key not in valid_attrs:
                raise DistronodeParserError("'%s' is not a valid attribute for a %s" % (key, self.__class__.__name__), obj=ds)
    ...
    def _load_module_defaults(self, name, value):
        ...
        for defaults_dict in value:
            ...
            for defaults_entry, defaults in defaults_dict.items():
                ...
                # action_groups that are not fully qualified are part of the 'distronode.legacy'
                ...
                if defaults_entry.startswith('group/'):
                    ...
                    if len(defaults_entry.split('.')) < 3:
                        defaults_entry = 'distronode.legacy.' + defaults_entry

                    resolved_action = self._resolve_action(defaults_entry)
                    if resolved_action:
                        validated_defaults_dict[resolved_action] = defaults

                    # If the defaults_entry is an distronode.legacy plugin, these defaults
                    # are inheritable by the 'distronode.builtin' subset, but are not
                    # required to exist.
                    if defaults_entry.startswith('distronode.legacy.'):
                        resolved_action = self._resolve_action(
                            defaults_entry.replace('distronode.legacy.', 'distronode.builtin.'),
                            mandatory=False
                        )
                        if resolved_action:
                            validated_defaults_dict[resolved_action] = defaults
    ...
    def _resolve_group(self, fq_group_name, mandatory=True):
        if not DistronodeCollectionRef.is_valid_fqcr(fq_group_name):
        ...
        try:
            ...
            raise DistronodeParserError("Error loading module_defaults: could not resolve the module_defaults group %s" % fq_group_name)
        ...
        for action in action_group:
            ...
            if not DistronodeCollectionRef.is_valid_fqcr(action):
        ...
        for include_group in include_groups:
            if not DistronodeCollectionRef.is_valid_fqcr(include_group):
    ...
    def _resolve_action(self, action_name, mandatory=True):
        ...
        if mandatory:
            raise DistronodeParserError("Could not resolve action %s in module_defaults" % action_name)
    ...
    def copy(self):
        ...
        try:
            ...
            raise DistronodeError("Exceeded maximum object depth. This may have been caused by excessive role recursion", orig_exc=e)
    ...
    def get_validated_value(self, name, attribute, value, templar):
        if attribute.isa == 'string':
            ...
            if attribute.listof is not None:
                for item in value:
                    if not isinstance(item, attribute.listof):
                        raise DistronodeParserError("the field '%s' should be a list of %s, "
                                                 "but the item '%s' is a %s" % (name, attribute.listof, item, type(item)), obj=self.get_ds())
                    elif attribute.required and attribute.listof == string_types:
                        if item is None or item.strip() == "":
                            raise DistronodeParserError("the field '%s' is required, and cannot have empty values" % (name,), obj=self.get_ds())
            ...
            raise DistronodeAssertionError(f"Unknown value for attribute.isa: {attribute.isa}")
        return value
        ...
        for (name, attribute) in self.fattributes.items():
            ...
            if getattr(self, name) is None:
                if not attribute.required:
                    ...
                    raise DistronodeParserError("the field '%s' is required but was not set" % name)
            ...
            try:
                ...
                value = getattr(self, name)
                raise DistronodeParserError("the field '%s' has an invalid value (%s), and could not be converted to an %s."
                ...
                if templar._fail_on_undefined_errors and name != 'name':
                    ...
                    raise DistronodeParserError(msg, obj=self.get_ds(), orig_exc=e)
    ...
    def _load_vars(self, attr, ds):
        ...
        try:
            ...
            raise DistronodeParserError("Vars in a %s must be specified as a dictionary" % self.__class__.__name__,
                                     obj=ds, orig_exc=e)
        except TypeError as e:
            raise DistronodeParserError("Invalid variable name in vars specified for %s: %s" % (self.__class__.__name__, e), obj=ds, orig_exc=e)
    ...
    def deserialize(self, data):
        ...
        if not isinstance(data, dict):
            raise DistronodeAssertionError('data (%s) should be a dict but is a %s' % (data, type(data)))
