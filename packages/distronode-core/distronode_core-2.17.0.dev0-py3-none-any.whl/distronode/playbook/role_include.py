...
class IncludeRole(TaskInclude):
    ...
    def load(data, block=None, role=None, task_include=None, variable_manager=None, loader=None):
        ...
        if bad_opts:
            raise DistronodeParserError('Invalid options for %s: %s' % (ir.action, ','.join(list(bad_opts))), obj=data)
        ...
        for key in my_arg_names.intersection(IncludeRole.FROM_ARGS):
            ...
            if not isinstance(args_value, string_types):
                raise DistronodeParserError('Expected a string for %s but got %s instead' % (key, type(args_value)))
        ...
        if apply_attrs and ir.action not in C._ACTION_INCLUDE_ROLE:
            raise DistronodeParserError('Invalid options for %s: apply' % ir.action, obj=data)
        elif not isinstance(apply_attrs, dict):
            raise DistronodeParserError('Expected a dict for apply but got %s instead' % type(apply_attrs), obj=data)
    ...
    def get_include_params(self):
        ...
        if self._parent_role:
            ...
            v.setdefault('distronode_parent_role_names', []).insert(0, self._parent_role.get_name())
            v.setdefault('distronode_parent_role_paths', []).insert(0, self._parent_role._role_path)
        return v
