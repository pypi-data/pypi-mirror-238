...
from distronode.executor.task_executor import remove_omit
        ...
        for res in results:
            ...
            if original_task.action in C._ACTION_ALL_INCLUDES:
                ...
                for include_result in include_results:
                    ...
                    special_vars = {}
                    loop_var = include_result.get('distronode_loop_var', 'item')
                    index_var = include_result.get('distronode_index_var')
                    ...
                    if '_distronode_item_label' in include_result:
                        task_vars['_distronode_item_label'] = special_vars['_distronode_item_label'] = include_result['_distronode_item_label']
                    if 'distronode_loop' in include_result:
                        task_vars['distronode_loop'] = special_vars['distronode_loop'] = include_result['distronode_loop']
                    if original_task.no_log and '_distronode_no_log' not in include_args:
                        task_vars['_distronode_no_log'] = special_vars['_distronode_no_log'] = original_task.no_log

                    # get search path for this task to pass to lookup plugins that may be used in pathing to
                    # the included file
                    task_vars['distronode_search_path'] = original_task.get_search_path()

                    # ensure basedir is always in (dwim already searches here but we need to display it)
                    if loader.get_basedir() not in task_vars['distronode_search_path']:
                        task_vars['distronode_search_path'].append(loader.get_basedir())
