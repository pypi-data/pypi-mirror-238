...
from distronode.playbook.helpers import load_list_of_blocks, load_list_of_roles
from distronode.playbook.role import Role, hash_params
from distronode.playbook.task import Task
from distronode.playbook.taggable import Taggable
from distronode.vars.manager import preprocess_vars
...
class Play(Base, Taggable, CollectionSearch):
    ...
    def _validate_hosts(self, attribute, name, value):
        ...
        if 'hosts' in self._ds:
            if not value:
                raise DistronodeParserError("Hosts list cannot be empty. Please check your playbook")

            if is_sequence(value):
                # Make sure each item in the sequence is a valid string
                for entry in value:
                    if entry is None:
                        raise DistronodeParserError("Hosts list cannot contain values of 'None'. Please check your playbook")
                    elif not isinstance(entry, (binary_type, text_type)):
                        raise DistronodeParserError("Hosts list contains an invalid host value: '{host!s}'".format(host=entry))

            elif not isinstance(value, (binary_type, text_type)):
                raise DistronodeParserError("Hosts list must be a sequence or string. Please check your playbook.")
    ...
    def preprocess_data(self, ds):
        ...
        if not isinstance(ds, dict):
            raise DistronodeAssertionError('while preprocessing data (%s), ds should be a dict but was a %s' % (ds, type(ds)))
        ...
        if 'user' in ds:
            ...
            if 'remote_user' in ds:
                raise DistronodeParserError("both 'user' and 'remote_user' are set for this play. "
    ...
    def _load_tasks(self, attr, ds):
        ...
        try:
            ...
            raise DistronodeParserError("A malformed block was encountered while loading tasks: %s" % to_native(e), obj=self._ds, orig_exc=e)
    ...
    def _load_pre_tasks(self, attr, ds):
        ...
        try:
            ...
            raise DistronodeParserError("A malformed block was encountered while loading pre_tasks", obj=self._ds, orig_exc=e)
    ...
    def _load_post_tasks(self, attr, ds):
        ...
        try:
            ...
            raise DistronodeParserError("A malformed block was encountered while loading post_tasks", obj=self._ds, orig_exc=e)
    ...
    def _load_handlers(self, attr, ds):
        ...
        try:
            ...
            raise DistronodeParserError("A malformed block was encountered while loading handlers", obj=self._ds, orig_exc=e)
    ...
    def _load_roles(self, attr, ds):
        ...
        try:
            ...
            raise DistronodeParserError("A malformed role declaration was encountered.", obj=self._ds, orig_exc=e)
        ...
        if new_ds is not None:
            for prompt_data in new_ds:
                if 'name' not in prompt_data:
                    raise DistronodeParserError("Invalid vars_prompt data structure, missing 'name' key", obj=ds)
                for key in prompt_data:
                    if key not in ('name', 'prompt', 'default', 'private', 'confirm', 'encrypt', 'salt_size', 'salt', 'unsafe'):
                        raise DistronodeParserError("Invalid vars_prompt data structure, found unsupported key '%s'" % key, obj=ds)
