...
from distronode.module_utils.common.yaml import HAS_LIBYAML, yaml_load
...
class DistronodeVersion(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        distronode_version = to_native(version(getattr(parser, 'prog')))
        print(distronode_version)
        parser.exit()
...
def unfrack_path(pathsep=False, follow=True):
    """Turn an Option's data into a single path in Distronode locations"""
    def inner(value):
        if pathsep:
...
def version(prog=None):
    ...
    result.append("  distronode python module location = %s" % ':'.join(distronode.__path__))
    result.append("  distronode collection location = %s" % ':'.join(C.COLLECTIONS_PATHS))
...
def create_base_parser(prog, usage="", desc=None, epilog=None):
    """
    Create an options parser for all distronode scripts
    """
    # base opts
    ...
    parser.add_argument('--version', action=DistronodeVersion, nargs=0, help=version_help)
...
def add_verbosity_options(parser):
    ...
    parser.add_argument('-v', '--verbose', dest='verbosity', default=C.DEFAULT_VERBOSITY, action="count",
                        help="Causes Distronode to print more debug messages. Adding multiple -v will increase the verbosity, "
...
def add_runas_options(parser):
    ...
    runas_group.add_argument('--become-method', dest='become_method', default=C.DEFAULT_BECOME_METHOD,
                             help='privilege escalation method to use (default=%s)' % C.DEFAULT_BECOME_METHOD +
                                  ', use `distronode-doc -t become -l` to list valid choices.')
