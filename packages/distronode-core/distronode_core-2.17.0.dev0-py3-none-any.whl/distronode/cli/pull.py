...
from distronode.utils.cmd_functions import run_cmd
...
class PullCLI(CLI):
    ''' Used to pull a remote copy of distronode on each managed node,
        each set to run via cron and update playbook source via a source repository.
        This inverts the default *push* architecture of distronode into a *pull* architecture,
        ...
        The setup playbook can be tuned to change the cron frequency, logging locations, and parameters to distronode-pull.
        This is useful both for extreme scale-out as well as periodic remediation.
        Usage of the 'fetch' module to retrieve logs from distronode-pull runs would be an
        excellent way to gather and analyze remote logs from distronode-pull.
    '''

    name = 'distronode-pull'
    ...
    ARGUMENTS = {'playbook.yml': 'The name of one the YAML format files to run as an Distronode playbook.'
                                 'This can be a relative path within the checkout. By default, Distronode will'
    ...
    def init_parser(self):
        ...
        self.parser.add_argument('-m', '--module-name', dest='module_name', default=self.DEFAULT_REPO_TYPE,
                                 help='Repository module name, which distronode will use to check out the repo. Choices are %s. Default is %s.'
    ...
    def post_process_args(self, options):
        ...
        if not options.dest:
            ...
            options.dest = os.path.join(C.DISTRONODE_HOME, 'pull', hostname)
        ...
        if os.path.exists(options.dest) and not os.path.isdir(options.dest):
            raise DistronodeOptionsError("%s is not a valid or accessible directory." % options.dest)
        ...
        if options.sleep:
            try:
                ...
                raise DistronodeOptionsError("%s is not a number." % options.sleep)

        if not options.url:
            raise DistronodeOptionsError("URL for repository not specified, use -h for help")

        if options.module_name not in self.REPO_CHOICES:
            raise DistronodeOptionsError("Unsupported repo module %s, choices are %s" % (options.module_name, ','.join(self.REPO_CHOICES)))
    ...
    def run(self):
        ...
        display.display(now.strftime("Starting Distronode Pull at %F %T"))
        display.display(' '.join(sys.argv))

        # Build Checkout command
        # Now construct the distronode command
        node = platform.node()
        host = socket.getfqdn()
        ...
        if not inv_opts:
            ...
            inv_opts += '-e %s ' % shlex.quote('distronode_python_interpreter=%s' % sys.executable)
        ...
        if context.CLIARGS['module_name'] == 'git':
            ...
            raise DistronodeOptionsError('Unsupported (%s) SCM module for pull, choices are: %s'
        ...
        if path is None:
            raise DistronodeOptionsError(("module '%s' not found.\n" % context.CLIARGS['module_name']))

        bin_path = os.path.dirname(os.path.abspath(sys.argv[0]))
        # hardcode local and inventory/host as this is just meant to fetch the repo
        cmd = '%s/distronode %s %s -m %s -a "%s" all -l "%s"' % (bin_path, inv_opts, base_opts,
                                                              context.CLIARGS['module_name'],
        ...
        display.debug("running distronode with VCS module to checkout repo")
        ...
        if playbook is None:
            raise DistronodeOptionsError("Could not find a playbook to run.")

        # Build playbook command
        cmd = '%s/distronode-playbook %s %s' % (bin_path, base_opts, playbook)
        ...
        display.debug("running distronode-playbook to do actual work")
