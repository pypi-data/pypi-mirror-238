...
from distronode.module_utils.connection import Connection, ConnectionError, send_data, recv_data
from distronode.module_utils.service import fork_process
...
from distronode.plugins.loader import connection_loader, init_plugin_loader
from distronode.utils.path import unfrackpath, makedirs_safe
from distronode.utils.display import Display
from distronode.utils.jsonrpc import JsonRpcServer
...
class ConnectionProcess(object):
    ...
    '''
    def __init__(self, fd, play_context, socket_path, original_path, task_uuid=None, distronode_playbook_pid=None):
        ...
        self._distronode_playbook_pid = distronode_playbook_pid
    ...
    def start(self, options):
        ...
        try:
            ...
            self.connection = connection_loader.get(self.play_context.connection, self.play_context, '/dev/null',
                                                    task_uuid=self._task_uuid, distronode_playbook_pid=self._distronode_playbook_pid)
            try:
            ...
            self.fd.write(json.dumps(result, cls=DistronodeJSONEncoder))
            self.fd.close()
    ...
    def shutdown(self):
        ...
        """
        lock_path = unfrackpath("%s/.distronode_pc_lock_%s" % os.path.split(self.socket_path))
...
def main(args=None):
    ...
    parser = opt_help.create_base_parser(prog='distronode-connection')
    ...
    if rc == 0:
        ...
        distronode_playbook_pid = args.playbook_pid
        task_uuid = args.task_uuid
        cp = ssh._create_control_path(play_context.remote_addr, play_context.port, play_context.remote_user, play_context.connection, distronode_playbook_pid)
        ...
        lock_path = unfrackpath("%s/.distronode_pc_lock_%s" % os.path.split(socket_path))
            ...
            if not os.path.exists(socket_path):
                ...
                if pid == 0:
                    try:
                        os.close(r)
                        wfd = os.fdopen(w, 'w')
                        process = ConnectionProcess(wfd, play_context, socket_path, original_path, task_uuid, distronode_playbook_pid)
                    ...
                    os.close(w)
                    rfd = os.fdopen(r, 'r')
                    data = json.loads(rfd.read(), cls=DistronodeJSONDecoder)
    ...
    if 'exception' in result:
        rc = 1
        sys.stderr.write(json.dumps(result, cls=DistronodeJSONEncoder))
    else:
        rc = 0
        sys.stdout.write(json.dumps(result, cls=DistronodeJSONEncoder))
