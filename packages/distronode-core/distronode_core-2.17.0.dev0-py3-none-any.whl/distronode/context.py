...
In the future we *may* create Context objects to allow running multiple Distronode plays in parallel
with different contexts but that is currently out of scope as the Distronode library is just for
running the distronode command line tools.
...
from distronode.utils.context_objects import CLIArgs, GlobalCLIArgs
