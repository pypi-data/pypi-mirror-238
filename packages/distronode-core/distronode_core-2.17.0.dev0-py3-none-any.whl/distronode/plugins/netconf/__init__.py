...
def ensure_ncclient(func):
    ...
    def wrapped(self, *args, **kwargs):
        if not HAS_NCCLIENT:
            raise DistronodeError("%s: %s" % (missing_required_lib('ncclient'), to_native(NCCLIENT_IMP_ERR)))
        return func(self, *args, **kwargs)
    return wrapped


class NetconfBase(DistronodePlugin):
    """
    A base class for implementing Netconf connections

    .. note:: Unlike most of Distronode, nearly all strings in
        :class:`TerminalBase` plugins are byte strings.  This is because of
        how close to the underlying platform these plugins operate.  Remember
        to mark literal strings as byte string (``b"string"``) and to use
        :func:`~distronode.module_utils.common.text.converters.to_bytes` and
        :func:`~distronode.module_utils.common.text.converters.to_text` to avoid unexpected
        problems.
        ...
        Note: the 'result' or 'error' from response should to be converted to object
            ...
            Usage:
            from distronode.module_utils.connection import Connection
