...
from distronode.plugins.loader import cache_loader
...
class BaseCacheModule(DistronodePlugin):
...
class BaseFileCacheModule(BaseCacheModule):
    ...
    def validate_cache_connection(self):
        if not self._cache_dir:
            raise DistronodeError("error, '%s' cache plugin requires the 'fact_caching_connection' config option "
        ...
        if not os.path.exists(self._cache_dir):
            try:
                ...
                raise DistronodeError("error in '%s' cache plugin while trying to create cache dir %s : %s" % (self.plugin_name, self._cache_dir, to_bytes(e)))
        else:
            for x in (os.R_OK, os.W_OK, os.X_OK):
                if not os.access(self._cache_dir, x):
                    raise DistronodeError("error in '%s' cache, configured path (%s) does not have necessary permissions (rwx), disabling plugin" % (
    ...
    def get(self, key):
        ...
        if key not in self._cache:
            ...
            try:
                ...
                self.delete(key)
                raise DistronodeError("The cache file %s was corrupt, or did not otherwise contain valid data. "
                                   "It has been removed, so you can re-run your command now." % cachefile)
            except (OSError, IOError) as e:
                display.warning("error in '%s' cache plugin while trying to read %s : %s" % (self.plugin_name, cachefile, to_bytes(e)))
                raise KeyError
            except Exception as e:
                raise DistronodeError("Error while decoding the cache file %s: %s" % (cachefile, to_bytes(e)))
    ...
    def _load(self, filepath):
        ...
        returned should be what Distronode would expect if it were uncached data.
...
class CachePluginAdjudicator(MutableMapping):
    ...
    def __init__(self, plugin_name='memory', **kwargs):
        ...
        if not self._plugin:
            raise DistronodeError('Unable to load the cache plugin (%s).' % plugin_name)
