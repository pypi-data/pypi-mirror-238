...
from distronode.config.manager import ensure_type
from distronode.errors import DistronodeError, DistronodeFileNotFound, DistronodeAction, DistronodeActionFail
...
from distronode.template import generate_distronode_template_vars, DistronodeEnvironment
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        for s_type in ('src', 'dest', 'state', 'newline_sequence', 'variable_start_string', 'variable_end_string', 'block_start_string',
            ...
            if s_type in self._task.args:
                ...
                if value is not None and not isinstance(value, string_types):
                    raise DistronodeActionFail("%s is expected to be a string, but got %s instead" % (s_type, type(value)))
        ...
        try:
            ...
            if state is not None:
                raise DistronodeActionFail("'state' cannot be specified on a template")
            elif source is None or dest is None:
                raise DistronodeActionFail("src and dest are required")
            elif newline_sequence not in allowed_sequences:
                raise DistronodeActionFail("newline_sequence needs to be one of: \n, \r or \r\n")
            else:
                try:
                    source = self._find_needle('templates', source)
                except DistronodeError as e:
                    raise DistronodeActionFail(to_text(e))
            ...
            try:
                ...
                raise DistronodeActionFail("could not find src=%s, %s" % (source, to_text(e)))
            ...
            try:
                with open(b_tmp_source, 'rb') as f:
                    try:
                        ...
                        raise DistronodeActionFail("Template source files must be utf-8 encoded")

                # set jinja2 internal search path for includes
                searchpath = task_vars.get('distronode_search_path', [])
                ...
                # add distronode 'template' vars
                temp_vars = task_vars.copy()
                # NOTE in the case of DISTRONODE_DEBUG=1 task_vars is VarsWithSources(MutableMapping)
                ...
                temp_vars.update(generate_distronode_template_vars(self._task.args.get('src', None), source, dest))

                # force templar to use DistronodeEnvironment to prevent issues with native types
                # https://github.com/distronode/distronode/issues/46169
                templar = self._templar.copy_with_new_env(environment_class=DistronodeEnvironment,
            ...
            except DistronodeAction:
                raise
            except Exception as e:
                raise DistronodeActionFail("%s: %s" % (type(e).__name__, to_text(e)))
            finally:
            ...
            try:
                ...
                # call with distronode.legacy prefix to eliminate collisions with collections while still allowing local override
                copy_action = self._shared_loader_obj.action_loader.get('distronode.legacy.copy',
                                                                        task=new_task,
