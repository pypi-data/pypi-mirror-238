...
from distronode.errors import DistronodeError, DistronodeAction, _DistronodeActionDone, DistronodeActionFail, DistronodeActionSkip
        ...
        try:
            ...
            if creates:
                ...
                if self._remote_file_exists(creates):
                    raise DistronodeActionSkip("%s exists, matching creates option" % creates)
            ...
            if removes:
                ...
                if not self._remote_file_exists(removes):
                    raise DistronodeActionSkip("%s does not exist, matching removes option" % removes)
            ...
            if chdir:
                ...
                if getattr(self._connection._shell, "_IS_WINDOWS", False) and \
                    ...
                    raise DistronodeActionFail('chdir %s must be an absolute path for a Windows remote node' % chdir)
                # Every other shell is unix-path-aware.
                if not getattr(self._connection._shell, "_IS_WINDOWS", False) and not chdir.startswith('/'):
                    raise DistronodeActionFail('chdir %s must be an absolute path for a Unix-aware remote node' % chdir)
            ...
            if self._task.check_mode:
                ...
                if new_module_args['creates'] or new_module_args['removes']:
                    result['changed'] = True
                    raise _DistronodeActionDone(result=result)
                ...
                # but since the script may override 'changed', just skip instead of guessing.
                    ...
                    raise DistronodeActionSkip('Check mode is not supported for this task.', result=result)
            ...
            if 'rc' in result and result['rc'] != 0:
                raise DistronodeActionFail('non-zero return code')
