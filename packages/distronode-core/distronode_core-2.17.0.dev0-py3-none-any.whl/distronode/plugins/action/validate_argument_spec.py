...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if 'argument_spec' not in self._task.args:
            raise DistronodeError('"argument_spec" arg is required in args: %s' % self._task.args)
        ...
        if not isinstance(argument_spec_data, dict):
            raise DistronodeError('Incorrect type for argument_spec, expected dict and got %s' % type(argument_spec_data))

        if not isinstance(provided_arguments, dict):
            raise DistronodeError('Incorrect type for provided_arguments, expected dict and got %s' % type(provided_arguments))
