...
from distronode.errors import DistronodeError, DistronodeFilterError, DistronodeFilterTypeError
from distronode.module_utils.six import string_types, integer_types, reraise, text_type
...
from distronode.module_utils.common.yaml import yaml_load, yaml_load_all
from distronode.parsing.ajson import DistronodeJSONEncoder
from distronode.parsing.yaml.dumper import DistronodeDumper
from distronode.template import recursive_check_defined
from distronode.utils.display import Display
from distronode.utils.encrypt import do_encrypt, PASSLIB_AVAILABLE
from distronode.utils.hashing import md5s, checksum_s
from distronode.utils.unicode import unicode_wrap
from distronode.utils.vars import merge_hash

display = Display()

UUID_NAMESPACE_DISTRONODE = uuid.UUID('361E6D51-FAEC-444A-9079-341386DA8E2E')
...
def to_yaml(a, *args, **kw):
    ...
    try:
        transformed = yaml.dump(a, Dumper=DistronodeDumper, allow_unicode=True, default_flow_style=default_flow_style, **kw)
    except Exception as e:
        raise DistronodeFilterError("to_yaml - %s" % to_native(e), orig_exc=e)
...
def to_nice_yaml(a, indent=4, *args, **kw):
    '''Make verbose, human-readable yaml'''
    try:
        transformed = yaml.dump(a, Dumper=DistronodeDumper, indent=indent, allow_unicode=True, default_flow_style=False, **kw)
    except Exception as e:
        raise DistronodeFilterError("to_nice_yaml - %s" % to_native(e), orig_exc=e)
    return to_text(transformed)
...
def to_json(a, *args, **kw):
    ...
    return json.dumps(a, cls=DistronodeJSONEncoder, *args, **kw)
...
def strftime(string_format, second=None, utc=False):
    ...
    if second is not None:
        try:
            ...
            raise DistronodeFilterError('Invalid value for epoch value (%s)' % second)
...
def regex_replace(value='', pattern='', replacement='', ignorecase=False, multiline=False, count=0, mandatory_count=0):
    ...
    if mandatory_count and mandatory_count != subs:
        raise DistronodeFilterError("'%s' should match %d times, but matches %d times in '%s'"
...
def regex_search(value, regex, *args, **kwargs):
    ...
    for arg in args:
        if arg.startswith('\\g'):
            ...
            raise DistronodeFilterError('Unknown argument')
...
def regex_escape(string, re_type='python'):
    ...
    # https://remram44.github.io/regex-cheatsheet/regex.html#programs
        ...
        raise DistronodeFilterError('Regex type (%s) not yet implemented' % re_type)
    else:
        raise DistronodeFilterError('Invalid regex type (%s)' % re_type)
...
def rand(environment, end, start=None, step=None, seed=None):
    ...
    if isinstance(end, integer_types):
        ...
        if start or step:
            raise DistronodeFilterError('start and step can only be used with integer values')
        return r.choice(end)
    else:
        raise DistronodeFilterError('random can only be used on sequences and integers')
...
def get_encrypted_password(password, hashtype='sha512', salt=None, salt_size=None, rounds=None, ident=None):
    ...
    try:
        ...
        reraise(DistronodeFilterError, DistronodeFilterError(to_native(e), orig_exc=e), sys.exc_info()[2])
    except Exception as e:
        if unknown_passlib_hashtype:
            ...
            raise DistronodeFilterError(f"{hashtype} is not in the list of supported passlib algorithms: {choices}") from e
        raise


def to_uuid(string, namespace=UUID_NAMESPACE_DISTRONODE):
    uuid_namespace = namespace
    if not isinstance(uuid_namespace, uuid.UUID):
        try:
            uuid_namespace = uuid.UUID(namespace)
        except (AttributeError, ValueError) as e:
            raise DistronodeFilterError("Invalid value '%s' for 'namespace': %s" % (to_native(namespace), to_native(e)))
...
def mandatory(a, msg=None):
    ...
    if isinstance(a, Undefined):
        ...
        if msg is not None:
            raise DistronodeFilterError(to_native(msg))
        raise DistronodeFilterError("Mandatory variable %s not defined." % name)
...
def combine(*terms, **kwargs):
    ...
    if kwargs:
        raise DistronodeFilterError("'recursive' and 'list_merge' are the only valid keyword arguments")
...
def subelements(obj, subelements, skip_missing=False):
    ...
    if isinstance(obj, dict):
        ...
        raise DistronodeFilterError('obj must be a list of dicts or a nested dict')
    ...
    if isinstance(subelements, list):
        ...
        raise DistronodeFilterTypeError('subelements must be a list or a string')
        ...
        for subelement in subelement_list:
            try:
                ...
                if skip_missing:
                    values = []
                    break
                raise DistronodeFilterError("could not find %r key in iterated item %r" % (subelement, values))
            except TypeError:
                raise DistronodeFilterTypeError("the key %s should point to a dictionary, got '%s'" % (subelement, values))
        if not isinstance(values, list):
            raise DistronodeFilterTypeError("the key %r should point to a list, got %r" % (subelement, values))
...
def dict_to_list_of_dict_key_value_elements(mydict, key_name='key', value_name='value'):
    ...
    if not isinstance(mydict, Mapping):
        raise DistronodeFilterTypeError("dict2items requires a dictionary, got %s instead." % type(mydict))
...
def list_of_dict_key_value_elements_to_dict(mylist, key_name='key', value_name='value'):
    ...
    if not is_sequence(mylist):
        raise DistronodeFilterTypeError("items2dict requires a list, got %s instead." % type(mylist))

    try:
        return dict((item[key_name], item[value_name]) for item in mylist)
    except KeyError:
        raise DistronodeFilterTypeError(
            "items2dict requires each dictionary in the list to contain the keys '%s' and '%s', got %s instead."
            % (key_name, value_name, mylist)
        )
    except TypeError:
        raise DistronodeFilterTypeError("items2dict requires a list of dictionaries, got %s instead." % mylist)
...
def path_join(paths):
    ...
    if is_sequence(paths):
        return os.path.join(*paths)
    raise DistronodeFilterTypeError("|path_join expects string or sequence, got %s instead." % type(paths))
...
def commonpath(paths):
    ...
    if not is_sequence(paths):
        raise DistronodeFilterTypeError("|path_join expects sequence, got %s instead." % type(paths))
...
class FilterModule(object):
    ...
    def filters(self):
        return {
            ...
            'sha1': checksum_s,
            # checksum of string as used by distronode for checksumming files
            'checksum': checksum_s,
            # generic hashing
