...
class LookupModule(LookupBase):
    ...
    The standard Distronode key-value form is accepted as well.  For example:
    ...
    def parse_simple_args(self, term):
        ...
        if start is not None:
            try:
                ...
                raise DistronodeError("can't parse start=%s as integer" % start)
        if end is not None:
            try:
                end = int(end, 0)
            except ValueError:
                raise DistronodeError("can't parse end=%s as integer" % end)
        if stride is not None:
            try:
                stride = int(stride, 0)
            except ValueError:
                raise DistronodeError("can't parse stride=%s as integer" % stride)
    ...
    def sanity_check(self):
        if self.count is None and self.end is None:
            raise DistronodeError("must specify count or end in with_sequence")
        elif self.count is not None and self.end is not None:
            raise DistronodeError("can't specify both count and end in with_sequence")
        ...
        if self.stride > 0 and self.end < self.start:
            raise DistronodeError("to count backwards make stride negative")
        if self.stride < 0 and self.end > self.start:
            raise DistronodeError("to count forward don't make stride negative")
        if self.format.count('%') != 1:
            raise DistronodeError("bad formatting string: %s" % self.format)
        ...
        for term in terms:
            try:
                ...
                try:
                    ...
                    raise DistronodeError("unknown error parsing with_sequence arguments: %r. Error was: %s" % (term, e))
