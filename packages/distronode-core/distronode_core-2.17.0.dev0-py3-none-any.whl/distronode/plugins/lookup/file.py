...
DOCUMENTATION = """
    ...
    description:
        - This lookup returns the contents from a file on the Distronode controller's file system.
    options:
      _terms:
...
- distronode.builtin.debug:
    msg: "the value of foo.txt is {{ lookup('distronode.builtin.file', '/etc/foo.txt') }}"
...
from distronode.errors import DistronodeError, DistronodeOptionsError, DistronodeLookupError
        ...
        for term in terms:
            ...
            try:
                ...
                if lookupfile:
                    ...
                    raise DistronodeOptionsError("file not found, use -vvvvv to see paths searched")
            except DistronodeError as e:
                raise DistronodeLookupError("The 'file' lookup had an issue accessing the file '%s'" % term, orig_exc=e)
