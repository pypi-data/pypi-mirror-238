...
DOCUMENTATION = """
    ...
    notes:
      ...
      - 'As all lookups, this runs on the Distronode host as the user running the playbook, and "become" does not apply,
...
- name: create a mysql user with a random password
  community.mysql.mysql_user:
    name: "{{ client }}"
    password: "{{ lookup('distronode.builtin.password', 'credentials/' + client + '/' + tier + '/' + role + '/mysqlpassword', length=15) }}"
    priv: "{{ client }}_{{ tier }}_{{ role }}.*:ALL"

- name: create a mysql user with a random password using only ascii letters
  community.mysql.mysql_user:
    name: "{{ client }}"
    password: "{{ lookup('distronode.builtin.password', '/tmp/passwordfile', chars=['ascii_letters']) }}"
    priv: '{{ client }}_{{ tier }}_{{ role }}.*:ALL'

- name: create a mysql user with an 8 character random password using only digits
  community.mysql.mysql_user:
    name: "{{ client }}"
    password: "{{ lookup('distronode.builtin.password', '/tmp/passwordfile', length=8, chars=['digits']) }}"
    priv: "{{ client }}_{{ tier }}_{{ role }}.*:ALL"

- name: create a mysql user with a random password using many different char sets
  community.mysql.mysql_user:
    name: "{{ client }}"
    password: "{{ lookup('distronode.builtin.password', '/tmp/passwordfile', chars=['ascii_letters', 'digits', 'punctuation']) }}"
    priv: "{{ client }}_{{ tier }}_{{ role }}.*:ALL"

- name: create lowercase 8 character name for Kubernetes pod name
  distronode.builtin.set_fact:
    random_pod_name: "web-{{ lookup('distronode.builtin.password', '/dev/null', chars=['ascii_lowercase', 'digits'], length=8) }}"

- name: create random but idempotent password
  distronode.builtin.set_fact:
    password: "{{ lookup('distronode.builtin.password', '/dev/null', seed=inventory_hostname) }}"
"""
...
from distronode.utils.encrypt import BaseHash, do_encrypt, random_password, random_salt
...
def _format_content(password, salt, encrypt=None, ident=None):
    ...
    if not salt:
        raise DistronodeAssertionError('_format_content was called with encryption requested but no salt value')
...
def _get_lock(b_path):
    ...
    first_process = False
    b_pathdir = os.path.dirname(b_path)
    lockfile_name = to_bytes("%s.distronode_lockfile" % hashlib.sha1(b_path).hexdigest())
    ...
    while os.path.exists(lockfile) and not first_process:
        ...
        if counter >= 2:
            raise DistronodeError("Password lookup cannot get the lock in 7 seconds, abort..."
...
class LookupModule(LookupBase):
    ...
    def _parse_parameters(self, term):
        ...
        See https://github.com/distronode/distronode-modules-core/issues/1968#issuecomment-136842156
        ...
        if len(first_split) <= 1:
            ...
            if '_raw_params' in params:
                ...
                if not term.startswith(relpath):
                    ...
                    raise DistronodeError('Unrecognized value after key=value parameters given to password lookup')
        ...
        if invalid_params:
            raise DistronodeError('Unrecognized parameter(s) given to password lookup: %s' % ', '.join(invalid_params))
        ...
        for term in terms:
            ...
            try:
                ...
                if not ident:
                    ...
                    raise DistronodeError('The ident parameter provided (%s) does not match the stored one (%s).' % (ident, params['ident']))
