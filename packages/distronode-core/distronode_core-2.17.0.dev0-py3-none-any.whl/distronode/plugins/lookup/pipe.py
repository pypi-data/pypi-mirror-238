...
DOCUMENTATION = r"""
    ...
    notes:
      - Like all lookups this runs on the Distronode controller and is unaffected by other keywords, such as become,
        so if you need to different permissions you must change the command or run Distronode as another user.
...
- name: raw result of running date command
  distronode.builtin.debug:
    msg: "{{ lookup('distronode.builtin.pipe', 'date') }}"

- name: Always use quote filter to make sure your variables are safe to use with shell
  distronode.builtin.debug:
    msg: "{{ lookup('distronode.builtin.pipe', 'getent passwd ' + myuser | quote ) }}"
"""
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for term in terms:
            ...
            if p.returncode == 0:
                ...
                raise DistronodeError("lookup_plugin.pipe(%s) returned %d" % (term, p.returncode))
        return ret
