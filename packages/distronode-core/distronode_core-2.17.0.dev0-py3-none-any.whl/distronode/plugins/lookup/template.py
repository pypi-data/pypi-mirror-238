...
- name: show templating results
  distronode.builtin.debug:
    msg: "{{ lookup('distronode.builtin.template', './some_template.j2') }}"

- name: show templating results with different variable start and end string
  distronode.builtin.debug:
    msg: "{{ lookup('distronode.builtin.template', './some_template.j2', variable_start_string='[%', variable_end_string='%]') }}"

- name: show templating results with different comment start and end string
  distronode.builtin.debug:
    msg: "{{ lookup('distronode.builtin.template', './some_template.j2', comment_start_string='[#', comment_end_string='#]') }}"
"""
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for term in terms:
            ...
            if lookupfile:
                ...
                searchpath = variables.get('distronode_search_path', [])
                if searchpath:
                ...
                # plus some added by distronode (e.g., template_{path,mtime}),
                # plus anything passed to the lookup with the template_vars=
                # argument.
                vars = deepcopy(variables)
                vars.update(generate_distronode_template_vars(term, lookupfile))
                ...
                if (C.DEFAULT_JINJA2_NATIVE and not jinja2_native) or not convert_data_p:
                    ...
                    # not to be processed by literal_eval anywhere in Distronode
                    res = NativeJinjaText(res)

                ret.append(res)
            else:
                raise DistronodeError("the template file %s could not be found for the lookup" % term)
