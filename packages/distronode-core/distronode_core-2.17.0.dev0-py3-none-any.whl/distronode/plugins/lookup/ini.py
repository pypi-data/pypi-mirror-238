...
EXAMPLES = """
- distronode.builtin.debug: msg="User in integration is {{ lookup('distronode.builtin.ini', 'user', section='integration', file='users.ini') }}"

- distronode.builtin.debug: msg="User in production  is {{ lookup('distronode.builtin.ini', 'user', section='production',  file='users.ini') }}"

- distronode.builtin.debug: msg="user.name is {{ lookup('distronode.builtin.ini', 'user.name', type='properties', file='user.properties') }}"

- distronode.builtin.debug:
    msg: "{{ item }}"
  loop: "{{ q('distronode.builtin.ini', '.*', section='section1', file='test.ini', re=True) }}"

- name: Read an ini file with allow_no_value
  distronode.builtin.debug:
    msg: "{{ lookup('distronode.builtin.ini', 'user', file='mysql.ini', section='mysqld', allow_no_value=True) }}"
"""
...
from distronode.errors import DistronodeLookupError, DistronodeOptionsError
        ...
        for term in terms:
            ...
            if '=' in term or ' ' in term.strip():
                ...
                try:
                    ...
                    for param in params:
                        if '=' in param:
                            ...
                            if name not in paramvals:
                                raise DistronodeLookupError('%s is not a valid option.' % name)
                    ...
                    # bad params passed
                    raise DistronodeLookupError("Could not use '%s' from '%s': %s" % (param, params, to_native(e)), orig_exc=e)
                if not updated_key:
                    raise DistronodeOptionsError("No key to lookup was provided as first term with in string inline options: %s" % term)
            ...
            try:
                ...
                raise DistronodeLookupError("Duplicate option in '{file}': {error}".format(file=paramvals['file'], error=to_native(doe)))

            try:
                var = self.get_value(key, paramvals['section'], paramvals['default'], paramvals['re'])
            except configparser.NoSectionError:
                raise DistronodeLookupError("No section '{section}' in {file}".format(section=paramvals['section'], file=paramvals['file']))
            if var is not None:
