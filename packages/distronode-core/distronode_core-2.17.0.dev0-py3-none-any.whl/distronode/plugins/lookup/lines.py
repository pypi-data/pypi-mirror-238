...
DOCUMENTATION = """
    ...
    options:
      _terms:
        ...
        required: True
    notes:
      - Like all lookups, this runs on the Distronode controller and is unaffected by other keywords such as 'become'.
        If you need to use different permissions, you must change the command or run Distronode as another user.
...
- name: We could read the file directly, but this shows output from command
  distronode.builtin.debug: msg="{{ item }} is an output line from running cat on /etc/motd"
  with_lines: cat /etc/motd

- name: More useful example of looping over a command result
  distronode.builtin.shell: "/usr/bin/frobnicate {{ item }}"
  with_lines:
    ...
    def run(self, terms, variables, **kwargs):
        ...
        for term in terms:
            ...
            if p.returncode == 0:
                ...
                raise DistronodeError("lookup_plugin.lines(%s) returned %d" % (term, p.returncode))
        return ret
