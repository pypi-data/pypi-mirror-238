...
- C(distronode_managed) (configurable via the C(defaults) section of C(distronode.cfg)) contains a string which can be used to
...
options:
  src:
    description:
    - Path of a Jinja2 formatted template on the Distronode controller.
...
- Since Distronode 0.9, templates are loaded with O(trim_blocks=True).
- >
