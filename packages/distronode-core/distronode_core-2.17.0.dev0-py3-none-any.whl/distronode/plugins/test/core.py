...
from distronode.module_utils.compat.version import LooseVersion, StrictVersion
...
def failed(result):
    ...
    if not isinstance(result, MutableMapping):
        raise errors.DistronodeFilterError("The 'failed' test expects a dictionary")
...
def unreachable(result):
    ...
    if not isinstance(result, MutableMapping):
        raise errors.DistronodeFilterError("The 'unreachable' test expects a dictionary")
...
def changed(result):
    ...
    if not isinstance(result, MutableMapping):
        raise errors.DistronodeFilterError("The 'changed' test expects a dictionary")
...
def skipped(result):
    ...
    if not isinstance(result, MutableMapping):
        raise errors.DistronodeFilterError("The 'skipped' test expects a dictionary")
...
def started(result):
    ...
    if not isinstance(result, MutableMapping):
        raise errors.DistronodeFilterError("The 'started' test expects a dictionary")
...
def finished(result):
    ...
    if not isinstance(result, MutableMapping):
        raise errors.DistronodeFilterError("The 'finished' test expects a dictionary")
...
def version_compare(value, version, operator='eq', strict=None, version_type=None):
    ...
    if strict is not None and version_type is not None:
        raise errors.DistronodeFilterError("Cannot specify both 'strict' and 'version_type'")

    if not value:
        raise errors.DistronodeFilterError("Input version value cannot be empty")

    if not version:
        raise errors.DistronodeFilterError("Version parameter to compare against cannot be empty")

    if version_type == 'pep440' and not HAS_PACKAGING:
        raise errors.DistronodeFilterError("The pep440 version_type requires the Python 'packaging' library")
    ...
    if strict:
        ...
        try:
            ...
            raise errors.DistronodeFilterError(
    ...
    try:
        ...
        raise errors.DistronodeFilterError('Version comparison failed: %s' % to_native(e))
