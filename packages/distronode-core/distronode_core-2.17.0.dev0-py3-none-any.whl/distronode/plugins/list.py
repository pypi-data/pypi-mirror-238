...
from distronode.collections.list import list_collections
...
from distronode.plugins import loader
from distronode.utils.display import Display
from distronode.utils.collection_loader._collection_finder import _get_collection_path
...
def get_composite_name(collection, name, path, depth):
    ...
    if '.' not in name:
        ...
        if collection == 'distronode.legacy' and name.startswith('distronode.builtin.'):
                    ...
                    if os.path.isdir(full_path):
                        ...
                        if ptype in ('test', 'filter'):
                            ...
                            for plugin in file_plugins:
                                plugin_name = get_composite_name(collection, plugin.distronode_name, os.path.dirname(to_native(full_path)), depth)
...
def list_collection_plugins(ptype, collections, search_paths=None):
    ...
    try:
        ...
        raise DistronodeError('Cannot list plugins, incorrect plugin type supplied: {0}'.format(ptype))

    # get plugins for each collection
    for collection in collections.keys():
        if collection == 'distronode.builtin':
            # dirs from distronode install, but not configured paths
            dirs = [d.path for d in ploader._get_paths_with_context() if d.internal]
        elif collection == 'distronode.legacy':
            ...
            # acr = DistronodeCollectionRef.try_parse_fqcr(collection, ptype)
            # if acr:
            #     dirs = acr.subdirs
            # else:
...
def list_plugins(ptype, collections=None, search_paths=None):
    ...
    if collections is None:
        ...
        plugin_collections['distronode.builtin'] = b''
        plugin_collections['distronode.legacy'] = b''
        plugin_collections.update(list_collections(search_paths=search_paths, dedupe=True))
    else:
        for collection in collections:
            if collection == 'distronode.legacy':
                ...
                try:
                    ...
                    raise DistronodeError("Cannot use supplied collection {0}: {1}".format(collection, to_native(e)), orig_exc=e)
...
def list_plugin_names(ptype, collection=None):
    return [plugin.distronode_name for plugin in list_plugins(ptype, collection)]
