...
DOCUMENTATION = '''
    ...
    options:
      plugin:
         ...
         required: True
         choices: ['distronode.builtin.generator', 'generator']
      hosts:
        description:
...
EXAMPLES = '''
    ...
    # remember to enable this inventory plugin in the distronode.cfg before using
    # View the output using `distronode-inventory -i inventory.config --list`
    plugin: distronode.builtin.generator
    hosts:
    ...
    def add_parents(self, inventory, child, parents, template_vars):
        for parent in parents:
            try:
                ...
                raise DistronodeParserError("Element %s has a parent with no name element" % child['name'])
