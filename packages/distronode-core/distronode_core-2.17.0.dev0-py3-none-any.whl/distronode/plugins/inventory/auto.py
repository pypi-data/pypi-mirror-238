...
class InventoryModule(BaseInventoryPlugin):
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        if not plugin_name:
            raise DistronodeParserError("no root 'plugin' key found, '{0}' is not a valid YAML inventory plugin config file".format(path))

        plugin = inventory_loader.get(plugin_name)

        if not plugin:
            raise DistronodeParserError("inventory config '{0}' specifies unknown plugin '{1}'".format(path, plugin_name))

        if not plugin.verify_file(path):
            raise DistronodeParserError("inventory source '{0}' could not be verified by inventory plugin '{1}'".format(path, plugin_name))
