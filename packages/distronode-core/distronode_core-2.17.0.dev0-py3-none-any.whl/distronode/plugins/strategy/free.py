...
DOCUMENTATION = '''
    ...
    description:
        - Task execution is as fast as possible per batch as defined by C(serial) (default all).
          Distronode will not wait for other hosts to finish the current task before queuing more tasks for other hosts.
...
from distronode.plugins.loader import action_loader
from distronode.plugins.strategy import StrategyBase
                    ...
                    if not self._blocked_hosts.get(host_name, False):
                        ...
                        if run_once:
                            if action and getattr(action, 'BYPASS_HOST_LOOP', False):
                                raise DistronodeError("The '%s' module bypasses the host loop, which is currently not supported in the free strategy "
