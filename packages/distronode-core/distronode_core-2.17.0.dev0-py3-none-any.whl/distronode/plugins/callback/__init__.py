...
from distronode.utils.unsafe_proxy import DistronodeUnsafeText, NativeJinjaUnsafeText
...
_YAML_TEXT_TYPES = (text_type, DistronodeUnicode, DistronodeUnsafeText, NativeJinjaUnsafeText)
...
class _DistronodeCallbackDumper(DistronodeDumper):
...
for data_type in _YAML_TEXT_TYPES:
    _DistronodeCallbackDumper.add_representer(
        data_type,
        _pretty_represent_str
    )


class CallbackBase(DistronodePlugin):

    '''
    This is a base distronode callback class that does nothing. New callbacks should
    ...
    def host_label(result):
        ...
        if result._task.delegate_to and result._task.delegate_to != result._host.get_name():
            ...
            ahost = result._result.get('_distronode_delegated_vars', {}).get('distronode_host', result._task.delegate_to)
            if result._task.delegate_to != ahost:
                label += "(%s)" % ahost
        return label

    def _run_is_verbose(self, result, verbosity=0):
        return ((self._display.verbosity > verbosity or result._result.get('_distronode_verbose_always', False) is True)
                and result._result.get('_distronode_verbose_override', False) is False)
    ...
    def _dump_results(self, result, indent=None, sort_keys=True, keep_invocation=False, serialize=True):
        ...
        indent_conditions = (
            result.get('_distronode_verbose_always'),
        ...
        # All result keys stating with _distronode_ are internal, so remove them from the result before we output anything.
        ...
        if result_format == 'json':
            try:
                return json.dumps(abridged_result, cls=DistronodeJSONEncoder, indent=indent, ensure_ascii=False, sort_keys=sort_keys)
            except TypeError:
                ...
                return json.dumps(OrderedDict(sorted(abridged_result.items(), key=to_text)),
                                  cls=DistronodeJSONEncoder, indent=indent,
            ...
            return '\n%s' % textwrap.indent(
                yaml.dump(
                    abridged_result,
                    allow_unicode=True,
                    Dumper=_DistronodeCallbackDumper(lossy=lossy),
    ...
    def _get_item_label(self, result):
        ...
        if result.get('_distronode_no_log', False):
            item = "(censored due to no_log)"
        else:
            item = result.get('_distronode_item_label', result.get('item'))
        return item
    ...
    def v2_runner_on_async_poll(self, result):
        host = result._host.get_name()
        jid = result._result.get('distronode_job_id')
    ...
    def v2_runner_on_async_failed(self, result):
        ...
        if not jid and 'async_result' in result._result:
            jid = result._result['async_result'].get('distronode_job_id')
