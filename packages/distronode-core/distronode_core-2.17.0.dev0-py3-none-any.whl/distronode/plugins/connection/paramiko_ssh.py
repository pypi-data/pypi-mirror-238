...
DOCUMENTATION = """
    ...
    description:
        ...
        - This is needed on the Distronode control machine to be reasonably efficient with connections.
    ...
    options:
      ...
      port:
          ...
          env:
            ...
            - name: DISTRONODE_REMOTE_PARAMIKO_PORT
              version_added: '2.15'
          vars:
            - name: distronode_port
            - name: distronode_ssh_port
            - name: distronode_paramiko_port
              version_added: '2.15'
          keyword:
            - name: port
      remote_user:
        ...
        vars:
            ...
            - name: distronode_paramiko_user
        env:
            - name: DISTRONODE_REMOTE_USER
            - name: DISTRONODE_PARAMIKO_REMOTE_USER
              version_added: '2.5'
        ini:
            - section: defaults
              key: remote_user
      ...
      password:
        ...
        vars:
            ...
            - name: distronode_paramiko_pass
            - name: distronode_paramiko_password
              version_added: '2.5'
      use_rsa_sha2_algorithms:
        ...
        vars:
            - name: distronode_paramiko_use_rsa_sha2_algorithms
        ini:
      ...
      host_key_auto_add:
        ...
        env: [{name: DISTRONODE_PARAMIKO_HOST_KEY_AUTO_ADD}]
        ini:
      ...
      look_for_keys:
        ...
        env: [{name: DISTRONODE_PARAMIKO_LOOK_FOR_KEYS}]
        ini:
      ...
      proxy_command:
        ...
        type: string
        env: [{name: DISTRONODE_PARAMIKO_PROXY_COMMAND}]
        ini:
          - {key: proxy_command, section: paramiko_connection}
        vars:
          - name: distronode_paramiko_proxy_command
            version_added: '2.15'
      ssh_args:
      ...
      record_host_keys:
        ...
        env: [{name: DISTRONODE_PARAMIKO_RECORD_HOST_KEYS}]
        ini:
          - section: paramiko_connection
            key: record_host_keys
        type: boolean
      host_key_checking:
        description: 'Set this to "False" if you want to avoid host key checking by the underlying tools Distronode uses to connect to the host'
        type: boolean
        default: True
        env:
          ...
          - name: DISTRONODE_SSH_HOST_KEY_CHECKING
            version_added: '2.5'
          - name: DISTRONODE_PARAMIKO_HOST_KEY_CHECKING
            version_added: '2.5'
        ini:
          - section: defaults
      ...
      use_persistent_connections:
        ...
        default: False
        env:
          - name: DISTRONODE_USE_PERSISTENT_CONNECTIONS
        ini:
          - section: defaults
      ...
      banner_timeout:
        ...
        ini:
          - section: paramiko_connection
            key: banner_timeout
        env:
          - name: DISTRONODE_PARAMIKO_BANNER_TIMEOUT
      timeout:
        type: int
        default: 10
        ...
        env:
          ...
          - name: DISTRONODE_SSH_TIMEOUT
            version_added: '2.11'
          - name: DISTRONODE_PARAMIKO_TIMEOUT
            version_added: '2.15'
        vars:
          - name: distronode_ssh_timeout
            version_added: '2.11'
      ...
      private_key_file:
          ...
          env:
            ...
            - name: DISTRONODE_PARAMIKO_PRIVATE_KEY_FILE
              version_added: '2.15'
          vars:
            - name: distronode_private_key_file
...
from distronode.module_utils.compat.paramiko import PARAMIKO_IMPORT_ERR, paramiko
    ...
    def missing_host_key(self, client, hostname, key) -> None:
        ...
        if all((self.connection.get_option('host_key_checking'), not self.connection.get_option('host_key_auto_add'))):
            ...
            if self.connection.get_option('use_persistent_connections') or self.connection.force_persistence:
                ...
                raise DistronodeError(AUTHENTICITY_MSG[1:92] % (hostname, ktype, fingerprint))
            ...
            if inp not in ['yes', 'y', '']:
                raise DistronodeError("host connection rejected by user")

        key._added_by_distronode_this_time = True
...
class Connection(ConnectionBase):
    ...
    def _parse_proxy_command(self, port: int = 22) -> dict[str, t.Any]:
        proxy_command = None
        # Parse distronode_ssh_common_args, specifically looking for ProxyCommand
        ssh_args = [
    ...
    def _connect_uncached(self) -> paramiko.SSHClient:
        ...
        if paramiko is None:
            raise DistronodeError("paramiko is not installed: %s" % to_native(PARAMIKO_IMPORT_ERR))
        ...
        try:
            ...
            raise DistronodeConnectionFailure('host key mismatch for %s' % e.hostname)
            ...
            if u"PID check failed" in msg:
                raise DistronodeError("paramiko version issue, please upgrade paramiko on the machine running distronode")
    ...
    def exec_command(self, cmd: str, in_data: bytes | None = None, sudoable: bool = True) -> tuple[int, bytes, bytes]:
        ...
        if in_data:
            raise DistronodeError("Internal Error: this module does not support optimized module pipelining")
        ...
        try:
            ...
            if text_e:
                msg += u": %s" % text_e
            raise DistronodeConnectionFailure(to_native(msg))

        # sudo usually requires a PTY (cf. requiretty option), therefore
        # we give it one by default (pty=True in distronode.cfg), and we try
            ...
            if self.become and self.become.expect_prompt():
                ...
                while not (become_sucess or passprompt):
                    ...
                    if not chunk:
                        if b'unknown user' in become_output:
                            ...
                            raise DistronodeError('user %s does not exist' % n_become_user)
                        else:
                            break
                            # raise DistronodeError('ssh connection closed waiting for password prompt')
                    become_output += chunk
                ...
                if passprompt:
                    if self.become:
                        ...
                        raise DistronodeError("A password is required but none was supplied")
                else:
                    no_prompt_out += become_output
                    no_prompt_err += become_output
        except socket.timeout:
            raise DistronodeError('ssh timed out waiting for privilege escalation.\n' + to_text(become_output))
    ...
    def put_file(self, in_path: str, out_path: str) -> None:
        ...
        if not os.path.exists(to_bytes(in_path, errors='surrogate_or_strict')):
            raise DistronodeFileNotFound("file or module does not exist: %s" % in_path)

        try:
            self.sftp = self.ssh.open_sftp()
        except Exception as e:
            raise DistronodeError("failed to open a SFTP connection (%s)" % e)

        try:
            self.sftp.put(to_bytes(in_path, errors='surrogate_or_strict'), to_bytes(out_path, errors='surrogate_or_strict'))
        except IOError:
            raise DistronodeError("failed to transfer file to %s" % out_path)
    ...
    def fetch_file(self, in_path: str, out_path: str) -> None:
        ...
        try:
            ...
            raise DistronodeError("failed to open a SFTP connection (%s)" % to_native(e))

        try:
            self.sftp.get(to_bytes(in_path, errors='surrogate_or_strict'), to_bytes(out_path, errors='surrogate_or_strict'))
        except IOError:
            raise DistronodeError("failed to transfer file from %s" % in_path)

    def _any_keys_added(self) -> bool:

        for hostname, keys in self.ssh._host_keys.items():
            for keytype, key in keys.items():
                added_this_time = getattr(key, '_added_by_distronode_this_time', False)
                if added_this_time:
                    return True
        return False
