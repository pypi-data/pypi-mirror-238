...
DOCUMENTATION = """
    ...
    description:
        ...
        - This plugin allows extra arguments to be passed that are supported by the protocol but not explicitly defined here.
          They should take the form of variables declared with the following pattern C(distronode_winrm_<option>).
    version_added: "2.0"
    ...
    options:
      ...
      scheme:
        ...
        choices: [http, https]
        vars:
          - name: distronode_winrm_scheme
        type: str
      path:
        description: URI path to connect to
        default: '/wsman'
        vars:
          - name: distronode_winrm_path
        type: str
      transport:
        description:
        ...
        elements: string
        vars:
          - name: distronode_winrm_transport
      kerberos_command:
        description: kerberos command to use to request a authentication ticket
        default: kinit
        vars:
          - name: distronode_winrm_kinit_cmd
        type: str
      kinit_args:
        description:
        - Extra arguments to pass to C(kinit) when getting the Kerberos authentication ticket.
        - By default no extra arguments are passed into C(kinit) unless I(distronode_winrm_kerberos_delegation) is also
        ...
        type: str
        vars:
          - name: distronode_winrm_kinit_args
        version_added: '2.11'
      kinit_env_vars:
        description:
        ...
        - section: winrm
          key: kinit_env_vars
        vars:
          - name: distronode_winrm_kinit_env_vars
        version_added: '2.12'
      kerberos_mode:
        description:
            - kerberos usage mode.
            - The managed option means Distronode will obtain kerberos ticket.
            - While the manual one means a ticket must already have been obtained by the user.
            - If having issues with Distronode freezing when trying to obtain the
              Kerberos ticket, you can either set this to V(manual) and obtain
              it outside Distronode or install C(pexpect) through pip and try
              again.
        choices: [managed, manual]
        vars:
          - name: distronode_winrm_kinit_mode
        type: str
      connection_timeout:
        description:
            ...
            - The read timeout sets the client HTTP-request timeout and specifies how long the
              client (distronode controller) will wait for data from the server to come back over
              the HTTP-connection (timeout for waiting for in-between messages from the server).
              When this timer expires, an exception will be thrown and the distronode connection
            ...
            - The default value is whatever is set in the installed version of
              pywinrm.
        vars:
          - name: distronode_winrm_connection_timeout
        type: int
"""
...
from distronode.plugins.shell.powershell import ShellBase as PowerShellBase
from distronode.utils.hashing import secure_hash
...
try:
    ...
    # https://github.com/distronode/distronode/issues/43462
...
class Connection(ConnectionBase):
    ...
    def _build_winrm_kwargs(self) -> None:
        ...
        if unsupported_transports:
            raise DistronodeError('The installed version of WinRM does not support transport(s) %s' %
        ...
        passed_winrm_args = {v.replace('distronode_winrm_', '') for v in self.get_option('_extras')}
        unsupported_args = passed_winrm_args.difference(supported_winrm_args)

        # warn for kwargs unsupported by the installed version of pywinrm
        for arg in unsupported_args:
            display.warning("distronode_winrm_{0} unsupported by pywinrm (is an up-to-date version of pywinrm installed?)".format(arg))

        # pass through matching extras, excluding the list we want to treat specially
        for arg in passed_winrm_args.difference(internal_kwarg_mask).intersection(supported_winrm_args):
            self._winrm_kwargs[arg] = self.get_option('_extras')['distronode_winrm_%s' % arg]
    ...
    def _kerb_auth(self, principal: str, password: str) -> None:
        ...
        # Stores various flags to call with kinit, these could be explicit args set by 'distronode_winrm_kinit_args' OR
        # '-f' if kerberos delegation is requested (distronode_winrm_kerberos_delegation).
        ...
        elif boolean(self.get_option('_extras').get('distronode_winrm_kerberos_delegation', False)):
        ...
        if HAS_PEXPECT:
            ...
            try:
                ...
                raise DistronodeConnectionFailure(err_msg)

            try:
                child.expect(".*:")
                child.sendline(password)
            except OSError as err:
                # child exited before the pass was sent, Distronode will raise
    ...
    def _winrm_connect(self) -> winrm.Protocol:
        ...
        for transport in self._winrm_transport:
            ...
            try:
                ...
                if re.search(to_text(r'Operation\s+?timed\s+?out'), err_msg, re.I):
                    raise DistronodeError('the connection attempt timed out')
        ...
        if errors:
            raise DistronodeConnectionFailure(', '.join(map(to_native, errors)))
        else:
            raise DistronodeError('No transport found for WinRM connection')
        ...
        try:
            ...
            if stdin_push_failed:
                ...
                try:
                    ...
                    raise DistronodeError('winrm send_input failed; \nstdout: %s\nstderr %s'
                                       % (to_native(response.std_out), to_native(stderr)))

            return response
        except requests.exceptions.Timeout as exc:
            raise DistronodeConnectionFailure('winrm connection error: %s' % to_native(exc))
        finally:
            if command_id:
    ...
    def _connect(self) -> Connection:
        ...
        if not HAS_WINRM:
            raise DistronodeError("winrm or requests is not installed: %s" % to_native(WINRM_IMPORT_ERR))
        elif not HAS_XMLTODICT:
            raise DistronodeError("xmltodict is not installed: %s" % to_native(XMLTODICT_IMPORT_ERR))
    ...
    def put_file(self, in_path: str, out_path: str) -> None:
        ...
        if result.status_code != 0:
            raise DistronodeError(to_native(result.std_err))
        ...
        try:
            ...
            if stderr.startswith(b"#< CLIXML"):
                stderr = _parse_clixml(stderr)
            raise DistronodeError('winrm put_file failed; \nstdout: %s\nstderr %s' % (to_native(result.std_out), to_native(stderr)))

        remote_sha1 = put_output.get("sha1")
        if not remote_sha1:
            raise DistronodeError("Remote sha1 was not returned")

        local_sha1 = secure_hash(in_path)

        if not remote_sha1 == local_sha1:
            raise DistronodeError("Remote sha1 hash {0} does not match local hash {1}".format(to_native(remote_sha1), to_native(local_sha1)))
        ...
        try:
            ...
            while True:
                try:
                    ...
                    traceback.print_exc()
                    raise DistronodeError('failed to transfer file to "%s"' % to_native(out_path))
        finally:
            if out_file:
                out_file.close()
