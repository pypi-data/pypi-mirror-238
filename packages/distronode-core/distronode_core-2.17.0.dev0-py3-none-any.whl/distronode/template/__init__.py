...
from distronode.errors import (
    DistronodeAssertionError,
    DistronodeError,
    DistronodeFilterError,
    DistronodeLookupError,
    DistronodeOptionsError,
    DistronodeUndefinedVariable,
)
...
from distronode.plugins.loader import filter_loader, lookup_loader, test_loader
from distronode.template.native_helpers import distronode_native_concat, distronode_eval_concat, distronode_concat
from distronode.template.template import DistronodeJ2Template
...
from distronode.utils.native_jinja import NativeJinjaText
from distronode.utils.unsafe_proxy import to_unsafe_text, wrap_var

display = Display()


__all__ = ['Templar', 'generate_distronode_template_vars']
...
def generate_distronode_template_vars(path, fullpath=None, dest_path=None):
    ...
    temp_vars['distronode_managed'] = to_unsafe_text(time.strftime(to_native(managed_str), time.localtime(os.path.getmtime(b_path))))
...
def _create_overlay(data, overrides, jinja_env):
    ...
    if has_override_header:
        ...
        for pair in line.split(','):
            if ':' not in pair:
                raise DistronodeError("failed to parse jinja2 override '%s'."
...
def recursive_check_defined(item):
    ...
    if isinstance(item, MutableMapping):
        ...
        if isinstance(item, Undefined):
            raise DistronodeFilterError("{0} is undefined".format(item))
...
def _wrap_native_text(func):
    ...
    in ``distronode_native_concat`` to indicate that it is a text
...
class DistronodeUndefined(StrictUndefined):
    '''
    ...
    def __getattr__(self, name):
        if name == '__UNSAFE__':
            # DistronodeUndefined should never be assumed to be unsafe
    ...
    def __repr__(self):
        return 'DistronodeUndefined(hint={0!r}, obj={1!r}, name={2!r})'.format(
...
class DistronodeContext(Context):
    '''
    A custom context, which intercepts resolve_or_missing() calls and sets a flag
    internally if any variable lookup returns an DistronodeUnsafe value. This
    flag is checked post-templating, and (when set) will result in the
    final templated result being wrapped in DistronodeUnsafe.
    '''
    def __init__(self, *args, **kwargs):
        super(DistronodeContext, self).__init__(*args, **kwargs)
        self.unsafe = False
    ...
    def _is_unsafe(self, val):
        ...
        lose the DistronodeUnsafe value.
        '''
    ...
    def resolve_or_missing(self, key):
        val = super(DistronodeContext, self).resolve_or_missing(key)
    ...
    def get_all(self):
        ...
        This is to prevent from running ``DistronodeJ2Vars`` through dict():

            ``dict(self.parent, **self.vars)``

        In Distronode this means that ALL variables would be templated in the
        process of re-creating the parent because ``DistronodeJ2Vars`` templates
        each variable in its ``__getitem__`` method. Instead we re-create the
        parent via ``DistronodeJ2Vars.add_locals`` that creates a new
        ``DistronodeJ2Vars`` copy without templating each variable.
        ...
        Also see ``DistronodeJ2Template``and
        ...
        if isinstance(self.parent, DistronodeJ2Vars):
            return self.parent.add_locals(self.vars)
        else:
            # can this happen in Distronode?
...
class JinjaPluginIntercept(MutableMapping):
    ...
    def __getitem__(self, key):
        ...
        if key not in self._seen_it:
            ...
            except (DistronodeError, KeyError) as e:
                original_exc = e
...
def _fail_on_undefined(data):
    ...
    if isinstance(data, Mapping):
        ...
        if isinstance(data, StrictUndefined):
            ...
            # handled.
            # See https://github.com/distronode/distronode/issues/52158
...
@_unroll_iterator
def _distronode_finalize(thing):
...
class DistronodeEnvironment(NativeEnvironment):
    '''
    Our custom environment, which simply allows us to override the class-level
    values for the Template and Context classes used by jinja2 internally.
    '''
    context_class = DistronodeContext
    template_class = DistronodeJ2Template
    concat = staticmethod(distronode_eval_concat)  # type: ignore[assignment]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.undefined = DistronodeUndefined
        self.finalize = _distronode_finalize


class DistronodeNativeEnvironment(DistronodeEnvironment):
    concat = staticmethod(distronode_native_concat)  # type: ignore[assignment]
...
class Templar:
    ...
    def __init__(self, loader, variables=None):
        ...
        environment_class = DistronodeNativeEnvironment if C.DEFAULT_JINJA2_NATIVE else DistronodeEnvironment
    ...
    def copy_with_new_env(self, environment_class=DistronodeEnvironment, **kwargs):
        ...
        new_templar.jinja2_native = environment_class is DistronodeNativeEnvironment
    ...
    def _get_extensions(self):
        ...
        If some extensions are set via jinja_extensions in distronode.cfg, we try
    ...
    def available_variables(self, variables):
        ...
        if not isinstance(variables, Mapping):
            raise DistronodeAssertionError("the type of 'variables' should be a Mapping but was a %s" % (type(variables)))
    ...
    def _fail_lookup(self, name, *args, **kwargs):
        raise DistronodeError("The lookup `%s` was found, however lookups were disabled from templating" % name)
    ...
    def _lookup(self, name, /, *args, **kwargs):
        ...
        if instance is None:
            raise DistronodeError("lookup plugin (%s) not found" % name)
        ...
        try:
        ...
        except (DistronodeUndefinedVariable, UndefinedError) as e:
            raise DistronodeUndefinedVariable(e)
        except DistronodeOptionsError as e:
            # invalid options given to lookup, just reraise
            raise e
        except DistronodeLookupError as e:
            ...
            if errors == 'warn':
                ...
                raise DistronodeError(to_native(msg), orig_exc=e)
            return [] if wantlist else None
        ...
        if not is_sequence(ran):
            display.deprecated(
                ...
                'This will be an error in Distronode 2.18',
                version='2.18'
            )
        ...
        if ran and allow_unsafe is False:
            ...
            try:
                ...
                if not isinstance(ran, Sequence):
                    raise DistronodeError("The lookup plugin '%s' did not return a list."
                                       % name)
                ...
                # See https://github.com/distronode/distronode/pull/77789
                ran = wrap_var(",".join(ran))
    ...
    def _make_undefined(self, hint=None):
        ...
        return DistronodeUndefined(hint)
    ...
    def do_template(self, data, preserve_trailing_newlines=True, escape_backslashes=True, fail_on_undefined=None, overrides=None, disable_lookups=False,
        ...
        try:
            ...
            # when templating nested variables in DistronodeJ2Vars where Templar.environment is used, not the overlay.
            ...
            try:
                ...
                raise DistronodeError("template error while templating string: %s. String: %s" % (to_native(e), to_native(data)), orig_exc=e)
            except Exception as e:
                if 'recursion' in to_native(e):
                    raise DistronodeError("recursive loop detected in template string: %s" % to_native(data), orig_exc=e)
                else:
                    return data
            ...
            jvars = DistronodeJ2Vars(self, t.globals)
            ...
            # the concat function is set for each Distronode environment,
            ...
            if not self.jinja2_native and not convert_data:
                myenv.concat = distronode_concat
            ...
            try:
                ...
                if 'DistronodeUndefined' in to_native(te):
                    errmsg = "Unable to look up a name or access an attribute in template string (%s).\n" % to_native(data)
                    errmsg += "Make sure your variable name does not contain invalid characters like '-': %s" % to_native(te)
                    raise DistronodeUndefinedVariable(errmsg, orig_exc=te)
                else:
                    display.debug("failing because of a type error, template data is: %s" % to_text(data))
                    raise DistronodeError("Unexpected templating type error occurred on (%s): %s" % (to_native(data), to_native(te)), orig_exc=te)
            finally:
            ...
            if isinstance(res, string_types) and preserve_trailing_newlines:
                ...
                if data_newlines > res_newlines:
                    ...
                    if unsafe:
                        res = wrap_var(res)
            return res
        except (UndefinedError, DistronodeUndefinedVariable) as e:
            if fail_on_undefined:
                raise DistronodeUndefinedVariable(e, orig_exc=e)
            else:
