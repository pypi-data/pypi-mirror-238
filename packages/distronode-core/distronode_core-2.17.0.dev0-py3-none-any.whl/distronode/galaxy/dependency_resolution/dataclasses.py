...
from distronode.galaxy.collection import HAS_PACKAGING, PkgReq
...
class _ComputedReqKindsMixin:
    ...
    def from_string(cls, collection_input, artifacts_manager, supplemental_signatures):
        req = {}
        if _is_concrete_artifact_pointer(collection_input) or DistronodeCollectionRef.is_valid_collection_name(collection_input):
    ...
    def from_requirement_dict(cls, collection_req, art_mgr, validate_signature_options=True):
        ...
        if req_type is None:
            if (  # FIXME: decide on the future behavior:
                ...
                src_path = req_source
                    ...
                    and DistronodeCollectionRef.is_valid_collection_name(req_name)
            ):
                req_type = 'galaxy'
            elif (
                    req_name is not None
                ...
                raise DistronodeError(  # NOTE: I'd prefer a ValueError instead
        ...
        if req_type is None:
            if _is_git_url(src_path):
                ...
                raise DistronodeError(  # NOTE: this is never supposed to be hit
    ...
    def construct_galaxy_info_path(self, b_collection_path):
        ...
        # collections/distronode_collections/ns.coll-1.0.0.info/GALAXY.yml
    ...
    def is_pinned(self):
        ...
        See https://github.com/distronode/distronode/pull/81606 for extra context.
        """
...
class Candidate(
    ...
    def with_signatures_repopulated(self):  # type: (Candidate) -> Candidate
        ...
        :raises DistronodeAssertionError: If the supplied candidate is not sourced from a Galaxy-like index.
        """
        if self.type != 'galaxy':
            raise DistronodeAssertionError(f"Invalid collection type for {self!r}: unable to get signatures from a galaxy server.")
