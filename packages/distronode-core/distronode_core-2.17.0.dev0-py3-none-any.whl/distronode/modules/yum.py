...
description:
     ...
     - This module only works on Python 2. If you require Python 3 support see the M(distronode.builtin.dnf) module.
options:
  use_backend:
    description:
      - This module supports V(yum) (as it always has), this is known as C(yum3)/C(YUM3)/C(yum-deprecated) by
        upstream yum developers. As of Distronode 2.7+, this module also supports C(YUM4), which is the
        "new yum" and it has an V(dnf) backend. As of distronode-core 2.15+, this module will auto select the backend
        based on the C(distronode_pkg_mgr) fact.
      - By default, this module will select the backend based on the C(distronode_pkg_mgr) fact.
    default: "auto"
  ...
  enablerepo:
    description:
      ...
      - As of Distronode 2.7, this can alternatively be a list instead of C(",")
        separated string
    type: list
    elements: str
    default: []
...
attributes:
    action:
        details: In the case of yum, it has 2 action plugins that use it under the hood, M(distronode.builtin.yum) and M(distronode.builtin.package).
        support: partial
    async:
        support: none
    bypass_host_loop:
...
notes:
  ...
  - 'Yum itself has two types of groups.  "Package groups" are specified in the
    ...
    apparent to distronode users because distronode needs to operate on the group
  ...
  - 'The yum module does not support clearing yum cache in an idempotent way, so it
    ...
    https://github.com/distronode/distronode/pull/31450#issuecomment-352889579'
...
- name: List distronode packages and register result to print with debug later
  distronode.builtin.yum:
    list: distronode
  register: result
...
from distronode.module_utils.common.respawn import has_respawned, respawn_module
from distronode.module_utils.common.text.converters import to_native, to_text
from distronode.module_utils.yumdnf import YumDnf, yumdnf_argument_spec
...
class YumModule(YumDnf):
    """
    Yum Distronode module back-end implementation
    """
        ...
        if pkgs:
            ...
            for pkg in pkgs:
                ...
                if installed:
                    ...
                    # https://github.com/distronode/distronode/issues/35672
    ...
    def ensure(self, repoq):
                    ...
                    try:
                        ...
                        for i in new_repos:
                            if i not in current_repos:
                                ...
                                a = rid.repoXML.repoid  # nopep8 - https://github.com/distronode/distronode/pull/21475#pullrequestreview-22404868
        ...
        if self.state == 'latest' or self.update_only:
            ...
            # should be caught by DistronodeModule argument_spec
    ...
    def run(self):
        ...
        if not HAS_RPM_PYTHON:
            error_msgs.append('The Python 2 bindings for rpm are needed for this module. If you require Python 3 support use the `dnf` Distronode module instead.')
        if not HAS_YUM_PYTHON:
            error_msgs.append('The Python 2 yum module is needed for this module. If you require Python 3 support use the `dnf` Distronode module instead.')
