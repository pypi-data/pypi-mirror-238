...
description:
     ...
     - This is useful and should only be done in a few cases. A common
       ...
       the M(distronode.builtin.shell) or M(distronode.builtin.command) module is much more appropriate.
     ...
     - This module does not require python on the remote system, much like
       the M(distronode.builtin.script) module.
...
notes:
    ...
    - If you want to execute a command securely and predictably, it may be
      better to use the M(distronode.builtin.command) or M(distronode.builtin.shell) modules instead.
...
- name: Bootstrap a host without python2 installed
  distronode.builtin.raw: dnf install -y python2 python2-dnf libselinux-python

- name: Run a command that uses non-posix shell-isms (in this example /bin/sh doesn't handle redirection and wildcards together but bash does)
  distronode.builtin.raw: cat < /tmp/*txt
  args:
    executable: /bin/bash

- name: Safely use templated variables. Always use quote filter to avoid injection issues.
  distronode.builtin.raw: "{{ package_mgr|quote }} {{ pkg_flags|quote }} install {{ python|quote }}"

- name: List user accounts on a Windows system
  distronode.builtin.raw: Get-WmiObject -Class Win32_UserAccount
'''
