...
description:
  - The M(distronode.builtin.script) module takes the script name followed by a list of space-delimited arguments.
  ...
  - This module does not require Python on the remote system, much like the M(distronode.builtin.raw) module.
...
options:
  ...
  executable:
    ...
    type: str
notes:
  - It is usually preferable to write Distronode modules rather than pushing scripts. Convert your script to an Distronode module for bonus points!
  - The P(distronode.builtin.ssh#connection) connection plugin will force pseudo-tty allocation via C(-tt) when scripts are executed.
    ...
    please switch to a set of tasks that comprises M(distronode.builtin.copy) with M(distronode.builtin.command) instead of using M(distronode.builtin.script).
...
- name: Run a script with arguments (free form)
  distronode.builtin.script: /some/local/script.sh --some-argument 1234

- name: Run a script with arguments (using 'cmd' parameter)
  distronode.builtin.script:
    cmd: /some/local/script.sh --some-argument 1234

- name: Run a script only if file.txt does not exist on the remote node
  distronode.builtin.script: /some/local/create_file.sh --some-argument 1234
  args:
    creates: /the/created/file.txt

- name: Run a script only if file.txt exists on the remote node
  distronode.builtin.script: /some/local/remove_file.sh --some-argument 1234
  args:
    removes: /the/removed/file.txt

- name: Run a script using an executable in a non-system path
  distronode.builtin.script: /some/local/script
  args:
    executable: /some/remote/executable

- name: Run a script using an executable in a system path
  distronode.builtin.script: /some/local/script.py
  args:
    executable: python3
