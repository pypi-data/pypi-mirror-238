...
options:
  ...
  fail_msg:
    description:
      ...
      - This argument was called 'msg' before Distronode 2.7, now it is renamed to 'fail_msg' with alias 'msg'.
    type: str
    aliases: [ msg ]
    version_added: "2.7"
  success_msg:
...
- module: distronode.builtin.meta
author:
...
EXAMPLES = r'''
- distronode.builtin.assert: { that: "distronode_os_family != 'RedHat'" }
...
- name: Please use 'msg' when distronode version is smaller than 2.7
  distronode.builtin.assert:
    that:
      - my_param <= 100
      - my_param >= 0
