...
description:
  ...
  - 'When crontab jobs are managed: the module includes one line with the description of the
    crontab entry C("#Distronode: <name>") corresponding to the "name" passed to the module,
    which is used by future distronode/module calls to find/check the state. The "name"
...
options:
  name:
    description:
      ...
      - This parameter is always required as of distronode-core 2.12.
    type: str
    required: yes
  user:
    description:
...
- name: Ensure a job that runs at 2 and 5 exists. Creates an entry like "0 5,2 * * ls -alh > /dev/null"
  distronode.builtin.cron:
    name: "check dirs"
    minute: "0"
    hour: "5,2"
    job: "ls -alh > /dev/null"

- name: 'Ensure an old job is no longer present. Removes any job that is prefixed by "#Distronode: an old job" from the crontab'
  distronode.builtin.cron:
    name: "an old job"
    state: absent
...
- name: Creates a cron file under /etc/cron.d
  distronode.builtin.cron:
    ...
    cron_file: distronode_yum-autoupdate
...
class CronTab(object):
    ...
    def do_comment(self, name):
        return "%s%s" % (self.distronode, name)
    ...
    def find_job(self, name, job=None):
        # attempt to find job by 'Distronode:' header comment
        comment = None
        for l in self.lines:
            if comment is not None:
                if comment == name:
                    ...
                    comment = None
            elif re.match(r'%s' % self.distronode, l):
                comment = re.sub(r'%s' % self.distronode, '', l)

        # failing that, attempt to find job by exact match
        if job:
            for i, l in enumerate(self.lines):
                if l == job:
                    # if no leading distronode header, insert one
                    if not re.match(r'%s' % self.distronode, self.lines[i - 1]):
                        self.lines.insert(i, self.do_comment(name))
                        return [self.lines[i], l, True]
                    # if a leading blank distronode header AND job has a name, update header
    ...
    def get_cron_job(self, minute, hour, day, month, weekday, job, special, disabled):
        # normalize any leading/trailing newlines (distronode/distronode-modules-core#3791)
        job = job.strip('\r\n')
    ...
    def get_jobnames(self):
        ...
        for l in self.lines:
            if re.match(r'%s' % self.distronode, l):
                jobnames.append(re.sub(r'%s' % self.distronode, '', l))
    ...
    def _update_job(self, name, job, addlinesfunction):
        distronodename = self.do_comment(name)
        newlines = []
        comment = None
...
def main():
    ...
    # Would produce:
    # PATH=/bin:/usr/bin
    # # Distronode: check dirs
    # * * 5,2 * * ls -alh > /dev/null
    # # Distronode: do the job
    # * * 5,2 * * /some/dir/job.sh
