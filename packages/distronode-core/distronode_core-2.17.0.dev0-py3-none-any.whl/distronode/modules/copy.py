...
description:
    - The M(distronode.builtin.copy) module copies a file or a directory structure from the local or remote machine to a location on the remote machine.
    ...
    - Get meta-information with the M(distronode.builtin.stat) module.
    - Set meta-information with the M(distronode.builtin.file) module.
    - Use the M(distronode.builtin.fetch) module to copy files from remote locations to the local box.
    - If you need variable interpolation in copied files, use the M(distronode.builtin.template) module.
      Using a variable with the O(content) parameter produces unpredictable results.
    - For Windows targets, use the M(distronode.windows.win_copy) module instead.
options:
  src:
    description:
  ...
  content:
    ...
    - For advanced formatting or if O(content) contains a variable, use the
      M(distronode.builtin.template) module.
    type: str
    version_added: '1.1'
  dest:
    description:
  ...
  mode:
    ...
    - For those used to C(/usr/bin/chmod) remember that modes are actually octal numbers.
      You must either add a leading zero so that Distronode's YAML parser knows it is an octal number
      (like V(0644) or V(01777)) or quote it (like V('644') or V('1777')) so Distronode receives a string
      and can do its own conversion from string into number. Giving Distronode a number without following
      one of these rules will end up with a decimal number which will have unexpected results.
    - As of Distronode 1.8, the mode may be specified as a symbolic mode (for example, V(u+rwx) or V(u=rw,g=r,o=r)).
    - As of Distronode 2.3, the mode may also be the special string V(preserve).
  ...
  checksum:
    ...
    - If this is not provided, distronode will use the local calculated checksum of the src file.
    type: str
    version_added: '2.5'
...
notes:
    - The M(distronode.builtin.copy) module recursively copy facility does not scale to lots (>hundreds) of files.
seealso:
    - module: distronode.builtin.assemble
    - module: distronode.builtin.fetch
    - module: distronode.builtin.file
    - module: distronode.builtin.template
    - module: distronode.posix.synchronize
...
src:
    ...
    returned: changed
    type: str
    sample: /home/httpd/.distronode/tmp/distronode-tmp-1423796390.97-147729857856000/source
md5sum:
...
# basic::DistronodeModule() until then but if so, make it a private function so that we don't have to
...
def split_pre_existing_dir(dirname):
    ...
    if not os.path.exists(b_head):
        if head == '/':
            raise DistronodeModuleError(results={'msg': "The '/' directory doesn't exist on this machine."})
...
def main():
    ...
    if dest.endswith(os.sep):
        ...
        if not os.path.exists(b_dirname):
            ...
            except DistronodeModuleError as e:
