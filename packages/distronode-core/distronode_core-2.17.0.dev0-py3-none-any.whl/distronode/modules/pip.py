...
options:
  ...
  virtualenv_python:
    description:
      - The Python executable used for creating the virtual environment.
        ...
        Python version used to run the distronode module is used. This parameter
  ...
  state:
    description:
      - The state of module
      - The 'forcereinstall' option is only available in Distronode 2.1 and above.
    type: str
  ...
  executable:
    description:
      - The explicit executable or pathname for the pip executable,
        if different from the Distronode Python interpreter. For
      ...
      - Does not affect the Distronode Python interpreter.
      - The setuptools package must be installed for both the Distronode Python interpreter
...
notes:
   ...
   - Although it executes using the Distronode Python interpreter, the pip module shells out to
     run the actual pip command, so it can use any pip version you specify with O(executable).
     By default, it uses the pip version for the Distronode Python interpreter. For example, pip3 on python 3, and pip2 or pip on python 2.
   - The interpreter used by Distronode
     (see R(distronode_python_interpreter, distronode_python_interpreter))
  ...
  block:
    - name: Ensure the virtual environment exists
      pip:
        ...
        virtualenv_command: "{{ distronode_python_interpreter }} -m venv"
    ...
    - name: Run a module inside the virtual environment
      ...
      vars:
        ...
        distronode_python_interpreter: "{{ venv_python }}"
...
cmd:
  ...
  returned: success
  type: str
  sample: pip2 install distronode six
name:
  description: list of python modules targeted by pip
  returned: success
  type: list
  sample: ['distronode', 'six']
requirements:
...
from distronode.module_utils.basic import DistronodeModule, is_executable, missing_required_lib
