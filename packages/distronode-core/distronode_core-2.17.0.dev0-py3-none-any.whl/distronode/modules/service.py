...
description:
    ...
    - This module is a proxy for multiple more specific service manager modules
      (such as M(distronode.builtin.systemd) and M(distronode.builtin.sysvinit)).
      ...
      service manager discovered by M(distronode.builtin.setup).  If M(distronode.builtin.setup) was not yet run, this module may run it.
    - For Windows targets, use the M(distronode.windows.win_service) module instead.
options:
    name:
        description:
        - Name of the service.
    ...
    use:
        ...
        - Normally it uses the value of the 'distronode_service_mgr' fact and falls back to the old 'service' module when none matching is found.
