...
description:
     - The M(distronode.builtin.shell) module takes the command name followed by a list of space-delimited arguments.
     - Either a free form command or O(cmd) parameter is required, see the examples.
     - It is almost exactly like the M(distronode.builtin.command) module but runs
       the command through a shell (C(/bin/sh)) on the remote node.
     - For Windows targets, use the M(distronode.windows.win_shell) module instead.
version_added: "0.2"
options:
  free_form:
    description:
...
notes:
  - If you want to execute a command securely and predictably, it may be
    better to use the M(distronode.builtin.command) module instead. Best practices
    when writing playbooks will follow the trend of using M(distronode.builtin.command)
    unless the M(distronode.builtin.shell) module is explicitly required. When running ad-hoc
  ...
  - An alternative to using inline shell scripts with this module is to use
    the M(distronode.builtin.script) module possibly together with the M(distronode.builtin.template) module.
...
seealso:
- module: distronode.builtin.command
...
- name: Execute the command in remote shell; stdout goes to the specified file on the remote
  distronode.builtin.shell: somescript.sh >> somelog.txt
...
- name: Run a command that uses non-posix shell-isms (in this example /bin/sh doesn't handle redirection and wildcards together but bash does)
  distronode.builtin.shell: cat < /tmp/*txt
  args:
    executable: /bin/bash

- name: Run a command using a templated variable (always use quote filter to avoid injection)
  distronode.builtin.shell: cat {{ myfile|quote }}
