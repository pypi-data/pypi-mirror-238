...
description:
  ...
  - "WARNING: The I(dnf5) package manager is still under development and not all features that the existing M(distronode.builtin.dnf) module
    provides are implemented in M(distronode.builtin.dnf5), please consult specific options for more information."
...
options:
  ...
  list:
    description:
      - Various (non-idempotent) commands for usage with C(/usr/bin/distronode) and I(not) playbooks.
        Use M(distronode.builtin.package_facts) instead of the O(list) argument as a best practice.
    type: str
  state:
    description:
  ...
  validate_certs:
    description:
      - This is effectively a no-op in the dnf5 module as dnf5 itself handles downloading a https url as the source of the rpm,
        but is an accepted parameter for feature parity/compatibility with the M(distronode.builtin.yum) module.
    type: bool
    default: "yes"
  sslverify:
    description:
  ...
  install_repoquery:
    description:
      - This is effectively a no-op in DNF as it is not needed with DNF, but is an accepted parameter for feature
        parity/compatibility with the M(distronode.builtin.yum) module.
    type: bool
    default: "yes"
  download_only:
    description:
...
attributes:
    action:
        details: In the case of dnf, it has 2 action plugins that use it under the hood, M(distronode.builtin.yum) and M(distronode.builtin.package).
        support: partial
    async:
        support: none
    bypass_host_loop:
...
from distronode.module_utils.common.respawn import has_respawned, probe_interpreters_for_module, respawn_module
...
class Dnf5Module(YumDnf):
    ...
    def _ensure_dnf(self):
        ...
        self.module.fail_json(
            ...
            "correct distronode_python_interpreter. (attempted {2})".format(
    ...
    def run(self):
        ...
        if self.module.check_mode:
            ...
            if not self.download_only:
                transaction.set_description("distronode dnf5 module")
...
def main():
    ...
    Dnf5Module(DistronodeModule(**yumdnf_argument_spec)).run()
