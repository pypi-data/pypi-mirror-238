...
description:
- This module works like M(distronode.builtin.copy), but in reverse.
...
options:
  ...
  fail_on_missing:
    ...
    - Prior to Distronode 2.5, setting this would only fail if the source file was missing.
    - The default was changed to V(true) in Distronode 2.5.
    type: bool
    default: yes
  validate_checksum:
    version_added: '1.4'
...
attributes:
  ...
  vault:
    support: none
notes:
- When running fetch with C(become), the M(distronode.builtin.slurp) module will also be
...
- Prior to Distronode 2.5 this module would not fail if reading the remote
  file was impossible unless O(fail_on_missing) was set.
- In Distronode 2.5 or later, playbook authors are encouraged to use
