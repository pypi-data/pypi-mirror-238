...
attributes:
    ...
    platform:
        platforms: posix
notes:
  - When accessing the RV(distronode_facts.services) facts collected by this module,
    ...
    C(distronode_facts.services.zuul-gateway). It is instead recommended to
    using the string value of the service name as the key in order to obtain
    the fact data value like C(distronode_facts.services['zuul-gateway'])
...
- name: Populate service facts
  distronode.builtin.service_facts:

- name: Print service facts
  distronode.builtin.debug:
    var: distronode_facts.services
'''

RETURN = r'''
distronode_facts:
  description: Facts to add to distronode_facts about the services on the system
  returned: always
  type: complex
  contains:
    services:
...
def main():
    ...
    if len(all_services) == 0:
        ...
        results = dict(distronode_facts=dict(services=all_services))
