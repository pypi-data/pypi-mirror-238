...
---
module: add_host
short_description: Add a host (and alternatively a group) to the distronode-playbook in-memory inventory
description:
...
attributes:
    ...
    platform:
        platforms: all
notes:
- The alias O(host) of the parameter O(name) is only available on Distronode 2.4 and newer.
- Since Distronode 2.4, the C(inventory_dir) variable is now set to V(None) instead of the 'global inventory source',
...
seealso:
- module: distronode.builtin.group_by
author:
...
- name: Add a host alias that we reach through a tunnel (Distronode 1.9 and older)
  distronode.builtin.add_host:
    hostname: '{{ new_ip }}'
    distronode_ssh_host: '{{ inventory_hostname }}'
    distronode_ssh_port: '{{ new_port }}'

- name: Add a host alias that we reach through a tunnel (Distronode 2.0 and newer)
  distronode.builtin.add_host:
    hostname: '{{ new_ip }}'
    distronode_host: '{{ inventory_hostname }}'
    distronode_port: '{{ new_port }}'

- name: Ensure inventory vars are set to the same value as the inventory_hostname has (close to pre Distronode 2.4 behaviour)
  distronode.builtin.add_host:
    hostname: charlie
