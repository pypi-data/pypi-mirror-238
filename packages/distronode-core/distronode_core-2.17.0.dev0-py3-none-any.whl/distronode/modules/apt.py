...
options:
  ...
  cache_valid_time:
    description:
      ...
      - As of Distronode 2.4, if explicitly set, this sets O(update_cache=yes).
    type: int
    default: 0
  purge:
    description:
  ...
  install_recommends:
    description:
      - Corresponds to the C(--no-install-recommends) option for I(apt). V(true) installs recommended packages.  V(false) does not install
        recommended packages. By default, Distronode will use the same defaults as the operating system. Suggested packages are never installed.
  ...
  deb:
     description:
       ...
       - If :// in the path, distronode will attempt to download deb before installing. (Version added 2.1)
...
class PolicyRcD(object):
    ...
    def __init__(self, module):
        ...
        if os.path.exists('/usr/sbin/policy-rc.d'):
            self.backup_dir = tempfile.mkdtemp(prefix="distronode")
        else:
...
def expand_pkgspec_from_fnmatches(m, pkgspec, cache):
    ...
    # https://github.com/distronode/distronode-modules-core/issues/1258
    new_pkgspec = []
    if pkgspec:
...
def mark_installed_manually(m, packages):
    ...
    # https://github.com/distronode/distronode/issues/40531
...
# https://github.com/distronode/distronode-modules-core/issues/2951
