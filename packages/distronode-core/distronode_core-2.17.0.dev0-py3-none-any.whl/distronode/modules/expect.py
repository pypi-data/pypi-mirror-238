...
description:
     - The M(distronode.builtin.expect) module executes a command and responds to prompts.
...
notes:
  ...
  - The M(distronode.builtin.expect) module is designed for simple scenarios.
    For more complex needs, consider the use of expect code with the M(distronode.builtin.shell)
    or M(distronode.builtin.script) modules. (An example is part of the M(distronode.builtin.shell) module documentation).
