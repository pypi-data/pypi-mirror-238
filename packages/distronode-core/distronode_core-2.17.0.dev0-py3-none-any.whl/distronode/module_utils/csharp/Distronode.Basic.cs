...
namespace Distronode.Basic
{
    public class DistronodeModule
    {
        ...
        {
            // null values means no mapping, not used in Distronode.Basic.DistronodeModule
        ...
        public string DistronodeVersion { get; private set; }
        ...
        public string Tmpdir
        {
            get
            {
                if (tmpdir == null)
                {
                    ...
                    string dirName = String.Format("distronode-moduletmp-{0}-{1}-{2}", dateTime, System.Diagnostics.Process.GetCurrentProcess().Id,
        ...
        public DistronodeModule(string[] args, IDictionary argumentSpec, IDictionary[] fragments = null)
        {
            ...
            try
            {
                ...
                // Used by distronode-test to retrieve the module argument spec, not designed for public use.
                if (_DebugArgSpec)
                {
                    ...
                    ScriptBlock.Create("Set-Variable -Name distronodeTestArgSpec -Value $args[0] -Scope Global"
                        ).Invoke(argumentSpec);
                    throw new Exception("distronode-test validate-modules check");
                }

                // Now make sure all the metadata keys are set to their defaults, this must be done after we've
                // potentially output the arg spec for distronode-test.
            ...
            {
                if (e.Message == "distronode-test validate-modules check")
                    Exit(0);
            ...
            List<string> legalInputs = passVars.Keys.Select(v => "_distronode_" + v).ToList();
            ...
            // Set a Distronode friendly invocation value in the result object
        ...
        public static DistronodeModule Create(string[] args, IDictionary argumentSpec, IDictionary[] fragments = null)
        {
            return new DistronodeModule(args, argumentSpec, fragments);
        }
        ...
        public static IDictionary GetParams(string[] args)
        {
            if (args.Length > 0)
            {
                ...
                if (!rawParams.ContainsKey("DISTRONODE_MODULE_ARGS"))
                    throw new ArgumentException("Module was unable to get DISTRONODE_MODULE_ARGS value from the argument path json");
                return (IDictionary)rawParams["DISTRONODE_MODULE_ARGS"];
            }
            else
            {
                    ...
                    if (choices.Count > 0)
                    {
                        ...
                        else if (caseDiffList.Count > 0)
                        {
                            ...
                            string msg = String.Format(
                                "value of {0} was a case insensitive match of {1}: {2}. Checking of choices will be case sensitive in a future Distronode release. Case insensitive matches were: {3}",
        ...
        private void CheckUnsupportedArguments(IDictionary param, List<string> legalInputs)
        {
            ...
            foreach (DictionaryEntry entry in param)
            {
                ...
                if (!legalInputs.Contains(paramKey, StringComparer.OrdinalIgnoreCase))
                    ...
                    // change in a future Distronode release.
                    caseUnsupportedParameters.Add(paramKey);
                else if (paramKey.StartsWith("_distronode_"))
                {
                    removedParameters.Add(paramKey);
                    string key = paramKey.Replace("_distronode_", "");
                    ...
                    PropertyInfo property = typeof(DistronodeModule).GetProperty(propertyName);
                    FieldInfo field = typeof(DistronodeModule).GetField(propertyName, BindingFlags.NonPublic | BindingFlags.Instance);
                    if (property != null)
                        property.SetValue(this, value, null);
                    else if (field != null)
                        field.SetValue(this, value);
                    else
                        FailJson(String.Format("implementation error: unknown DistronodeModule property {0}", propertyName));
                }
            }
            ...
            {
                legalInputs.RemoveAll(x => passVars.Keys.Contains(x.Replace("_distronode_", "")));
            ...
            if (caseUnsupportedParameters.Count > 0)
            {
                ...
                msg = String.Format("{0}. Module options will become case sensitive in a future Distronode release. Supported parameters include: {1}",
    ...
    // https://github.com/distronode/distronode/pull/80247
