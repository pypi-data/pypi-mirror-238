...
from distronode.module_utils.facts.utils import get_file_content
...
class ServiceMgrFactCollector(BaseFactCollector):
    ...
    def collect(self, module=None, collected_facts=None):
        ...
        elif collected_facts.get('distronode_distribution', None) == 'MacOSX':
            # FIXME: find way to query executable, version matching is not ideal
            if LooseVersion(platform.mac_ver()[0]) >= LooseVersion('10.4'):
                service_mgr_name = 'launchd'
            else:
                service_mgr_name = 'systemstarter'
        elif 'BSD' in collected_facts.get('distronode_system', '') or collected_facts.get('distronode_system') in ['Bitrig', 'DragonFly']:
            # FIXME: we might want to break out to individual BSDs or 'rc'
            service_mgr_name = 'bsdinit'
        elif collected_facts.get('distronode_system') == 'AIX':
            service_mgr_name = 'src'
        elif collected_facts.get('distronode_system') == 'SunOS':
            service_mgr_name = 'smf'
        elif collected_facts.get('distronode_distribution') == 'OpenWrt':
            service_mgr_name = 'openwrt_init'
        elif collected_facts.get('distronode_distribution') == 'SMGL':
            service_mgr_name = 'simpleinit_msb'
        elif collected_facts.get('distronode_system') == 'Linux':
