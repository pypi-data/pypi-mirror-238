...
Function Get-DistronodeParam {
    ...
    slightly more pythonic
    .EXAMPLE
    $attr = Get-DistronodeParam $response "code" -default "1"
    .EXAMPLE
    Get-DistronodeParam -obj $params -name "State" -default "Present" -ValidateSet "Present","Absent" -resultobj $resultobj -failifempty $true
    Get-DistronodeParam also supports Parameter validation to save you from coding that manually
...
#>
    ...
    try {
        ...
        if ($ValidateSet) {
            ...
            if ($ValidateSet -contains ($obj.$name)) {
                ...
                if ($null -eq $ValidateSetErrorMessage) {
                    ...
                    $ValidateSetErrorMessage = "Get-DistronodeParam: Argument $name needs to be one of $($ValidateSet -join ",") but was $($obj.$name)."
                }
        ...
        if ($failifempty -eq $false) {
            ...
            if (-not $emptyattributefailmessage) {
                $emptyattributefailmessage = "Get-DistronodeParam: Missing required argument: $name"
            }
    ...
    if ($type -eq "path") {
        ...
        if (-not (Test-Path -IsValid $value)) {
            ...
            if ($path_invalid) {
                Fail-Json -obj $resultobj -message "Get-DistronodeParam: Parameter '$name' has an invalid path '$value' specified."
            }
        }
    }
    elseif ($type -eq "str") {
    ...
    elseif ($type -eq "list") {
        ...
        elseif ($value -is [int]) {
            ...
            Fail-Json -obj $resultobj -message "Get-DistronodeParam: Parameter '$name' is not a YAML list."
        }
...
#Alias Get-attr-->Get-DistronodeParam for backwards compat. Only add when needed to ease debugging of scripts
If (-not(Get-Alias -Name "Get-attr" -ErrorAction SilentlyContinue)) {
    New-Alias -Name Get-attr -Value Get-DistronodeParam
}
...
Function Parse-Args {
    <#
    .SYNOPSIS
    Helper function to parse Distronode JSON arguments from a "file" passed as
