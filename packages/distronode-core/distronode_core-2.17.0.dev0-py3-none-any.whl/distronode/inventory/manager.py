...
from distronode.inventory.data import InventoryData
...
from distronode.plugins.loader import inventory_loader
from distronode.utils.helpers import deduplicate_list
...
from distronode.vars.plugins import get_vars_from_inventory_sources
...
class InventoryManager(object):
    ...
    def _fetch_inventory_plugins(self):
        ...
        if not plugins:
            raise DistronodeError("No inventory plugins available to generate inventory, make sure you have at least one enabled.")
    ...
    def parse_sources(self, cache=False):
        ...
        if parsed:
            ...
            if C.INVENTORY_UNPARSED_IS_FAILED:
                raise DistronodeError("No inventory was parsed, please check your configuration and options.")
    ...
    def parse_source(self, source, cache=False):
            ...
            for plugin in self._fetch_inventory_plugins():
                ...
                if plugin_wants:
                    try:
                        ...
                        tb = ''.join(traceback.format_tb(sys.exc_info()[2]))
                        failures.append({'src': source, 'plugin': plugin_name, 'exc': DistronodeError(e), 'tb': tb})
                else:
        ...
        if parsed:
            ...
            if source != '/etc/distronode/hosts' or os.path.exists(source):
                ...
                if C.INVENTORY_ANY_UNPARSED_IS_FAILED:
                    raise DistronodeError(u'Completely failed to parse inventory source %s' % (source))
                else:
    ...
    def _match_list(self, items, pattern_str):
        ...
        try:
            ...
            raise DistronodeError('Invalid host list pattern: %s' % pattern_str)
        ...
        if pattern_list:
            ...
            if order in ['sorted', 'reverse_sorted']:
                ...
                if order == 'shuffle':
                    ...
                    raise DistronodeOptionsError("Invalid 'order' specified for inventory hosts: %s" % order)
    ...
    def _match_one_pattern(self, pattern):
        ...
        if pattern not in self._pattern_cache:
            ...
            try:
                ...
                raise DistronodeError("No hosts matched the subscripted pattern '%s'" % pattern)
    ...
    def subset(self, subset_pattern):
        ...
        Corresponds to --limit parameter to distronode-playbook
        """
            ...
            for x in subset_patterns:
                ...
                if x[0] == "@":
                    ...
                    if not os.path.exists(b_limit_file):
                        raise DistronodeError(u'Unable to find limit file %s' % b_limit_file)
                    if not os.path.isfile(b_limit_file):
                        raise DistronodeError(u'Limit starting with "@" must be a file, not a directory: %s' % b_limit_file)
    ...
    def add_dynamic_group(self, host, result_item):
        ...
        if real_host is None:
            if host.name == self.localhost.name:
                ...
                raise DistronodeError('%s cannot be matched in inventory' % host.name)
            else:
