...
from distronode.inventory.group import Group
...
class InventoryData(object):
        ...
        if self.localhost:
            ...
            if not py_interp:
                ...
                display.warning('Unable to determine python interpreter from sys.executable. Using /usr/bin/python default. '
                                'You can correct this by setting distronode_python_interpreter for localhost')
            new_host.set_variable("distronode_python_interpreter", py_interp)
            new_host.set_variable("distronode_connection", 'local')
    ...
    def add_group(self, group):
        ...
        if group:
            if not isinstance(group, string_types):
                raise DistronodeError("Invalid group name supplied, expected a string but got %s for %s" % (type(group), group))
            ...
            raise DistronodeError("Invalid empty/false group name provided: %s" % group)
    ...
    def add_host(self, host, group=None, port=None):
        ...
        if host:
            if not isinstance(host, string_types):
                raise DistronodeError("Invalid host name supplied, expected a string but got %s for %s" % (type(host), host))
            ...
            if group:
                if group in self.groups:
                    ...
                    raise DistronodeError("Could not find group %s in inventory" % group)
            ...
            raise DistronodeError("Invalid empty host name provided: %s" % host)
    ...
    def set_variable(self, entity, varname, value):
        ...
        if entity in self.groups:
            ...
            raise DistronodeError("Could not identify group or host named %s" % entity)
    ...
    def add_child(self, group, child):
        ...
        if group in self.groups:
            ...
            if child in self.groups:
                ...
                raise DistronodeError("%s is not a known host nor group" % child)
            self._groups_dict_cache = {}
            display.debug('Group %s now contains %s' % (group, child))
        else:
            raise DistronodeError("%s is not a known group" % group)
        return added
