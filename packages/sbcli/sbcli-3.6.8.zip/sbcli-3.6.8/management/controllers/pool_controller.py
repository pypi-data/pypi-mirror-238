...
def get_io_stats(pool_id, history):
    pool = db_controller.get_pool_by_id(pool_id)
    if not pool:
        logger.error(f"Pool not found {pool_id}")
        return False
    ...
    for lvol_id in pool.lvols:
        ...
        # if not record:
        #     continue
        records = db_controller.get_lvol_stats(lvol, history)
