...
def degrade_cluster(cl_id):
    ...
    ec.log_event_cluster(
        ...
        message=f"Cluster status changed from {old_status} to {Cluster.STATUS_DEGRADED}")
...
def get_iostats(cluster_id, history):
    db_controller = DBController()
    cls = db_controller.get_clusters(id=cluster_id)
    if not cls:
        logger.error(f"Cluster not found {cluster_id}")
    ...
    if history is not None and history > 0:
        return get_iostats_history(cluster_id, history)
...
def _get_cluster_io_data(nodes, history):
    db_controller = DBController()
    data = []
    for node in nodes:
        for dev in node.nvme_devices:
            stats = DeviceStat().read_from_db(db_controller.kv_store, id="%s/%s" % (node.get_id(), dev.get_id()),
                                              limit=history, reverse=True)
            for index, record in enumerate(stats):
                if index < len(data):
                    ...
                    data[index]["Date"] = time.strftime("%H:%M:%S, %d/%m/%Y", time.gmtime(record.date))
                    data[index]["read_bytes_per_sec"] = record.read_bytes_per_sec
                    data[index]["read_iops"] = record.read_iops
                    data[index]["write_bytes_per_sec"] = record.write_bytes_per_sec
                    data[index]["write_iops"] = record.write_iops
...
def get_iostats_history(cluster_id, history=20):
    db_controller = DBController()
    cls = db_controller.get_clusters(id=cluster_id)
    if not cls:
        logger.error(f"Cluster not found {cluster_id}")
    ...
    records = _get_cluster_io_data(nodes, history)
