...
class IndicateResponseTestCase(unittest.TestCase):
    ...
    @parameterized.expand([
        ['from_dpa', data_ok, IndicateResponse.from_dpa(data_ok['dpa']), False],
        ['from_dpa', data_ok_1, IndicateResponse.from_dpa(data_ok_1['dpa']), False],
        ['from_json', data_ok, IndicateResponse.from_json(generate_json_response(data_ok)), True],
        ['from_json', data_ok_1, IndicateResponse.from_json(generate_json_response(data_ok_1)), True],
        ['from_dpa_error', data_error, IndicateResponse.from_dpa(data_error['dpa']), False],
        ['from_json_error', data_error, IndicateResponse.from_json(generate_json_response(data_error)), True]
    ])
