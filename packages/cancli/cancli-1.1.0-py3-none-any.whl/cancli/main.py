...
class App:
	...
	def init_config(self) -> None:
		...
		self.config_file = ConfigFile(appname=self.APPNAME, notification_level=Config('notification-level.config-file', NotificationLevel.ERROR))
		self.cli = ConfigFile(appname=__package__, notification_level=Config('notification-level.cli', NotificationLevel.INFO))
		self.cli.command_dict['include'].config_file = self.config_file
		...
		self.config_file.set_ui_callback(on_config_message)
		self.cli.set_ui_callback(on_config_message)

	def create_parser(self) -> argparse.ArgumentParser:
		p = argparse.ArgumentParser(description=__doc__)
		p.add_argument('-v', '--version', action=CallAction, callback=self.print_version_and_exit, help="show the version and exit")
		p.add_argument('-e', '--edit-config', action=CallAction, callback=self.edit_config_and_quit, help="open the config file")
		return p

	def print_version_and_exit(self) -> None:
		print(f"{self.APPNAME} {__version__}")
		sys.exit(0)

	def edit_config_and_quit(self) -> None:
		self.editor.replace(SubprocessCommandWithAlternatives.WC_FILE_NAME, self.config_file.save(if_not_existing=True)).run(context=None)
		sys.exit(0)

	def run(self) -> None:
		p: 'PromptSession[str]' = PromptSession(FormattedText([(self.color_prompt, self.prompt)]), completer=ConfigFileCompleter(self.cli))
		while True:
			Message.reset()
			try:
				self.cli.parse_line(p.prompt())
			except EOFError:
				break
