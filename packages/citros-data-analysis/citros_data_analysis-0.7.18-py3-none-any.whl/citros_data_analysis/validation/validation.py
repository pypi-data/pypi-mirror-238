...
class Validation():
    ...
    def __init__(self, df = None, data_label = None, param_label = None, method = 'scale', num = 100, units = '',
                 omit_nan_rows = True, inf_vals = 1e308):
        self._set_data_table(df, data_label, param_label, method = method, num = num, units = units, 
                             omit_nan_rows = omit_nan_rows, inf_vals = inf_vals)
        
    def _set_data_table(self, df, data_label, param_label, method = 'scale', num = 100, units = '', 
                        omit_nan_rows = True, inf_vals = 1e308):
        '''
            ...
            if isinstance(df, pd.DataFrame):
                ...
                if data_label is None:
                    ...
                    if param_label is None:
                        ...
                        if method == 'bin':
                            self.db=dataset.bin_data(n_bins = num, param_label = param_label, show_fig = False)
                        elif method == 'scale':
                            self.db = dataset.scale_data(n_points = num, param_label = param_label, show_fig = False)
                        else:
    ...
    def std_bound_test(self, limits: Union[float, list] = 1.0, n_std: int = 3, nan_passed: bool = True, std_color: str = 'b', 
                       connect_nan_std: bool = False, std_area: bool = False, std_lines: bool = True):
        '''
        ...
        std_bound_test: passed
        '''
        fig, ax = self.db._plot_statistics(self.stat, fig_title = 'Std boundary test', show_fig = False, return_fig = True,
                                            n_std = n_std, std_color = std_color, connect_nan_std = connect_nan_std, 
                                            std_area = std_area, std_lines = std_lines)
        lower_limit, upper_limit = self._get_limit_values(limits, len(self.db.data.columns))
        ...
        if if_nan_sigma.any() and nan_passed:
            result.where(~nan_std, True, inplace = True) #chnage test results to True if the value was nan

        # insert column with `param_label` as a first column
        result.insert(0, self.db.x_label, self.db.addData.groupby(self.db.xid_label).first()[self.db.x_label])
        ...
        for col in self.db.data.columns:
            if if_nan_sigma[col]:
                log[col]['nan_std'] = da.CitrosDict(result[self.db.x_label].loc[nan_std[col]].to_dict())
            else:
    ...
    def std_test(self, limits: Union[float, list] = 1.0, n_std: int = 3, nan_passed: bool = True, std_color: str = 'b', 
                       connect_nan_std: bool = False, std_area: bool = False, std_lines: bool = True):
        '''
        ...
        for i, y_col in enumerate(self.stat.sigma.columns):
            ...
            if connect_nan_std:
                ...
                if res_sigm.notna().all():
                    ...
                    line_style_custom = '.-'
            if std_lines:
                ax[i].plot(result[self.db.x_label][filter_st], n_std*res_sigm[filter_st], line_style_custom, color = std_color, label = str(n_std)+ r'$\sigma$',markersize = 0.7)
                label = None
            else:
                label = str(n_std)+ r'$\sigma$'
            if std_area:
                ax[i].fill_between(result[self.db.x_label][filter_st].values, n_std*res_sigm[filter_st].values, y_0[filter_st].values, color = std_color, label = label, alpha = 0.12)

            ax[i].plot([min(result[self.db.x_label]), max(result[self.db.x_label])], [upper_limit[i], upper_limit[i]], 'r', label='limit')
            ylabel = 'std '+y_col + ", ["+self.db.units+"]" if self.db.units != '' else ''
    ...
    def mean_test(self, limits: Union[float, list] = 1.0, nan_passed: bool = True):
        ...
        mean_test: passed
        '''
        fig, ax = self.db._plot_statistics(self.stat, fig_title = 'Mean test', show_fig = False, return_fig = True, 
                                           n_std = None, std_color = 'b')
        lower_limit, upper_limit = self._get_limit_values(limits, len(self.db.data.columns))
        if lower_limit is None:
            return None, None, None
        ...
        if if_nan_mean.any() and nan_passed:
            result.where(~nan_mean, True, inplace = True) #change test results to True if the value was nan
    ...
    def _get_mean_std_log(self, init_param, result):
        ...
        try:
            pass_rate = da.CitrosDict((result[self.db.data.columns].\
                                        apply(pd.Series.value_counts).loc[True]/len(result)).\
                                        fillna(0).apply(round, ndigits = 3))
        except KeyError:
        ...
        for col in self.db.data.columns:
            ...
            log[col]['failed'] = da.CitrosDict(result[self.db.x_label].loc[result.index[result[col] == False]].to_dict())
    ...
    def _plot_bounds(self, result, lower_limit, upper_limit, fig):
        ...
        for i, ax in enumerate(axes):
            if isinstance(lower_limit, np.ndarray) and len(lower_limit.shape) != 0:
                ax.plot([min(result[self.db.x_label]),max(result[self.db.x_label])], [lower_limit[i], lower_limit[i]], 'r')
            elif isinstance(lower_limit, pd.DataFrame) and len(lower_limit.shape) != 0:
                ax.plot(result[self.db.x_label], lower_limit, 'r')
            else:
                ax.plot([min(result[self.db.x_label]),max(result[self.db.x_label])], [lower_limit, lower_limit], 'r')
                
            if isinstance(upper_limit, np.ndarray) and len(upper_limit.shape) != 0:
                ax.plot([min(result[self.db.x_label]),max(result[self.db.x_label])], [upper_limit[i], upper_limit[i]], 'r')
            elif isinstance(upper_limit, pd.DataFrame) and len(upper_limit.shape) != 0:
                ax.plot(result[self.db.x_label], upper_limit, 'r')
            else:
                ax.plot([min(result[self.db.x_label]), max(result[self.db.x_label])], [upper_limit, upper_limit], 'r')

        line = Line2D([0], [0], label='test bounds', color='r')
        handles, labels = axes[-1].get_legend_handles_labels()
        fig.legend(handles+[line], labels+['test bounds'], bbox_to_anchor=(1.0, 0.94),loc ='upper left')
        fig.legends.pop(0)
        fig.tight_layout()
    ...
    def sid_test(self, limits: Union[float, list] = 1.0, nan_passed: bool = True):
        ...
        sid_test: passed
        '''
        fig, ax = self.db._plot_statistics(self.stat, fig_title = 'Sid test', show_fig = False, return_fig = True, 
                                           n_std = None, std_color = 'b')
        ...
        if if_nan_val.any() and nan_passed:
            result.where(~nan_val, True, inplace = True) #change test results to True if the value was nan
        
        # insert column with `param_label` as a first column
        result.insert(0, self.db.x_label, self.db.addData[self.db.x_label])

        try:
            pass_rate = (result.groupby('sid').\
                agg({col : 'value_counts' for col in self.db.data.columns}).xs(True, level = 1)/\
                result[self.db.data.columns].groupby('sid').count()).\
                fillna(0).apply(round, ndigits = 3).to_dict()
        except KeyError:
            # in case all points failed test and there is no True values
            pass_rate = da.CitrosDict()
            for col in self.db.data.columns:
        ...
        try:
            pass_rate_overall = (result.groupby('sid').all().\
                agg({col : 'value_counts' for col in self.db.data.columns}).loc[True]/\
        ...
        for col in self.db.data.columns:
            ...
            if log[col]['passed']:
                ...
                log[col]['failed'] = da.CitrosDict(result[result[col] == False].swaplevel().groupby('sid').\
                    apply(lambda f: da.CitrosDict(f.xs(f.name)[self.db.x_label].to_dict())).to_dict())
            # log[col]['table'] = result[[col]]
    ...
    def norm_test(self, norm_type: str = 'L2', limits: Union[float, list] = 1.0):
        ...
        try:
            pass_rate = (result.agg({col : 'value_counts' for col in self.db.data.columns}).loc[True]/result.count().values).\
                    fillna(0).apply(round, ndigits = 3).to_dict()
        except KeyError:
            pass_rate = da.CitrosDict()
            for col in self.db.data.columns:
        ...
        for i, ax in enumerate(axes):
            ...
            ylim = ax.get_ylim()
            if isinstance(upper_limit, np.ndarray) and len(upper_limit.shape) != 0:
                ax.plot([upper_limit[i], upper_limit[i]],[min(index)-1,max(index)+1], 'r', label = 'limit')
            else:
                ax.plot([upper_limit, upper_limit], [min(index)-1, max(index)+1], 'r', label = 'limit')
            ax.set_ylim(ylim)
            ax.xaxis.grid(True)
    ...
    def _get_norm(self, norm_type = 'L2'):
        ...
        '''
        if norm_type == 'L2':
            norm = self.db.data.apply(lambda x : x**2).groupby('sid').sum().apply(lambda x: np.sqrt(x))
        elif norm_type == 'Linf':
            norm = (abs(self.db.data)).groupby('sid').max()
        else:
            print("error: can not recognise the norm type, the allowed types are 'L2' and 'Linf'")
            norm = None
        return norm

    def set_tests(self, test_method: dict = {'std_bound' : {'limits' : 1.0, 'n_std': 3, 'nan_passed': True},
        ...
        bound_tests = {'std_bound' : self.std_bound_test, 'mean': self.mean_test, 'sid': self.sid_test}
        norm_tests = {'norm_L2' : 'L2', 'norm_Linf': 'Linf'}

        for method in methods:
            if method in bound_tests.keys():
                logs[method], tables[method], figures[method] = bound_tests[method](**test_method[method])
            elif method in norm_tests.keys():
                logs[method], tables[method], figures[method] = self.norm_test(norm_type = norm_tests[method], **test_method[method])
    ...
    def _get_limit_values(self, limits, data_length):
        ...
        if isinstance(limits, (list, tuple)): 
            ...
            if len(limits) == 1:
            ...
            elif len(limits) == 2 and not any(map(lambda x: isinstance(x, (list, tuple)), limits)):
                # [0.1, 0.2]
                lower_limit = limits[0]
                ...
                if len(limits) != data_length:
                    print("error: 'limits' length does not match the number of data columns.")
                    return None, None
                else:
