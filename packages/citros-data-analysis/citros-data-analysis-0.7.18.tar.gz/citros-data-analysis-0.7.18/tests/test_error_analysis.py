...
def test_get_mu():
    ...
    path_table = r'example_table.csv'
    F=pd.read_csv(path_table)
...
def test_CitrosData():
    ...
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x'], units = 'm', parameter_label='data.p')
    ...
    assert dataset.type is 'x', 'CitrosData: wrong type'
    ...
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x.x_1','data.x.x_2'], units = 'm')
    ...
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x.x_1'], units = 'm', parameter_label=['data.p','data.t'])
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1','data.y.y_2'], units = 'm')
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1','data.y.y_2'], units = 'm', inf_vals = None)
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1','data.y.y_2'], units = 'm', omit_nan_rows=False)
    assert (((dataset.data['data.y.y_1'].abs()-10**307)>=0) | dataset.data['data.y.y_1'].isna()).\
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1','data.y.y_2'], units = 'm', inf_vals=None, omit_nan_rows=False)
...
def test_bin_data():
    ...
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x'], units = 'm')
...
def test_bin_data_with_nan_1col():
    #test bin with nan and inf values
    #tests bin with nan and inf vals, 1 column
    df = citros.topic('A').data(['data.y.y_1', 'data.y.y_2'])
    dataset = analysis.CitrosData(df, data_label = 'data.y.y_1', units = 'm')
...
def test_bin_data_with_nan_2col_omit_nan_rows():
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1', 'data.y.y_3'], units = 'm', omit_nan_rows=True)
...
def test_bin_data_with_nan_1co2():
    ...
    df = citros.topic('A').data(['data.y.y_1', 'data.y.y_2', 'data.y.y_3'])
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1', 'data.y.y_3'], units = 'm', omit_nan_rows=False)
    db = dataset.bin_data(n_bins = 1, param_label = 'data.y.y_2', show_fig = True)
...
def test_get_statistics():
    ...
    #bin
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_1', 'data.y.y_3'], units = 'm')
    db = dataset.bin_data(n_bins = 10, param_label = 'data.y.y_2', show_fig = False)
    stat = db.get_statistics(return_format='citrosStat')
...
@image_comparison(baseline_images=['show_statistics'], remove_text=True, extensions=['png'], style='mpl20',\
...
@image_comparison(baseline_images=['show_statistics_1col_nan_rows_false'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
def test_show_statistics_1col_nan_rows_false():
    plt.close('all')
    df = citros.topic('A').data(['data.y.y_1', 'data.y.y_2', 'data.time'])
    dataset = analysis.CitrosData(df, data_label = 'data.y.y_1', units = 'm', omit_nan_rows = False)
    db = dataset.scale_data(n_points = 10, param_label = 'data.y.y_2', show_fig = False)
...
@image_comparison(baseline_images=['show_statistics_1col_nan_rows_true'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
def test_show_statistics_1col_nan_rows_true():
    plt.close('all')
    df = citros.topic('A').data(['data.y.y_1', 'data.y.y_2', 'data.time'])
    dataset = analysis.CitrosData(df, data_label = 'data.y.y_1', units = 'm', omit_nan_rows = True)
...
@image_comparison(baseline_images=['show_statistics_1col_nan_rows_true_lines'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
def test_show_statistics_1col_nan_rows_true():
    plt.close('all')
    df = citros.topic('A').data(['data.y.y_1', 'data.y.y_2', 'data.time'])
    dataset = analysis.CitrosData(df, data_label = 'data.y.y_1', units = 'm', omit_nan_rows = True)
    db = dataset.bin_data(n_bins = 50, param_label = 'data.y.y_2', show_fig = False)
    fig, axes = db.show_statistics(return_fig = True, connect_nan_std = False, std_area = False, std_lines = True)
    fig.supxlabel("")
    fig.suptitle('')
    for leg in fig.legends:
        leg.set_visible(False)
...
@image_comparison(baseline_images=['show_statistics_2col_nan_rows_true'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
...
@image_comparison(baseline_images=['show_statistics_2col_nan_rows_false'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
...
@image_comparison(baseline_images=['show_statistics_1col'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
def test_show_statistics_1col():
    plt.close('all')
    fig = plt.figure(figsize = (6,6))
    df = citros.topic('C').data(['data.x.x_1', 'data.time', 'data.p'])
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x.x_1'], units = 'm')
    #dataset.data = dataset.data.add(p)
    db_sc = dataset.scale_data(n_points = 35, 
                            param_label = 'data.time', 
                            show_fig = False)
...
def test_show_correlation_output():
    ...
    assert n_lines == 2, 'show_correlation_output: numbder of lines is wrong'
    assert n_patches == 3, 'show_correlation_output: numbder of patches is wrong'
...
def test_show_correlation_output_2():
    ...
    n_lines = len(ax.get_lines())
    n_patches = len(ax.patches)
    assert n_lines == 2, 'show_correlation_output_2: numbder of lines is wrong'
    assert n_patches == 3, 'show_correlation_output_2: numbder of patches is wrong'
...
def test_show_correlation_nan_inf():
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_2', 'data.y.y_3'], units = 'm', omit_nan_rows = True)
    ...
    dataset = analysis.CitrosData(df, data_label = ['data.y.y_2', 'data.y.y_3'], units = 'm', omit_nan_rows = False)
    db = dataset.scale_data(n_points = 10, param_label = 'data.y.y_1', show_fig = False)
    for i in list(set(db.data.index.get_level_values('data.y.y_1_id'))):
        fig, ax = db.show_correlation(x_col = 'data.y.y_2', y_col = 'data.y.y_3', slice_id = i, bounding_error=True, display_id=False, return_fig = True)
        assert len(ax.get_lines()) == n_lines_answ_sc[i], f'show_correlation_nan_inf: the number of lines for slice_id = {i} is incorrect'
...
def test_show_correlation_output_2db():
    ...
    n_patches = len(ax.patches)
    assert n_lines == 2, 'show_correlation_output_2: numbder of lines is wrong'
    assert n_patches == 1, 'show_correlation_output_2: numbder of patches is wrong'
...
def test_show_correlation_output_bound_error():
    ...
    for ax in fig.axes:
        n_lines = len(ax.get_lines())
        n_patches = len(ax.patches)
    assert n_lines == 5, 'show_correlation_output_bound_error: numbder of lines is wrong'
    assert n_patches == 2, 'show_correlation_output_bound_error: numbder of patches is wrong'

def test_set_parameter():
    #I parameter setting by parameter_label
    df = citros.topic('A').set_filter({'data.t':[10]}).data(['data.x.x_1', 'data.time', 'data.p', 'data.t'])
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x.x_1'], units = 'm', parameter_label='data.t')
    ...
    df = citros.topic('A').set_filter({'data.t':[10]}).data(['data.x.x_1', 'data.time', 'data.p', 'data.t'])
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x.x_1'], units = 'm', parameter_label=['data.t', 'data.p'])
    ...
    df = citros.topic('A').set_filter({'data.t':[10]}).data(['data.x.x_1', 'data.time', 'data.p', 'data.t'])
    dataset = analysis.CitrosData(df, type_name = 'x', data_label=['data.x.x_1'], units = 'm', parameters = {'m': 55, 'k': 94})
...
@image_comparison(baseline_images=['show_prediction'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
...
@image_comparison(baseline_images=['show_prediction_2par'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
...
@image_comparison(baseline_images=['show_prediction_3col'], remove_text=True, extensions=['png'], style='mpl20',\
                   savefig_kwarg = {'bbox_inches':'tight'})
...
def test_get_prediction_with_nan_inf():
    ...
    def compare(omit_nan_rows, method):
        ...
        for t in list_t:
            ...
            dataset = analysis.CitrosData(df, data_label=['data.y.y_1', 'data.y.y_2'], units = 'm', omit_nan_rows = omit_nan_rows )
