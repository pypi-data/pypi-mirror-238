...
from regscale.core.app.public.fedramp.fedramp import process_fedramp_docx_by_profile_id
...
def process_file():
    ...
    return load_results(csv_path, result_output, file, implementation_results)


def load_results(csv_path, result_output, file, implementation_results):
...
async def process_stig_file():
    ...
    if ckl_file:
        ...
        if non_ckl_count := upload_count - file_count:
            warning_msg = (
                f"{non_ckl_count}/{upload_count} "
                "file(s) will not processed because they are not .ckl files."
            )
        if non_ckl_count == upload_count:
            logger.error(
                "No file(s) were selected for processing because none of them are .ckl files."
            )
    ...
    result_msg = (
        f"Processed {file_count} STIG file(s) for SSP #{ssp_id} in {time_elapsed}"
    )
    return render_template(
