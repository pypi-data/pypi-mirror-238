...
class Aerosol(object):
    ...
    def prepare_data(self, begin_date=None, end_date=None):
        ...
        total_data = company_data.merge(
            sector_data, on=['code'],
            how='left').merge(metrics_qrank_data,
                              on=['code']).merge(gd_qrank_data,
                                                 on=['code', 'date'],
                                                 how='left')
    ...
    def calculate_result(self, total_data, feature_data, threshold):
        ...
        for industry in weighted.industry_id1.unique().tolist():
            ...
            industry_data['score'] = (
                industry_data[factor_columns].fillna(0).values *
                industry_data[w_list].values * 5).sum(axis=1)
            industry_data['industry'] = industry
            res[industry] = industry_data

            #res.append(
            #    industry_data.drop(w_list, axis=1).set_index(
            #        ['date', 'code', 'industry']))

        return res

    def run(self, begin_date, end_date, threshold=0.4):
