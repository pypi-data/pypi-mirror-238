...
class OMFITobject(object):
    ...
    """
    # static variable when the objects were cleaned the last time
    cleanup_time = [0]
    ...
    def __init__(
        ...
        **kw,
        ...
        if not len(OMFITaux['prun_process']):
            # do not call cleaning more often then once per second
            if time.time() - self.cleanup_time[0] > 1:
                _cleanup()
                self.cleanup_time[0] = time.time()
        ...
        # this is the full true path fo the filename
...
def is_server(serverA, serverB):
    ...
    cache_id = (str(serverA), str(serverB))
    if cache_id in SERVER_cache:
        return SERVER_cache[cache_id]

    # generate list of all possible aliases for server A
    serverA_ = tolist(evalExpr(serverA))
    for serverA in serverA_:
        serverA = SERVER(serverA)
        if serverA not in serverA_:
            serverA_.append(serverA)
    for serverA in serverA_:
        try:
            serverA = SERVER[serverA]
        except KeyError:
            continue
        serverA = parse_server(serverA['server'])[2]
        if serverA not in serverA_:
            serverA_.append(serverA)
    ...
    for serverA in serverA_:
        for serverBname in serverB_:

            # generate list of all possible aliases for server B
            serverB__ = [serverBname]
            for serverB in serverB__:
                serverB = SERVER(serverB)
                if serverB not in serverB__:
                    serverB__.append(serverB)
            for serverB in serverB__:
                try:
                    serverB = SERVER[serverB]
                except KeyError:
                    continue
                serverB = parse_server(serverB['server'])[2]
                if serverB not in serverB__:
                    serverB__.append(serverB)
            ...
            for serverB in serverB__:
                ...
                if not tmp and is_localhost(serverA) and is_localhost(serverB):
                    tmp = True
                if tmp:
                    SERVER_cache[cache_id] = serverBname
                    return serverBname  # return original server B name as typed by user

    SERVER_cache[cache_id] = False
    return False
...
server_picker_cache = dict()


def server_picker(root, MainSettings):
    """
    Picks appropriate server with managed GA software for a known device
    Unknown devices will default to a default GA server, which currently
    is iris.
    Previous servers are cached

    :param root: reference to the root namespace for the specified module

    :param MainSettings: reference to the MainSettings in the OMFIT namespace
    """
    try:
        experiment_branch = root['SETTINGS']['EXPERIMENT']
    except KeyError:
        experiment_branch = MainSettings['EXPERIMENT']

    device = experiment_branch['device']

    if device in server_picker_cache:
        return server_picker_cache[device]

    if is_localhost('iris'):
        server_picker_cache[device] = 'iris'
    elif is_localhost('omega'):
        server_picker_cache[device] = 'omega'
    elif is_localhost('cori'):
        server_picker_cache[device] = 'cori'
    elif is_localhost('perlmutter'):
        server_picker_cache[device] = 'perlmutter'
    elif is_localhost(['pppl_sunfire', 'nx', 'portal', 'sunfire'], looseCheck=True):
        server_picker_cache[device] = 'portal'
    elif is_localhost(['shenma', 'login110', 'login111', 'login112', 'login113']):
        server_picker_cache[device] = 'shenma'
    else:
        # The decision for server is device dependent
        if is_device(experiment_branch['device'], 'DIII-D') and MainSettings['SERVER']['GA_username']:
            server_picker_cache[device] = 'iris'
        elif is_device(experiment_branch['device'], ['JET', 'MAST']) and MainSettings['SERVER']['CCFE_username']:
            if is_localhost('freia'):
                server_picker_cache[device] = 'freia'
            else:
                server_picker_cache[device] = 'heimdall'
        elif is_device(experiment_branch['device'], 'CMOD') and MainSettings['SERVER']['MIT_username']:
            server_picker_cache[device] = 'cmodws'
        elif is_device(experiment_branch['device'], 'KSTAR') and MainSettings['SERVER']['KFE_username']:
            server_picker_cache[device] = 'ukstar'
        elif is_device(experiment_branch['device'], 'TCV') and MainSettings['SERVER']['EPFL_username']:
            server_picker_cache[device] = 'lac'
        elif MainSettings['SERVER']['GA_username']:  # Unkown Device but valid GA credentials
            server_picker_cache[device] = 'iris'
        else:
            raise ValueError('OMFIT Managed Code needs to be run on a known host')
...
print(repo_str)
print('Python executable'.ljust(22) + ': ' + os.path.realpath(sys.executable))
