...
class IndiCamServiceClient:
    ...
    def __init__(self, service_url: str, api_key: str) -> None:
        ...
        self._session = aiohttp.ClientSession(headers=req_headers)
    ...
    async def test_connect(self) -> int:
        ...
        async with self._session as session:
            try:
                async with session.get(f'{self._base_url}/indicams/') as response:
    ...
    async def get_indicam_id(self, name: str) -> int | None:
        ...
        async with self._session as session:
            async with session.get(f'{self._base_url}/indicams/?handle={name}') as response:
                if response.status != 200:
                    _LOGGER.error(
    ...
    async def get_camconfig(self, indicam_id: int) -> CamConfig | None:
        ...
        async with self._session as session:
            async with session.get(f'{self._base_url}/indicams/{indicam_id}/camconfig_current/') as response:
                if response.status != 200:
                    _LOGGER.error(
    ...
    async def create_camconfig(self, indicam_id: int, camconfig: CamConfig) -> bool:
        ...
        async with self._session as session:
            ...
            async with session.post(f'{self._base_url}/camconfigs/', data=data) as response:
    ...
    async def measurement_ready(self, image_id: int) -> bool:
        ...
        async with self._session as session:
            async with session.get(f'{self._base_url}/measurements/?src_image={image_id}') as response:
