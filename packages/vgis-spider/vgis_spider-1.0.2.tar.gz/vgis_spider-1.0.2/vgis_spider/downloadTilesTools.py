...
class downloadTilesHelper:
    ...
    # 根据坐标范围计算WMTS/TMS瓦片的行列号
    def compute_col_row_by_coords(self, xmin, xmax, ymin, ymax):
        tileMinCol, tileMaxCol, tileMinRow, tileMaxRow = None, None, None, None
        image_service_type = self.image_service_type
        # 为补偿浮点计算的不准确性
        epsilon = 1e-6
        # 左上角为原点
        if image_service_type == "wmts" or image_service_type == "xyz":
    ...
    def loop_download_tiles(self, xmin, xmax, ymin, ymax):
        tileMinCol, tileMaxCol, tileMinRow, tileMaxRow = self.compute_col_row_by_coords(xmin, xmax, ymin, ymax)
        if self.image_service_type == "wmts":
            for col in range(tileMinCol, tileMaxCol + 1):
                for row in range(tileMinRow, tileMaxRow + 1):
                    ...
                    # 下载瓦片
                    self.retry_download_curent = 0
                    self.download_each_tile(tile_url, "row=" + str(row) + "&col=" + str(col), col)
            # 同时计算左上角瓦片的地图单位
            # 瓦片原点在左上角
            left_top_tile_bounds = self.get_bounds_of_geoserver_wmts_tile(self.image_service_info["zoom_level"],
            ...
            left_top_tile_bounds = self.get_bounds_of_geoserver_wmts_tile(self.image_service_info["zoom_level"],
                                                                          tileMaxRow,
                                                                          tileMinCol)
        elif self.image_service_type == "xyz":
            pass
            # 同时计算左上角瓦片的地图单位
            # 瓦片原点在左上角
            left_top_tile_bounds = self.get_bounds_of_geoserver_wmts_tile(self.image_service_info["zoom_level"],
    ...
    # 获取瓦片的地图坐标范围
    def get_bounds_of_geoserver_wmts_tile(self, level, row, col):
        ...
        try:
            ...
            if not os.path.exists(os.path.join(self.out_image_path, self.merge_pic_name)):
                if self.image_service_type == "wms":
                    ...
                    # 沿着X方向,逐步长滑动
                    while xmin < xmax:
                        self.loop_download_wms_tiles_follow_y(xmin, ymin, ymax)
                        xmin += (self.resolution * self.tile_size)
                elif self.image_service_type == "wmts" or self.image_service_type == "tms" or self.image_service_type == "xyz":
                    left_top_tile_bounds = self.loop_download_tiles(xmin, xmax, ymin, ymax)

                logger.info('瓦片下载完成')

                # 拼接瓦片图片
                logger.info('正在拼接瓦片为图片')
                try:
                    ...
                    logger.error("瓦片拼接失败，请检查瓦片：{}".format(str(tm_exp)))
                    logger.error(tm_exp.__traceback__.tb_frame.f_globals["__file__"])  # 发生异常所在的文件
                    logger.error(tm_exp.__traceback__.tb_lineno)  # 发生异常所在的行数
            else:
                logger.warning("之前已拼接过图片")
            self.delete_cache()
        except Exception as exp:
            logger.error("下载瓦片失败：{}".format(str(exp)))
    ...
    def vconcat_tileImage(self, tile_stride_dir):
        ...
        elif self.image_service_type == "wmts":
            # wmts的原点在左下方，因此COL值越小越往上，不做反转
            names = sorted(names)
    ...
    # 沿着Y方向循环下载wms瓦片
    # 方向为从上往下
    def loop_download_wms_tiles_follow_y(self, xmin, ymin, ymax):
        # 沿着Y方向
        while ymax > ymin:
            ...
            # 下载瓦片
            self.retry_download_curent = 0
            self.download_each_tile(tile_url, "y=" + str(ymax) + "&x=" + str(xmin), xmin)
    ...
    def build_wms_tile_pic_url(self, bbox):
        ...
        # 卫星中心一张图2米WMS服务
        if self.image_service_info["service_type"] == "satellite_center_wms":
            data = {
                'SERVICE': 'WMS',
                'REQUEST': 'GetMap',
                # 'SRS': 'EPSG%3A4326',
            ...
            for k, v in data.items():
                lt.append(k + '=' + str(v))
            # 拼接url
            url = self.image_service_info["service_url"] + "/OneMap" + self.image_service_info[
                "layer_name"] + "/wms?" + '&'.join(lt)

        # logger.info("瓦片地址：" + url)
        return url

    # 下载每个小瓦片
    def download_each_tile(self, url, pic_name, pic_dir_name):
        if self.retry_download_curent <= 5:
            try:
                if self.retry_download_curent > 0:
                    logger.info("self.retry_download_curent:{}".format(self.retry_download_curent))
                ...
                if not os.path.exists(path):
                    # 创建文件夹
                    self.mkdir(path)
                    logger.info("创建文件夹:" + path + "--" + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                    ...
                    try:
                        ...
                        if img_cv is not None:
                            if img_cv.shape[2] == 4:
                                ...
                                pass
                                # logger.info("瓦片已存在且完整不再下载---" + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                ...
                logger.error("下载失败" + str(ex) + "---" + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                logger.error(url)
                if str(ex) != "HTTP Error 404: Not Found":
                    logger.error("正在重新下载---" + datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                    # 瓦片下载失败，重试
                    self.download_each_tile(url, pic_name, pic_dir_name)
