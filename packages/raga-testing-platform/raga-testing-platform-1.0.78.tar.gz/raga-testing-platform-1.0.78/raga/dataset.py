...
class Dataset:
    ...
    def init(self):
        from raga import EMPTY_DATA_FRAME, INVALID_DATA_ARG, INVALID_SCHEMA
    ...
    def load_data_frame(self, org=None):
        ...
        job_id = res_data.get('data').get('jobId')
        if job_id:
            self.wait_for_status(job_id)
    ...
    def wait_for_status(self, job_id: int, status: Optional[str]='Completed'):
