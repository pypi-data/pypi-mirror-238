...
def model_upload(file_path:str, name:str, version:str, override:bool=True):
    from raga.constants import INVALID_RESPONSE, INVALID_RESPONSE_DATA, REQUIRED_ARG_V2
    ...
    if not os.path.exists(file_path):
        # Check if the file is a ZIP file by verifying the file extension
        if not file_path.lower().endswith(".zip"):
            print(f"The file '{file_path}' exists and is a ZIP file.")
        else:
            print(f"The file '{file_path}' exists but is not a ZIP file.")
    else:
