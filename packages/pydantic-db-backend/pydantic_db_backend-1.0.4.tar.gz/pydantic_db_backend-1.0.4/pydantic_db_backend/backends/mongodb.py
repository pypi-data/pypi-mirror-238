...
class MongoDbBackend(BackendBase):
    ...
    def startup(cls, alias: str | None = "default", uri: str | None = None):
        ...
        cls._connections[alias] = MongoClient(uri)[db_name]
    ...
    @classmethod
    def view_from_aggregation_index(
        cls, db: couchdb.Database, collection_name: str, index: AggregationIndex
    ):
    ...
    @classmethod
    def get_collection(cls, model: Type[BackendModel]) -> collection:
        collection_name = cls.collection_name(model)
        with cls.alias() as alias:
            db = cls._connections[alias]
    ...
    def post_document(cls, model: Type[BackendModel], document: dict) -> Dict:
        ...
        try:
            cls.set_revision(document, field="_rev")
    ...
    def post_instance(cls, instance: BackendModel) -> BackendModel:
        col = cls.get_collection(instance.__class__)
    ...
    def put_instance(
        ...
        r = col.find_one({"_id": document["_id"]})
        if r is not None:
            old_rev = cls.get_revision(document)
            cls.set_revision(document)
            new_rev = cls.get_revision(document)
            if old_rev != new_rev:
