...
def nearest(major_ts: np.ndarray, minor_ts: np.ndarray, trust: int) -> np.ndarray:
    ...
    left_diff = np.abs(np.take(minor_ts[:, 0], indexes - 1, mode="wrap") - major_ts[:, 0])
    right_diff = np.abs(np.take(minor_ts[:, 0], indexes, mode="wrap") - major_ts[:, 0])
    nearest_indexes = np.where(left_diff < right_diff, indexes - 1, indexes)
...
def sync_ts(
    major_ts: np.ndarray, minor_ts: np.ndarray, method: Literal["backward", "forward", "nearest"] = "backward", trust: int = np.inf
) -> np.ndarray:
    """_summary_
