...
from abstract_utilities import create_new_name,get_any_key,unified_json_loader,get_sleep,eatAll,safe_json_loads,read_from_file,make_list,ThreadManager,HistoryManager
...
class GptManager:
    ...
    def update_feedback(self,key):
        ...
        content = get_any_key(self.last_content,key)
        if content:
    ...
    def update_text_with_responses(self):
        input(self.current_output)
        output_keys = []
        for key in ['response','prompt','title']:
            output_keys.append(get_any_key(safe_json_loads(self.current_output),key))
        ...
        self.last_content = get_any_key(safe_json_loads(self.last_response),'content')
        self.bot_notation =get_any_key(safe_json_loads(self.last_content),'notation')
        self.api_response =get_any_key(safe_json_loads(self.last_content),'api_response')
