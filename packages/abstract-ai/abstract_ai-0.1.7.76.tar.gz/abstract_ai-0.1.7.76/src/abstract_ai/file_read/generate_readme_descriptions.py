from abstract_gui import expandable,sg,make_component,ensure_nested_list
...
def construct_layout(files_list,window_size):
    ...
    mass_files = [
        ...
        [make_component("Button",'Clear Selection'), make_component("Button",'Exit'), make_component("Button",'add')]
    ]
    layout = [mass_files]
    ...
    layout.append([make_component("Button",'Submit'), make_component("Button",'Exit')])
    return layout
...
def read_me_window(file_path_list=[]):
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == 'Exit':
            ...
            #sg.popup_scrolled(result, title="Modules Overview", size=(60, 30))
