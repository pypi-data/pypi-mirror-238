...
class GptManager:
    ...
    def update_prompt_mgr(self,prompt_data=None,token_dist=None,completion_percentage=None,bot_notation=None,chunk=None,chunk_type='TEXT'):
        ...
        self.window_mgr.update_value(key='-PROMPT-',value=self.prompt_mgr.create_prompt(self.chunk_number))
    ...
    def get_content(self,response):
        content = safe_json_loads(find_keys(response,'content'))
        if content:
            if isinstance(content,list):
                content=safe_json_loads(content[0])
        return content
    def update_last_response_file(self):
        ...
        self.latest_output = self.response_manager.output
        self.update_output(0)
    def update_feedback(self,key,current_response_content):
        value_key = text_to_key(key,section='feedback')
        content = get_any_key(current_response_content,key)
        if content:
    ...
    def update_text_with_responses(self,output):
        self.current_response_content=safe_json_loads(get_content(output))
        title = get_any_key(self.current_response_content,'title')
        bot_notation = get_any_key(self.current_response_content,'notation')
        api_response = get_any_key(self.current_response_content,'api_response')
        self.window_mgr.update_value(key=text_to_key('title input'),value=title)
        self.window_mgr.update_value(key=text_to_key('notation',section='feedback'),value=bot_notation)
        for pre_key in self.instruction_pre_keys:
            self.update_feedback(pre_key,self.current_response_content)
        if isinstance(response,dict): 
            self.append_output('-RESPONSE-',f"#TITLE#{self.get_new_line(1)}{title}{self.get_new_line(2)}#USER QUERY#{self.get_new_line(1)}{self.request}{self.get_new_line(2)}#{self.model_mgr.selected_model_name} RESPONSE#{self.get_new_line(2)}{api_response}{self.get_new_line(2)}")
            print('response:\n',self.current_response_content)
            for key in self.current_response_content.keys():
                if key not in self.instruction_pre_keys:
                    ...
                    if gui_key in self.window_mgr.get_values().keys():
                        self.window_mgr.update_value(key=gui_key,value=text_to_key(text=key,section='feedback'))
                    elif key not in ['api_response','generate_title']:
                        self.window[text_to_key(text='other',section='feedback')].update(visible=True)
                        self.append_output(text_to_key(text='other',section='feedback'),f"{key}: {self.current_response_content[key]}"+'\n')
        else:
            self.append_output('-RESPONSE-',str(self.current_response_content))
    def update_output(self,output_iteration):
        if output_iteration < len(self.latest_output) and output_iteration >=0:
            ...
            self.update_text_with_responses(output = self.current_output[self.response_text_number_actual])
    ...
    def update_progress_chunks(self,done=False):
        ...
        if done == True:
            ...
            self.window_mgr.update_value(key='-QUERY_COUNT-', value=f"chunk {min(i_query,1)} of {min(chunk,1)} being sent")
    ...
    def update_chunk_info(self):
        self.window_mgr.update_value(key=self.sectioned_chunk_data_key, value=self.token_dist[self.chunk_number]['chunk']['data'])
        self.window_mgr.update_value(key=self.sectioned_chunk_text_number_key,value=self.chunk_number)
    ...
    def while_window(self):
        ...
        while True:
            ...
            if self.event in self.instruction_keys:
                ...
                self.response_text_number_display = self.window_mgr.get_from_value('-RESPONSE_TEXT_NUMBER-')
                ...
                if self.event == '-RESPONSE_TEXT_BACK-':
                    if self.response_text_number_actual >0:
                        self.response_text_number_actual-=1
                        self.response_text_number_display-=1
                        update_output(output_iteration=self.response_text_number_actual)
                        self.window_mgr.update_value(key='-RESPONSE_TEXT_NUMBER-',value=self.response_text_number_display)
...
=self.response_text_number_display)
