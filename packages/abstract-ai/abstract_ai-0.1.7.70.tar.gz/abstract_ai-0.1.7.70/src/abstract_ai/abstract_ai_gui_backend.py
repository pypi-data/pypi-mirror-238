...
class GptManager:
    ...
    def update_text_with_responses(self):
        self.last_title_content = get_any_key(self.current_output,"title")
        if self.last_title_content:
            if isinstance(self.last_title_content,list):
                self.last_title_content=self.last_title_content[0]
        self.last_prompt_content = get_any_key(self.current_output,"prompt")
        self.last_response_content = get_any_key(self.current_output,'response')
