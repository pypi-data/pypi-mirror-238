...
df[["Exam 1 Score", "Exam 2 Score", "Exam 3 Score"]] = imputer.fit_transform(
...
df[["Exam 1 Score", "Exam 2 Score", "Exam 3 Score"]] = scaler.fit_transform(
    df[["Exam 1 Score", "Exam 2 Score", "Exam 3 Score"]]
)
# Normalization
scaler = MinMaxScaler()
...
stacking_classifier = VotingClassifier(estimators=base_models, voting="soft", n_jobs=-1)
...
plt.subplot(1, 3, 3)
plt.quiver(
    0, 0, first_principal_component[0], first_principal_component[1], angles="xy"
)
...
imputer = SimpleImputer(
    strategy="mean"
)  # You can choose strategy='median' or 'most_frequent' as well
...
scaler = StandardScaler()
data[["age", "rooms", "area", "price"]] = scaler.fit_transform(
    data[["age", "rooms", "area", "price"]]
)


normalizer = MinMaxScaler()
data[["age", "rooms", "area", "price"]] = normalizer.fit_transform(
    data[["age", "rooms", "area", "price"]]
)
...
encoded_features = pd.DataFrame(
    encoder.fit_transform(data[["rooms"]]).toarray(),
...
stacking_classifier = StackingClassifier(
    estimators=base_estimators, final_estimator=DecisionTreeClassifier(), cv=5
)
...
def gradient_descent(learning_rate, num_iterations):
    ...
    for iteration in range(num_iterations):
        ...
        print(
            f"Iteration {iteration + 1}: x = {x:.4f}, Gradient = {gradient:.4f}, Cost = {cost:.4f}"
        )
