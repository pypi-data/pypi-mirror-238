...
- `labcrawler list projects` to list all the projects in a group and its subgroups
- `labcrawler list groups` to list all the subgroups in a group and its subgroups
- `labcrawler move` to move project between namespaces

_More docs to come_

From your GitLab instance (or GitLab.com), you will also need a [personal access token](https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html) or [group access token](https://docs.gitlab.com/ee/user/group/settings/group_access_tokens.html) with API access to the group(s) and project(s) you wish to query. LabCrawler uses WizLib, which supports several options for token storage. For everyday use, the best option is to store the token in a password manager, then copy-and-paste it into LabCrawler when requested.
