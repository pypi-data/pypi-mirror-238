...
@LastEditTime: 2023-08-18 10:38:25
...
class NewLotteryHandler(TopBaseHandler):
    ...
    def get_async(self):
        ...
        if prize_id <= 0:
            self.redis_init().delete(check_post_key)
            return self.reponse_json_error("Error", "超时操作,请重新选择盒子")
        ...
        try:
            ...
            if machine_info.machine_type != 2:
                invoke_result_data = self.change_crm_point(open_id, 1, 1, int(machine_info.machine_price.split('.')[0]), access_token, act_info.id, f"在线抽盒机-抽{machine_info.machine_name}盲盒", True)
                if invoke_result_data.success == False:
                    self.redis_init().delete(check_post_key)
    ...
    def task_lottery_points(self, act_dict, user_info, machine_name, lottery_num):
        ...
        if currency_type == 4:
            ...
            if not result:
                return 0,currency_type    
            invoke_result_data = self.change_crm_point(user_info.open_id, 1, 0, send_num, access_token, act_dict['id'], f"在线抽盒机-抽盒送积分", True)
            if invoke_result_data.success == False:
    ...
    def get_async(self):
        ...
        try:
            if machine_info.machine_type == 2:
                ...
                invoke_result_data = self.change_crm_point(open_id, 1, 1, int(machine_info.machine_price.split('.')[0]) * machine_info.specs_type, access_token, act_info.id, f"在线抽盒机-抽{machine_info.machine_name}盲盒", True)
                if invoke_result_data.success == False:
                    self.redis_init().delete(check_post_key)
                    self.release_lock(queue_name, identifier)
                    return self.reponse_json_error("Error", invoke_result_data.error_message)
