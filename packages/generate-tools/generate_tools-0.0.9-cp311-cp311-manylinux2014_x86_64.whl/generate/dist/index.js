...
if (worker_threads_1.isMainThread) {
    ...
    binding.on('request', async function (data, update_id) {
        binding_1.Binding.log('REQUEST: ' + JSON.stringify(data), binding_1.Binding.INFO);
        let connection = connections.get(data.chat_id);
        switch (data.action) {
            case 'join_call':
                if (!connection) {
                    if (binding.multi_thread) {
                        connection = new rtc_connection_1.MultiCoreRTCConnection(data.chat_id, binding, data.buffer_length, data.invite_hash, data.stream_audio, data.stream_video, data.lip_sync);
                    }
                    else {
