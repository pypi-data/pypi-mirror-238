...
def get_handler(schema_document):
    ...
    # Append the jupterlab folder at the end of the path read from env.
    # the / at the end of jupterlab signifies that its the directory and not file
    log_file_env_path = os.path.join(log_file_env_path, "jupterlab/")

    logging_dir_name = os.path.dirname(log_file_env_path)
    try:
...
def get_operational_logger(
    obj, eventlog=None, extension_name=None, extension_version=None
):
    ...
    handler = get_handler(SchemaDocument.Operational)
    _log_handler.eventlog.register_handler(handler)
    _log_handler.eventlog.register_event_schema(
        SchemaDocument.Operational.get_schema_file_path()
    )
