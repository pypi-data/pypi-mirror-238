...
def load(fo):
    ...
    if 'events' in ydata:
        ...
        for event in ydata['events']:
            ...
            if 'event' in event:
                eventdata['event'] = str(event['event'])
                try:
                    eventdata['event_number'] = int(event['event'],10)
                except TypeError:
                    eventdata['event_number'] = int(event['event']) #yaml already converted it to a number
                except ValueError:
                    #print("couldn't convert event_num, leaving as string",event_num_str)
                    eventdata['event_number'] = None
            else:
                eventdata['event'] = None
                eventdata['event_number'] = None
