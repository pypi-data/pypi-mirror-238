...
from inhandtest.base_page.table_tr import IgTable, ErTable
...
class Page_:
    ...
    def table_tr(self, table: list, locator: Locator, value: list, locale: dict = None,
        ...
        """
        if self.model in ('IG902', 'IG502'):
            tr = IgTable(table, locator, locale, log_desc)
            exist_tr = []
            if value:
                for value_ in value:
                    if value_[0] in ('add', 'install'):
                        tr.add(agg_function, **value_[1])
                        ...
                        tr.edit(agg_function, value_[1], **value_[2])
                ...
                return exist_tr
        elif self.model in ('ER805', 'ER605', 'FWA02', 'ODU2002', 'EAP600'):
            tr = ErTable(table, locator, locale, log_desc)
            exist_tr = []
            if value:
                for value_ in value:
                    if value_[0] == 'add':
