...
def crack_and_chunk_and_embed(
                ...
                if existing_embedded_source.source.mtime is not None and existing_embedded_source.source.mtime >= mtime:
                    ...
                    for doc_id in existing_embedded_source.document_ids:
                        if verbosity >= DOCUMENT_LOGGING_VERBOSITY:
                            logger.info(f"REUSING: chunk '{doc_id}' from source '{existing_embedded_source.source.filename}'")
    ...
    if output_path is not None:
        ...
        embeddings = EmbeddingsContainer.load(Path(output_path).name, str(Path(output_path).parent), documents_only=True)
        ...
        data_to_embed = [document.load_data() for document in documents_to_embed(chunked_docs)]
