...
class Copert:
    ...
    class_Euro_6 = 13
    class_Euro_6c = 14
    ## Print names for copert_params class
...
"""
    ...
    efc_gasoline_passenger_car \
        = np.fromstring(emission_factor_string, sep=' ')
...
"""
    cold_start_emission_quotient \
        = np.fromstring(cold_start_emission_quotient_string, sep=' ')
...
"""
    ldv_parameter_pre_euro_1 \
        = np.fromstring(ldv_parameter_pre_euro_1_string, sep=' ')
...
"""
    ldv_reduction_percentage \
        = np.fromstring(ldv_reduction_percentage_string, sep=' ')
...
"""
    moped_parameter = np.fromstring(moped_parameter_string, sep=' ')
    ...
    def __init__(self, pc_parameter_file, ldv_parameter_file,
        ...
        self.pc_parameter = np.empty((7, 3, 4, 12), dtype=float)
        self.pc_parameter.fill(np.nan)
        ...
        ## Initialization
        self.ldv_parameter = np.empty((2, 3, 5, 12), dtype=float)
        self.ldv_parameter.fill(np.nan)
        ## Correspondence between strings and integer attributes in this class
        ...
        self.hdv_parameter = np.empty((2, 20, 8, 5, 3, 7, 10),
                                         dtype=float)
        self.hdv_parameter.fill(np.nan)
        ...
        ## Initialization
        self.moto_parameter = np.empty((4, 5, 4, 8), dtype=float)
        self.moto_parameter.fill(np.nan)
        ## Correspondence between strings and integer attributes in this class
        ## for motorcycles
                    ...
                    if engine_capacity == self.engine_capacity_0p8_to_1p4:
                        ...
                        if math.isnan(a) and copert_class <= self.class_Euro_3:
                            raise Exception("There is no formula to " \
                                            ...
                                            + "diesel passenger cars of copert_params class " \
    ...
    def HEFHeavyDutyVehicle(self, speed, vehicle_category, hdv_type,
        ...
        V = speed
        name_hdv_type \
            = list(self.corr_hdv_type.keys())[list(self.corr_hdv_type.values()).index(hdv_type)]
        name_slope \
            = list(self.corr_slope.keys())[list(self.corr_slope.values()).index(slope)]
        name_load \
            = list(self.corr_load.keys())[list(self.corr_load.values()).index(load)]
    ...
    def EFMotorcycle(self, pollutant, speed, engine_type, copert_class,
        ...
        if copert_class in [self.class_Improved_Conventional,
            ...
            name_engine_type \
                = list(self.corr_engine_type.keys())[list(self.corr_engine_type.values()).index(engine_type)]
            i_pollutant = self.index_pollutant[pollutant]
            i_copert_class = self.index_copert_class_moto[copert_class]
