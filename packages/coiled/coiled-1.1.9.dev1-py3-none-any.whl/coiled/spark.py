...
DEBUG_PORTS = [22, 8787, 4040, 8080]  # 22 is ssh, other ports expose dashboards without auth
...
def get_spark(client: Client, name="Coiled") -> SparkSession:
    ...
    if client.scheduler.address.startswith("tls://"):
        warnings.warn(
            f"Spark Connect session is neither encrypted nor secured. This operates in the open on {host}:{port}"
        )
    ...
    spark = (
        SparkSession.builder.remote(f"sc://{host}:{port}")  # TODO: security?
        .appName(name)
        .getOrCreate()
    )
    return spark
...
def get_spark_cluster(_open_debug_ports=False, *args, **kwargs):
    cluster = Cluster(
        ...
        **kwargs,
        backend_options={
            "ingress": [
                {"ports": [443, 8786, 7077, 15002, *(DEBUG_PORTS if _open_debug_ports else [])], "cidr": "0.0.0.0/0"},
            ],
        },
    )
    return cluster
