...
def get_ssh_connection(cloud, cluster_id):
    try:
        ...
        raise RuntimeError("--use-ssh requires fabric")
...
@click.option(
    ...
    hidden=True,
    help=(
        "Whether to run user-code via an ssh session or dask task submitted by client. "
        "SSH usually is the best option."
    ),
)
@click.option(
    "--interactive",
...
def start_run(
    ...
    if interactive and not use_ssh:
        raise RuntimeError("--interactive doesn't work with --use-dask")
    if detach and not use_ssh:
        raise RuntimeError("--detach doesn't work with --use-dask")

    if use_ssh:
        # while --use-ssh is hidden option we won't add fabric to client deps
        try:
            import fabric  # noqa: F401
        except ImportError:
            print(
                "[red][bold]Error:[/bold][/red] to use [green]--use-ssh[/green] "
                "you need to install [green]fabric[/green]"
            )
            return
            ...
            with coiled.Cluster(name=name, cloud=cloud, custom_widget=widget, **cluster_kwargs) as cluster:
                ...
                if not use_ssh:
                    with Client(cluster) as client:
                        ...
                        with span(cluster, callstack=callstack):
