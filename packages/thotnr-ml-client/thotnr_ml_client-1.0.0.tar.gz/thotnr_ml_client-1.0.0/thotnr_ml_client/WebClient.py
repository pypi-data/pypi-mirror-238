...
class WebClient:
    ...
    def __init__(self, base_url, config_manager):
        self.auth_token = ""
        self.configManager = config_manager
        self._host_name = self.configManager.get_host_name()
        self._base_url = self._host_name + "/" + base_url + "/" + self.configManager.get_session_code()
    ...
    def _attach_headers(self, headers):
        ...
        if self.configManager.get_current_time_label() is not None:
            headers[self.QUERY_PARAM_TIME_LABEL_PROP] = self.configManager.get_current_time_label()
        if self.configManager.get_current_bi_temporal_props() is not None:
            encoded_bytes = base64.urlsafe_b64encode(self.configManager.get_current_bi_temporal_props().encode())
            encoded_props = encoded_bytes.decode()
            headers[self.QUERY_PARAM_BITEMPORAL_PROPS] = encoded_props
