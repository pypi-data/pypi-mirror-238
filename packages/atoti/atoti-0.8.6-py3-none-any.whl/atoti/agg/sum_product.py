...
@overload
def sum_product(*factors: Column) -> MeasureDescription:
    ...


@overload
def sum_product(*factors: MeasureConvertible, scope: Scope) -> MeasureDescription:
    ...


@doc(scope=_SCOPE_DOC)
def sum_product(
    *factors: Union[Column, MeasureConvertible],
    ...
    return agg(
        ...
        plugin_key="ATOTI_SUM_PRODUCT",
        # The type checkers cannot see that the `@overload` above ensure that this call is valid.
        scope=scope,  # type: ignore[arg-type] # pyright: ignore[reportGeneralTypeIssues]
    )
