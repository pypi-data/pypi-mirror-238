...
class LocalSession(BaseSession[_LocalCubes, Security]):
    ...
    def __init__(  # noqa: C901
        ...
        *,
        ...
        # When there are no Atoti sessions or detached processes running, there will be no Py4J server listening on the default port: a `ConnectionRefusedError` will be raised.
            ...
            self._java_api = JavaApi(
                address=self._address,
                auth_token=server_subprocess.auth_token,
                distributed=distributed,
                py4j_java_port=server_subprocess.py4j_java_port,
            )
            self._server_subprocess = server_subprocess
    ...
    def close(self) -> None:
        ...
        assert not self.closed, "The session has already been closed."
        try:
            ...
            self._closed = True
            _get_local_sessions().remove(self)
