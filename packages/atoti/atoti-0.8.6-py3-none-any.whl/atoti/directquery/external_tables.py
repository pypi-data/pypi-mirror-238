...
class ExternalTables(Generic[ExternalTableT_co], ReprJsonable):
    ...
    Example:
        .. doctest:: ExternalTables
            ...
            >>> all_identifiers = [
            ...     ExternalTableIdentifier("my_db", "my_schema", "my_table")
            ... ]
            >>> key = "EXAMPLE"
    ...
    _tables: dict[str, dict[str, list[ExternalTableIdentifier]]]
    _database_key: str
    # Optional is just a workaround for mypy issue
    # https://github.com/python/mypy/issues/708
    _create_table: Optional[Callable[[ExternalTableIdentifier], ExternalTableT_co]]
    ...
    def _filter(
        ...
        *,
        ...
        return ExternalTables(
            ...
            _create_table=self._create_table,
        )

    def __getitem__(self, table_key: ExternalTableKey, /) -> ExternalTableT_co:
        identifier = self._resolve_table(table_key)
        assert self._create_table is not None
        table: ExternalTableT_co = self._create_table(identifier)
        return table
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, ExternalTables):
            return False
        return (
            self._database_key == other._database_key and self._tables == other._tables
        )
