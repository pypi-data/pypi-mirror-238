...
from typing import Optional as _Optional, Union
...
class AbstractGridEntity(LogicalEntity):
    ...
    @facing.setter
    def facing(self, facing: _Optional[Union[float, int, str]]):
...
class GridEntity(AbstractGridEntity):
    ...
    def occupy(self, cell_to_occupy: object = None):
        if cell_to_occupy != self.cell or self.last_cell is not None:
            if self.cell is not None:
                return
    ...
    def move(self, cell_to_move_to: object = None):
        ...
        if cell_to_move_to in [self.grid[cell] for cell in self.cell.adjacent]:
            self.actions['move'][self.movements - self._movements_remaining] = {'direction': f'{list(_DIRECTION_MAP.keys())[self.adjacent.index(cell_to_move_to.designation)]}', 'from': f'{self.cell.designation}', 'to': f'{cell_to_move_to.designation}'}
        else:
            print('Invalid move')
            return
        self.vacate()
