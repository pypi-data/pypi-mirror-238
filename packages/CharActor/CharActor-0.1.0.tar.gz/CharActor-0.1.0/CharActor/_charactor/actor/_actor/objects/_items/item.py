...
_GENERAL_ITEMS_DICT = _load_json('CharActor/_charactor/dicts/general_items.json')

_TRADE_ITEMS_DICT = _load_json('CharActor/_charactor/dicts/trade_items.json')
...
class AbstractItem:
    ...
    @property
    def position(self):
        return pymunk.Vec2d(*self.cell.coordinates)
    ...
    def _spawn(self, grid, cell = None):
        self._grid = grid
        cell = cell if cell is not None else grid.random_cell()
        while not cell.passable:
