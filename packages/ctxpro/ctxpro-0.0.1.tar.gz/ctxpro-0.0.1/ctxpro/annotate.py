...
try:
    ...
    raise Exception("Failed to import simalign. Have you upgraded networkx?\nTry: pip install -U networkx")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="CTXPRO: Annotate a document according to a set of rules for ambiguities which require context to resolve.\n"
        "   Example: ctxpro annotate -t deu -r rules.json -o output_dir/ input_dir/",
        usage="$(prog)s [options] input_dir/",
    ...
    parser.add_argument("--input_files", '-i', default=None,
                                                type=str,
                                                nargs='*',
                                                help = "Input tsv files to annotate. If not specified, reads from stdin. Expected columns are docid, src, tgt.")
    parser.add_argument("--rules", "-r", required=True,
                                                type=str,
                                                nargs='+',
                                                help="Name of rule set to use. If not found in pre-determined rule set, must be a file path.")
    parser.add_argument("--window-size", "-w", default=10,
                                                type=int,
                                                help="Window size for preceding context. Default 10 sentences.")
    parser.add_argument("--target", '-t', required=True,
                                                type=str,
                                                help="Target language code (ISO 639-1). Required.")
    parser.add_argument("--output_dir", '-o', default="annotations",
                                                type=str,
