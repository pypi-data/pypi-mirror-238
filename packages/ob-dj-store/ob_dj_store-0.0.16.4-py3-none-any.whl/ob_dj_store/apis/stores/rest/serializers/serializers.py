...
from ob_dj_store.core.stores.utils import PartnerAuth, distance, get_arabic_fields
...
class AttributeChoiceSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        static_fields = self.Meta.fields
        language = (
            kwargs.get("context").get("request").language
            if "context" in kwargs.keys()
            and "language" in kwargs.get("context").get("request").data
            else None
        )
        dynamic_fields = get_arabic_fields(self.Meta.model, static_fields)
        super().__init__(*args, **kwargs)
        self.Meta.fields = dynamic_fields if language else static_fields
