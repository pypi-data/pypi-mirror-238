...
class App(Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Special case to allow test cases have multiple subclasses of xt.State.
        if not is_testing_env:
            # Only one State class is allowed.
            if len(state_subclasses) > 1:
                raise ValueError(
                    "xt.State has been subclassed multiple times. Only one subclass is allowed"
                )

            # verify that provided state is valid
            if self.state and inferred_state and self.state is not inferred_state:
                console.warn(
                    f"Using substate ({self.state.__name__}) as root state in `xt.App` is currently not supported."
        ...
        if admin_dash and admin_dash.models:
            ...
            admin = (
                ...
                if admin_dash.admin
                    ...
                    logo_url="https://dotagent.dev/Nextpy.svg",
                )
            )
