...
def removePunctuation(sentance):
    punctuation = list(pd.read_csv("{currentDir}/ptrNaturalLanguage/punctuation.csv".format(currentDir = os.getcwd()))["symbol"])
...
def ridStopWords(sentance):
    stopWords = list(pd.read_csv("{currentDir}/ptrNaturalLanguage/stopWords.csv".format(currentDir = os.getcwd()))["Stop Words"])
    #
...
def saveSentiments(sentimentDictionary):
    with open("{currentDir}/ptrNaturalLanguage/trainedWords.csv".format(currentDir = os.getcwd()), "w", newline = "") as fileToWrite:
...
def checkSentiment(sentance):
    ...
    trainedWords = pd.read_csv("{currentDir}/ptrNaturalLanguage/trainedWords.csv".format(currentDir = os.getcwd()), encoding='latin1')
    ...
    elif sentenceSentiment < 0.5 and sentenceSentiment > 0.4:
