...
def peak_alignment(files_excel, rt_error=0.1, mz_error=0.015):
    ...
    for file in tqdm(files_excel, desc='Generating excel files with alignment information...'):
...
def gen_ref(files_excel, mz_error=0.015, rt_error=0.1):
    """
    Align all the mz & rt pairs, combine the rt & mz pairs with specific rt_error and mz_error, and generate reference mz & rt pairs.

    Args:
        files_excel: List of excel files path for extracted peaks.
        rt_error: RT error for combining.
        mz_error: MZ error for combining.

    Returns:
        Reference mz/rt pairs.
    """
    # Read data from all excel files
    data = [pd.read_excel(file).loc[:, ['rt', 'mz']].values for file in
            tqdm(files_excel, desc='Reading each excel file')]

    # Concatenate all peaks
    peak_all = np.vstack(data)

    # Align reference pairs
    def align_reference_pairs(pair):
        reference_pairs = []
        while len(pair) > 0:
            rt, mz = pair[0]
            rt = round(rt, 2)
            mz = round(mz, 4)
            index = np.where(
                (pair[:, 0] <= rt + rt_error) & (pair[:, 0] >= rt - rt_error) & (pair[:, 1] <= mz + mz_error) & (
                        pair[:, 1] >= mz - mz_error))
            reference_pairs.append([rt, mz])
            pair = np.delete(pair, index, axis=0)
        return np.array(reference_pairs)

    # Generate reference pairs
    reference_pairs = align_reference_pairs(peak_all)

    # Check for omitted pairs
    omitted_pairs = []
    for pair in tqdm(peak_all, desc='Second Check for gen_ref'):
        rt, mz = pair
        index = np.where(
            (np.abs(reference_pairs[:, 0] - rt) <= rt_error) & (np.abs(reference_pairs[:, 1] - mz) <= mz_error))
        if len(index[0]) == 0:
...
def multi_process_database_matching(path, database, processors=None, ms1_error=50, ms2_error=0.015, rt_error=0.1,
    ...
    with Pool(processes=processors) as pool:
        results = []
        for file in unique_files:
            result = pool.apply_async(database_match, args=(file, database, ms1_error, ms2_error, rt_error, mode,frag_DIA,frag_DDA ))
...
def database_match(file, database, ms1_error=50, ms2_error=0.015, rt_error=0.1, mode='pos',frag_DIA = 'frag_DIA',frag_DDA = 'frag_DDA'):
    """
    ...
    unique_ms2_match = ms2_matching(unique, database, ms1_error=ms1_error, ms2_error=ms2_error, mode=mode,frag_DIA = frag_DIA,frag_DDA = frag_DDA)
...
def swath_process(file, precursor_ion_start_mass=99.5, profile=True, peak_width=2,
    ...
    for k, v in all_data.items():
        ...
        print('   ')
        x = locals()[v]
        peak_all = split_peak_picking_swath(x, k, profile=profile, i_threshold=i_threshold, peak_width=peak_width)
...
def KMD_cal(exact_mass, functional_group):
    ...
    kendrick_mass = exact_mass * round(functional_group_masses[functional_group],0) / functional_group_masses[functional_group]
...
def build_molecular_network(data, parent_cmps,
                            ...
                            node_edge_width=1.0, node_edge_color='black',
    ...
    parent_pairs = data[(data['cmp1'].isin(parent_cmps) | data['cmp2'].isin(parent_cmps))&(data['same_frag_num']>=frag_num_for_network)]
    ...
    target_pairs = data[data['cmp1'].isin(target_cmp) & data['cmp2'].isin(target_cmp)&(data['same_frag_num']>=frag_num_for_network)]
    ...
    nx.draw(G, pos, with_labels=False, node_color=colors,font_size=label_font_size,
           ...
           linewidths=node_edge_width, edgecolors=node_edge_color)
...
def formula_prediction(mz,atoms = ['C', 'H', 'O', 'N'],
    ...
    def generate_formula(row):
        ...
        return formula
    # sort element list
    elements_sorted_list = ['C','H','O','N','S','Cl','Br','P','F','K','Na','Ciso','D','Oiso','Niso','Siso']
    ...
    if len(patterns_list)> max_possible_num:
        raise ValueError("The number of possible structures is so large that processing may take an extended period on your computer, increase the max_possible_num may solve the issue.")
    else:      
