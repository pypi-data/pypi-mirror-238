...
class ExternalIdentityMapping(IdentityMapping):
    ...
    def map_identity(self, identity_data: t.Mapping[str, str]) -> str | None:
        ...
        """
        p = Popen(
            self.command + ["-c", self.connector, "-s", self.storage_gateway],
        ...
        if p.returncode != 0:
            self.log.debug("Mapping error: stdout=%s", stdout)
            self.log.debug("Mapping error: stderr=%s", stderr)
            raise IdentityMappingError("Error mapping identity")
        ...
        if stdout:
            try:
                ...
                self.log.error(
                    "The mapping application %s yielded an invalid result %s, "
                    "causing %s",
                    self.command,
                    stdout,
                    str(e),
            ...
            if output["result"]:
                ...
                if str(first_result["id"]) != identity_data["id"]:
                    raise InvalidMappingError("Invalid response from mapping program")
    ...
    def map_identities(
        ...
        p = Popen(
            self.command + ["-c", self.connector, "-s", self.storage_gateway, "-a"],
            universal_newlines=True,
            stdout=PIPE,
            stderr=PIPE,
            stdin=PIPE,
        ...
        if p.returncode != 0:
            raise IdentityMappingError(
                f"Error running mapping program {self.command[0]}"
            )
        if stdout:
            input_ids = [i["id"] for i in identity_data]
            try:
                output = identity_mapping_output_schema.loads(stdout)
            except Exception as e:
                self.log.error(
