...
def has_tkinter() -> bool:
    ...
    try:
        ...
        colored_text("tkinter가 설치되지 않았습니다", new=RED)
        return False
    try:
        setup_tk()
    except tkinter.TclError:
        colored_text("tkinter 설정 오류", new=RED)
        return False
    return True
...
def select_file(
    ...
    if not has_tkinter():
        path = user_input_handler.colored_input(
            f"파일의 경로를 입력하세요 ({title}): "
        )
        if not os.path.isfile(path):
            colored_text("유효하지 않은 경로", new=RED)
            return ""
        return path
    setup_tk()
    from tkinter import filedialog
...
def load_save_file(path: str) -> dict[str, Any]:
    ...
    colored_text(f"게임 버전: &{country_code}&")
    ...
    if config_manager.get_config_value_category("START_UP", "CREATE_BACKUP"):
        ...
        colored_text(
            f"백업 생성 시간: &{os.path.abspath(path + '_backup')}&", new=GREEN
        )
    return {
        "save_data": save_data,
