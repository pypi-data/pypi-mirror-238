"use strict";
(self["webpackChunk_jupyterlab_application_top"] = self["webpackChunk_jupyterlab_application_top"] || []).push([[3245],{
...
__webpack_require__.d(__webpack_exports__, {
  "BaseSettings": () => (/* reexport */ BaseSettings),
  "DefaultSchemaValidator": () => (/* reexport */ DefaultSchemaValidator),
  "ISettingRegistry": () => (/* reexport */ ISettingRegistry),
  "SettingRegistry": () => (/* reexport */ SettingRegistry),
 ...
 */
const copy = index_js_.JSONExt.deepCopy;
...
class DefaultSchemaValidator {
     ...
     */
    constructor() {
        this._composer = new (ajv_default())({
            useDefaults: true,
            ...AJV_DEFAULT_OPTIONS
        });
        this._validator = new (ajv_default())({ ...AJV_DEFAULT_OPTIONS });
        this._composer.addSchema(plugin_schema_namespaceObject, 'jupyterlab-plugin-schema');
        this._validator.addSchema(plugin_schema_namespaceObject, 'jupyterlab-plugin-schema');
    }
    /**
...
class SettingRegistry {
    ...
    constructor(options) {
         ...
         */
        this.schema = plugin_schema_namespaceObject;
        /**
         * The collection of setting registry plugins.
         */
        this.plugins = Object.create(null);
        this._pluginChanged = new signaling_dist_index_es6_js_.Signal(this);
    ...
    async remove(plugin, key) {
        ...
        const raw = dist.parse(plugins[plugin].raw);
    ...
    isDefault(user) {
        for (const key in this.schema.properties) {
            ...
            if (value === undefined ||
                ...
                index_js_.JSONExt.deepEqual(value, index_js_.JSONExt.emptyObject) ||
                index_js_.JSONExt.deepEqual(value, index_js_.JSONExt.emptyArray)) {
                continue;
            }
            if (!index_js_.JSONExt.deepEqual(value, defaultValue)) {
                return false;
            }
        }
        return true;
...
(function (SettingRegistry) {
    ...
    function reconcileMenus(reference, addition, warn = false, addNewItems = true) {
        if (!reference) {
            return addition && addNewItems ? index_js_.JSONExt.deepCopy(addition) : [];
        }
        if (!addition) {
            return index_js_.JSONExt.deepCopy(reference);
        }
        const merged = index_js_.JSONExt.deepCopy(reference);
    ...
    function reconcileItems(reference, addition, warn = false, addNewItems = true) {
        if (!reference) {
            return addition ? index_js_.JSONExt.deepCopy(addition) : undefined;
        }
        if (!addition) {
            return index_js_.JSONExt.deepCopy(reference);
        }
        const items = index_js_.JSONExt.deepCopy(reference);
            ...
            switch ((_a = item.type) !== null && _a !== void 0 ? _a : 'command') {
                ...
                case 'submenu':
                    if (item.submenu) {
                        ...
                        if (refIndex < 0) {
                            if (addNewItems) {
                                items.push(index_js_.JSONExt.deepCopy(item));
                            }
                        }
                        else {
                            items[refIndex] = {
                    ...
                    if (item.command) {
                        const refIndex = items.findIndex(ref => {
                            ...
                            return ref.command === item.command &&
                                ...
                                index_js_.JSONExt.deepEqual((_a = ref.args) !== null && _a !== void 0 ? _a : {}, (_b = item.args) !== null && _b !== void 0 ? _b : {});
                        });
                        if (refIndex < 0) {
                            if (addNewItems) {
                                items.push({ ...item });
    ...
    function reconcileShortcuts(defaults, user) {
        ...
        user = user.filter(shortcut => {
            const keys = index_es6_js_.CommandRegistry.normalizeKeys(shortcut).join(RECORD_SEPARATOR);
            if (!keys) {
    ...
    function reconcileToolbarItems(reference, addition, warn = false) {
        ...
        addition.forEach(item => {
            ...
            if (refIndex < 0) {
                ...
                if (warn &&
                    index_js_.JSONExt.deepEqual(Object.keys(item), Object.keys(items[refIndex]))) {
...
(function (Private) {
    ...
    function reifyDefault(schema, root, definitions) {
        ...
        if (schema.type === 'object') {
            ...
            const result = index_js_.JSONExt.deepCopy(schema.default);
...
;// CONCATENATED MODULE: ../packages/settingregistry/lib/tokens.js
/* -----------------------------------------------------------------------------
| Copyright (c) Jupyter Development Team.
| Distributed under the terms of the Modified BSD License.
|----------------------------------------------------------------------------*/
 ...
 */
const ISettingRegistry = new index_js_.Token('@jupyterlab/coreutils:ISettingRegistry', `A service for the JupyterLab settings system.
  Use this if you want to store settings for your application.
  See "schemaDir" for more information.`);

;// CONCATENATED MODULE: ../packages/settingregistry/lib/index.js
/* -----------------------------------------------------------------------------
| Copyright (c) Jupyter Development Team.
| Distributed under the terms of the Modified BSD License.
|----------------------------------------------------------------------------*/
...
//# sourceMappingURL=3245.9b13a20b18c6757b2a06.js.map?v=9b13a20b18c6757b2a06