"use strict";
(self["webpackChunk_jupyterlab_application_top"] = self["webpackChunk_jupyterlab_application_top"] || []).push([[4176],{
...
__webpack_require__.d(__webpack_exports__, {
  "CodeConsole": () => (/* reexport */ CodeConsole),
  "ConsoleHistory": () => (/* reexport */ ConsoleHistory),
  "ConsolePanel": () => (/* reexport */ ConsolePanel),
  "ForeignHandler": () => (/* reexport */ ForeignHandler),
  "IConsoleTracker": () => (/* reexport */ IConsoleTracker)
});
...
;// CONCATENATED MODULE: ../packages/console/lib/foreign.js
...
;// CONCATENATED MODULE: ../packages/console/lib/history.js
// Copyright (c) Jupyter Development Team.
// Distributed under the terms of the Modified BSD License.
...
;// CONCATENATED MODULE: ../packages/console/lib/widget.js
// Copyright (c) Jupyter Development Team.
// Distributed under the terms of the Modified BSD License.
 ...
 */
class CodeConsole extends dist_index_es6_js_.Widget {
    /**
    ...
    constructor(options) {
        ...
        this._banner = null;
        this._executed = new index_es6_js_.Signal(this);
        this._mimetype = 'text/x-ipython';
        this._msgIds = new Map();
        this._msgIdCells = new Map();
        this._promptCellCreated = new index_es6_js_.Signal(this);
        this._dragData = null;
        this._drag = null;
        this._focusedCell = null;
        this._translator = (_a = options.translator) !== null && _a !== void 0 ? _a : translation_lib_index_js_.nullTranslator;
        ...
        this._cells = new observables_lib_index_js_.ObservableList();
        this._content = new dist_index_es6_js_.Panel();
        this._input = new dist_index_es6_js_.Panel();
    ...
    addBanner() {
        ...
        const model = this.modelFactory.createRawCell({
            sharedModel: (0,ydoc_lib_index_js_.createStandaloneCell)({
                cell_type: 'raw',
                source: '...'
            })
        });
        const banner = (this._banner = new cells_lib_index_js_.RawCell({
            model,
    ...
    serialize() {
        ...
        for (const cell of this._cells) {
            const model = cell.model;
            if ((0,cells_lib_index_js_.isCodeCellModel)(model)) {
    ...
    _evtMouseDown(event) {
        ...
        let cellIndex = cells_lib_index_js_.CellDragUtils.findCell(target, this._cells, cellFilter);
        if (cellIndex === -1) {
            ...
            cellIndex = cells_lib_index_js_.CellDragUtils.findCell(target, this._cells, cellFilter);
        }
        if (cellIndex === -1) {
            return;
        }
        const cell = this._cells.get(cellIndex);
        const targetArea = cells_lib_index_js_.CellDragUtils.detectTargetArea(cell, event.target);
    ...
    _evtMouseMove(event) {
        ...
        if (data &&
            cells_lib_index_js_.CellDragUtils.shouldStartDrag(data.pressX, data.pressY, event.clientX, event.clientY)) {
    ...
    _startDrag(index, clientX, clientY) {
        ...
        const dragImage = cells_lib_index_js_.CellDragUtils.createCellDragImage(this._focusedCell, selected);
    ...
    newPromptCell() {
        ...
        if (promptCell) {
            ...
            index_es6_js_.Signal.clearData(promptCell.editor);
     ...
     */
    onUpdateRequest(msg) {
        widget_Private.scrollToBottom(this._content.node);
    }
    /**
    ...
    _execute(cell) {
        ...
        return cells_lib_index_js_.CodeCell.execute(cell, this.sessionContext).then(onSuccess, onFailure);
    }
    /**
    ...
    _createCodeCellOptions() {
        ...
        return {
            ...
            contentFactory,
            editorConfig,
            editorExtensions: [state_dist_index_js_.Prec.high(onKeyDown)],
            placeholder: false,
...
(function (CodeConsole) {
     ...
     */
    class ContentFactory extends cells_lib_index_js_.Cell.ContentFactory {
        /**
        ...
        createCodeCell(options) {
            return new cells_lib_index_js_.CodeCell(options).initializeState();
        }
        /**
        ...
        createRawCell(options) {
            return new cells_lib_index_js_.RawCell(options).initializeState();
        }
    }
    ...
    class ModelFactory {
        ...
        constructor(options = {}) {
            this.codeCellContentFactory =
                options.codeCellContentFactory || cells_lib_index_js_.CodeCellModel.defaultContentFactory;
        }
        /**
        ...
        createCodeCell(options = {}) {
            ...
            return new cells_lib_index_js_.CodeCellModel(options);
        }
        /**
         ...
         */
        createRawCell(options) {
            return new cells_lib_index_js_.RawCellModel(options);
        }
    }
...
;// CONCATENATED MODULE: ../packages/console/lib/panel.js
// Copyright (c) Jupyter Development Team.
// Distributed under the terms of the Modified BSD License.
 ...
 */
class ConsolePanel extends index_js_.MainAreaWidget {
    /**
     * Construct a console panel.
     */
    constructor(options) {
        super({ content: new dist_index_es6_js_.Panel() });
        this._executed = null;
        ...
        const count = panel_Private.count++;
        if (!path) {
            path = lib_index_js_.PathExt.join(basePath || '', `console-${count}-${dist_index_js_.UUID.uuid4()}`);
        }
        sessionContext = this._sessionContext =
            sessionContext !== null && sessionContext !== void 0 ? sessionContext : new index_js_.SessionContext({
        ...
        const resolver = new rendermime_lib_index_js_.RenderMimeRegistry.UrlResolver({
            path,
        ...
        void sessionContext.initialize().then(async (value) => {
            var _a;
            if (value) {
                await ((_a = options.sessionDialogs) !== null && _a !== void 0 ? _a : new index_js_.SessionContextDialogs({ translator })).selectKernel(sessionContext);
            }
        ...
        this.title.icon = ui_components_lib_index_js_.consoleIcon;
     ...
     */
    _updateTitlePanel() {
        panel_Private.updateTitle(this, this._connected, this._executed, this.translator);
    }
}
/**
...
(function (ConsolePanel) {
     ...
     */
    ConsolePanel.IContentFactory = new dist_index_js_.Token('@jupyterlab/console:IContentFactory', 'A factory object that creates new code consoles. Use this if you want to create and host code consoles in your own UI elements.');
    ...
    function updateTitle(panel, connected, executed, translator) {
        ...
        if (sessionContext) {
            ...
            let caption = trans.__('Name: %1\n', sessionContext.name) +
                trans.__('Directory: %1\n', lib_index_js_.PathExt.dirname(sessionContext.path)) +
                trans.__('Kernel: %1', panel.console.sessionContext.kernelDisplayName);
            if (connected) {
                caption += trans.__('\nConnected: %1', lib_index_js_.Time.format(connected.toISOString()));
            }
            if (executed) {
                caption += trans.__('\nLast Execution: %1');
            }
...
;// CONCATENATED MODULE: ../packages/console/lib/tokens.js
// Copyright (c) Jupyter Development Team.
// Distributed under the terms of the Modified BSD License.
 ...
 */
const IConsoleTracker = new dist_index_js_.Token('@jupyterlab/console:IConsoleTracker', `A widget tracker for code consoles.
  Use this if you want to be able to iterate over and interact with code consoles
  created by the application.`);

;// CONCATENATED MODULE: ../packages/console/lib/index.js
// Copyright (c) Jupyter Development Team.
// Distributed under the terms of the Modified BSD License.
/**
...
//# sourceMappingURL=4176.1de59f6c40f33a7200b2.js.map?v=1de59f6c40f33a7200b2