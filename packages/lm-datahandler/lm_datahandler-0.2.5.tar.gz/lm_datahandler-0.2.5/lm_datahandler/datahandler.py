...
class DataHandler(object):
    ...
    def concat_data_file(self, eeg_path, acc_path, ble_path, sti_path):
        ...
        self.raw_eeg = np.concatenate((self.raw_eeg, np.zeros(round(time_gap*self.sf_eeg)).astype(self.raw_eeg.dtype), raw_eeg))
        self.raw_acc = np.concatenate((self.raw_acc, np.zeros([3, round(time_gap*self.sf_acc)]).astype(self.raw_acc.dtype), raw_acc), axis=1)
        ...
        self.acc = np.concatenate((self.acc, np.zeros([3, round(time_gap*self.sf_acc)]).astype(self.acc.dtype), acc), axis=1)
