...
from models.role import Role, ALL_ROLE_NAMES
from models.tier import Tier
from models.pool import Pool
from persistence.csv import CsvChampIO
from pool.yaml import YamlPoolIO
from utils.throttler import Throttler
from services.opgg import retrieve_opgg_data, parse_data
...
def completer(cls: Role | Champion | Region | Tier) -> Callable[[str], Iterable[str]]:
...
def add(
    ...
    print(
        f"Champion {champion} successfully added to {role} role pool with priority {priority}"
    )
...
def recommend(role: role_cli, enemy_champion: champ_cli):
    ...
    for champion, priority, winrate, games in sorted(matchup_data, key=lambda x: (-x[2], x[1], -x[3], x[0])):
        print(f"{champion: >15}: {winrate: 5.2f}% | {games: >5} games | Priority: {priority}")
    
    if len(no_data) > 0:
        print("No data for the following champions:")
        for champion, priority in sorted(no_data, key=lambda x: (x[1], x[0])):
            print(f"{champion: >15} | Priority: {priority}")
