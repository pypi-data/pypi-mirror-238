...
try:
    ...
    def textbox(x, y, text, color='fg', font=fontpath, fontsize=fontsize.text, linewidth=2, backgroundcolor='rfg', fill=True, **kwargs):
        plt.text(x, y, text, color=colorlist(color), fontproperties=fm.FontProperties(fname=font, size=fontsize), bbox=dict(boxstyle="square", fc=colorlist(backgroundcolor), ec=colorlist(color), fill=fill, alpha=1.0, linewidth=linewidth), **kwargs)

    def annotate(text, xy, xytext, color='fg', font=fontpath, fontsize=fontsize.text, **kwargs):
        plt.annotate(text, xy, xytext, color=colorlist(color), fontproperties=fm.FontProperties(fname=font, size=fontsize), **kwargs)

    def annotate_wedge(text, xy, xytext, arrowcolor='fg', color='fg', font=fontpath, fontsize=fontsize.text, linewidth=2, backgroundcolor='rfg', fill=True, box=True, **kwargs):
        if box == True:
            plt.annotate(text, xy, xytext, color=colorlist(color), fontproperties=fm.FontProperties(fname=font, size=fontsize), 
                        arrowprops=dict(arrowstyle="wedge", color=colorlist(arrowcolor)),
                        bbox=dict(boxstyle="square", fc=colorlist(backgroundcolor), ec=colorlist(arrowcolor), fill=fill, alpha=1.0, linewidth=linewidth), **kwargs)
        else:
            plt.annotate(text, xy, xytext, color=colorlist(color), fontproperties=fm.FontProperties(fname=font, size=fontsize), 
                        arrowprops=dict(arrowstyle="wedge", color=colorlist(arrowcolor)), **kwargs)
