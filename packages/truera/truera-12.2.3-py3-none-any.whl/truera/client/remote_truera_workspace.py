...
class RemoteTrueraWorkspace(BaseTrueraWorkspace):
    ...
    def add_project(
        ...
        try:
            ...
            if workspace_validation_utils.is_nontabular_project(input_type):
                self.set_influence_type("integrated-gradients")
    ...
    def set_influence_type(self, algorithm: str):
        ...
        if self.project:
            return self.cs_client.update_analytics_configuration(
                project_id=self.project.id,
                influence_algorithm_type=algorithm_enum
            )
    ...
    def set_model(self, model_name: str):
        ...
        self._print_model_associated_with_different_data_collection_message(
            self.logger, model_name, data_collection_name,
    ...
    def _infer_model_output_context(
        self, column_spec: Union[ColumnSpec, NLPColumnSpec],
        ...
        if column_info.prediction or column_info.feature_influences:
            ...
            if column_info.feature_influences:
                ...
                model_output_context = model_output_context.clone(
                    background_split_name=background_split_name,
                    influence_type=self.get_influence_type()
                )
            self.logger.info(
    ...
    def add_data(
        ...
        **kwargs
        ...
        #TODO: Remove is_production_data
        if is_production_data:
            raise ValueError(
                "Use of `is_production_data` in `add_data` is deprecated. Please use `add_production_data()`"
            )

        is_success = False
        if column_spec and not isinstance(column_spec, BaseColumnSpec):
