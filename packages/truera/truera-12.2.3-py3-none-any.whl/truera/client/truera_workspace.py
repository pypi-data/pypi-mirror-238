...
class TrueraWorkspace(BaseTrueraWorkspace):
    ...
    def set_influence_type(self, algorithm: str):
        if algorithm not in ["truera-qii", "shap", "integrated-gradients"]:
            return ValueError(
                "Influence algorithm type must be one of [\"truera-qii\", \"shap\", \"integrated-gradients\"]!"
            )
        ...
        if self._local_model_execution:
            if self.get_environment() == REMOTE_ENV:
                ...
                self.download_project(
                    ...
                    data_split_names=splits_to_download
                )
    ...
    def compute_feature_influences(
        ...
        feature_influences = self.get_explainer(data_split_name
