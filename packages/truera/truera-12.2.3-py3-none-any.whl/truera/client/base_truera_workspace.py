...
from truera.client.client_utils import _QOI_TO_STRING
...
class BaseTrueraWorkspace(ABC):
    ...
    def compute_feature_influences(
        ...
        return explainer.compute_feature_influences(
            start=start,
            stop=stop,
    ...
    def _print_add_model_message(
        self, logger: logging.Logger, model_name: str, data_collection_name: str
    ) -> None:
        logger.info(
            f"Model \"{model_name}\" is added and associated with data collection \"{data_collection_name}\". \"{model_name}\" is set as the model for the workspace context."
        )
    ...
    def compute_all(
        self,
        computations: Optional[Sequence[str]] = DEFAULT_COMPUTATIONS,
                ...
                for data_split in data_splits:
                    ...
                    for computation in computations:
                        if computation == "predictions":
                            # TODO(PI-813): Split compute and get for predictions
                            predictions = self.get_ys_pred()
                            self.add_model_predictions(
                                predictions.reset_index(),
                                id_col_name=predictions.index.name
                            )
