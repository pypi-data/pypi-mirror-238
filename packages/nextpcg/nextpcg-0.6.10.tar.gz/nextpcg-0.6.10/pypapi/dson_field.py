...
# _value is file name
class FileField(Field, ListSubFieldBase):
...
class StaticMeshField(Field):
    """
    _value is a StaticMesh
    """
    field_type_name = "paramType.StaticMesh"
    support_param = False
    ...
    def from_json(cls, json_object, input_name, field_desc:FieldDesc):
        if field_desc.work_path is None:
            raise Exception("StaticMesh from_json failed:work path need to be set, not allowed")
        # kivlin. todo
        value = StaticMesh.create_from_json(json_object, field_desc.work_path, field_desc.io_mode, input_name)
        return StaticMeshField(value)

    def get_input(self) -> StaticMesh:
        return self._value
