...
def _aopt(x_cand, pysr_model, batch_config):
    ...
    traces = np.empty((len(x_cand),))

    for i, cand in enumerate(x_cand):

        fim = get_fim(cand, pysr_model, single=True, difference=difference)
        traces[i] = np.trace(fim)
...
def _dopt(x_cand, pysr_model, batch_config):
    ...
    x_cand = np.array(x_cand)
    dets = np.empty((len(x_cand),))

    for i, cand in enumerate(x_cand):
        fim = get_fim(cand, pysr_model, single=True, difference=difference)
        dets[i] = np.linalg.det(fim)
...
def _eopt(x_cand, pysr_model, batch_config):
    ...
    x_cand = np.array(x_cand)
    mineigs = np.empty((len(x_cand),))

    for i, cand in enumerate(x_cand):
        fim = get_fim(cand, pysr_model, single=True, difference=difference)
        mineigs[i] = np.min(np.linalg.eigvals(fim))
