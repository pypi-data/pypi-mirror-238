...
from ._query_result_metadata import QueryResultMetadata
...
class QueryResult(pd.DataFrame):
    ...
    def __init__(
        ...
        *,
        ...
        self._atoti_metadata = QueryResultMetadata(
            context=context,
    ...
    @property
    def _atoti_metadata(self) -> Optional[QueryResultMetadata]:
        metadata = getattr(self, _METADATA_PROPERTY_NAME, None)

        if metadata is None:
            return None

        assert isinstance(metadata, QueryResultMetadata)
    ...
    def _atoti_metadata(self, value: QueryResultMetadata, /) -> None:
    ...
    def _repr_html_(self) -> Optional[str]:  # type: ignore[misc] # pyright: ignore[reportGeneralTypeIssues]
        html = self._atoti_metadata.formatted_values._repr_html_() if self._atoti_metadata else super()._repr_html_()  # type: ignore[misc, operator] # pyright: ignore[reportGeneralTypeIssues]
        assert isinstance(html, (type(None), str))
        return html

    @override  # `pandas-stubs` lacks the `_repr_latex_` method.
    def _repr_latex_(self) -> Optional[str]:  # type: ignore[misc]  # pyright: ignore[reportGeneralTypeIssues]
        latex = self._atoti_metadata.formatted_values._repr_latex_() if self._atoti_metadata else super()._repr_latex_()  # type: ignore[misc, operator] # pyright: ignore[reportGeneralTypeIssues]
    ...
    def _repr_mimebundle_(  # type: ignore[misc] # pyright: ignore[reportGeneralTypeIssues]
        ...
        if self._atoti_metadata and self._atoti_metadata.widget_conversion_details:
            mimebundle[_CONVERT_QUERY_RESULT_TO_WIDGET_MIME_TYPE] = asdict(
