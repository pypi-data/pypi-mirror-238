...
class AverageEpochs(InputOutputProcess):
    ...
    def get_snr_table(self):
        ...
        for _iw in range(len(roi_samples)):
            ...
            f_value, f_critic, df_num, df_den, p_value = f_test(
                snr=final_snr[:, _iw],
...
class ApplyFunctionToTimeROI(InputOutputProcess):
    ...
    def transform_data(self):
        ...
        for _iw in range(len(roi_samples)):
            ...
            _value = self.function_object(self.input_node.data[_ini: _end, ...])
            for _idx_ch, _ch in enumerate(self.input_node.layout):
                tests.append(TimeROIValue(function_value=_value[_idx_ch],
                                          label=_label,
                                          test_name=test_name,
                                          ini_time=_ini / self.input_node.fs,
                                          end_time=_end / self.input_node.fs,
    ...
    def get_roi_samples(self):
        if self.roi_windows is not None:
            ...
            roi_samples = [np.array([0, self.input_node.data.shape[0]])]
        return roi_samples
