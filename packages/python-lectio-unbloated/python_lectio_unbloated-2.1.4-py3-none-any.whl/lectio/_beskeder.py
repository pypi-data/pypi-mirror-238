...
def besked(self, message_id):
    ...
    for besked in beskederHtml:
        try:
            beskedOversigt[
                ...
                besked.find(
                    "div", {"class": "message-reply-summary viewed-persons-message"}
                )
    ...
    for besked in beskederHtml:
        ...
        beskedDict = {
            ...
            "padding_left": 0,
            "besked": besked.find(
                "div", {"class": "message-thread-message-content"}
            ).text.strip(),
            "vedhæftninger": [
                {
                    "navn": vedhæft.text,
...
def besvarBesked(self, message_id, id, titel, content, _from):
    content = (
        ...
        + "\n\n"
        + [
            "Sendt fra [url=github.com/BetterLectio/python-lectio] python-lectio [/url]",
    ...
    resp = self.session.post(
        f"https://www.lectio.dk/lectio/{self.skoleId}/beskeder2.aspx?type=showthread&id={message_id}&elevid={self.elevId}",
        data=payloadEncoded,
    )
    soup = BeautifulSoup(resp.text, "html.parser")
    ...
    ] = ""  # Skal tilføjes som option
    payload[
        "s$m$Content$Content$Notification"
    ] = "NotifyBtnAuthor"  # Skal tilføjes som option
    payload[
        "s$m$Content$Content$RepliesToResponseAllowed"
    ] = "on"  # Skal tilføjes som option
    ...
    resp = self.session.post(
        f"https://www.lectio.dk/lectio/{self.skoleId}/beskeder2.aspx?type=showthread&id={message_id}&elevid={message_id}",
        data=payloadEncoded,
