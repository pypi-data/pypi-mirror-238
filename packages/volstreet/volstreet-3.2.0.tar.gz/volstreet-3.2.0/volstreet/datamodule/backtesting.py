...
from volstreet.datamodule.eod_client import DataClient
import numpy as np
import pandas as pd
...
from volstreet.blackscholes import call, put, adjusted_iv_from_atm_iv
from volstreet.dealingroom import straddle_iv, calc_combined_premium
from volstreet.datamodule.db_manager import DBManager


@define
class BackTester:
    data_client: DataClient = field(factory=DataClient)
    db_manager: DBManager = field(factory=DBManager)
    ...
    def get_intraday_prices_on_expiry_day(
        self, underlying, one_min_df, start_time, end_time, between_hours, minutes
    ):
        """
        Get the intraday prices for the given expiry day.

        Parameters:
            underlying (str): The underlying index.
        ...
        """
        expiry_days = self.index_instances[underlying].expiry_dates
    ...
    def _fetch_option_prices_from_skeleton(
        ...
        query = self.db_manager.generate_query_for_option_prices_df(
            index=index,
            df=df,
        )

        option_prices = self.db_manager.fetch_option_prices_from_tsdb(query)
    ...
    def backtest_option_buying_strategy_on_expiry(
        ...
        day_info["strike"] = day_info.open.apply(lambda x: find_strike(x, strike_base))
        day_info["expiry"] = day_info.index.to_series().apply(
            lambda x: self.fetch_nearest_expiry_from_date(underlying, x).replace(
                hour=15, minute=30
            )
        )
    ...
    def backtest_intraday_strangle(
        ...
        day_info["start_time_to_expiry"] = day_info.date.apply(
            lambda x: self.historic_time_to_expiry(
                underlying, x.replace(hour=9, minute=15)
            )
        )

        day_info["end_time_to_expiry"] = day_info.date.apply(
            lambda x: self.historic_time_to_expiry(
                underlying, x.replace(hour=15, minute=30)
            )
        )

        day_info["traded_call_strike"] = (day_info.open * (1 + call_offset)).apply(
            lambda x: find_strike(x, strike_base)
        )
        ...
        day_info["start_premium"] = day_info.apply(
            lambda row: calc_combined_premium(
                ...
                call_iv=adjusted_iv_from_atm_iv(
                    row.iv,
                    row.traded_call_strike,
                    row.open,
                    row.start_time_to_expiry,
                ),
                put_iv=adjusted_iv_from_atm_iv(
                    row.iv,
