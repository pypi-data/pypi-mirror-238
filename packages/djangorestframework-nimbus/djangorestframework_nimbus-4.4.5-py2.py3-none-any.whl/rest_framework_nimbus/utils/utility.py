...
from django.utils.encoding import force_str, force_str, force_bytes
...
def uuid_filename_generator(instance, filename, path_format):
    ...
    basename = ''.join((force_str(uuid.uuid4()), filetype))

    dirname = force_str(datetime.datetime.now().strftime(force_str(path_format)))
