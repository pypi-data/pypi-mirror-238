...
# sources: unacast/catalog/v1/catalog.proto, unacast/catalog/v1/catalog_service.proto, unacast/catalog/v1/data_delivery.proto, unacast/catalog/v1/file_download_service.proto, unacast/catalog/v1/query_service.proto
...
class SearchMetricValuesRequest(betterproto.Message):
    ...
    """
    If true, the query will be executed even if the user filters on items it is
    not allowed to see
    """
    ...
    """
    Filter clause to limit the search to certain related Address Component
    Values.
    """
    ...
    """
    Filter clause to limit the search to certain Metric Values which have
    Numeric Value Kind(COUNT; NUMBER).
    """
...
class SearchLayerFeaturesRequest(betterproto.Message):
    ...
    """
    If the bounding box is set, only return Features that falls within or
    intersects the bounding box
    """

    query: str = betterproto.string_field(9)
    """
    free text search query. This only sorts by relevance, and does not filter
    out any results. Setting this works in combination with filters. The query
    has fuzziness with edit-distance of 2 per word and splits on whitespace.
...
class SearchLayerFeaturesResponse(betterproto.Message):
    ...
    """
    the score for each resulting record, zeros if not applicable higher score
    means higher relevance order is same as features, so match on index. e.g.
    features[0] has score[0]
    """
...
class QueryMetricFilters(betterproto.Message):
    ...
    related_address_component_filters: List[
        "__maps_v1__.AddressComponentFilter"
    ] = betterproto.message_field(7)
