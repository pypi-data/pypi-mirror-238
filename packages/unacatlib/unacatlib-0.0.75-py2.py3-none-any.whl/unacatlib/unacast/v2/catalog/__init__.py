...
class ListDataDeliveryInfoRequest(betterproto.Message):
    ...
    """
    SQL syntax, e.g. "last_activity desc" or "created_at ASC". Only one field
    can be used for sorting. Ascending is default. The default is
    "last_activity desc". Accepted values: - "last_activity": Last Activity,
    last DataDeliveryOrder `status_timestamp` - "created_at": Created at,
...
class DataDeliveryInfo(betterproto.Message):
    ...
    """
    True if the data delivery per its configuration has completed its final
    order. Typically this means that the configuration has an end-date and that
    this full date-range has been delivered.
    """
...
class ListDataDeliveryFilter(betterproto.Message):
    ...
    include_delivery_status_types: List[
        "DataDeliveryStatusType"
...
@dataclass(eq=False, repr=False)
class CreateSpecificDataDeliveryRequest(betterproto.Message):
    catalog_id: str = betterproto.string_field(1)
    metric_id: str = betterproto.string_field(2)
    billing_context: str = betterproto.string_field(4)
    filters: "DataDeliveryFilters" = betterproto.message_field(5)
    """Filters for this data delivery"""

    delivery_columns: List["DeliveryColumn"] = betterproto.message_field(6)
    storage_access_id: str = betterproto.string_field(13)
    file_config: "FileConfig" = betterproto.message_field(14)
...
class DataDeliveryFilters(betterproto.Message):
    ...
    related_address_component_filter: List[
        "_maps__.AddressComponentFilter"
    ] = betterproto.message_field(7)
    """
    Filter clause to limit the delivery to certain related Address Component
    Values.
    """
...
class DeliveryColumn(betterproto.Message):
    """
    Provided as a way of explicitly describe a column to deliver. Can translate
    to unacast.v2.metric.DataSchema that is usually provided with the metric
    creation, but removes the fuzz and allows ordering of columns
    """

    source_column: str = betterproto.string_field(1)
    """
    Structure: [Namespace].[column_id].[type] - to be provided alongside the
    Column in the default_schema to the frontend? Namespace because columns are
    sourced from different aspects and may not have unique IDs across asspects
    (address_component and features may both have a column with id X) Type
    because the same column can be included in different ways (ie id or value
    of related item). Maps internally to
    `unacast.v2.metric.ColumnKind`/`unacast.v2.metric.Column.type`
    """
...
class DataDeliveryServiceStub(betterproto.ServiceStub):
    ...
    async def create_specific_data_delivery(
        self,
        create_specific_data_delivery_request: "CreateSpecificDataDeliveryRequest",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
        ...
        return await self._unary_unary(
            "/unacast.v2.catalog.DataDeliveryService/CreateSpecificDataDelivery",
            create_specific_data_delivery_request,
            CreateDataDeliveryResponse,
            timeout=timeout,
            deadline=deadline,
            metadata=metadata,
...
class DataDeliveryServiceBase(ServiceBase):
    ...
    async def create_specific_data_delivery(
        self, create_specific_data_delivery_request: "CreateSpecificDataDeliveryRequest"
    ) -> "CreateDataDeliveryResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_create_specific_data_delivery(
        self,
        stream: "grpclib.server.Stream[CreateSpecificDataDeliveryRequest, CreateDataDeliveryResponse]",
    ) -> None:
        request = await stream.recv_message()
        response = await self.create_specific_data_delivery(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            ...
            "/unacast.v2.catalog.DataDeliveryService/CreateSpecificDataDelivery": grpclib.const.Handler(
                self.__rpc_create_specific_data_delivery,
                grpclib.const.Cardinality.UNARY_UNARY,
                CreateSpecificDataDeliveryRequest,
                CreateDataDeliveryResponse,
            ),
