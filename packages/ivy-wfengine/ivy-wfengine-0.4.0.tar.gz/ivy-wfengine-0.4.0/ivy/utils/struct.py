...
if sys.version_info.major < 3:
    ...
    class _Base(object, DictMixin):
        pass

else:

    class _Base(DictMixin):
        pass


class ImmutableStruct(_Base):
    """
    ...
    def __setitem__(self, name, val):
        raise IllegalAccessException(
            "Trying to modify immutable struct with: %s=%s" % (str(name), str(val))
        )
...
WorkflowState = Enum("RUN", "STOP", "EXIT", "RESUME")
