...
def _cmd_uncomment_selection():
    ...
    if text is not None:
        _uncomment_selection(text)
def _cmd_upload_images():
    ui_utils.show_dialog(ImageDialog(get_workbench()))
def _cmd_upload_images_plus():
    ...
    if file_path:
        ...
        # 指定目标目录
        target_dir = 'C:\latex_project'

        with open('C:\\test\\activeProject.txt', 'r') as f:
            content = f.read()
            if content:
                shutil.copy2(file_path, target_dir + '/' +content +'/'+ filename)
                messagebox.showinfo('成功', '图片上传成功')
            else:
                messagebox.showerror('错误', '你还没有选择项目')
...
def _cmd_install_images():
    text = _get_focused_writable_text()
    if text is not None:
        sel_range = _get_focused_code_range(text).lineno
        text.insert(str(sel_range) + ".0", '''
\includegraphics[width=1]{请写好路径}
''')
