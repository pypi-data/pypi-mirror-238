...
class Runner:
    ...
    # 检查是否有一个名为'isFirst.txt'的文件
    def check_is_first(self):
        if os.path.exists('C:\\test\\isFirst.txt'):
            return False
        else:
            # open('isFirst.txt', 'a')
            return True
    ...
    def _init_commands(self) -> None:
        ...
        get_workbench().add_command(
            ...
            # 使用cmd_run_current_script方法作为处理程序
        ...
        get_workbench().add_command(
            ...
            caption=RUN_COMMAND_CAPTION,
            # 使用cmd_run_current_script方法作为处理程序
            handler=self.cmd_run_current_script_latex,
            # 快捷键默认是<F5>
            default_sequence="<F9>",
            # 其他的额外的快捷键
            extra_sequences=[select_sequence("<Control-u>", "<Command-u>")],
            tester=self.cmd_run_current_script_enabled,
            # 菜单组是10
            group=10,
            # 图标是
        ...
        get_workbench().add_command(
            ...
            caption=RUN_COMMAND_CAPTION,
            # 使用cmd_run_current_script方法作为处理程序
            handler=self.cmd_run_current_add_latex,
            # 快捷键默认是<F5>
            default_sequence='<F5>',
            # 其他的额外的快捷键
            extra_sequences=[select_sequence("<Control-u>", "<Command-u>")],
        ...
        if self.check_is_first():
            get_workbench().add_command(
                ...
                caption=RUN_COMMAND_CAPTION,
                # 使用cmd_run_current_script方法作为处理程序
                handler=self.cmd_run_current_request_latex,
                # 快捷键默认是<F5>
                default_sequence='<F5>',
                # 其他的额外的快捷键
                extra_sequences=[select_sequence("<Control-u>", "<Command-u>")],
        ...
        get_workbench().add_command(
            ...
            caption=RUN_COMMAND_CAPTION,
            # 使用cmd_run_current_script方法作为处理程序
            handler=self.cmd_run_current_browser,
            tester=self.cmd_run_current_script_enabled,
            # 菜单组是10
            group=10,
            # 图标是
    ...
    def remove_txt_extension(self, string):
        if string.endswith(".tex"):
            return string[:-4]
        else:
            return string
    def cmd_run_current_request_latex(self):
        is_first = self.check_is_first()
        if not is_first:
            return messagebox.showerror('失败', '您已申请，无法再次申请')
        else:
            messagebox.showinfo('成功', '您已申请成功')
        user_uuid = str(uuid.uuid4()).replace('-', '')[0:6]
        req = Requester()
        res = req.post(url='/latex/create-directory?directory=/' + user_uuid)
        file_path = 'C:\\test\\isFirst.txt'
        # 检查文件是否存在，如果不存在则创建文件
    ...
    def cmd_run_current_add_latex(self):
        file_path = 'C:\\test\\isFirst.txt'
        if not os.path.exists(file_path):
            return messagebox.showerror('错误', '请先申请 latex 权限')
        self.add_root = tk.Tk()
        ...
        self.folder_name_var.set("请输入项目名称:")

        label = tk.Label(frame, text="请输入项目名称:")
        ...
        button = tk.Button(frame, text="创建", command=self.addProject)
        button.grid(row=2, column=0, sticky='nsew')
        self.add_root.mainloop()

    def is_chinese_string(self, s):
        """
        判断一个字符串是否为中文字符串
        """
        for c in s:
            if not (0x4E00 <= ord(c) <= 0x9FFF):  # 判断字符是否为中文字符
                return False
        return True

    def addProject(self):
        folder_name = self.add_entry.get()  # 获取输入框的值
        if self.is_chinese_string(folder_name):
            return messagebox.showerror('错误', '请输入英文')
        self.add_root.destroy()
        if not os.path.exists('C:\\latex_project\\' + folder_name):
            os.makedirs('C:\\latex_project\\' +folder_name)
            with open('C:\\test\\isFirst.txt', 'r') as f:
                content = f.read()
                print(content)
                req = Requester()
                res = req.post(url='/latex/create-project?project=/'+content+'/'+folder_name)
                messagebox.showinfo('成功', '项目创建成功')

        else:
            messagebox.showerror('错误', '项目已存在')

    def cmd_run_current_script_latex(self) -> None:
        file_path = 'C:\\test\\isFirst.txt'
        if not os.path.exists(file_path):
            return messagebox.showerror('错误', '请先申请 latex 权限')
        ...
        for file in os.listdir(current_dir):
            # 检查文件是否不以.py结尾且不是.db结尾，且是普通文件
            if not file.endswith('.py') and not file.endswith('.db') and not file.endswith('.ini') and not file.endswith('.jpg') and not file.endswith('.png') and not file.endswith('VERSION') and os.path.isfile(
        ...
        if get_workbench().in_simple_mode():
            get_workbench().hide_view("VariablesView")
        report_time("Before Run")
        content = get_workbench().get_editor_notebook().get_current_editor().get_content()
        name = str(uuid.uuid4()).replace('-', '')[0:8]
        ...
        with open('C:\\test\\activeProject.txt', 'r') as f:
            content = f.read()
            if content:
                res_path = 'C:\latex_project\\' + content
                self.list_files(res_path)
                name_list = res_path.split('\\')
                project_name = name_list[len(name_list) - 1]
                with open('C:\\test\\isFirst.txt', 'r') as f:
                    content = f.read()
                    if content:
                        user_latex_id = content
                requests.post('http://180.76.235.69:22001/latex/upload',
                                    data={'filePath': '/' + user_latex_id + '/' + project_name},
                                    files={'file': open(name+ '.tex', 'rb')})
                compile_res = requests.post('http://180.76.235.69:22001/latex/compile?filePath='+'/' + user_latex_id + '/' + project_name+'/'+name+ '.tex')
                print(compile_res)
                if compile_res.status_code != 200:
                    messagebox.showerror('失败', '编译失败')
                    return
        pdf_file_path = '/' + user_latex_id + '/' + project_name+'/'+ name + '.pdf'
        file_result = requests.get(url='http://180.76.235.69:22001/latex/getPDFFile?filename='+pdf_file_path)
        # 获取文件名
        ...
        for root, dirs, files in os.walk(dir_path):
            for file in files:
                ...
                if not file.endswith('.tex'):
                    print('/' + user_latex_id + '/'+project_name)
                    image_path = dir_path + '\\' + file
                    res = requests.post('http://180.76.235.69:22001/latex/upload',
                                        data={'filePath': '/' + user_latex_id + '/'+ project_name},
                                        files={'file': open(image_path, 'rb')})
    def cmd_run_current_browser(self):
        # 打开窗体
        get_workbench().show_view("BrowserView", set_focus=False)
        # 渲染视图
        get_workbench().event_generate("PreBrowserView", content='传递过来的内容')

    def compile_latex(self, name):
        # try:
        #     # 使用 pdflatex 编译 LaTeX 文件并指定输出目录
        #     if running_on_linux() or running_on_mac_os():
        #         process = subprocess.Popen(["pdflatex", name + ".tex"])
        #     else:
        #         process = subprocess.Popen(["pdflatex", name + ".tex"],  shell=True)  # Windows
        #     while True:
        #         try:
        #             process.wait(timeout=5)  # 设置超时时间为5秒
        #             break
        ...
        #         raise Exception('Compile failed with error code: ' + str(process.returncode))
        #
        #     # messagebox.showinfo('成功', '编译成功，请看右边')
        #     get_workbench().show_view("LatextView", set_focus=False)
        #     get_workbench().event_generate("PreviewLatex", content=name + '.pdf', type='yp')
        # except Exception as e:
        #     messagebox.showerror('错误', '你书写的latex存在语法错误，请检查')
        #     pro = 'taskkill /f /im pdflatex.exe'
        #     os.system(pro)
        get_workbench().show_view("LatextView", set_focus=False)
        get_workbench().event_generate("PreviewLatex", content=name + '.pdf', type='yp')
