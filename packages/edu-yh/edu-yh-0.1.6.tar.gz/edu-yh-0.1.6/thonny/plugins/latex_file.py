...
class FilesView1(tk.PanedWindow):
    ...
    def on_secondary_click(self, event):
        ...
        if is_exist:
            ...
            if not path_name == '':
                self.menu.add_command(label='使用电脑默认程序打开', command=lambda :open_with_default_app(path_name))
    ...
    def add_node(self):
        ...
        for item in os.listdir(root_path):
            ...
            if os.path.isdir(folder_path):
                ...
                root_node = self.tree.insert("", "end", text=item, iid=item)
                for i in os.listdir(folder_path):
                    children_arr = self.file_obj[len(self.file_obj) - 1]['children']
                    children_arr.append({'name': i})
                    self.tree.insert(root_node, 'end', text=i, iid=i)
