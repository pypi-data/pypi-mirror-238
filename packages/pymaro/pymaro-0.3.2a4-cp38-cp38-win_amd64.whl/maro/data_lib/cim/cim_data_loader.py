...
def _load_vessels(data_folder: str) -> (Dict[str, int], List[VesselSetting]):
    vessel_mapping: Dict[str, int] = {}
    vessels: List[VesselSetting] = []

    vessels_file_path = os.path.join(data_folder, "vessels.csv")

    for line in _read_csv_lines(vessels_file_path):
...
def _load_vessel_period(data_folder: str) -> List[int]:
    vessels_file_path = os.path.join(data_folder, "vessels.csv")
...
def _load_routes(data_folder: str) -> (Dict[str, int], List[List[RoutePoint]]):
    route_mapping: Dict[str, int] = {}
    routes: List[List[RoutePoint]] = []

    route_file_path = os.path.join(data_folder, "routes.csv")

    for line in _read_csv_lines(route_file_path):
...
def _load_stops_from_csv(stops_file_path: str, vessel_number: int) -> List[List[Stop]]:
    stops: List[List[Stop]] = []

    for _ in range(vessel_number):
        stops.append([])

    for line in _read_csv_lines(stops_file_path):
...
def _load_stops(data_folder: str, vessel_number: int) -> List[List[Stop]]:
    bin_path = os.path.join(data_folder, "stops.bin")
    if os.path.exists(bin_path):
        return _load_stops_from_bin(bin_path, vessel_number)
    else:
        print(f"No stops binary file in {data_folder}, read from csv file instead...")
        csv_path = os.path.join(data_folder, "stops.csv")
        return _load_stops_from_csv(csv_path, vessel_number)
...
def _load_order_proportions(data_folder: str) -> Dict[int, List[NoisedItem]]:
    ...
    for line in _read_csv_lines(proportion_file_path):
...
def _load_ports_dump(
    ...
    for line in _read_csv_lines(ports_file_path):
        port_name = line["name"]
...
def _load_ports_real(data_folder: str) -> Tuple[dict, List[PortSetting]]:
    ports_file_path = os.path.join(data_folder, "ports.csv")
...
def _load_orders_from_csv(order_file_path: str) -> Dict[int, List[Order]]:
    orders: Dict[int, List[Order]] = {}

    for line in _read_csv_lines(order_file_path):
        tick = int(line["tick"])
...
def load_from_folder(source_folder: str) -> CimSyntheticDataCollection:
    ...
    """
    # load from files
    misc_items = _load_misc(source_folder)
    order_target_proportion = _load_order_proportions(source_folder)
    port_mapping, ports = _load_ports_dump(source_folder, order_target_proportion)
    route_mapping, routes = _load_routes(source_folder)
    vessel_mapping, vessels = _load_vessels(source_folder)
    periods_without_noise = _load_vessel_period(source_folder)
    stops = _load_stops(source_folder, len(vessels))
    global_order_proportions = _load_global_order_proportions(source_folder)
...
def load_real_data_from_folder(source_folder: str) -> CimRealDataCollection:
    ...
    # load from files
    misc_items = _load_misc(source_folder)
    port_mapping, ports = _load_ports_real(source_folder)
    route_mapping, routes = _load_routes(source_folder)
    vessel_mapping, vessels = _load_vessels(source_folder)
    periods_without_noise = _calculate_vessel_period(vessels, routes, route_mapping)
    stops = _load_stops(source_folder, len(vessels))
    orders = _load_orders(source_folder)

    # construct data collection
    # NOTE: this is a namedtuple, so out-side cannot change it
    return CimRealDataCollection(
        port_settings=ports,
        port_mapping=port_mapping,
        vessel_settings=vessels,
        vessel_mapping=vessel_mapping,
