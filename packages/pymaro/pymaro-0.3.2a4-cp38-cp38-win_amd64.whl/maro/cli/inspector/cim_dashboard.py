...
def render_inter_view(source_path: str, epoch_num: int):
    ...
    attribute_option_candidates = CIMItemOption.quick_info + CIMItemOption.port_info + CIMItemOption.booking_info
...
def render_intra_view(source_path: str, epoch_num: int, prefix: str):
    ...
    attribute_option_candidates = CIMItemOption.quick_info + CIMItemOption.booking_info + CIMItemOption.port_info
...
def _render_intra_view_by_ports(
    ...
    _generate_intra_panel_accumulated_by_ports(
        data_ports,
        f"ports_{selected_port}",
        snapshot_num,
        selected_snapshot_sample_ratio,
    )
    # Detailed data.
...
def _generate_intra_panel_by_ports(
    data: pd.DataFrame,
    option_port_name: str,
    snapshot_num: int,
    snapshot_sample_num: float,
...
def _generate_intra_panel_vessel(data_vessel: pd.DataFrame, snapshot_index: int, vessels_num: int):
    ...
    data_vessel = data_vessel[data_vessel["frame_index"] == snapshot_index][CIMItemOption.vessel_info].reset_index(
        drop=True,
    )
...
def _generate_intra_heat_map(matrix_data: str):
    ...
    transfer_volume_heat_map = (
        alt.Chart(data_transfer_volume)
        .mark_rect()
        .encode(
            x="Dest_Port:O",
            y="Start_Port:O",
