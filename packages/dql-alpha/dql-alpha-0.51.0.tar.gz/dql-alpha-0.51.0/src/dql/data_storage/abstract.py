...
class AbstractDataStorage(ABC):
    ...
    def __init__(self, uri: str = "", partial_id: Optional[int] = None):
    ...
    @abstractmethod
    def clone(
        self, uri: str = "", partial_id: Optional[int] = None
    ) -> "AbstractDataStorage":
    ...
    @abstractmethod
    def init_db(self, uri: str, partial_id: int) -> None:
    ...
    def partials_table(self, uri: str):
    ...
    def dataset_rows(self, dataset_name: str, version: Optional[int] = None):
        ...
        return self.schema.dataset_row_cls(name, self.ddb.engine, self.ddb.metadata)
    ...
    def dataset_select_paginated(
        ...
        """
        This is equivalent to ddb.execute, but for selecting dataset rows in batches
        """
    ...
    def partials_table_name(self, uri: str) -> str:
    ...
    def get_next_partial_id(self, uri: str) -> int:
        if not uri:
    ...
    def get_valid_partial_id(
        self, uri: str, prefix: str, raise_exc: bool = True
    ) -> Optional[int]:
    ...
    def get_nodes_by_parent_id(
        ...
        query = n.select(*n.default_columns()).where(
            (n.c.is_latest == true()) & (n.c.parent_id == parent_id)
        )
    ...
    def select_node_fields_by_parent_id(
        ...
        return self.ddb.execute(
            n.select(*(getattr(n.c, f) for f in fields)).where(
                (n.c.parent_id == parent_id) & (n.c.is_latest == true())
            )
        )
    ...
    def update_partition_unique_id(self, tbl):
        ...
        rows_count = next(self.ddb.execute(select(func.count()).select_from(tbl)))[0]
        update_values = {
            PARTITION_UNIQUE_ID: tbl.c[PARTITION_COLUMN_ID] * rows_count + tbl.c.id,
        }
        self.ddb.execute(tbl.update().values(**update_values))
