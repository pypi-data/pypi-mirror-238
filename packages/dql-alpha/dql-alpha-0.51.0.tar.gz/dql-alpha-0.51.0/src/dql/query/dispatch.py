...
def udf_entrypoint() -> int:
    from dql.query.dataset import UDF
    ...
    for udf_output in udf_results:
        ...
        UDF.add_defaults(new_rows, udf)
        rows.extend(new_rows)
        if len(rows) > DEFAULT_BATCH_SIZE:
            for row_chunk in chunk(rows, DEFAULT_BATCH_SIZE):
                db_adapter.insert_rows(table, row_chunk)
            rows.clear()
...
class UDFDispatcher:
    ...
    def _call_udf(self, row):
        ...
        return self.udf(self.catalog, row, output_rows=self.is_generator)
