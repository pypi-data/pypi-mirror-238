...
from dql.sql.types import JSON, Array, Boolean, DateTime, Float, Float32, Int, String
...
def test_row_generator_with_limit(cloud_test_catalog, dogs_shadow_dataset):
    ...
    # one original row + two generated ones out of it
    assert len(parents_names) == 3
    # this depends on original ordering of the rows in a dataset table which
    # can differ in some databases other than SQLite (where it's always ordered
    # as inserted)
    input_row_parent = parents_names[0][0]
    input_row_name = parents_names[0][1]

    assert parents_names == [
        (input_row_parent, input_row_name),
        (f"{input_row_parent}/{input_row_name}", "subobject"),
        (f"{input_row_parent}/{input_row_name}", "subobject2"),
    ]
...
def test_row_generator_with_new_columns(cloud_test_catalog, dogs_shadow_dataset):
    # TODO remove when there is unification of empty values for sql types for all DBs
    skip_if_not_sqlite()
    # Setup catalog.
    catalog = cloud_test_catalog.catalog
    catalog.client_config = cloud_test_catalog.client_config
    ...
    assert col_values == [
        ("dogs", "dog1", None, None, None, None, None, None),
        ("dogs", "dog2", None, None, None, None, None, None),
        ("dogs", "dog3", None, None, None, None, None, None),
        ("dogs/others", "dog4", None, None, None, None, None, None),
        ("dogs/dog1", "subobject", 10, 1, 0.5, dumps({"a": 1}), now, dumps([0.8, 0.9])),
        ("dogs/dog2", "subobject", 10, 1, 0.5, dumps({"a": 1}), now, dumps([0.8, 0.9])),
        ("dogs/dog3", "subobject", 10, 1, 0.5, dumps({"a": 1}), now, dumps([0.8, 0.9])),
        (
            "dogs/others/dog4",
            "subobject",
            10,
...
def test_json_loader(cloud_test_catalog):
    ...
    @udf(
        parameters=(C.ext, LocalFilename("*.json")),
        output=json_output,
    )
    def attach_json(rows):
        ...
        signal_names = [name for (name, _) in json_output]
        signals = []
        for ext, _ in rows:
            if json_data and ext != ".json":
                signals.append([json_data.get(k) for k in signal_names])
            else:
                signals.append(None)
