...
from dql.data_storage.abstract import (
    SELECT_BATCH_SIZE,
    AbstractDataStorage,
    DatabaseEngine,
)
...
class SQLiteDataStorage(AbstractDataStorage):
    ...
    def dataset_select_paginated(
        ...
        while True:
            if limit:
                limit -= num_yielded
                if limit == 0:
                    # we fetched all results
                    break
                if limit < page_size:
                    paginated_query = paginated_query.limit(None).limit(limit)

            results = self.ddb.execute(paginated_query.where(order_by_col > next_id))
            new_next_id = next_id
    ...
    def _get_dataset_row_values(
        ...
        rows = self.ddb.execute(query, cursor=cur)
        for row in rows:
            yield dict(row)
