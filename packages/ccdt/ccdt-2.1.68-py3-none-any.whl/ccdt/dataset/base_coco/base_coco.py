...
class BaseCoco(BaseLabelme):
    ...
    def add_annotation(self, image_id: int, category_id: int, segmentation: list, bbox: list, ann_id: int = None, image_w_h: tuple = (), score: str = ''):
        """
        追加coco标注属性
        :param image_id:图像id
    ...
    def self2coco(self):
        ...
        约定规则，
        1、多边形和矩形框组合，存在关系需要打组。（用于抠出一个组的标注，即车中包含车牌，然后用多边形车牌转coco）
        2、关键点和矩形框组合，存在关系需要打组。（用于直接转换coco）
        3、coco中segmentation存储，车牌关键点标注坐标、车牌多边形点标注坐标，bbox存储车牌矩形框标注坐标及车牌多边形的点转矩形框的坐标。其中属于同一个组的，需要把两个shape合并为一个coco注释
        4、车牌多边形的点转矩形框
        6、车牌（多边形）和车（矩形框）两个shape组合为一个注释
        7、车牌（关键点）和车牌（矩形框）两个shape组合为一个注释
        8、没有组合的就直接一个标注shape转换为一个注释
        9、目前多边形标注的还有关键点标注，统一放到segmentation中存储
        10、矩形框的标注直接转coco,一个shape一个注释
        """
        ...
        for dataset in tqdm(self.datasets):
            ...
            if dataset.get('background') is False:  # 为真表示为处理背景图片
                ...
                for ann in one_img_ann_list:  # 一张图像的多个标注及组合标注在这里进行迭代，迭代前必须组合完成
                    category_id = 0
                    ...
                    self.add_annotation(self.result_addImage_id, category_id, ann.get('segmentation'), ann.get('bbox'), None, score=ann.get('score'))
    ...
    def labelme_shapes2coco_ann(self, dataset):
        ...
        for structure_shape_key, structure_shape_value in structure_shapes.items():
            ...
            if isinstance(structure_shape_key, int):  # 处理打组的数据
                ...
                # if len(structure_shape_value) == 5:  # 长度等于5，关键点和矩形框分组，转换为一个coco注释
                # 通过返回的shape_type来判断属于那种组合，是否需要追加到one_img_ann_list
                result = self.shape_processing(structure_shape_value, dataset, one_img_ann_list, point_list, rebuild_polygon_point_shape)
                # 关键点和矩形框分组，转换为一个coco注释。or 多边形且标签为车牌，同时存在分组，转换为一个coco注释。
                if ("point" and "polygon" and "plate" in result) and (len(result) == 3) or ("polygon" and "plate" in result) and (len(result) == 2):
                    if len(point_list) != 0:
                        convert_segmentation = sum(segmentation, point_list)  # 把列表中存储的多个元组元素，合并成一个列表且保持原有排列顺序
                        rebuild_polygon_point_shape.update({'segmentation': convert_segmentation})  # 新增关键点列表，用于segmentation赋值
                    one_img_ann_list.append(rebuild_polygon_point_shape)  # 把标注shape追加到转coco时的迭代列表集合中
                # if ("polygon" and "plate" in result) and (len(result) == 2):  # 多边形且标签为车牌，同时存在分组，转换为一个coco注释
                # one_img_ann_list.append(rebuild_polygon_point_shape)  # 把标注shape追加到转coco时的迭代列表集合中
                else:
                    print("分组的labelme转换为coco数据存在未按照约定规则进行处理，暂时跳过转换！当前约定规则为（point，polygon，plate，有分组）（polygon，plate，有分组）")
                    continue
            if structure_shape_key is None:  # 处理不打组的数据，即分组键为None，只有矩形框未分组的可以转coco注释
    ...
    def shape_processing(self, structure_shape_value, dataset, one_img_ann_list, point_list, rebuild_polygon_point_shape):
        """
        ...
        @param dataset:
        @param structure_shape_value:
        @param rebuild_polygon_point_shape:
        """
        shape_type = list()
        for shape in structure_shape_value:
            if shape.setdefault('shape_type') == 'polygon':
                res = self.polygon_shape(shape, dataset, one_img_ann_list, rebuild_polygon_point_shape)
                shape_type.append(shape.setdefault('shape_type'))
                shape_type.append(res)
            if shape.setdefault('shape_type') == 'rectangle':  # 目前存在组合：1、多边形和矩形框组合。2、点和矩形框组合
                res = self.rectangle_shape(shape, dataset, one_img_ann_list, rebuild_polygon_point_shape)
                shape_type.append(shape.setdefault('shape_type'))
                shape_type.append(res)
            if shape.setdefault('shape_type') == 'point':
                self.point_shape(shape, dataset, one_img_ann_list, point_list)
                shape_type.append(shape.setdefault('shape_type'))
        return set(shape_type)

    def polygon_shape(self, shape, dataset, one_img_ann_list, rebuild_polygon_point_shape):
        """
        处理标注形状为多边形的shape
        :param shape: 标注属性字典
        ...
        if len(shape['points']) == 4 and shape["group_id"] is not None:  # 多边形点为4个，分组不为空，即为多边形车牌转coco，把多边形变矩形框存储bbox，多边形的点存储segmentation
            ...
            if self.rectangle_cross_the_border(bbox, dataset, True):  # 如果函数返回为True，表示标注形状已经超出图像边界
                ...
                try:
                    ...
                    # 内存里更新shape标注属性
                    rebuild_polygon_point_shape.update({'bbox': bbox})  # 更新多边形点的坐标为矩形框坐标
                    rebuild_polygon_point_shape.update({'label': shape['label']})
                    if 'text' in shape:  # 判断 'text' 键是否存在
                        if shape['text'] is not None:  # 判断 'text' 值是否存在
                            rebuild_polygon_point_shape.update({'score': shape['text']})  # 如果阈值存在，就把阈值添加进去
                    rebuild_polygon_point_shape.update({'segmentation': convert_segmentation})  # 新增关键点列表，按照左上、右上、右下、左下的顺序排序
                    return shape['label']
        ...
        else:  # 语义分割转coco就会来这里，目前不做逻辑处理，数据结构未知
            print(f'车牌多边形标注不为4个点且标注未分配组id，请矫正数据，已经存储到自定义默认错误数据集目录，修改完成后覆盖真实数据集，并重新转换')
    ...
    @staticmethod
    def rectangle_shape(shape, dataset, one_img_ann_list, rebuild_polygon_point_shape):
        """
        处理标注形状为矩形的shape
        :param shape: 标注属性字典
        :param dataset:一张图像属性封装字典集合
        ...
        """
        # 矩形框标注打组与不打组处理逻辑都相同
        # segmentation = self.find_rect_sequential_coordinates(shape, dataset.get('full_path'))  # 这里是把矩形框转成了左上、右上、右下、左下的顺序排列4个顶点
        # convert_segmentation = sum(segmentation, [])  # 把列表中存储的多个元组元素，合并成一个列表且保持原有排列顺序
        # rebuild_rectangle_shape = {}  # 存储shape重构为coco标注属性字典变量
        rebuild_polygon_point_shape.update({'bbox': shape['points']})  # 更新多边形点的坐标为矩形框坐标
        rebuild_polygon_point_shape.update({'label': shape['label']})
        if 'text' in shape:  # 判断 'text' 键是否存在
            if shape['text'] is not None:  # 判断 'text' 值是否存在
                rebuild_polygon_point_shape.update({'score': shape['text']})  # 如果阈值存在，就把阈值添加进去
        ...
        # one_img_ann_list.append(rebuild_polygon_point_shape)  # 矩形框可以不做处理，直接追加标注
    ...
    def point_shape(self, shape, dataset, one_img_ann_list, point_list):
        ...
        if self.group_id_mark:  # 处理打组的shape
            point_list.append(shape['points'][0][0])  # 追加点标注
            point_list.append(shape['points'][0][1])  # 追加点标注
        ...
        #         # segmentation = self.find_poly_sequential_coordinates(rebuild_point_shape, dataset.get('full_path'))  # 这里把点标注进行排序，目前不需要了
        #         # convert_segmentation = sum(segmentation, [])  # 把列表中存储的多个元组元素，合并成一个列表且保持原有排列顺序
