...
def getAssetIds(CREDS, assetType, logger):
    try:
        ...
        if assetType is None:
            assetids="SELECT [AssetTypeId] FROM [dbo].[mstAssetType] WHERE [Name] IN ('Turbine','Production meter','Met mast','Inverter','Device group','Grid meter','Combiner box','String','Virtual Meteo Sensor')"
        else:
        ...
        while errors<3:
            try:
                ...
                with pyodbc.connect(conn_string) as conn:
                    with conn.cursor() as cursor:
                        #sqlSelect=f"DECLARE @assetid INT=(SELECT [AssetTypeId] FROM [dbo].[mstAssetType] WHERE [Name] IN ('{assetType}'));"
