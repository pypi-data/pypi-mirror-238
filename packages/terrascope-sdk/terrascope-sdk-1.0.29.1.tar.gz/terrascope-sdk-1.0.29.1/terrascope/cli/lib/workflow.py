...
def check_environment_complete(raise_on_failure=False, print_missing=True, verbose=False):
    ...
    missing = set()
    required_environment = ['TERRASCOPE_API_TOKEN', 'TERRASCOPE_API_ADMIN_TOKEN', 'TERRASCOPE_HOST']
    var_list = []
...
def create_algorithm_manifest(manifest_dict):
    ...
    # parameters
    for parameter in manifest_dict['parameters']:
...
async def create_algorithm_config(
    ...
    config.add_data_source(data_type=data_type, data_source=data_source,
...
async def download_algorithm_computation_result(
    ...
    Args:
        algorithm_computation_id (str): The ID of the completed algorithm computation.
        file_path (str, optional): The file path to save the downloaded results. Defaults to "test.zip".
...
async def download_analysis_computation_result(
        analysis_computation_id: str,
        file_path: str = "test.zip"
):
    """
    Download results from a completed analysis computation.

    This asynchronous function downloads the results from a completed analysis computation
    identified by the provided analysis computation ID. The results will be saved to the specified
    file path, with a default name of "test.zip" if not provided.

    Args:
        analysis_computation_id (str): The ID of the completed analysis computation.
        file_path (str, optional): The file path to save the downloaded results. Defaults to "test.zip".
    ...
    """
    if 1:
        raise RuntimeError("Direct download of analysis is not Implemented yet")
    else:
        sdk = TerraScopeSDK()
        await sdk.result.wait_and_download(algorithm_computation_ids=[analysis_computation_id],
                                           output=file_path)
    return file_path
