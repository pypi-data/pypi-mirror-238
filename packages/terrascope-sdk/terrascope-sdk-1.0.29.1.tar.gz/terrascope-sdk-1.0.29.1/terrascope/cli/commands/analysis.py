...
def analysis_get(ctx, analysis_id, analysis_name, truncate=False, verbose=False):
    ...
    for analysis in analyses:
        t = tsu.protobuf_to_dict(analysis).get('created_on', '        ')[:-8]
        data.append([analysis.name, analysis.id, t, analysis.author])
...
@click.pass_context
def analysis_update(ctx):
    click.echo("I'm sorry.  ana 'update' is not yet implemented")
