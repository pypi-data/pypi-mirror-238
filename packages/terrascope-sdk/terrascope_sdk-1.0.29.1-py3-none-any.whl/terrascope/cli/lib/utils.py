...
def set_version(version_input, manifest, do_overwrite=False):
    ...
    major, minor, patch = [int(v) for v in manifest_version.split('.')]
    if version_input:
        if version_input == 'patch':
            patch += 1
        elif version_input == 'minor':
            minor += 1
            patch = 0
        elif version_input == 'major':
            major += 1
            minor = 0
            patch = 0
        else:
            major, minor, patch = [int(v) for v in version_input.split('.')]
