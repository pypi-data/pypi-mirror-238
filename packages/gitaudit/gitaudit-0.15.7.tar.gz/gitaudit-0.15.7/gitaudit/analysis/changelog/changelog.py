...
from gitaudit.branch.hierarchy import linear_log_to_hierarchy_log, iter_commits
...
class GithubChangeLog:
    ...
    def get_change_log(  # noqa: C901
        ...
        first_parent=False,
        assert_start_ref_in_first_parent_log=False,
    ):
        """
        ...
        if assert_start_ref_in_first_parent_log:
            p_log = git.log_parentlog(end_ref)
            assert any(
                [entry.sha == start_ref for entry in p_log]
            ), f"Commit {start_ref} not found in parent log of {end_ref}!"

            hp_log = linear_log_to_hierarchy_log(p_log)
            fp_log_shas = [entry.sha for entry in hp_log]

            commit_found = False
            for commit in iter_commits(hp_log):
                if commit.sha == start_ref:
                    commit_found = True

                if commit_found and commit.sha in fp_log_shas:
        ...
        if has_submodules and follow_submodules:
            ...
            for submodule_start, submodule_end in both_submodules:
                ...
                submodule_changelog = self.get_change_log(
                    ...
                    assert_start_ref_in_first_parent_log=assert_start_ref_in_first_parent_log,
                )
