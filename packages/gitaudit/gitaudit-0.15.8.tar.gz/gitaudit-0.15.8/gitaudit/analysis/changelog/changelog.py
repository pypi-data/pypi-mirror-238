...
class GithubChangeLog:
    ...
    def _get_git_controller(self, owner, repo, commit_url_provider=None, issues_provider=None):
    ...
    def _get_commit(self, owner: str, repo: str, ref: str, git: Git) -> Commit:
        if "@" in ref:
            ...
            ref = git.get_last_sha_before(branch_name, date_time)
    ...
    def get_preprocessed_change_log(  # noqa: C901
        ...
        git, _ = self._get_git_controller(owner, repo, commit_url_provider, issues_provider)
        ...
        try:
            ...
            raise GitauditError(
                (f'Start expression "{start_ref}" does not exist in ' f'repository "{owner}/{repo}"')
            ) from esc
        ...
        try:
            ...
            raise GitauditError(
                (f'End expression "{end_ref}" does not exist in ' f'repository "{owner}/{repo}"')
            ) from esc
        ...
        fp_log = git.log_parentlog(end_ref, first_parent=True)
        if not any([entry.sha == start_commit.oid for entry in fp_log]):
            raise GitauditError(f'Commit "{start_ref}" not found in first parent log of "{end_ref}"!')
    ...
    def get_change_log(  # noqa: C901
        ...
        git, ssh_url = self._get_git_controller(owner, repo, commit_url_provider, issues_provider)
