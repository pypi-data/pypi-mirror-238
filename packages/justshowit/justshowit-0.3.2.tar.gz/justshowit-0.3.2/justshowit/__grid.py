...
def _resize_images(images: List[np.ndarray], config: Dict):
    ...
    for i, image in enumerate(images):
        ...
        if min_width and (min_width > w):
            image, h, w = __image_modifier.resize_image_respect_aspect_ratio(image, min_width, None, interpolation)
        if min_height and (min_height > h):
            image, h, w = __image_modifier.resize_image_respect_aspect_ratio(image, None, min_height, interpolation)
        if max_width and (max_width < w):
            image, h, w = __image_modifier.resize_image_respect_aspect_ratio(image, max_width, None, interpolation)
        if max_height and (max_height < h):
            image, h, w = __image_modifier.resize_image_respect_aspect_ratio(image, None, max_height, interpolation)
...
def _check_dict(image_spacing:dict, image_resizing:dict, image_text_config:dict, col_text_config:dict, row_text_config:dict, image_area_margins:dict, image_border:dict, drop_down_shadow:dict):
    ...
    if image_spacing.get("sep_line_color"):
        __checker.assert_color(image_spacing["sep_line_color"], "RGBA", 'image_spacing["sep_line_color"]')
    ...
    for (dict_to_check, dict_name, legal_placements) in to_check:
        ...
        if dict_to_check.get("color"):
            __checker.assert_color(dict_to_check["color"], "RGB", 'dict_to_check["color"]')
        if dict_to_check.get("placement"):
            __checker.assert_in(dict_to_check["placement"], legal_placements, f"{dict_name}['placement']")
    ...
    if image_border is not None:
        if image_border.get("color"):
            __checker.assert_color(image_border["color"], "RGB", 'image_border["color"]')
        __checker.assert_dict_types(
    ...
    if drop_down_shadow is not None:
        if drop_down_shadow.get("color"):
            __checker.assert_color(drop_down_shadow["shadow_color"], "RGB", 'drop_down_shadow["shadow_color"]')
        __checker.assert_dict_types(
...
def _get_font_grid(text, placement: str, font_size: int, font_thickness: int, italic: bool, color, adjust_draw_distance: int):
    ...
    font = __utils.get_font(font_size, font_thickness, italic)
...
def show_grid_configurable(
        ...
        c_image_border:       Optional[Dict] = {"border_size":1, "color":(200, 200, 200), "add_padding_on_the_outside":False},
        c_drop_down_shadow:   Optional[Dict] = {"shadow_offset_x":-1, "shadow_offset_y":-1, "shadow_color":(200, 200, 200), "iterations":3, "blur_kernel":(7, 7), "shadow_margins_lrtb":(25, 25, 25, 25)}
) -> Optional[ndarray]:
    ...
    __checker.assert_valid_universal_inputs(title, display_image, return_image, max_output_image_size_wh, save_image_path, resize_factor, BGR2RGB, parse_image)
    ...
    if (not_iterable and (not is_tensor) and (not is_numpy)) or (image_source is None):
        ...
        if (is_numpy or is_tensor) and (not parse_image):
            ...
            warnings.warn(msg)
        if isinstance(image_source, torch.Tensor) and __checker.assumed_torch_batch(image_source):
            image_source = __parser.parse_torch_image_batch_as_uint8_rgb_numpy_array(image_source, resize_factor, BGR2RGB)
        elif isinstance(image_source, np.ndarray) and __checker.assumed_numpy_batch(image_source):
            image_source = __parser.parse_numpy_image_batch_as_uint8_rgb_numpy_array(image_source, resize_factor, BGR2RGB)
        else:
        ...
        if parse_image:
            for i, image in enumerate(image_source):
                    image_source[i] = __parser.parse_image_as_uint8_rgb_numpy_array(image, resize_factor, BGR2RGB)
        else:
            image_source = __parser.quick_parse_image_source(image_source)
    ...
    if max_amount_reached:
        if (rows is None) and (cols is None):
            images = __parser.threshold_image_count(images)
        else:
    ...
    # List type checks
    if image_text: __checker.assert_list_types(image_text, str, "image_text")
    if col_text: __checker.assert_list_types(col_text, str, "col_text")
    if row_text: __checker.assert_list_types(row_text, str, "row_text")
    ...
    if c_drop_down_shadow is not None:
        ...
        canvas = __image_modifier.add_square_dropdown_shadow(canvas, shadow_upper_left_corner_xy, shadow_lower_right_corner_xy, background_color=canvas_background_color, **c_drop_down_shadow)
    ...
    if title is not None:
        canvas = __image_modifier.draw_image_title(canvas, title, top_padding_color=canvas_background_color)
    if max_output_image_size_wh is not None:
        canvas = __image_modifier.resize_universal_output_image(canvas, max_output_image_size_wh)
    if save_image_path is not None:
        __utils.save_image_to_disk(save_image_path, canvas)
    if display_image:
        __show.show_image(canvas)
    if return_image:
        return canvas
...
def show_grid(
    ...
    __checker.assert_valid_universal_inputs(title, display_image, return_image, max_output_image_size_wh, save_image_path, resize_factor, BGR2RGB, parse_image)
    if parse_image:
        image_source = __parser.parse_arbitrary_image_source(image_source, resize_factor, BGR2RGB)
    else:
        image_source = __parser.quick_parse_image_source(image_source)
    images = image_source  # A bit cumbersome to carry around, but I wanted to keep `image_source` for the sake of consistency between show-functions
    ...
    if len(images) == 1:
        return __show.show_image(image_source=images[0], title=title, display_image=display_image, return_image=return_image,
    ...
    # (2) If all images are wider than tall: resize to a shared width
        ...
        for i, image in enumerate(images):
            images[i] = __image_modifier.resize_image_respect_aspect_ratio(image, shared_width, None)[0]
    ...
    # (3) If all images are taller than wide: resize to a shared height
        ...
        for i, image in enumerate(images):
            images[i] = __image_modifier.resize_image_respect_aspect_ratio(image, None, shared_height)[0]
    ...
    if title is not None:
        final = __image_modifier.draw_image_title(final, title, top_padding_color=shared_settings["canvas_background_color"])
    if save_image_path is not None:
