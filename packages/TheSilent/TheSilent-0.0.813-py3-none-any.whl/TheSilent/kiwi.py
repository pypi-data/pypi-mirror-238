...
def kiwi(host,delay=0,scripts=False,root=False):
    clear()
    init_host = host
    hits = []
    ...
    for host in hosts:
        ...
        for _ in subdomains:
            ...
            if success:
                ...
                if scripts:
                    # run scripts
                    print(CYAN + f"pinging: {dns_host}")
                    time.sleep(delay)
                    os.system(f"ping -c 4 -W 10 {dns_host} > {dns_host}-ping.txt")

                    print(CYAN + f"running traceroute on: {dns_host}")
                    time.sleep(delay)
                    os.system(f"traceroute {dns_host} > {dns_host}-traceroute.txt")
                    
                    print(CYAN + f"running nmap vulnerabiltiy scan on: {dns_host}")
                    time.sleep(delay)
                    if root:
                        os.system(f"sudo nmap -Pn -p- -sT -sU --script vuln {dns_host} > {dns_host}-nmap.txt")

                    else:
                        os.system(f"nmap -Pn -p- --script vuln {dns_host} > {dns_host}-nmap.txt")

                    print(CYAN + f"running nikto scan on: http://{dns_host}")
                    time.sleep(delay)
                    os.system(f"nikto -host http://{dns_host} -Tuning x -Pause {delay} > {dns_host}-nikto.txt")

                    print(CYAN + f"running nikto scan on: https://{dns_host}")
                    time.sleep(delay)
                    os.system(f"nikto -host https://{dns_host} -Tuning x -Pause {delay} >> {dns_host}-nikto.txt")

                    print(CYAN + f"running sqlmap scan on: http://{dns_host}")
                    time.sleep(delay)
                    os.system(f"sqlmap -u http://{dns_host} --random-agent --level 5 --risk 3 --batch --current-user --flush-session --crawl 1000 --delay {delay} --threads 4 > {dns_host}-sqlmap.txt")
