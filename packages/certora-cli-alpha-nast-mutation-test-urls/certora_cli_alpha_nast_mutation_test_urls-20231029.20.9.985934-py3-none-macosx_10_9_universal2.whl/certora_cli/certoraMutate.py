...
def submit(args: argparse.Namespace) -> None:
    ...
    if args.prover_conf is not None:
        ...
        rule_report_link = args.orig_run
        local = False
        input_dir = fetch_and_extract_inputs_dir(rule_report_link, args)
...
def collect(args: argparse.Namespace) -> bool:
    ...
    original_url_to_status_page_url = original_url.replace(Constants.OUTPUT, Constants.JOB_DATA)
    original_results = get_results(args, original_url_to_status_page_url, fetcher)
    ...
    if any(
        [
            (Constants.RULE_REPORT_LINK not in mutant and Constants.LINK not in mutant) or
            (mutant.get(Constants.RULE_REPORT_LINK) is None and mutant.get(Constants.LINK) is None) or
            (mutant.get(Constants.RULE_REPORT_LINK) and not valid_link(mutant.get(Constants.RULE_REPORT_LINK))) or
            (mutant.get(Constants.LINK) and not valid_link(mutant.get(Constants.LINK)))
    ...
    for mutant in mutants_objs:
        mutant_link = mutant.get(Constants.RULE_REPORT_LINK)
        if mutant_link is None:
            mutant_link = mutant.get(Constants.LINK)
        mutants_results.append((mutant, get_results(args, mutant_link, fetcher)))
...
class WebFetcher(ReportFetcher):
    ...
    def get_url_path(self, url: str) -> Optional[str]:
        parsed_url = urllib3.util.parse_url(url)
