...
from spectral_cli.ezkl_wrapper.ezkl_wrapper import dump_model
...
@click.group()
def cli():
    """Modelers CLI provides tools to interact with Spectral platform and taking part in competitions."""
    pass
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if config_manager.get('global', 'wallet_private_key') is None and config_manager.get('global', 'wallet_address') is None:
                ...
                config_updated = True
            if config_manager.get('global', 'wallet_private_key') and config_manager.get('global', 'multisig_wallet_address') is None and not config_updated:
...
def configure(config_manager):
    """Configure CLI"""
    click.echo(f"Spectral CLI is configured!")
