...
import numpy as np
from .astronomy import observer_position, gmst, _days
...
class Orbital(object):
    ...
    def get_lonlatalt(self, utc_time):
        ...
        lon = ((np.arctan2(pos_y * XKMPER, pos_x * XKMPER) - gmst(utc_time))
               % (2 * np.pi))
    ...
    def get_orbit_number(self, utc_time, tbus_style=False, as_float=False):
        """Calculate orbital number at specified time.

        Args:
            tbus_style: If True, use TBUS-style orbital numbering (TLE orbital number + 1)
            as_float: Return a continuous orbital number as float.
        """
        utc_time = np.datetime64(utc_time)
        try:
            dt = _days(utc_time - self.orbit_elements.an_time)
            orbit_period = _days(self.orbit_elements.an_period)
    ...
    def get_equatorial_crossing_time(self, tstart, tend, node='ascending', local_time=False,
                                     rtol=1E-9):
        """Estimate the equatorial crossing time of an orbital.

        The crossing time is determined via the orbital number, which increases by one if the
        spacecraft passes the ascending node at the equator. A bisection algorithm is used to find
        the time of that passage.

        Args:
            tstart: Start time of the orbital
            tend: End time of the orbital. Orbit number at the end must be at least one greater than
            ...
            rtol: Tolerance of the bisection algorithm. The smaller the tolerance, the more accurate
                the result.
        """
        # Determine orbital number at the start and end of the orbital.
        ...
        def _nprime(time_f):
            """Continuous orbital number as a function of time."""
