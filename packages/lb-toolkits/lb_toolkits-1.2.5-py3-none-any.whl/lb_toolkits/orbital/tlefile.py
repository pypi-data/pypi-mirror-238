...
class Tle(object):
    ...
    def _read_tle(self):
        ...
        if self._line1 is not None and self._line2 is not None:
            ...
            for url in urls:
                ...
                outname = os.path.join(PKG_CONFIG_DIR, 'tle',nowdate.strftime('%Y%m%d'),
                                       basename)
    ...
    def _parse_tle(self):
        ...
        self.satnumber = self._line1[2:7]               # 卫星编号
        self.classification = self._line1[7]            #
        self.id_launch_year = self._line1[9:11]         # 发射年
        self.id_launch_number = self._line1[11:14]      # 在这一年发射序列号
        self.id_launch_piece = self._line1[14:17]       #
        self.epoch_year = self._line1[18:20]            # 当前轨道数据的时间点
        ...
        self.epoch_time = \
            dt.datetime.strptime(self.epoch_year, "%y") + dt.timedelta(days=self.epoch_day - 1)
        self.mean_motion_derivative = float(self._line1[33:43])     # 平均运动对时间一阶导数
        self.mean_motion_sec_derivative = _read_tle_decimal(self._line1[44:52]) # 平均运动对时间二阶导数
        self.bstar = _read_tle_decimal(self._line1[53:61])  # BSTAR阻力系数
        try:
            self.ephemeris_type = int(self._line1[62])  # 轨道模型类型， SGP4/SDP4
        except ValueError:
            self.ephemeris_type = 0
        self.element_number = int(self._line1[64:68])

        self.inclination = float(self._line2[8:16])             # 轨道倾角
        self.right_ascension = float(self._line2[17:25])        # 升交点赤经
        self.excentricity = int(self._line2[26:33]) * 10 ** -7  # 轨道偏心率
        self.arg_perigee = float(self._line2[34:42])            # 近地点俯角
        self.mean_anomaly = float(self._line2[43:51])           # 平近点角
        self.mean_motion = float(self._line2[52:63])            # 周期
        self.orbit = int(self._line2[63:68])                    # 飞行圈数
