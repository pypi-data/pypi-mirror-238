...
class GitMigrationHelper:
    ...
    def clone(
        ...
        :param password: Password for transports that needs it (http(s), svn).
        :return:
        """
        if exclude is None:
            exclude = []
        ...
        if password:
            init_command += f"--password={password} "
            # @TODO: Add support for password authentication this should be echo password | git svn init
        if not metadata:
        ...
        run_command(init_command, exit_on_error=True)
        ...
        run_command(fetch_command, exit_on_error=True)
    ...
    def get_branches(self, tags_base: str = "tags") -> GitBranches:
        ...
        result_local = run_command("git branch -l --no-color --format='%(refname:short)'")
        local_branches = result_local.splitlines()
        result_remote = run_command("git branch -r --no-color --format='%(refname:short)'")
    ...
    def fix_tags(self, branches: GitBranches, tags_base: str = "tags") -> None:
        ...
        current: dict[str, str] = {
            "user.name": run_command("git config user.name", exit_on_error=False).splitlines()[0],
            "user.email": run_command("git config user.email", exit_on_error=False).splitlines()[0],
        }
        ...
        for tag in branches.tags:
            ...
            subject = (
                run_command(f"git log -1 --pretty=format:'%s' \"{escape_quotes(tag)}\"").splitlines()[0].strip("'")
            )
            logger.debug(f"Subject: {subject}")
            date = run_command(f"git log -1 --pretty=format:'%ci' \"{escape_quotes(tag)}\"").splitlines()[0].strip("'")
            logger.debug(f"Date: {date}")
            author = (
                run_command(f"git log -1 --pretty=format:'%an' \"{escape_quotes(tag)}\"").splitlines()[0].strip("'")
            )
            logger.debug(f"Author: {author}")
            email = run_command(f"git log -1 --pretty=format:'%ae' \"{escape_quotes(tag)}\"").splitlines()[0].strip("'")
    ...
    def fix_branches(self, branches: GitBranches, rebase: bool = False, trunk_branch: str = "trunk") -> None:
        ...
        svn_branches = branches.remotes - branches.tags
    ...
    def verify_working_tree_is_clean(self) -> None:
        ...
        """
        result = run_command("git status --porcelain --untracked-files=no")
        if result != "":
    ...
    def get_config_command(self) -> str:
        ...
        """
        result = run_command("git config --local --get user.name", exit_on_error=False)
