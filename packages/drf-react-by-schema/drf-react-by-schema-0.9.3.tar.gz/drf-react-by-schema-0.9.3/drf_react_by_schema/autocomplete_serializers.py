...
class GenericAutocompleteSerializer(serializers.ModelSerializer):
    ...
    label = type('SerializerMethodField', (serializers.SerializerMethodField, serializers.CharField), dict())(
        'get_source'
    )
    ...
    for name, model in apps.all_models[app].items():
        ...
        if isinstance(model, type):
            ...
            if not serializer_name in dir():
                meta = type("Meta",(GenericAutocompleteSerializer.Meta, ),{
                    'model':model
                })
                generated_class = type(serializer_name, (GenericAutocompleteSerializer,), {
                    'Meta': meta
                })
