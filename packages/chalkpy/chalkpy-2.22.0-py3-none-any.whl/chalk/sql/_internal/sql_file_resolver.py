...
def get_sql_file_resolver(sources: Iterable[BaseSQLSource], sql_string_result: SQLStringResult) -> ResolverResult:
    ...
    if len(outputs) == 0:
        errors.append(
            ResolverError(
                display=f"No outputs detected. Make sure that all select outputs are aliased to features "
                f"defined within the feature set referenced by the `resolves` parameter.",
                path=path,
                parameter=None,
            )
        )
...
def _get_sql_glot(sql_string: str, path: str, sources: Iterable[BaseSQLSource]) -> GlotResult:
    ...
    if len(glot.selects) > len(glot.named_selects):
        errors.append(
            ResolverError(
                display=f"SQL query {sql_string} has at least one unnamed select. "
