...
def _convert_type(underlying_type: Type) -> _ConvertedType:
    ...
    if isinstance(underlying_type, Feature):
        return _convert_type(underlying_type.typ.underlying)
    ...
    if isinstance(t, Feature):
        ...
        if t.is_has_one:
            if t.join is None:
                raise TypeError(
                    (
                        f"Feature '{t.fqn}' is annotated with feature set '{t.typ}' but no join was found. "
                        f"Please ensure that join between keys is specified between '{t.namespace}' and the foreign feature set."
                    )
                )
