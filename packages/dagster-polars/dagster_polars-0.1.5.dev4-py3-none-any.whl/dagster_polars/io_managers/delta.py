...
class PolarsDeltaIOManager(BasePolarsUPathIOManager):
    ...
    def dump_df_to_path(self, context: OutputContext, df: pl.DataFrame, path: UPath):
        ...
        table = DeltaTable(
            str(path), storage_options={k.upper(): v for k, v in storage_options.items()} if storage_options else None
        )
    ...
    def scan_df_from_path(self, path: UPath, context: InputContext) -> pl.LazyFrame:
        ...
        version = DeltaTable(
            str(path),
            storage_options={k.upper(): v for k, v in storage_options.items()} if storage_options else None,
            version=version,
        ).version()
