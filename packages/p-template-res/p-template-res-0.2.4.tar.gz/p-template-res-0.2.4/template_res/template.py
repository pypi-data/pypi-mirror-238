...
def updateResources(rootDir):
    getOssResource(rootDir, "https://m.mecordai.com/res/template-res_20230517.zip", "9bb0765d04c0bb5c5d10a7c573c503f0", "template_res.zip.py")
    getOssResource(rootDir, "https://m.mecordai.com/res/music_20230426.zip", "375b2f1394557f35f132eed417b9fbff", "music.zip.py")
...
def listTemplate(searchPath, tid):
    ...
    for root,dirs,files in os.walk(tpDir):
        ...
        if root != files:
            break
    if len(tid) > 0:
        tid_dir = template_search_service.getTemplateWithTid(tid)
        if tid_dir:
            inputList = findInputList(tid_dir)
            if inputList:
                tid_params = inputConfig(inputList)
            else:
                tid_params = {}
            tid_params["name"] = tid
