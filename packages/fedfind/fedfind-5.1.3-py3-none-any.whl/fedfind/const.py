...
SUBVARIANTS = (
    ...
    # similar images with both appliance-creator and imagefactory;
    # previously they had identical subvariant, type, format and arch,
    # and this caused issues for productmd's parser. Before Fedora 33,
    # the subvariant value for these images didn't have _Appliance on
    # the end. We set no strings for these tuples because we *don't*
    # want to use these subvariant values when synthesizing metadata,
    # as we should only do that for old stable releases, and the true
    # value for those composes was the old one without _Appliance
    LoadTuple("Workstation_Appliance", tuple()),
    LoadTuple("Server_Appliance", tuple()),
    LoadTuple("Minimal_Appliance", tuple()),
    LoadTuple("Xfce_Appliance", tuple()),
...
RESPINRE = re.compile(
    r"F(?P<release>\d+)-(?P<subv>[^-]+)-(?P<arch>[^-]+)(-LIVE)?-(?P<date>\d{8,8})\.iso", re.I
)
