...
def merge_reports(output_dir: str, patient_id: str, results: List[Dict]):
...
def parse_report_then_write(
    ...
    try:
        ...
        label = results['label']
        merge_reports(output_dir, label, [results])
        if vcf:
            if not vcf_header:
                vcf_header = VCF.create_simple_vcf_header()
