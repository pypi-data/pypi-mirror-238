...
def parse_variant(
    ...
    if page:
        ...
        if len(all_variants) > 0:
            ...
            if vcf:
                vcf_header, vcf = vcf.archer_to_vcf('Archer', pid, all_variants, output_dir)
...
def parse_archer(
        ...
        vcf: VCF,
        pdf_file_path: str,
        output_dir: str) -> (str, List[Dict[str, str]]):
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    vcf_header, vcf = parse_variant(patient_id, results, doc, output_dir, maf, vcf)
