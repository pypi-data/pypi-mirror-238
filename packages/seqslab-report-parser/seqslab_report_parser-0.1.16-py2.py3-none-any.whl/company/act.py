...
def parse_v110(
    ...
    if vcf:
        vcf_header, vcf = vcf.to_vcf('ACT', info[REPORT_PATIENT_ID][0], var9, output_dir)
        return results, vcf_header, vcf
    else:
        return results, None, List()
...
def parse_v111(
    ...
    if vcf:
        vcf_header, vcf = vcf.to_vcf('ACT', info[REPORT_PATIENT_ID][0], var6 + var7, output_dir)
        return results, vcf_header, vcf
    else:
        return results, None, List()
...
def parse_act(
    ...
    if search1 or search2:
        return parse_v110(test_name, patient_id, maf, vcf, pdf_file_path, output_dir)
    else:
        return parse_v111(test_name, patient_id, maf, vcf, pdf_file_path, output_dir)
