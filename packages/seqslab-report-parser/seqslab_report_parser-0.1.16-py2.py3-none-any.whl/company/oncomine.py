...
def parse_variants(
    ...
    for v in all_variants:
        ...
        r.append(substitute_variables(t, v))
    ...
    if vcf:
        vcf_header, vcf = vcf.to_vcf('ONCOMINE', pid, all_variants, output_dir)
        return vcf_header, vcf
    else:
        return None, List()


def get_cnv(pid: str, page: fitz.Page, search1, search2) -> List[Dict[str, List[str]]]:
...
def parse_cnv(pid: str, results: Dict[str, str | Dict[str, str] | List[Dict[str, str]]], doc: fitz.Document) -> None:
    # TODO:
    ...
    var = get_cnv(pid, page, search1, search2)
    r = []
    for v in var:
...
def parse_brca(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> (Dict, str, List[Dict[str, str]]):
    ...
    vcf_header, vcf = parse_variants(patient_id, results, doc, get_clinvar_variants,
                                     ...
                                     output_dir, maf, vcf)
    parse_cnv(patient_id, results, doc)
    parse_fusion(results, doc)

    return results, vcf_header, vcf


def parse_focus(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> (Dict, str, List[Dict[str, str]]):
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
...
def parse_myeloid(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> (Dict, str, List[Dict[str, str]]):
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
    vcf_header, vcf = parse_variants(patient_id, results, doc, get_variants,
...
def parse_tumor_mutation_load(test_name: str, patient_id: str, maf: MAF, vcf: VCF, pdf_file_path: str, output_dir: str) -> (Dict, str, List[Dict[str, str]]):
    (doc, results) = init(test_name, pdf_file_path, patient_id)
    parse_basic_info(patient_id, results, doc)
    parse_biomarkers(results, doc, template_onco_biomarker_sens, template_onco_biomarker_res, template_onco_biomarker_other)
    vcf_header, vcf = parse_variants(patient_id, results, doc, get_variants,
...
def parse_oncomine(
        ...
        vcf: VCF,
        pdf_file_path: str,
        output_dir: str) -> (Dict, str, List[Dict[str, str]]):
    if 'BRCA' in test_name:
        return parse_brca(test_name, patient_id, maf, vcf, pdf_file_path, output_dir)
    elif test_name == 'Oncomine Focus Assay':
        return parse_focus(test_name, patient_id, maf, vcf, pdf_file_path, output_dir)
    elif test_name == 'Oncomine Myeloid Assay':
        return parse_myeloid(test_name, patient_id, maf, vcf, pdf_file_path, output_dir)
    elif 'Tumor Mutation Load' in test_name:
        return parse_tumor_mutation_load(test_name, patient_id, maf, vcf, pdf_file_path, output_dir)
    else:
