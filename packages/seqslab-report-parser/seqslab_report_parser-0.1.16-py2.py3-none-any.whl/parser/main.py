...
def parse_report(
    ...
    try:
        ...
        if test_name == 'ACTOnco+':
            results, vcf_header, vcf_content = act.parse_act(test_name, pid, maf, vcf, pdf_file_path, txt_file_path, output_dir)
        elif test_name.startswith('FoundationOne'):
            results, vcf_header, vcf_content = foundation_one.parse_foundation_one(test_name, pid, maf, vcf, pdf_file_path, xml_file_path, output_dir)
        elif test_name == 'Guardant360':
            results, vcf_header, vcf_content = guardant_360.parse_guardant_360(test_name, pid, maf, vcf, pdf_file_path, txt_file_path, output_dir)
        elif test_name.startswith('Oncomine') or test_name.startswith('Tumor Mutation Load Assay'):
            results, vcf_header, vcf_content = oncomine.parse_oncomine(test_name, pid, maf, vcf, pdf_file_path, output_dir)
        elif test_name.startswith('Archer'):
            results, vcf_header, vcf_content = archer.parse_archer(test_name, pid, maf, vcf, pdf_file_path, output_dir)
        else:
...
def merge_reports(output_dir: str, patient_id: str, results: List[Dict], vcf_header: str, vcfs: List[List[Dict[str, str]]]):
    ...
    def merge_vcf(vcfs: List[List[Dict[str, str]]]) -> List[Dict[str, str]]:
        m = []
        for v in vcfs:
            m += v
    ...
    merged_v = merge_vcf(vcfs)
    VCF.write_vcf(output_dir, patient_id, vcf_header, merged_v)
...
def parse_report_then_write(
        ...
        xml_file_path: str,
        output_dir: str) -> None:
    results, vcf_header, vcf_content = parse_report(patient_id, maf, vcf, pdf_file_path, txt_file_path, xml_file_path, output_dir)
    merge_reports(output_dir, patient_id, [results], vcf_header, vcf_content)
...
def single(args):
    ...
    for root, dirs, files in os.walk(args.input_dir):
        ...
        for p in pdf_file_paths:
            r, h, v = parse_report(pid, maf, vcf, p, txt_file_path, xml_file_path, out)
            results.append(r)
            vcf_header = h
            vcfs.append(v)

        merge_reports(out, pid, results, vcf_header, vcfs)
