...
class AbstractLLMApi(LLMApi):
    ...
    def _get_messages(self, prompt: Union[str, List[BaseMessage]]) -> List[BaseMessage]:
        ...
        if self.llm_config.llm_module_config.system_prompt is not None:
            messages.append(ChatMessage(role="system", content=self.llm_config.llm_module_config.system_prompt))
        if isinstance(prompt, str):
            messages.append(ChatMessage(role="user", content=prompt))
        else:
            for msg in prompt:
                if self.llm_config.llm_module_config.system_prompt is not None and\
                        to_message_in_req(msg).get('role') == "system":
                    continue
                messages.append(msg)
        return messages
