...
class ModelSelector:
    ...
    def _fetch_model_name(self):
        """ fetch model name from metadata file."""
        ...
        if os.path.isfile(model_metadata_file):
            ...
            return metadata.get(ModelSelectorConstants.MODEL_NAME, None)
        else:
            logger.warning("unable to fetch model name from the model provided through the"
    ...
    def convert_mlflow_model_to_pytorch_model(
        ...
        try:
            ...
            extra_files_dir = Path(mlflow_model_path, "extra_files")
            ...
            if os.path.isdir(extra_files_dir):
                logger.info("downloading extra files")
                shutil.copytree(extra_files_dir, download_dir, dirs_exist_ok=True)
    ...
    def run_workflow(self) -> None:
        ...
        if self.pytorch_model is not None:
            asset_id = self.get_model_asset_id(is_mlflow_model=False)
            logger.info(f"asset id of input model: {asset_id}")
            ...
            asset_id = self.get_model_asset_id(is_mlflow_model=True)
            logger.info(f"asset id of input model: {asset_id}")
