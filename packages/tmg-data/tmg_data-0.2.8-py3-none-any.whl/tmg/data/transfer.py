...
class Client:
    ...
    def bq_to_gs(self, table, bucket_name, separator=',', print_header=True, compress=False, partition=False):
        """Extract BigQuery table into the GoogleStorage in CSV format.
        ...
        logs.client.logger.info('Extracting from {table} to gs://{bucket_name}/{obj}_*{extension}'.format(
            bucket_name=bucket_name, obj=obj, table=table, extension=extension)
        )

        extract_job = bq_client.extract_table(
            source=table_ref,
            destination_uris='gs://{bucket_name}/{obj}_*{extension}'.format(
        ...
        bucket_folder = bucket_name.split('/')[0]
        bucket_prefix = '/'.join(bucket_name.split('/')[1:])
        blobs = storage_client.list_blobs(bucket_or_name=bucket_folder, prefix=bucket_prefix)
