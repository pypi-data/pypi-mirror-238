...
def get_sound_file_path(filename: str):
    package_data = get_data(__name__, f'sounds/{filename}')
    if package_data:
        return package_data
    else:
...
def play_dot_sound() -> None:
    ...
    dot_sound_path = get_sound_file_path('Dot.wav')
...
def play_dash_sound() -> None:
    # Get the absolute path to the sound file within the project directory
    dash_sound_path = get_sound_file_path('Dash.wav')
