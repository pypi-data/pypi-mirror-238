...
class HyperOptimizer(object):
  ...
  def _backward_step(self, input_valid, target_valid):
    loss, ncc, grad, _, _ = self.model._loss(input_valid, target_valid)
    loss.backward()

  def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
    unrolled_model = self._compute_unrolled_model(input_train, target_train, eta, network_optimizer)
    # unrolled_loss, ncc, grad, _, _ = unrolled_model._loss(input_valid, target_valid)
    unrolled_loss, ncc, multi, grad = unrolled_model._loss(input_valid, target_valid)
