...
from ..services.bank_from_iban_getter import BankFromIbanGetter
...
class ResPartner(models.Model):
    ...
    def _validate_bank_ids(self, bank_ids):
        for bank_id in bank_ids:
            if bank_id[0] == 0:
                acc_number = bank_id[2]['acc_number']
                bank = BankFromIbanGetter(self.env, acc_number).get_bank()
                if not bank:
    ...
    def create(self, vals):
        ...
        if bank_ids:
            self._validate_bank_ids(bank_ids)
