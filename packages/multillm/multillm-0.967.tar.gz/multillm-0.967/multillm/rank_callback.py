...
def return_ranking_result(args_dict):
    ...
    scores_sum = {llm: 0 for llm in LLM}
    scores_count = {llm: 0 for llm in LLM}
    ...
    average_scores = {llm: scores_sum[llm] / scores_count[llm] for llm in LLM}
...
def rank_CB(responses, config=None):
    ...
    while not exit:
        ...
        if len(func_args_json) == len(LLM)*4:
            exit = True
        else:
