...
def create_item_reservation(request, current_user, start, end, item_type: ReservationItemType, item_id):
    ...
    if item_type == ReservationItemType.TOOL:
        ...
        if tool_accessories and not accessories_configured:
            ...
            return render(
                request, "calendar/accessories.html", {"tool_accessories": tool_accessories, "conflicts": conflicts}
            )

        if tool_accessories and accessories_configured:
            selected_accessories = extract_tool_accessories(request)
            if selected_accessories:
                policy_problems = policy.check_accessories_available_for_reservation(
                    new_reservation, selected_accessories
                )
                if policy_problems:
                    return render(
                        request,
