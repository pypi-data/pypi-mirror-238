...
def generate_CRs(mean_fname, sig_fname=None, res_fnames=None, out_dir=None, c=None, 
    ...
    # Create mask for non-zero sigma values
    for i in np.arange(m):
        ...
        if i == 0:
            ...
            mask_nz = mask_current

        # Otherwise, take the intersection of the current mask and the previous mask
        else:

            mask_nz = np.logical_and(mask_nz, mask_current)

    # If the mask is a string then read it in
    if isinstance(mask, str):

        # Read in mask
        mask = np.logical_and(read_images(mask),mask_nz)

    # If the mask isn't a file path, it must be raw data
    ...
    for i in np.arange(m):
        ...
        AcHat_bdry_map = get_bdry_maps(muHat, c, 1*mask.reshape(muHat.shape))
    ...
    FcHat_bdry_map = get_bdry_maps(cap_muHat, c, 1*mask.reshape(cap_muHat.shape))
