...
from .cubic_approximation import (
    CubicApproximation,
    piecewise_cubic,
)
from .rot_approximation import (
    rot_average_slerp,
    rot_average_window,
)
from .settings import RotApprox
...
class TrajectoryApproximation(Trajectory):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            else TrajectoryProcessingState(),
        )
        self._has_valid_sorting = sort_index is not None
        self.config = settings if settings is not None else ApproximationSettings()
        self.state_before_approximation = (
            copy.deepcopy(state) if state is not None else TrajectoryProcessingState()
        )
    ...
    def _rotation_approximation(self, settings: ApproximationSettings):
        ...
        if (
            settings.rot_approx_technique == RotApprox.INTERP
            and self.state.sorting_known
        ):
            ...
            raise ValueError(
                "Please provide a sorter object for 'INTERP' rotation approximation"
            )
