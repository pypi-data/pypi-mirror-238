...
from spatialsorter import detect_laps_by_length
...
def rot_average_window(
    function_of: np.ndarray, quat: np.ndarray, win_size: float = 0.15
) -> np.ndarray:
    """
    ...
    # define window
    steps = rndodd(
        len(function_of) / len(np.unique(np.round(function_of / (win_size))))
    )
