...
from pyconnectwise.endpoints.manage.ExpensePaymenttypesEndpoint import (
    ExpensePaymenttypesEndpoint,
)
...
class ExpenseEndpoint(ConnectWiseEndpoint):
    def __init__(self, client, parent_endpoint=None) -> None:  # noqa: ANN001
        ConnectWiseEndpoint.__init__(
            self, client, "expense", parent_endpoint=parent_endpoint
        )

        self.info = self._register_child_endpoint(
            ExpenseInfoEndpoint(client, parent_endpoint=self)
        )
