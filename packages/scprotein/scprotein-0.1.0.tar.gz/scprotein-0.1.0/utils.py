...
def load_sc_proteomic_features(stage1):
    ...
    if stage1:
        ...
        if os.path.exists('peptide_uncertainty_data.npy'):
            # peptide_uncertainty = np.load('./peptide_uncertainty_estimation/peptide_uncertainty_data.npy')
            # peptide_uncertainty = np.load('scprotein/peptide_uncertainty_estimation/peptide_uncertainty.npy')
            # data_bytes = pkgutil.get_data('peptide_uncertainty_estimation', 'peptide_uncertainty.npy')
            # data_bytes = pkgutil.get_data('scprotein', 'peptide_uncertainty.npy')
            # print(data_bytes)
            # peptide_uncertainty = np.load(data_bytes)
            data_filename = "peptide_uncertainty_estimation/peptide_uncertainty.npy"
            data_file = pkg_resources.resource_filename(__name__, data_filename)
            peptide_uncertainty = np.load(data_file)
