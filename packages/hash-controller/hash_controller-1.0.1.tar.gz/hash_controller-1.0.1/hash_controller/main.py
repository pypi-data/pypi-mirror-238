...
class HashClient:
    ...
    def exist(customer_uuid: str, type: str, obj) -> bool:
        ...
        item = HashClient._client[customer_uuid][type].find_one(id)
        if item is None:
            return False
        else:
            return True
    ...
    def exist_many(customer_uuid: str, type: str, objs: list) -> list[bool]:
        ...
        items = HashClient._client[customer_uuid][type].find(filter={"_id": {"$in": ids_to_find}}, projection="_id")
        items = list(items)
    ...
    def create(customer_uuid: str, type: str, obj) -> str:
        ...
        return HashClient._client[customer_uuid][type].insert_one(document).inserted_id
    ...
    def create_many(customer_uuid: str, type: str, objs: list) -> list[str]:
        ...
        return HashClient._client[customer_uuid][type].insert_many(objs_to_add).inserted_ids

    @staticmethod
    def get_many(customer_uuid: str, type: str, filter: dict = {}, date_comparasion: str = "$gt") -> list[dict]:
        """retrive a list of objects filtered from hash collection

        Args:
            customer_uuid (str): customer_uuid as database
            type (str): type as collection
        ...
        items = HashClient._client[customer_uuid][type].find(filter=filter)
        return list(items)
