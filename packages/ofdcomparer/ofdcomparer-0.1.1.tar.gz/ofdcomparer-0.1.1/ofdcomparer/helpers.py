...
def compare_json_structure(json_template_path, fn_doc_from_fn):
    ...
    # Получаем ключи из обоих JSON объектов
...
def convert_fn_format(data):
    str_tags = ['1018', '1226', '1016', '1041', '1074', '1075', '1073', '1037', '1264', '1171']
    int_tags = ['1213', '1040', '1135']
...
def convert_receipt_format(data):
    def format_datetime(value):
        try:
            parsed_datetime = datetime.strptime(value, "%d.%m.%Y %H:%M:%S")
            adjusted_datetime = parsed_datetime.replace(hour=parsed_datetime.hour - 3)
        ...
        for tag_info in item['value']:
            ...
            if isinstance(tag_value, list):
                ...
                for nested_item in tag_value:
                    ...
                    if isinstance(nested_tag_value, list):
                        ...
                        for sub_nested_item in nested_tag_value:
                            ...
                            if sub_nested_tag_name == 1012 or sub_nested_tag_name == 1098:
                                sub_nested_dict[str(sub_nested_tag_name)] = format_datetime(sub_nested_tag_value)
                            elif sub_nested_tag_name == 1016:
                                sub_nested_dict[str(sub_nested_tag_name)] = str(sub_nested_tag_value).strip()
                            elif sub_nested_tag_name == 1262:
                                sub_nested_dict[str(sub_nested_tag_name)] = str(sub_nested_tag_value).zfill(3)
                            elif sub_nested_tag_name in str_tags:
                                sub_nested_dict[str(sub_nested_tag_name)] = [
                                    sub_nested_tag_value]  # Always format as a list
                            else:
                                sub_nested_dict[str(sub_nested_tag_name)] = convert_value(
                                    sub_nested_tag_value).strip() if isinstance(convert_value(sub_nested_tag_value),
                                                                                str) else convert_value(
                                    sub_nested_tag_value)
                        nested_dict[str(nested_tag_name)] = sub_nested_dict
                    else:
                        if nested_tag_name == 1012 or nested_tag_name == 1098:
                            nested_dict[str(nested_tag_name)] = format_datetime(nested_tag_value)
                        elif nested_tag_name == 1016:
                            nested_dict[str(nested_tag_name)] = str(nested_tag_value).strip()
                        elif nested_tag_name == 1262:
                            nested_dict[str(nested_tag_name)] = str(nested_tag_value).zfill(3)
                        elif nested_tag_name in str_tags:
                            nested_dict[str(nested_tag_name)] = [nested_tag_value]  # Always format as a list
                        else:
                            nested_dict[str(nested_tag_name)] = convert_value(nested_tag_value).strip() if isinstance(
                                convert_value(nested_tag_value), str) else convert_value(nested_tag_value)
                ...
                if tag_name == 1012 or tag_name == 1098:
                    new_item[str(tag_name)] = format_datetime(tag_value)
                elif tag_name == 1016:
                    ...
                    new_item[str(tag_name)] = convert_value(tag_value).strip() if isinstance(convert_value(tag_value),
                                                                                             str) else convert_value(
                        tag_value)
