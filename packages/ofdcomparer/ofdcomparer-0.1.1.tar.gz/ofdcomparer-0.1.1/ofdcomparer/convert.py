...
DOSSymbols = ' !"#$%&`()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~' \
              ' АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдежзийклмноп                                              ' \
              '  рстуфхцчшщъыьэюяЁё          №   '

KKTSymbols = ' !"#№%&`()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~' \
              ' АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдежзийклмноп                                              ' \
              '  рстуфхцчшщъыьэюяЁё€         $   '

InKKTSymbols = 'АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ !"#№%&`()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^' \
               '_`abcdefghijklmnopqrstuvwxyz{|}~ абвгдежзийклмнопрстуфхцчшщъыьэюя$€-                            ' \
               '                                                 Ёё            \t '

OutKKTSymbols = '         \t                       !"#№%&`()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^' \
...
def HexStringToString_InP5(hex_string):  # декодер из кодировки ККТ- позволяет читать данные в привычной кодировке
    result = ''
...
def StringToHexString(string):  # кодировщик в кодировку ККТ (просто вводить аргументы для команды через ';')
    string = str(string)
    result = ''
    for char in string:
        b = bytes(char, 'cp866')
...
def CalcINN10(inn10):
    ...
    c = 2 * a[0] + 4 * a[1] + 10 * a[2] + 3 * a[3] + 5 * a[4] + 9 * a[5] + 4 * a[6] + 6 * a[7] + 8 * a[8]
    c = (c % 11) % 10
...
def CalcINN12(inn12):
    ...
    c1 = 7 * a[0] + 2 * a[1] + 4 * a[2] + 10 * a[3] + 3 * a[4] + 5 * a[5] + 9 * a[6] + 4 * a[7] + 6 * a[8] + 8 * a[9]
    c1 = (c1 % 11) % 10
    c2 = 3 * a[0] + 7 * a[1] + 2 * a[2] + 4 * a[3] + 10 * a[4] + 3 * a[5] + 5 * a[6] + 9 * a[7] + 4 * a[8] + 6 * a[
        9] + 8 * c1
    c2 = (c2 % 11) % 10
...
#         data = sock.recv(1024)
#         if data == 'OK':
#             print('OK - Succesfully sent socket data ' + Data)
#         else:
#             print('"{}"'.format(data))
#         sock.close()
#         time.sleep(5)
#         del sock
#     except:
#         print('BAD (time out!) - not sent socket data ' + Data)
...
def End(code, driver=None):
    ...
    try:
        ...
        print('Не удалось отправить данные скрипта', e)
        pass
