...
def get_token():
    ...
    """
    # print("get_token()")
    headers = {"Content-Type": "application/json", "Integrator-ID": Config.INTEGRATOR_ID }
    ...
    try:
        if response.status_code == 301:
            response = requests.post(response.headers['Location'], headers=headers, json=Config.USER_DATA)
    ...
    try:
        ...
        while not time.time() - start_time > timeout:
            ...
            if response.status_code == 200:
                ...
                #formatted_fd_taxcom = json.dumps(fd_taxcom, sort_keys=True, indent=4, ensure_ascii=False)
                return fd_taxcom
    except requests.exceptions.RequestException as e:
