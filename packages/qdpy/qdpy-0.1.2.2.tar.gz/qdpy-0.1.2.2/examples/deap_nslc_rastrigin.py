...
# Iteration callback to modify the parameters of the archive at the end of each iteration
def iteration_callback(iteration, batch, container, logbook):
    if container.threshold_novelty > 0.02:
        container.threshold_novelty -= 0.003
...
if __name__ == "__main__":
    ...
    # Create container
    #parent_archive = Container(fitness_domain=fitness_domain, features_domain=features_domain, storage_type=list)   # Will contain ALL added individuals, and will be used to compute novelty
    parent_archive = Container(storage_type=list)   # Will contain ALL added individuals, and will be used to compute novelty
    container = NoveltyArchive(k=k, threshold_novelty=threshold_novelty, fitness_domain=fitness_domain, features_domain=features_domain, storage_type=list, parents=[parent_archive])   # Will only contain individuals that verify the novelty criterion, forward all added individuals to ``parent_archive``

    with ParallelismManager(args.parallelismType, toolbox=toolbox) as pMgr:
        # Create a QD algorithm
        algo = DEAPQDAlgorithm(pMgr.toolbox, container, init_batch_size = init_batch_size, batch_size = batch_size, niter = nb_iterations,
                verbose = True, results_infos = results_infos, iteration_callback_fn = iteration_callback,
    ...
    grid = Grid(parent_archive, shape=nb_bins, max_items_per_bin=max_items_per_bin, fitness_domain=fitness_domain, features_domain=features_domain, storage_type=list)
    print("\tDone !")
    print(grid.summary())
