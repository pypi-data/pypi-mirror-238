...
class QDAlgorithmLike(Protocol):
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
            features: Optional[FeaturesLike] = None, scores: Optional[ScoresDictLike] = None,
            elapsed: Optional[float] = None, verbose: bool = False) -> bool: ...
    ...
    def optimise(self, evaluate: Callable, budget: Optional[int] = None, 
            ...
            send_several_suggestions_to_fn: bool = False, max_nb_suggestions_per_call: int = 10,
            verbose: bool = False) -> IndividualLike: ...
...
# Inspiration from the nevergrad library
class QDAlgorithm(abc.ABC, QDAlgorithmLike, Summarisable, Saveable, Copyable, CreatableFromConfig):
    ...
    def __init__(self, container: Container, budget: int, batch_size: Optional[int] = None,
        ...
        tell_container_parents: bool = False
        ...
        if isinstance(tell_container_at_init, bool):
            ...
            if tell_container_at_init.lower() == "yes":
                tell_container = True
            elif tell_container_at_init.lower() == "parents":
                tell_container_parents = True
        if tell_container_parents:
            if len(self.container.parents) > 0:
                raise ValueError("`tell_container_parents` can only be set to 'parents' if `container` possesses at least one parent.")
            for p in self.container.parents:
                for e in p:
    ...
    # TODO init fitness_weights of container, instead of algorithm
    ...
    def optimisation_task(self, optimisation_task: Union[str, Sequence[float]]) -> None:
        ...
        self.container.fitness_weights = self._fitness_weights
    ...
    def ask(self) -> IndividualLike:
        ...
        #suggestion.fitness.weights = self._fitness_weights # XXX
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
            features: Optional[FeaturesLike] = None, scores: Optional[ScoresDictLike] = None,
            elapsed: Optional[float] = None, verbose: bool = False, tell_container = True) -> bool:
        ...
        :param scores: Optional[ScoresDictLike]
            If provided, set `individual.scores` to `scores`.
        ...
        if scores is not None:
            ind.scores = scores
        ind_fit = self.container.get_ind_fitness(ind)
        if fitness is not None:
            if isinstance(fitness, FitnessLike):
                ind_fit = fitness
                ind_fit.weights = self._fitness_weights
            else:
                ind_fit.weights = self._fitness_weights
                ind_fit.values = fitness
            self.container.set_ind_fitness(ind, ind_fit)
        if len(ind_fit.values) != self._nb_objectives:
        #if len(ind.fitness.values) != self._nb_objectives:
            #print(f"DEBUG: scores: {ind.scores} ind_fit: {self.container.get_ind_fitness(ind)} individual.scores: {individual.scores} individual.fitness: {individual.fitness} fitness: {fitness}(type: {type(fitness)}) ind_fit: {ind_fit}")
            raise ValueError(f"`individual` must possess a `fitness` attribute of dimension {self._nb_objectives} (specified through parameter `nb_objectives` when creating a QDAlgorithm sub-class), instead of {len(ind_fit.values)}.")
        ...
        added: bool = self.container.update([ind], issue_warning=verbose) == 1 if tell_container and allowed_to_tell else False
    ...
    def _tell_after_container_update(self, individual: IndividualLike, added: bool, xattr: Mapping[str, Any] = {}) -> None:
        ...
        for fn in self._callbacks.get("tell"):
            fn(self, individual, added, xattr)
        ...
        def optimisation_loop(budget_fn: Callable):
            ...
            while remaining_evals > 0 or len(futures) > 0:
                ...
                if send_several_suggestions_to_fn:
                    ...
                    remaining_suggestions = nb_suggestions
                    while remaining_suggestions > 0:
                        eval_id: int = remaining_evals
                        nb_suggestions2 = min(max_nb_suggestions_per_call, nb_suggestions)
                        inds: Sequence[IndividualLike] = [self.ask() for _ in range(nb_suggestions2)]
                        ...
                        remaining_suggestions -= len(inds)
                    ...
                    if ind_exc is None:
                        ...
                        for i in range(len(inds)):
                            if isinstance(ind_res[i], IndividualLike):
                                self.tell(ind_res[i], elapsed=ind_elapsed, verbose=verbose)
                            else:
                                if len(ind_res[i]) == 0:
                                    self.tell(inds[i], elapsed=ind_elapsed, verbose=verbose)
                                if len(ind_res[i]) == 2:
                                    self.tell(inds[i], fitness=ind_res[i][0], features=ind_res[i][1], elapsed=ind_elapsed, verbose=verbose)
                                elif len(ind_res[i]) == 3:
                                    self.tell(inds[i], fitness=ind_res[i][0], features=ind_res[i][1], scores=ind_res[i][2], elapsed=ind_elapsed, verbose=verbose)
                                else:
                                    warnings.warn(f"Evaluation function returned malformatted results.")
                    ...
                    if ind_exc is None:
                        if isinstance(ind_res, IndividualLike):
                            self.tell(ind_res, elapsed=ind_elapsed, verbose=verbose)
                        else:
                            ind = individuals[ind_id] # type: ignore
                            if len(ind_res) == 0:
                                self.tell(ind, elapsed=ind_elapsed, verbose=verbose)
                            elif len(ind_res) == 2:
                                self.tell(ind, fitness=ind_res[0], features=ind_res[1], elapsed=ind_elapsed, verbose=verbose)
                            elif len(ind_res) == 3:
                                self.tell(ind, fitness=ind_res[0], features=ind_res[1], scores=ind_res[2], elapsed=ind_elapsed, verbose=verbose)
                            else:
                                warnings.warn(f"Evaluation function returned malformatted results.")
                    else:
                        warnings.warn(f"Individual evaluation raised the following exception: {ind_exc} !")
...
class AlgWrapper(QDAlgorithm):
    ...
    def __init__(self, algorithms: Any, container: Optional[Container] = None, budget: Optional[int] = None,
            ...
            name: Optional[str] = None, batch_mode: bool = False,
            tell_all: bool = False, **kwargs):
        if len(algorithms) == 0:
...
#        return {**entries, **res}
    ...
    @abc.abstractmethod
    def next(self, verbose: bool = False) -> None:
...
#            self.switch_to(0)
    ...
    def switch_to(self, idx: int, verbose: bool = False):
        ...
        if (isinstance(self.tell_container_when_switching, bool) and self.tell_container_when_switching) or \
            ...
            try:
                self.current.tell(previous.best(), verbose=verbose) # Exception when no individual is present
            except Exception:
                pass
...
#        return ind
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
            features: Optional[FeaturesLike] = None, scores: Optional[ScoresDictLike] = None,
            elapsed: Optional[float] = None, verbose: bool = False, tell_container = False) -> bool:
        if self.tell_all:
            added: bool = False
            for a in self.algorithms:
                added |= a.tell(individual, fitness, features, scores, elapsed, verbose=verbose)
        else:
            added = self.current.tell(individual, fitness, features, scores, elapsed, verbose=verbose)
...
class AlternatingAlgWrapper(AlgWrapper):
    def next(self, verbose: bool = False) -> None:
        """Switch to the next algorithm in Sequence `self.algorithms`, if there is one."""
        if self.current_idx < len(self.algorithms) - 1:
            self.switch_to(self.current_idx + 1, verbose)
        else:
            self.switch_to(0, verbose)
...
class Sq(QDAlgorithmLike, Summarisable, Saveable, Copyable, CreatableFromConfig):
    ...
    def tell(self, individual: IndividualLike, fitness: Optional[Any] = None,
            features: Optional[FeaturesLike] = None, scores: Optional[ScoresDictLike] = None,
            elapsed: Optional[float] = None, verbose: bool = False) -> bool:
        return self.current.tell(individual, fitness, features, scores, elapsed, verbose=verbose)
    ...
    def optimise(self, evaluate: Callable, budget: Optional[int] = None, 
        ...
        for _ in range(self.current_idx, len(self.algorithms)):
            try:
                res = self.current.optimise(evaluate, budget, batch_mode, executor,
                        send_several_suggestions_to_fn = send_several_suggestions_to_fn,
                        max_nb_suggestions_per_call = max_nb_suggestions_per_call,
                        verbose = verbose)
            except Exception as e:
                warnings.warn(f"Optimisation failed with algorithm '{self.current.name}': {e}")
                traceback.print_exc()
