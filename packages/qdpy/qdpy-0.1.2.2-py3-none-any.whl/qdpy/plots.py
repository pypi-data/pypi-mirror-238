...
def drawGridInAx(data, ax, cmap, featuresBounds, fitnessBounds, aspect="equal", xlabel = "", ylabel = "", nbBins=None, nbTicks = 5):
    ...
    # Fix cmap if data is a null matrix
    if vmin == vmax and cmap.N == 0:
        cmap.N = 1
...
def default_plots_grid(logger, output_dir=None, suffix=None, to_grid_parameters={}, fitness_domain=None):
    ...
    plot_evals(logger, os.path.join(output_dir, f"./evals_fitnessmax0{suffix}.pdf"), "max0", ylabel="Fitness")
    ylim_contsize = (0, len(container)) if np.isinf(container.capacity) else (0, container.capacity)
    plot_evals(logger, os.path.join(output_dir, f"./evals_contsize{suffix}.pdf"), "cont_size", ylim=ylim_contsize, ylabel="Container size")
    plot_iterations(logger, os.path.join(output_dir, f"./iterations_nbupdated{suffix}.pdf"), "nb_updated", ylabel="Number of updated bins")
    ...
    plot_path = os.path.join(output_dir, f"activityGrid{suffix}.pdf")
