...
__all__ = ["LoggerStat", "AlgorithmLogger", "TQDMAlgorithmLogger", "default_algorithm_logger"]


import os
from typing import Optional, Tuple, List, Iterable, Iterator, Any, TypeVar, Generic, Union, Sequence, MutableSet, MutableSequence, Type, Callable, Generator, Mapping, MutableMapping, overload, NamedTuple
import warnings
import numpy as np
import pandas as pd
import threading
...
class LoggerStat(NamedTuple):
    name: str
    fn: Callable
    every_iteration: bool = True
...
class AlgorithmLogger(Saveable):
    ...
    _cols_names: MutableSequence[str] = ["alg_name", "iteration", "cont_size", "evals", "nb_updated", "avg", "std", "min", "max", "ft_min", "ft_max", "qd_score", "elapsed"]
    _cols_size: MutableSequence[int]
    ...
    _custom_stats: MutableSequence[LoggerStat]
    ...
    def _stats_iteration(self, algo: QDAlgorithmLike, batch_elapsed: float) -> Sequence[Any]:
        alg_name = algo.name
        ...
        fitness_values = np.array([algo.container.get_ind_fitness(ind).values for ind in algo.container])
        ...
        custom_stats_vals = [catch(s.fn, lambda e: None, algo) for s in self._custom_stats]

        # Regroup all stats into formatted list of strings
        with np.printoptions(precision=4, suppress=True):
            res = list(map(str, [alg_name, iteration, cont_size, evals, nb_updated, avg_values, std_values, min_values, max_values, ft_min_values, ft_max_values, qd_score, batch_elapsed_str] + custom_stats_vals))
        return res


    def _init_cols(self, content: Sequence[Any]) -> None:
        #assert(len(content) == len(self._cols_size))
        cont2 = content[:len(self._cols_size)]
        c_size: Sequence[int] = [len(f"{c}") for c in cont2]
    ...
    def _tell(self, algo: QDAlgorithmLike, ind: IndividualLike, added: bool, xattr: Mapping[str, Any] = {}) -> None:
        self._lock_evals_data.acquire()
        ...
        # Compute custom stats
        for s in self._custom_stats:
            if s.every_iteration:
                continue
            val = s.fn(algo, ind)
            self._evals_data.setdefault(s.name, []).append(val)
    ...
    def _iteration(self, algo: QDAlgorithmLike, batch_elapsed: float) -> None:
        stats: Sequence[Any] = self._stats_iteration(algo, batch_elapsed)
        if self.verbose:
        ...
        for i in range(len(self._cols_names)):
            if i < len(stats):
                ...
                self._iterations_data[self._cols_names[i]].append(np.nan)
        self._lock_iterations_data.release()
    ...
    def register_stat(self, *stat: LoggerStat) -> None:
        #if len(self.algorithms) > 0:
        #    raise RuntimeError("`register_stat` cannot be called while the logger is already monitoring one algorithm.")
        for s in stat:
            self._custom_stats.append(s)
            if s.every_iteration:
                self._cols_names.append(s.name)
                self._iterations_data[s.name] = []
                self._cols_size.append(len(s.name))
...
try:
    ...
    class TQDMAlgorithmLogger(AlgorithmLogger):
        ...
        def _started_optimisation(self, algo: QDAlgorithmLike) -> None:
            ...
            self._tqdm_pbar = tqdm.tqdm(total=algo.budget, file=self._orig_out_err[0], unit="eval", leave=False)
