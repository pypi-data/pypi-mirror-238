...
import logging
import math
from .base_classes import Basedata, Weather
...
class Illumination(Basedata):
    ...
    def __init__(self,
        ...
        if requested_day is not None:
            ...
            logging.info('setting requested_day in illumination object: {self.requested_day}')
            weather.requested_day = self.requested_day
            logging.info('setting requested_day in weather object: {weather.requested_day}')
        ...
        if requested_hour is not None:
            ...
            logging.info('setting requested_hour in illumination object: {self.requested_hour}')
            weather.requested_hour = self.requested_hour
            logging.info('setting requested_hour in weather object: {weather.requested_hour}')
    ...
    def et_illuminance(self):
        ...
        _et_illuminance = float(1367) * (1 + 0.034 * math.cos(((2 * math.pi)/356) * (_day_of_the_year - 2)))
    ...
    def cloud_coefficients(self):
        ...
        """
        _cloud_coverage = self.cloud_coverage
        if _cloud_coverage < 30:
            _cloud_coefficients = {
                "c": 0.21,
                "A": 0.8,
                "B": 15.5,
    ...
    def horizontal_illuminance(self):
        ...
        _horizontal_illuminance = _direct_illuminance * math.sin(math.radians(_altitude))
    ...
    def horizontal_sky_illuminance(self):
        ...
        if isinstance(_C_temp, float):
            ...
            _C = float(_C_temp)
        _sky_illuminance = _A + (_B * (math.sin(math.radians(_altitude)))**_C)
    ...
    def daylight_illuminance(self):
        if self.day is True:
            ...
            _daylight = _sky_illuminance + _horizontal_illuminance
