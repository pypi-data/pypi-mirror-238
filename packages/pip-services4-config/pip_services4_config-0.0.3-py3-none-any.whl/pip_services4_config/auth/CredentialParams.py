# -*- coding: utf-8 -*-
"""
    pip_services4_config.auth.CredentialParams
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class CredentialParams(ConfigParams):
    ...
    def use_credential_store(self) -> bool:
        """
        Checks if these credential parameters shall be retrieved from :class:`ICredentialStore <pip_services4_config.auth.ICredentialStore.ICredentialStore>`.
        The credential parameters are redirected to :class:`ICredentialStore <pip_services4_config.auth.ICredentialStore.ICredentialStore>` when store_key parameter is set.

        :return: true if credentials shall be retrieved from :class:`ICredentialStore <pip_services4_config.auth.ICredentialStore.ICredentialStore>`
        """
        return "store_key" in self

    def get_store_key(self) -> str:
        """
        Gets the key to retrieve these credentials from :class:`ICredentialStore <pip_services4_config.auth.ICredentialStore.ICredentialStore>`.
    ...
    def set_store_key(self, value: str):
        """
        Sets the key to retrieve these parameters from :class:`ICredentialStore <pip_services4_config.auth.ICredentialStore.ICredentialStore>`.
