...
class TemplateAttribute:
    ...
    def apply(self, df: pd.DataFrame):
        df = df.copy()
        if self.per_90:
            df[self.name] = self.calculation(df) / df["minutes"] * 90
        else:
...
FOULS = TemplateAttribute("Fouls", lambda df: df["fouls"], False, columns_used=["fouls"])
...
HEADERS_WON_PCT = TemplateAttribute(
    "Headers Won %",
    lambda df: (100 * df["aerials_won"] / (df["aerials_won"] + df["aerials_lost"])).fillna(0),
    True,
...
NPXG = TemplateAttribute("NPxG", lambda df: df["npxg"], True, columns_used=["npxg"], sig_figs=2)

NPXG_PER_SHOT = TemplateAttribute(
    "NPxG/Shot",
    lambda df: df["npxg"] / df["shots_total"],
    True,
...
PCT_DRIBBLERS_TACKLED = TemplateAttribute(
    ...
    lambda df: (100 * df["tackles_vs_dribbles_won"] / df["tackles_vs_dribbles"]).fillna(0),
    True,
...
PASSING_PCT = TemplateAttribute(
    name="Passing %",
    calculation=lambda df: 100 * (df["passes_completed"] / df["passes"]).fillna(0),
    ascending_rank=True,
...
PASSES_PROGRESSIVE_PCT = TemplateAttribute(
    "Progressive Passes %",
    lambda df: 100 * df["progressive_passes"] / df["passes"],
    True,
...
SHOOTING_PCT = TemplateAttribute(
    "Shooting %",
    lambda df: (100 * (df["goals"] - df["pens_made"]) / df["shots_total"]).fillna(0),
    True,
    columns_used=["goals", "pens_made", "shots_total"],
    per_90=False,
)
SHOTS = TemplateAttribute("Shots", lambda df: df["shots_total"], True, columns_used=["shots_total"])
...
XA = TemplateAttribute("xA", lambda df: df["xa"], True, columns_used=["xa"], sig_figs=2)
...
SELF_CREATED_SHOT_PCT = TemplateAttribute(
    "Self-Created Shot %",
    lambda df: 100 * df["self_created_shots"] / df["shots_total"],
    True,
...
GoalkeeperTemplate = [
    ...
    TemplateAttribute(
        "Save %",
        lambda df: 100 * (df["saves"] / df["shots_on_target_against"]).fillna(0),
        True,
    ...
    TemplateAttribute(
        "SSVA per 100 Shots",
        lambda df: 100 * (df["psxg_gk"] - df["goals_conceded"]) / df["shots_on_target_against"],
        True,
    ...
    TemplateAttribute(
        "Long Pass % Completed",
        lambda df: 100 * (df["passes_completed_long"] / df["passes_long"]).fillna(0),
        True,
...
TeamTemplate = [
    ...
    TemplateAttribute(
        "Shots",
        lambda df: df["shots_total" + "_team"] / df["matches"],
        True,
        columns_used=["shots_total"],
    ),
    TemplateAttribute(
        "Shots Conceded",
        lambda df: df["shots_total" + "_opp"] / df["matches"],
        False,
        columns_used=["shots_total"],
    ),
    TemplateAttribute(
    ...
    TemplateAttribute(
        "Long Ball %",
        lambda df: 100 * df["passes_long" + "_team"] / df["passes" + "_team"],
        True,
        columns_used=["passes_long", "passes"],
    ),
    TemplateAttribute(
        "Possession %",
        lambda df: 100 * df["touches" + "_team"] / (df["touches" + "_opp"] + df["touches" + "_team"]),
        True,
        columns_used=["touches"],
    ),
    TemplateAttribute(
        "PAdj Final 3rd Tackles",
        lambda df: df["tackles_att_3rd" + "_team"]
        / df["matches"]
        / (df["touches" + "_opp"] / (df["touches" + "_opp"] + df["touches" + "_team"]))
        * 0.5,
        True,
    ...
    TemplateAttribute(
        "PPDA",
        lambda df: df["passes_opp"] / (df["tackles_team"] + df["interceptions_team"] + df["fouls_team"]),
        False,
        columns_used=["passes", "tackles", "interceptions", "fouls"],
    ),
    TemplateAttribute(
        "PAdj Fouls Committed",
        lambda df: df["fouls" + "_team"]
        / df["matches"]
        / (df["touches" + "_opp"] / (df["touches" + "_opp"] + df["touches" + "_team"]))
        * 0.5,
        False,
    ...
    TemplateAttribute(
        "PAdj Dribbles",
        lambda df: df["dribbles" + "_team"]
        / df["matches"]
        / (df["touches" + "_team"] / (df["touches" + "_opp"] + df["touches" + "_team"]))
        * 0.5,
        True,
