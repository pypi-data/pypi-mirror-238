...
class AstronomicalRoutines:
    ...
    def utc_to_local_diff(self):
        ...
        local = self.utc_to_local2(datetime.now())
    ...
    def calculate_sun(self):
        ...
        try:
            self._sun_next_setting_civil = self._sun_observer.next_setting(ephem.Sun(), use_center=True).datetime().replace(tzinfo=pytz.utc)
        ...
        try:
            self._sun_next_rising_nautical = self._sun_observer_nautical.next_rising(
                ephem.Sun(), use_center=True
        ...
        try:
            self._sun_next_setting_nautical = self._sun_observer_nautical.next_setting(
                ephem.Sun(), use_center=True
            ).datetime().replace(tzinfo=pytz.utc)
        ...
        try:
            self._sun_next_rising_astro = self._sun_observer_astro.next_rising(ephem.Sun(), use_center=True).datetime().replace(tzinfo=pytz.utc)
        ...
        try:
            self._sun_previous_rising_astro = self._sun_observer_astro.previous_rising(
                ephem.Sun(), use_center=True
            ).datetime().replace(tzinfo=pytz.utc)
        ...
        try:
            self._sun_previous_setting_astro = self._sun_observer_astro.previous_setting(
                ephem.Sun(), use_center=True
            ).datetime().replace(tzinfo=pytz.utc)
    ...
    async def moon_next_full_moon(self) -> float:
        ...
        localtime = self._forecast_time
        if self._moon_next_full_moon is None or localtime > self._moon_next_full_moon:
    ...
    async def deep_sky_darkness_moon_sets(self) -> bool:
        ...
        if self._moon_next_setting < self._sun_next_rising_astro and self._moon_next_setting > start_timestamp:
            return True
        return False
    ...
    async def deep_sky_darkness(self) -> float:
        ...
        # self.utc_to_local(datetime.utcnow())

    async def deep_sky_darkness2(self) -> float:
        """Returns the remaining timespan of deep sky darkness"""
        ...
        # Are we already in darkness?
        if self._sun_next_setting_astro > self._sun_next_rising_astro:
            # print("start_timestamp = self._sun_previous_setting_astro")
            start_timestamp = self._sun_previous_setting_astro
        else:
            # print("start_timestamp = self._sun_next_setting_astro")
            start_timestamp = self._sun_next_setting_astro

        # print("_sun_next_setting_astro", self._sun_next_setting_astro)
        # print("_sun_next_rising_astro", self._sun_next_rising_astro)
        # print("_sun_previous_setting_astro", self._sun_previous_setting_astro)
        # print("_moon_next_rising", self._moon_next_rising)
        # print("_moon_next_setting", self._moon_next_setting)
        # print("_moon_day_after_next_setting", self._moon_day_after_next_setting)
        ...
        if self._moon_next_rising > start_timestamp and self._moon_next_rising < self._sun_next_rising_astro:
            ...
            # print("dsd = dsd - (self._sun_next_rising_astro - self._moon_next_rising)")
            dsd = dsd - (self._sun_next_rising_astro - self._moon_next_rising)
        # else:
        # print("moon does not rise during darkness")
        ...
        if self._moon_next_setting < self._sun_next_rising_astro and self._moon_next_setting > start_timestamp:
            ...
            # print("dsd = dsd - (self._moon_next_setting - start_timestamp)")
            dsd = dsd - (self._moon_next_setting - start_timestamp)
        # else:
        # print("moon does not set during darkness")

        # print("dsd2", dsd)

        # Moon up during darkness
        if self._moon_next_rising < self._moon_next_setting:
            # print("moon is not up")
            if self._moon_next_rising < start_timestamp and self._moon_next_setting > self._sun_next_rising_astro:
                # _LOGGER.debug("Astronomical calculations Moon up during darkness")
                # print("dsd = timedelta(0)")
                dsd = timedelta(0)
        else:
            # print("moon is up")
            if (
                # self._moon_next_rising < start_timestamp
                # and self._moon_day_after_next_setting > self._sun_next_rising_astro
                self._moon_day_after_next_setting
