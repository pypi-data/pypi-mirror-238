...
def transcribe(src, dst, media_filepath, subtitle_format, event, n_media_filepaths):
    global thread_transcribe, thread_transcribe_starter, pool, do_translate, completed_tasks, pbar
    ...
    msg = "Processing {} :".format(file_display_name)
    print(msg)
...
def start_transcribe(media_filepaths, src, dst, subtitle_format, concurrency):
    global pool, thread_transcribe, thread_transcribe_starter, do_translate, completed_tasks, pbar

    #print(media_filepaths)
    n_media_filepaths = len(media_filepaths)
    completion_events = {}  # Dictionary to store completion events
    pool = {media_filepath: multiprocessing.Pool(concurrency) for media_filepath in media_filepaths}
...
def main():
    ...
    #thread_transcribe_starter = Thread(target=start_transcribe, args=(media_filepaths, args.src_language, args.dst_language, args.format, args.concurrency), daemon=True)
    ...
    for file in media_filepaths:
        thread_transcribe = Thread(target=transcribe, args=(args.src_language, args.dst_language, file, args.format, completion_events[media_filepath], n_media_filepaths), daemon=True)
        thread_transcribe.start()
