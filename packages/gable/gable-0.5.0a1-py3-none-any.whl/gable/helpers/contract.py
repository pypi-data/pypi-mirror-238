...
def contract_files_to_post_contract_request(
    ...
    for contract_file in contract_files:
        ...
        logger.debug(f"Git info ({contract_file.name}): {jsonpickle.dumps(git_info)}")
        relative_path = os.path.relpath(
            contract_file.name, git_info["localRepoRootDir"]
        )
        logger.debug(f"Relative path ({contract_file.name}): {relative_path}")
        ...
        try:
            ...
            logger.debug(
                f"Contract Input ({contract_file.name}): {contract_input.json()}"
            )
