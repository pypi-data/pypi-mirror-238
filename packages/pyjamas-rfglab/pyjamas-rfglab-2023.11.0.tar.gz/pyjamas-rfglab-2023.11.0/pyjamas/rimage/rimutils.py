...
class rimutils:
    ...
    def flow(cls, firstslice: numpy.ndarray, secondslice: numpy.ndarray,
        ...
        if filter_output:  # Needs checking - straight from Matlab rflow.
            ...
            # the __call__ method).
            interpXfield = interpolate.RectBivariateSpline(Y0[:, 0], X0[0, :], Xval)
            interpYfield = interpolate.RectBivariateSpline(Y0[:, 0], X0[0, :], Yval)
        ...
        print("before")
        Xvaldesired = interpolate.griddata(numpy.vstack((y0, x0)).T, xval, numpy.vstack((Yf, Xf)).T,
                                                 method='linear', fill_value=0.0)
        print("mid")
        Yvaldesired = interpolate.griddata(numpy.vstack((y0, x0)).T, yval, numpy.vstack((Yf, Xf)).T,
                                                 method='linear', fill_value=0.0)
        print("after")
    ...
    def mode(cls, image: numpy.ndarray) -> int:
        return stats.mode(image, axis=None, nan_policy='omit')[0]
