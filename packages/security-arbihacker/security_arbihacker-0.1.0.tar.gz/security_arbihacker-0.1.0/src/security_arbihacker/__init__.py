...
def check_token_and_scopes(
    ...
    def maybe_raise_credentials_exception(reason=None):
        if reason is not None:
            logging.warning(
                {
                    "message": f"Token invalid: {reason}",
                    "reason": reason,
                    "sub": subject,
                }
            )
    ...
    for security_scope, payload_scope in itertools.product(security_set, payload_set):
        ...
        scope_match = True
        for security_f, payload_f in itertools.zip_longest(
            security_fragments, payload_fragments, fillvalue="*"
        ):
            if not (
                security_f == "*"
                or payload_f == {"all", "admin", "superuser", "*"}
                or security_f == payload_f
            ):
                scope_match = False
        if scope_match:
            break
