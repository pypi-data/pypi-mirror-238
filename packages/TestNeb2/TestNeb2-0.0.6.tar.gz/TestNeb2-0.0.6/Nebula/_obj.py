...
class Entity(core.pygame.sprite.Sprite):
    def __init__(self, game, _id:int=core.random.randint(999,9999), position:core.pygame.math.Vector2()=core.pygame.math.Vector2(), size:int=32, rectSize=core.pygame.math.Vector2(32, 32), spriteGroups:list=[], color:list=[10, 30, 20]):
...
class Renderer:
    ...
    def render(self, surface: core.pygame.Surface):
        ...
        for assetID, position, offset in self.renderData:
            ...
            try:
                ...
                self.dirtyRects.append(core.pygame.Rect(position.x, position.y, assetImage[0].get_width(), assetImage[0].get_height()))
            except AttributeError:
                surface.blit(asset, position - offset)
                self.dirtyRects.append(core.pygame.Rect(position.x, position.y, asset.get_width(), asset.get_height()))
...
class Tilemap:
    ...
    def load(self, path):
        ...
        self.mapSize = core.pygame.math.Vector2(map_data['map width']*self.tileSize, map_data['map height']*self.tileSize)
    ...
    def render(self, surface, offset=core.pygame.math.Vector2(), zoomFactor=1):
        ...
        # offset += core.pygame.math.Vector2(0.2, 0.4)
        for layer in self.tilemap.values():
            for location, tile in list(layer.items()):
...
"""NEBULA"""
# [monitor := m for m in core.get_monitors()]
# core.os.environ['SDL_VIDEO_WINDOW_POS'] = f'{str(int(monitor.width)/2)},{str(int(monitor.height)/2)}'
# NebulaEnv(
