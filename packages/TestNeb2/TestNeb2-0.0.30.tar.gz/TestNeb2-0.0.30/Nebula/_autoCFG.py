...
def Main(projectType:str, projectName:str, projectPath:str) -> bool:
    ...
    try:
        open(f"{projectDir}\\.ncfg", "r")
        ncfg = True
    except FileNotFoundError:
        return 'ncfg not found'
    
    if ncfg:
        with open(f"{projectDir}\\.ncfg", "r") as cfgReader:
            cfgData = json.load(cfgReader)
            cfgReader.close()

        if cfgData['env']['configured'] == 'False':
            with open(f"{projectDir}{projectName}\\.ncfg", "w") as cfgWriter:
            ...
            return 'Nebula project was pre-configured???\n'
    else:
        return 'ncfg not found!\n'
