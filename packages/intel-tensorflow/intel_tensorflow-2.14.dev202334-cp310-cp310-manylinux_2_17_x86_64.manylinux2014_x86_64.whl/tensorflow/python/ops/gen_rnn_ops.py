...
_AUGRUBlockCellOutput = collections.namedtuple(
    "AUGRUBlockCell",
    ["r", "u", "c", "h"])
...
@tf_export('augru_block_cell')
def augru_block_cell(x, h_prev, au_x, w_ru, w_c, b_ru, b_c, lbr=False, training=False, name=None):
  r"""TODO: add doc.

  Args:
    x: A `Tensor`. Must be one of the following types: `float32`, `bfloat16`.
    h_prev: A `Tensor`. Must have the same type as `x`.
    au_x: A `Tensor`. Must have the same type as `x`.
    ...
    lbr: An optional `bool`. Defaults to `False`.
    training: An optional `bool`. Defaults to `False`.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "AUGRUBlockCell", name, x, h_prev, au_x, w_ru, w_c, b_ru, b_c,
        "lbr", lbr, "training", training)
      _result = _AUGRUBlockCellOutput._make(_result)
      return _result
      ...
      pass
    try:
      _result = _dispatcher_for_augru_block_cell(
          (x, h_prev, au_x, w_ru, w_c, b_ru, b_c, lbr, training, name,), None)
      if _result is not NotImplemented:
        return _result
      return augru_block_cell_eager_fallback(
          x, h_prev, au_x, w_ru, w_c, b_ru, b_c, lbr=lbr, training=training,
          name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
      _result = _dispatch.dispatch(
            augru_block_cell, (), dict(x=x, h_prev=h_prev, au_x=au_x,
                                       w_ru=w_ru, w_c=w_c, b_ru=b_ru, b_c=b_c,
                                       lbr=lbr, training=training, name=name)
          )
  ...
  if lbr is None:
    lbr = False
  lbr = _execute.make_bool(lbr, "lbr")
  if training is None:
    training = False
  training = _execute.make_bool(training, "training")
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "AUGRUBlockCell", x=x, h_prev=h_prev, au_x=au_x, w_ru=w_ru, w_c=w_c,
                          b_ru=b_ru, b_c=b_c, lbr=lbr, training=training,
                          name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          augru_block_cell, (), dict(x=x, h_prev=h_prev, au_x=au_x, w_ru=w_ru,
                                     w_c=w_c, b_ru=b_ru, b_c=b_c, lbr=lbr,
  ...
  if _execute.must_record_gradient():
    _attrs = ("T", _op._get_attr_type("T"), "lbr", _op._get_attr_bool("lbr"),
              "training", _op._get_attr_bool("training"))
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "AUGRUBlockCell", _inputs_flat, _attrs, _result)
  _result = _AUGRUBlockCellOutput._make(_result)
  return _result

AUGRUBlockCell = tf_export("raw_ops.AUGRUBlockCell")(_ops.to_raw_op(augru_block_cell))
_dispatcher_for_augru_block_cell = augru_block_cell._tf_type_based_dispatcher.Dispatch


def augru_block_cell_eager_fallback(x, h_prev, au_x, w_ru, w_c, b_ru, b_c, lbr, training, name, ctx):
  if lbr is None:
    lbr = False
  lbr = _execute.make_bool(lbr, "lbr")
  if training is None:
    training = False
  training = _execute.make_bool(training, "training")
  _attr_T, _inputs_T = _execute.args_to_matching_eager([x, h_prev, au_x, w_ru, w_c, b_ru, b_c], ctx, [_dtypes.float32, _dtypes.bfloat16, ])
  (x, h_prev, au_x, w_ru, w_c, b_ru, b_c) = _inputs_T
  _inputs_flat = [x, h_prev, au_x, w_ru, w_c, b_ru, b_c]
  _attrs = ("T", _attr_T, "lbr", lbr, "training", training)
  _result = _execute.execute(b"AUGRUBlockCell", 4, inputs=_inputs_flat,
...
def gru_block_cell(x, h_prev, w_ru, w_c, b_ru, b_c, lbr=False, training=False, name=None):
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "GRUBlockCell", name, x, h_prev, w_ru, w_c, b_ru, b_c, "lbr",
        lbr, "training", training)
    ...
    try:
      return gru_block_cell_eager_fallback(
          x, h_prev, w_ru, w_c, b_ru, b_c, lbr=lbr, training=training,
          name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  ...
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "GRUBlockCell", x=x, h_prev=h_prev, w_ru=w_ru, w_c=w_c, b_ru=b_ru,
                        b_c=b_c, lbr=lbr, training=training, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("T", _op._get_attr_type("T"), "lbr", _op._get_attr_bool("lbr"),
              "training", _op._get_attr_bool("training"))
...
def gru_block_cell_eager_fallback(x, h_prev, w_ru, w_c, b_ru, b_c, lbr, training, name, ctx):
  if lbr is None:
    lbr = False
  lbr = _execute.make_bool(lbr, "lbr")
  if training is None:
    training = False
  training = _execute.make_bool(training, "training")
  _attr_T, _inputs_T = _execute.args_to_matching_eager([x, h_prev, w_ru, w_c, b_ru, b_c], ctx, [_dtypes.float32, _dtypes.bfloat16, ])
