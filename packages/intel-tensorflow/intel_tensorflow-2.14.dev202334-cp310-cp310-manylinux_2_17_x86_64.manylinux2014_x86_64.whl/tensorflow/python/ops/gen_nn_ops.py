...
def approx_top_k(input, k, reduction_dimension=-1, recall_target=0.95, is_max_k=True, reduction_input_size_override=-1, aggregate_to_topk=True, name=None):
...
def conv2d_backprop_filter_v2(input, filter, out_backprop, strides, padding, use_cudnn_on_gpu=True, explicit_paddings=[], data_format="NHWC", dilations=[1, 1, 1, 1], name=None):
  r"""Computes the gradients of convolution with respect to the filter.
...
def conv2d_backprop_input_v2(input, filter, out_backprop, strides, padding, use_cudnn_on_gpu=True, explicit_paddings=[], data_format="NHWC", dilations=[1, 1, 1, 1], name=None):
  r"""Computes the gradients of convolution with respect to the input.
...
def _fused_batch_norm(x, scale, offset, mean, variance, epsilon=0.0001, exponential_avg_factor=1, data_format="NHWC", is_training=True, name=None):
  ...
  Args:
    x: A `Tensor`. Must be one of the following types: `float32`, `half`.
...
def _fused_batch_norm_eager_fallback(x, scale, offset, mean, variance, epsilon, exponential_avg_factor, data_format, is_training, name, ctx):
  ...
  if is_training is None:
    is_training = True
  is_training = _execute.make_bool(is_training, "is_training")
  _attr_T, _inputs_T = _execute.args_to_matching_eager([x, scale, offset, mean, variance], ctx, [_dtypes.float32, _dtypes.half, ])
...
def _gelu(features, approximate=False, name=None):
  r"""TODO: add doc.

  Args:
    features: A `Tensor`. Must be one of the following types: `half`, `float32`, `float64`, `bfloat16`.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "Gelu", name, features, "approximate", approximate)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
      pass
    try:
      return _gelu_eager_fallback(
          features, approximate=approximate, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  if approximate is None:
    approximate = False
  approximate = _execute.make_bool(approximate, "approximate")
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "Gelu", features=features, approximate=approximate, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("T", _op._get_attr_type("T"), "approximate",
              _op._get_attr_bool("approximate"))
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "Gelu", _inputs_flat, _attrs, _result)
  _result, = _result
  return _result

Gelu = tf_export("raw_ops.Gelu")(_ops.to_raw_op(_gelu))


def _gelu_eager_fallback(features, approximate, name, ctx):
  if approximate is None:
    approximate = False
  approximate = _execute.make_bool(approximate, "approximate")
  _attr_T, (features,) = _execute.args_to_matching_eager([features], ctx, [_dtypes.half, _dtypes.float32, _dtypes.float64, _dtypes.bfloat16, ])
  _inputs_flat = [features]
  _attrs = ("T", _attr_T, "approximate", approximate)
  _result = _execute.execute(b"Gelu", 1, inputs=_inputs_flat, attrs=_attrs,
                             ctx=ctx, name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
        "Gelu", _inputs_flat, _attrs, _result)
...
def gelu_grad(gradients, features, approximate=False, name=None):
  r"""TODO: add doc.

  Args:
    gradients: A `Tensor`. Must be one of the following types: `half`, `float32`, `float64`, `bfloat16`.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "GeluGrad", name, gradients, features, "approximate",
        approximate)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
      ...
      pass
    try:
      return gelu_grad_eager_fallback(
          gradients, features, approximate=approximate, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  if approximate is None:
  ...
  approximate = _execute.make_bool(approximate, "approximate")
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "GeluGrad", gradients=gradients, features=features,
                    approximate=approximate, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("T", _op._get_attr_type("T"), "approximate",
              _op._get_attr_bool("approximate"))
    _inputs_flat = _op.inputs
    _execute.record_gradient(
        "GeluGrad", _inputs_flat, _attrs, _result)
  _result, = _result
  return _result

GeluGrad = tf_export("raw_ops.GeluGrad")(_ops.to_raw_op(gelu_grad))


def gelu_grad_eager_fallback(gradients, features, approximate, name, ctx):
  if approximate is None:
    approximate = False
  approximate = _execute.make_bool(approximate, "approximate")
  _attr_T, _inputs_T = _execute.args_to_matching_eager([gradients, features], ctx, [_dtypes.half, _dtypes.float32, _dtypes.float64, _dtypes.bfloat16, ])
  (gradients, features) = _inputs_T
  _inputs_flat = [gradients, features]
  _attrs = ("T", _attr_T, "approximate", approximate)
  _result = _execute.execute(b"GeluGrad", 1, inputs=_inputs_flat,
                             attrs=_attrs, ctx=ctx, name=name)
  if _execute.must_record_gradient():
    _execute.record_gradient(
        "GeluGrad", _inputs_flat, _attrs, _result)
...
def quantized_mat_mul_with_bias_and_dequantize(a, b, bias, min_a, max_a, min_b, max_b, min_freezed_output, max_freezed_output, Toutput, transpose_a=False, transpose_b=False, input_quant_mode="MIN_FIRST", name=None):
  ...
  Args:
    ...
    min_freezed_output: A `Tensor` of type `float32`.
    max_freezed_output: A `Tensor` of type `float32`.
    Toutput: A `tf.DType` from: `tf.float32, tf.bfloat16`.
    transpose_a: An optional `bool`. Defaults to `False`.
    transpose_b: An optional `bool`. Defaults to `False`.
    input_quant_mode: An optional `string` from: `"MIN_FIRST", "SCALED"`. Defaults to `"MIN_FIRST"`.
    name: A name for the operation (optional).
...
def top_kv2(input, k, sorted=True, index_type=_dtypes.int32, name=None):
  r"""Finds values and indices of the `k` largest elements for the last dimension.
...
def top_kv2_eager_fallback(input, k, sorted, index_type, name, ctx):
  if sorted is None:
    sorted = True
  sorted = _execute.make_bool(sorted, "sorted")
  if index_type is None:
