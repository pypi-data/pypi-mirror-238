...
class ScanTask(Task):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # UID has a foreign key to the item, so we need to flush the items before the
        # uids
            ...
            if isinstance(scan_result, T.Item):
                ...
                if self.ingest_config.detect_duplicates:
                    ...
                    detect_duplicates.detect_uid_conflicts_in_item(
                        ...
                        scan_result.uids,
                        self.insert_errors,
                        self.ingest_config.detect_duplicates_override,
                    )
    ...
    def extract_uids(self, item: T.Item) -> None:
        """Create new task for extracting UIDs. In case of DICOM scanner it's already
        extracted"""
                ...
                try:
                    ...
                    if not project or project.container_type != "project":
                        self.insert_errors.push(
                            T.Error(
                                ...
                                message=(
                                    f"Skipping {path} because it's not a valid "
                                    "container path"
                                ),
                    ...
                    if not self.fw.auth_info.is_admin:
                        if not required_actions.issubset(having_actions):
                            ...
                            msg = (
                                f"User does not have the required permissions "
                                f"({req_str}) in '{path}'. Missing permissions: "
                                f"{missing_str}"
                            )
