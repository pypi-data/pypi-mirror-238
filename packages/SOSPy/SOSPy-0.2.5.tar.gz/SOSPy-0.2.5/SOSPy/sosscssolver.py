...
def sosscssolver(At,b,c,K,options={},verbose=1):
    ...
    # Initialize solver
    solver = scs.SCS(data, cone, eps_abs=1e-9, eps_rel=1e-9, max_iters=max_iters, normalize=normalize, scale=scale, adaptive_scale=adaptive_scale, rho_x=rho_x, eps_infeas=eps_infeas, alpha=alpha, time_limit_secs=time_limit_secs, verbose=verbose, warm_start=warm_start, acceleration_lookback=acceleration_lookback, acceleration_interval=acceleration_interval)
    # Solve
