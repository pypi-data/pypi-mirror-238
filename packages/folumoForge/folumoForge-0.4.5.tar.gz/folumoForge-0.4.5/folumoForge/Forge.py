...
class Screen:
    ...
    def SwitchScreen(self):
        if self.fullScreen:
            self.root = pygame.display.set_mode(self.size, DOUBLEBUF | OPENGL | FULLSCREEN, 0, self.monitorNum)
        else:
            self.root = pygame.display.set_mode(self.size, DOUBLEBUF | OPENGL, display=self.monitorNum)
