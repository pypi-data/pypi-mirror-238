...
from pygame import Surface, DOUBLEBUF, OPENGL, FULLSCREEN
...
class Screen:
    ...
    def SwitchScreen(self):
        if self.fullScreen:
            # self.root.MainRoot = pygame.display.set_mode(self.size, DOUBLEBUF | OPENGL | FULLSCREEN, 0, self.monitorNum)
            self.root.MainRoot = pygame.display.set_mode(self.size, DOUBLEBUF | FULLSCREEN, 0, self.monitorNum)
        else:
            # self.root.MainRoot = pygame.display.set_mode(self.size, DOUBLEBUF | OPENGL, display=self.monitorNum)
            self.root.MainRoot = pygame.display.set_mode(self.size, DOUBLEBUF, display=self.monitorNum)

        # GL.glOrtho(0, self.size[0], self.size[1], 0, -1, 1)
