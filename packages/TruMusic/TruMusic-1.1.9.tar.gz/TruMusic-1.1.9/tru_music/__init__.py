...
class TruMusic:
    ...
    @staticmethod
    def _print_title_options(results):
        for index, item in enumerate(results, 1):
            print(f"[{index}] {item.get('title')}")

    # pylint: disable=too-many-locals)
    def download_artist_album(self, artist_name, album_name, clean=False):
        search_results = self.yt_music.search(artist_name, filter='albums')

        matching_titles = [
            item for item in search_results if album_name is None or album_name in item.get('title').lower()
        ]
        ...
        if match_count == 1 or (match_count > 0 and self.quiet):
            ...
            while selected_item is None:
                self._print_title_options(matching_titles)
                selection = input(f"{match_count} matching results found; select one (q to quit): ")
                if selection == "q":
                    return False
            ...
            self.logger.error(f"No match found for \"{artist_name}\" - \"{album_name}\"")
            return False

        _artist_name = selected_item.get('artists')[0]['name']
        thumbnails = selected_item.get("thumbnails")
        album = self.yt_music.get_album(selected_item['browseId'])
        if clean:
            is_clean = not selected_item['isExplicit']
            if not is_clean:
                for other_version in album.get('other_versions', []):
                    is_clean = not other_version['isExplicit']
                    if is_clean:
                        thumbnails = other_version.get("thumbnails")
                        album = self.yt_music.get_album(other_version.get('browseId'))
                        break

            if is_clean:
                self.logger.warning("Getting clean version")
            else:
                self.logger.warning("Clean version not available")

        self.dest_dir = f"{_artist_name}/{album.get('title')}"
        self.image_file = thumbnails

        self.logger.info(
            f'{_artist_name} :: {album.get("title")} ({album.get("year")}) [ {album.get("trackCount")} tracks ]'
        )
        self._set_log_prefix([_artist_name, album.get('title')])
        for index, track in enumerate(album.get("tracks"), 1):
            track_num = str(index).zfill(2)
            self.logger.info(
                f"DOWNLOADING: {track_num} - {track['title']}{' [E]' if track['isExplicit'] else ''}"
                f" ({track['duration']})"
            )
            if not self.dry_run:
