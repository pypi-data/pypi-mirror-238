...
Note on lite objects: SermonAudio provides "lite" versions of objects with
minimal fields to make API calls faster.

Prefer LiteSermon when displaying sermons. This object is the most lightweight
sermon object possible, containing the most minimal fields necessary to display
metadata about a sermon.

If sermon media is necessary for sermon playback (we recommend using LiteSermon
for lists, and only using the full sermon object with media for playback),
perfer using the liteBroadcaster flag for sermons. This flag returns a
LiteContainedBroadcaster object instead of the full broadcaster
object. Typically, for sermon playback, the full broadcaster object is not
necessary.

If neither of the above options contains enough information, then get the full
sermon object.

Always prefer WebcastLite to Webcast. The lite version of a webcast contains
all the information necessary to display and begin playback of a
webcast. Consider the full Webcast to be deprecated at this point.
"""
...
class SoloSite(Model):
    """Information relating to a particular SoloSite"""

    def __init__(self, obj: dict):
        super().__init__(obj)

        self.solo_id = obj["solo_id"]
        self.broadcaster_id = obj["broadcaster_id"]
        self.domain = obj["domain"]
        self.status = SoloStatus(obj["status"])
        self.create_date = (
            datetime.datetime.fromtimestamp(obj["create_date"], tz=pytz.utc) if obj["create_date"] else None
        )
        self.update_date = (
            datetime.datetime.fromtimestamp(obj["update_date"], tz=pytz.utc) if obj["update_date"] else None
        )


class LiteContainedBroadcaster(Model):
    """Class has minimal fields for broadcaster.

    The lite contained broadcaster is a lightweight version of the
    Broadcaster object which contains fields needed for displaying
    broadcaster information for an object like a sermon or webcast which
    contains a Broadcaster. Rather than always generate the full
    Broadcaster object, when another object contains a broadcaster and
    needs only display fields, it can request the lite broadcaster
    object.

    The full Broadcaster object is field-compatible with the
    LiteContainedBroadcaster object, and therefore derives from it.
    """
    ...
    def __init__(self, obj: dict):
        ...
        self.solo_site = SoloSite.parse(obj["soloSite"]) if obj.get("soloSite") else None

        self.solo_site_disabled = bool(obj.get("soloSiteDisabled"))

        self.disabled = bool(obj.get("disabled"))
...
class Broadcaster(LiteContainedBroadcaster):
    def __init__(self, obj: dict):
        super().__init__(obj)
        ...
        # Regular RSS feed of latest sermons.
        self.rss_feed_url = obj["rssFeedURL"]
        assert isinstance(self.rss_feed_url, str)

        # Feed of latest sermons in Atom format (https://validator.w3.org/feed/docs/rfc4287.html).
        self.rss_feed_atom_url = obj["rssFeedAtomURL"]
        assert isinstance(self.rss_feed_atom_url, str)
...
class Media(Model):
    def __init__(self, obj: dict):
        ...
        assert isinstance_or_none(self.language, str)
        ...
        self.auto_generated = obj.get("autoGenerated", False)
        assert isinstance_or_none(self.auto_generated, bool)
...
class MediaSet(Model):
    def __init__(self, obj: dict):
        ...
        self.caption = list(filter(is_valid, [Media.parse(rec) for rec in obj.get("caption", [])]))


class LiteSermonBroadcaster(Model):
    def get_album_art_url(self, size: int):  # pragma: no cover
        return self.album_art_url_format.replace("{size}", str(size))
...
class LiteSermon(Model):
    def __init__(self, obj: dict):
        ...
        self.has_audio = obj["hasAudio"]
        assert isinstance(self.has_audio, bool)
        ...
        self.broadcaster = LiteSermonBroadcaster(obj["broadcaster"])
        self.speaker = LiteSermonSpeaker(obj["speaker"])
...
class Sermon(Model):
    def __init__(self, obj: dict):
        ...
        if obj["broadcaster"]["type"] == "lite_contained_broadcaster":
            self.broadcaster = LiteContainedBroadcaster.parse(obj["broadcaster"])
        else:
...
class WebcastLite(Model):
    def __init__(self, obj: dict):
        super().__init__(obj)

        self.audio_stream_url = obj["audioStreamURL"]
        assert isinstance_or_none(self.audio_stream_url, str)

        self.video_stream_url = obj["videoStreamURL"]
        assert isinstance_or_none(self.video_stream_url, str)

        self.broadcaster_id = obj["broadcasterID"]
        assert isinstance_or_none(self.broadcaster_id, str)

        self.broadcaster_location = obj["broadcasterLocation"]
        assert isinstance_or_none(self.broadcaster_location, str)

        self.broadcaster_minister = obj["broadcasterMinister"]
        assert isinstance_or_none(self.broadcaster_minister, str)

        self.display_name = obj["displayName"]
        assert isinstance_or_none(self.display_name, str)

        self.group = obj["group"]
        assert isinstance_or_none(self.group, str)
...
@enum.unique
class SeriesPodcastLinkType(enum.Enum):
    """The type of a series podcast link.

    The podcast type is a general category for identifying and grouping
    podcast links. The podcast type and label form a unique podcast feed
    link, so porcast types may be reused (particularly OTHER) as long as
...
class SermonSeriesFeedLink(Model):
    def __init__(self, obj: dict):
        super().__init__(obj)
        self.feed_link_id = obj["feedLinkID"]
        self.series_id = obj["seriesID"]
        self.podcast_type = SeriesPodcastLinkType(obj["podcastType"]) if obj["podcastType"] else None
...
class SermonSeries(Model):
    def __init__(self, obj: dict):
        ...
        self.podcast_enabled = obj["podcastEnabled"]
        self.podcast_speaker = obj["podcastSpeaker"]
        self.image = obj["image"]
        self.image_resizable = obj["imageResizable"]
        self.rss_url = obj["rssURL"]  # Regular RSS feed of latest sermons in this series.
        self.rss_atom_url = obj["rssAtomURL"]  # Feed in Atom format (https://validator.w3.org/feed/docs/rfc4287.html).

        if obj["feedLinks"]:
            self.feed_links = [SermonSeriesFeedLink(f) for f in obj["feedLinks"]]
        else:
            self.feed_links = None
...
class Language(Model):
    ...
    def __init__(self, obj: dict):
        ...
        self.iso_code3 = obj["languageCode3"]
        self.no_captions = obj["noCaptions"]
