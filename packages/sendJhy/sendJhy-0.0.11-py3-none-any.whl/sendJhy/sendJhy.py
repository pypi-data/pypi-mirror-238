...
def post_api(endpoint,data):
    ...
    if response.status_code == 200:
        ...
        result['Post_Sucess'] = f"Response from server/{endpoint}: {response_data}"
    else:
        # 요청이 실패한 경우 오류 코드 출력
        result['Post_Error'] =  f"Request failed with status code/{endpoint}: {response.status_code}"
    return result
...
def get_api(endpoint):
    ...
    if response.status_code == 200:
        response_data = response.json()
        result['Get_Sucess'] = f"Response from server/{endpoint}: {response_data}"
    else:
        # 요청이 실패한 경우 오류 코드 출력
        result['Get_Error'] =  f"Request failed with status code/{endpoint}: {response.status_code}"
    return result
