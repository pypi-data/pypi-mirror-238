...
import requests
import json
MAIN_URL = f"http://localhost:8000"
...
def get_ipynb_directory_and_index2(current_input=None):
    if current_input is None:
        # 현재 입력이 제공되지 않는 경우, IPython 환경에서 현재 입력을 가져옵니다.
        from IPython import get_ipython
        if get_ipython() is not None:
            current_input = get_ipython().get_parent().all_cells[-1].source
        else:
            raise ValueError("현재 입력이 제공되지 않았고 IPython 환경에서 실행 중이 아닙니다.")
    ...
    if matching_cell_index is not None and now_ipynb_file is not None:
        return (current_directory + "\\" + now_ipynb_file, matching_cell_index)
    else:
        return None
...
def post_api(endpoint,data):
    ...
    if response.status_code == 200:
        ...
        result['Post_Sucess'] = f"Response from server/{endpoint}"
        result.update(response.json())
    else:
        # 요청이 실패한 경우 오류 코드 출력
        result['Post_Error'] =  f"Request failed with status code{response.status_code}/{endpoint}"
    
    return result

def get_api(endpoint,data):
    result = {}
    url = f"{MAIN_URL}/{endpoint}"
    response = requests.get(url, json= data)
    if response.status_code == 200:
        response_data = response.json()
        result['Get_Sucess'] = f"Response from server/{endpoint}"
    else:
        # 요청이 실패한 경우 오류 코드 출력
        result['Get_Error'] =  f"Request failed with status code/{endpoint}"
    result.update(response.json())
    return result
