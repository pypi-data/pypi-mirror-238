...
def segment_image(image: Union[str, np.ndarray], 
    ...
    if image.ndim > 2:
        ...
        masks, _, _ = model.eval(img, diameter=model.diameter, min_size=model.min_size)
    else:
...
def extract_patches(image: Union[str, np.ndarray], 
    ...
    masks, original_image = segment_image(image, model, cellpose_kwargs=cellpose_kwargs, nuclear_channel=nuclear_channel)
