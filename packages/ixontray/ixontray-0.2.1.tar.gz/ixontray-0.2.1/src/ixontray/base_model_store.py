...
class BaseModelStore(Generic[T]):
    ...
    def load(self) -> T:
        ...
        with self._lock:
            logger.info(f"Loading {self._determine_model()} from: {self._filepath}")
            ...
            if not self._valid_file_ensured():
                ...
                self._data = self._determine_model().parse_obj(parsed)
    ...
    def save(self, data: T) -> None:
        ...
        with self._lock:
            ...
            with self._filepath.open("w+") as stream:
                logging.info(f"Saving to: {self._filepath}")
