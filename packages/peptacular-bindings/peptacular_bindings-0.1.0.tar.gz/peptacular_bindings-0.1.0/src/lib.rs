...
fn get_modifications(peptide: &str) -> HashMap<isize, String> {
...
fn strip_modifications(modified_peptide: &str) -> String {

    let (modified_peptide, _c_terminal_mod) = extract_c_terminal_modifications(modified_peptide);
    let (modified_peptide, _n_terminal_mod) = extract_n_terminal_modifications(&modified_peptide);
...
#[pyfunction]
fn get_modifications_py(peptide: &str) -> PyResult<HashMap<isize, String>> {
    let modifications = get_modifications(peptide);
    Ok(modifications) // Wrap the result in `Ok(...)`
}

// Python bindings for strip_peptide
#[pyfunction]
fn strip_modifications_py(modified_peptide: &str) -> PyResult<String> {
    let unmodified_peptide = strip_modifications(modified_peptide);
    Ok(unmodified_peptide) // Wrap the result in `Ok(...)`

}

#[pymodule]
fn peptacular_bindings(_py: Python, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(get_modifications_py, m)?)?;
    m.add_function(wrap_pyfunction!(strip_modifications_py, m)?)?;
    Ok(())
}
