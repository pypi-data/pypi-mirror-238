...
from anyscale.utils.ray_version_utils import get_alias_for_base_image_name
    ...
    def build_cluster_environment(
        ...
        if not cluster_environments:
            ...
            if _is_create_byod_cluster_environment(create_cluster_environment):
                ...
                if base_image_name != get_alias_for_base_image_name(base_image_name): # We need to make sure base_image_name after ray 2.7.0, before 2.8.0 have a optimized suffix 
                    self.log.error(f"Starting from Ray version 2.7.0 and before version 2.8.0, you might be required to include the suffix 'optimized' in the base_image name for that specific Ray version. Try using {self.log.highlight(get_alias_for_base_image_name(base_image_name))}")
                    return
