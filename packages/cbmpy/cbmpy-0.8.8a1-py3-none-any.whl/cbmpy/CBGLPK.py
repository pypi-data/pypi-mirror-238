...
try:
    import swiglpk as sw

    lp = sw.glp_create_prob()
...
def glpk_constructLPfromFBA(fba, fname=None):
    ...
    lp = sw.glp_create_prob()
    sw.glp_set_prob_name(lp, fba.getId())
    sw.glp_add_cols(lp, len(fba.N.col))
    ...
    for n_ in range(Nmat.shape[1]):
        ...
        sw.glp_set_col_name(lp, 1 + n_, fba.N.col[n_])
    ...
    try:
        ...
        if osense in ['minimize', 'minimise', 'min']:
            sw.glp_set_obj_dir(lp, sw.GLP_MIN)
        elif osense in ['maximize', 'maximise', 'max']:
            sw.glp_set_obj_dir(lp, sw.GLP_MAX)
        else:
    ...
    sw.glp_add_rows(lp, Nmat.shape[0])
    ...
    for r_ in range(Nmat.shape[0]):
        ...
        for c_ in range(Nmat.shape[1]):
            ...
            ind_arr.append(1 + c_)
            val_arr.append(Nmat[r_, c_])

        # lp.rows[r_].name = fba.N.row[r_]
        # lp.rows[r_].matrix = newCon
        sw.glp_set_row_name(lp, 1 + r_, fba.N.row[r_])
        sw.glp_set_mat_row(
            lp,
            1 + r_,
            Nmat.shape[1],
            sw.as_intArray(ind_arr),
            sw.as_doubleArray(val_arr),
        )
    ...
    if fba.CM != None and fba.CM.shape[0] > 0:
        ...
        sw.glp_add_rows(lp, CMmat.shape[0])
        ...
        for r_ in range(CMmat.shape[0]):
            ...
            for c_ in range(CMmat.shape[1]):
                newCon.append((c_, CMmat[r_, c_]))
                ind_arr.append(1 + c_)
                val_arr.append(CMmat[r_, c_])

            # lp.rows[baseRows + r_].name = fba.CM.row[r_]
            # lp.rows[baseRows + r_].matrix = newCon
            sw.glp_set_row_name(lp, 1 + baseRows + r_, CMmat[r_])
            sw.glp_set_mat_row(
                lp,
                1 + baseRows + r_,
                CMmat.shape[1],
            ...
            if fba.CM.operators[r_] in ['<=', '<', 'L']:
                ...
                sw.glp_set_row_bnds(lp, 1 + baseRows + r_, sw.GLP_UP, None, rhs)
            elif fba.CM.operators[r_] in ['>=', '>', 'G']:
                # lp.rows[baseRows + r_].bounds = rhs, None
                sw.glp_set_row_bnds(lp, 1 + baseRows + r_, sw.GLP_LO, rhs, None)
            elif fba.CM.operators[r_] in ['=', 'E']:
                # lp.rows[baseRows + r_].bounds = rhs
                sw.glp_set_row_bnds(lp, 1 + baseRows + r_, sw.GLP_FX, rhs, rhs)
            else:
                raise RuntimeError('INFO: invalid operator: %s' % fba.N.operators[r_])

    # add bounds
    for r_b in fba.reactions:
        lb = ub = None
        lb = fba.getReactionLowerBound(r_b.getId())
        ub = fba.getReactionUpperBound(r_b.getId())
        ...
        if ub != GLPK_INFINITY and lb != -GLPK_INFINITY and ub == lb:
            ...
            sw.glp_set_col_bnds(lp, varMap[r_b.getId()] + 1, sw.GLP_DB, lb, ub)

        elif ub != GLPK_INFINITY:
            # lp.cols[varMap[r_b.getId()]].bounds = None, ub
            sw.glp_set_col_bnds(lp, varMap[r_b.getId()] + 1, sw.GLP_UP, ub, ub)
        elif lb != -GLPK_INFINITY:
            # lp.cols[varMap[r_b.getId()]].bounds = lb, None
            sw.glp_set_col_bnds(lp, varMap[r_b.getId()] + 1, sw.GLP_LO, lb, lb)
        else:
            # lp.cols[varMap[r_b.getId()]].bounds = None
            sw.glp_set_col_bnds(lp, varMap[r_b.getId()] + 1, sw.GLP_FR, 0, 0)
