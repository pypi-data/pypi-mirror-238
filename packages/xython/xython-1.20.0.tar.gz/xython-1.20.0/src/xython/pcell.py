...
import win32gui, win32api, win32con  # pywin32의 모듈
...
class pcell:
	...
	def make_ppt_table_from_xl_data(self, ):
		...
		"""
		excel = pcell.pcell("")
		activesheet_name = excel.read_name_for_activesheet()
		[x1, y1, x2, y2] = excel.read_address_for_selection()
		print([x1, y1, x2, y2])
		...
		for y in range(y1, y2 + 1):
			for x in range(x1, x2 + 1):
				value = excel.read_value_in_cell(activesheet_name, [x, y])
