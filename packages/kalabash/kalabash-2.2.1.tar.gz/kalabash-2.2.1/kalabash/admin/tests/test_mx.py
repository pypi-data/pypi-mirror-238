...
class MXTestCase(KbashTestCase):
    ...
    def test_single_domain_update(
        ...
        management.call_command(
            "kbash", "check_mx", "--domain", self.domain.name)
        self.assertTrue(
            models.MXRecord.objects.filter(domain=self.domain).exists())
        self.assertFalse(
        ...
        management.call_command(
            "kbash", "check_mx", "--domain", str(self.bad_domain.pk))
        self.assertFalse(
            models.MXRecord.objects.filter(domain=self.bad_domain).exists())
        self.assertEqual(len(mail.outbox), 1)

        management.call_command(
            "kbash", "check_mx", "--domain", "toto.com")
...
class DNSBLTestCase(KbashTestCase):
    ...
    def test_notifications(
        ...
        management.call_command(
            "kbash", "check_mx", "--email", "user@example.test")
