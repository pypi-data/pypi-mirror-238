...
A set of tools to use DMARC through Kalabash.

This feature is still in BETA stage, for now it only parses XML aggregated
reports and generate visual reports on a per-domain basis.
...
.. note::
   ...
   kinstaller can automatically set it up for you.
...
First, declare a new service in ``/etc/postfix/master.cf``::
  ...
  dmarc-rua-parser unix  -       n       n       -       -       pipe
    flags= user=vmail:vmail argv=<path to python> <path to kalabash instance>/manage.py import_aggregated_report --pipe
