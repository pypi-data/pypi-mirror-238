...
class AccountViewSetTestCase(KbashAPITestCase):
    ...
    def test_failed_api_token(self):
        ...
        with self.assertLogs(logger='kalabash.auth', level='WARNING') as log:

            resp = self.client.post(url, data, format="json")
            self.assertEqual(resp.status_code, 401)
            self.assertIn(
                "WARNING:kalabash.auth:Failed connection attempt from '127.0.0.1'"
...
class AuthenticationTestCase(KbashAPITestCase):
