...
__all__: Sequence[str] = ('ChatSSAComponent',)
...
class ChatSSAComponent:
    """ChatSSA Streamlit Component."""

    # some typing for clarity to developers/maintainers
    # =================================================
    Uid: type = int | str | UUID  # accepted type(s) for unique IDs of ChatSSAComponent instances & SSA conversations
    ...
    SSAS_SSS_KEY: str = '_ssms'
    SSA_CONVO_IDS_SSS_KEY: str = '_ssm_convo_ids'
    SSA_INTROS_SSS_KEY: str = '_ssm_intros'

    SSA_CONVO_NEXTQ_SSS_KEY: str = '_ssm_nextq'
    SSA_CONVO_QBOX_SSS_KEY: str = '_ssm_qbox'

    def __init__(self, unique_name: Uid,
                 knowledge_src_path: DirOrFilePath = '', knowledge_src_file_relpaths: FilePathSet = frozenset()):
        """Initialize and start running ChatSSAComponent instance."""
        ...
        # set Unique Name
        assert unique_name, ValueError('ChatSSAComponent instance requires explicit Unique Name')
    ...
    def _init_sss(cls):
        ...
        if cls.SSAS_SSS_KEY not in sss:
            sss[cls.SSAS_SSS_KEY]: defaultdict[cls.KnowledgeSrcHash, RAGSSM | None] = defaultdict(lambda: None)
    ...
    def ssm(self) -> RAGSSM | None:
        if ssm := sss[self.SSAS_SSS_KEY][self._hashable_knowledge_src_repr]:
            return ssm
        ...
        if st.button(label='Build SSA',
            ...
            sss[self.SSAS_SSS_KEY][self._hashable_knowledge_src_repr]: RAGSSM = ssm
            return ssm
