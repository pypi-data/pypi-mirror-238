...
def noise(timelst, sample, tau, sigma,
    ...
    init_val    -> Set initial value for the noise for each sample. Is an array of length 
                   equal to the number of samples.
                   WARNING: This is for [noise_tl]. Making noise with fixed initial value 
                            results in incorrect simulation.
...
def noise_tl(timeline, sample, tau, sigma, seed = None):
    ...
    I mean that the random numbers generated are equal, but the results might not be
    ...
    new_seed = [seed * new_rng.integers(12345, 56789) for i in range(l-1)]

    out_arr = [noise(timelst = timeline[0], sample = sample, 
                     tau=tau, sigma=sigma, seed=seed)]
    for i in range(l-1):
        ...
        out_arr.append(noise(timelst = timeline[i + 1], sample = sample, 
                             tau = tau, sigma = sigma, seed = new_seed[i], 
                             init_val = init_val))
