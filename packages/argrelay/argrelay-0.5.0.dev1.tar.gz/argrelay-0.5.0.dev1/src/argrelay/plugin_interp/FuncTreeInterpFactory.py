...
class FuncTreeInterpFactory(FuncArgsInterpFactory):
    ...
    def load_func_envelopes(
        ...
        for delegator_plugin_id in context_config[delegator_plugin_ids_]:
            assert delegator_plugin_id in server_config.action_delegators, f"plugin `{delegator_plugin_id}` must be activated via {plugin_instance_id_activate_order_dac_}"
