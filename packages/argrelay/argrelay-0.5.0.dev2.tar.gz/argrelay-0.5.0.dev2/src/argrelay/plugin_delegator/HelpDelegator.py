...
class HelpDelegator(InterceptDelegator):
    ...
    def get_supported_func_envelopes(
        ...
        func_envelopes = [{
            instance_data_: {
                func_id_: SpecialFunc.help_func.name,
            ...
            ReservedArgType.FuncId.name: SpecialFunc.help_func.name,
        }]
    ...
    def invoke_action(invocation_input: InvocationInput):
        ...
        if get_func_id_from_invocation_input(invocation_input) == SpecialFunc.help_func.name:
            for data_envelope in (
                ...
                # TODO: Print all `tree_path_selector_prefix_`-related attributes somehow.
                #       Currently, it is based on `search_control` but this is incomplete if there are
                #       several interps to reach func envelope.
                #       Maybe just print all `tree_path_selector_prefix_`?
