...
class InterpTreeInterp(AbstractInterp):
    ...
    def next_interp(self) -> "AbstractInterp":
        # TODO: Selected `interp_factory_id` cannot identify one of the paths in the tree
        #       because each `interp_factory_id` can be plugged into multiple leaves.
        #       populate `interp_tree_path` based on `InterpTreeContext` on creation of this interp
        #       plus based on the path to that `interp_factory_id` in the tree.
