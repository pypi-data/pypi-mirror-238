...
def ms_to_datetime(ms: float|int|str) -> Optional[DateTime]:
    ...
    if dt < YEAR_2000:
        log.warning(f"{ms} seems like it's probably epoch seconds seconds not milliseconds...")
...
def seconds_to_datetime(seconds: float|int|str) -> DateTime:
    ...
    try:
        ...
        log.warning(f"{seconds} seems like it's probably epoch milliseconds seconds not seconds...")
