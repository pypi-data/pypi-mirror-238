...
class light_curve(observation):
    ...
    def read_from_fits(self, filename, data_ind = 1, gti_ind = 2):
        ...
        # We cannot use errors >0 for sparse LCs
        mask = np.isfinite(self._rates_) & np.isfinite(self._drates_) & np.isfinite(self._times_) #& (self._drates_>0)

        self._times_ = self._times_[mask]
        self._rates_ = self._rates_[mask]
        self._drates_ = self._drates_[mask]
        ...
        try:
            ...
            logger.warning("Not found GTI, ignoring")
            self._gti_header_ = None
    ...
    def dump_info(self):
        fname = getattr(self, '_filename_', 'none')
        logger.info("The light curve %s has %d bin" % (fname, len(self._times_)))
        if hasattr(self, '_gti_start_'):
            logger.info("The light curve %s has %d GTI bin" % (fname, len(self._gti_start_)))
    ...
    def plot_lc(self, source_name=None, systematic_fraction=0, ng_sig_limit=3):
...
def common_times_lc(soft, hard):

    new_times, ind_soft, ind_hard = np.intersect1d(soft._times_, hard._times_, assume_unique=False, return_indices=True)

    soft._times_ = new_times
    hard._times_ = new_times

    soft._dtimes_ = soft._dtimes_[ind_soft]
    hard._dtimes_ = hard._dtimes_[ind_hard]
    
    soft._rates_ = soft._rates_[ind_soft]
    soft._drates_ = soft._drates_[ind_soft]

    hard._rates_ = hard._rates_[ind_hard]
    hard._drates_ = hard._drates_[ind_hard]

    # hard._frac_exp_ = hard._frac_exp_[ind_hard]
    # hard._frac_exp_ = hard._frac_exp_[ind_hard]

    logger.info('After finding common times ligh curves have the properties:')
    soft.dump_info()
    hard.dump_info()
...
def hratio_func(soft_lcname, hard_lcname, hratio_fname, min_sn: float, max_time=1e4, gti_threshold=100, flag_rebin=1,
    ...
    soft_lc, hard_lc = common_times_lc(soft_lc, hard_lc)
    ...
    if  soft_lc.rebin(table) > 0:
        raise Exception('No rebin table')
    if hard_lc.rebin(table) > 0:
        raise Exception('No rebin table')
