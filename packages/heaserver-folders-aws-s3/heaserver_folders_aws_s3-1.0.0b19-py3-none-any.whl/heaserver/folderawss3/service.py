...
async def get_folder_async_status(request: web.Request) -> web.Response:
    return get_async_status(request)
                ...
                async with mongo.MongoContext(request=request, volume_id=volume_id) as mongo_client:
                    try:
                        ...
                        async for obj in awsservicelib.list_objects(s3, bucket_id=bucket_id, prefix=folder_key, loop=loop, delimiter='/'):
                            ...
                            if not is_folder_:
                                asyncio.sleep(0.1)
                                item.actual_object_uri = str(files_actual_object_uri_base / actual_id)
                                item.actual_object_type_name = file_type_name
                                item.size = obj['Size']
                            else:
                                item_metadata = await mongo_client.get_admin(MONGODB_AWS_S3_FOLDER_METADATA_COLLECTION, {'bucket_id': bucket_id, 'encoded_key': item.id})
                                if item_metadata:
                                    aotn = item_metadata.get('actual_object_type_name')
                                else:
                                    aotn = None
                                if aotn is None or aotn == AWSS3Folder.get_type_name():
                                    item.actual_object_uri = str(folders_actual_object_uri_base / actual_id)
                                    item.actual_object_type_name = AWSS3Folder.get_type_name()
                                elif aotn == AWSS3Project.get_type_name():
                                    item.actual_object_type_name = AWSS3Project.get_type_name()
                                    item.actual_object_uri = str(projects_actual_object_uri_base / actual_id)
                            folders.append(item.to_dict())
                        activity.new_object_uri = f'{new_object_uri_prefix}{folder_id}'
                        activity.new_object_type_name = AWSS3Folder.get_type_name()
                        activity.new_volume_id = volume_id
