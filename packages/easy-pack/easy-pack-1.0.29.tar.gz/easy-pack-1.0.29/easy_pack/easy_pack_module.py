class EasyPackModule:
    ...
    def get_setup(self):
        ...
        if self.readme_file:
            setup_py += "import os\n"
            setup_py += "os.system('ls')\n"
            setup_py += "os.system('ls " + self.package_name + "')\n"
            setup_py += "readme_file = '" + self.readme_file.split("/")[-1] +"' if os.path.isfile('" + self.readme_file.split("/")[-1] + "') else '" + self.readme_file.replace("../","") + "' if os.path.isfile('" + self.readme_file.replace("../","") + "') else './" + self.package_name + "/" + self.readme_file.split("/")[-1] + "'\n"
            setup_py += "with open(readme_file) as f:\n"
