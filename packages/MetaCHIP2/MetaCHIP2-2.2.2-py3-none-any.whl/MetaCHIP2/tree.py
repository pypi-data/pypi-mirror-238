...
MetaCHIP2 tree -o get_tree_wd -i gnm_dir -x fna -t 12 -f -db db_dir
MetaCHIP2 tree -o get_tree_wd -i gnm_dir -x fna -t 12 -f -db db_dir -c gnm_taxon.txt
...
def tree(args):
    ...
    msa_bac120_gz                   = '%s/align/gtdb.bac120.user_msa.fasta.gz'      % tmp_dir
    msa_bac120                      = '%s/align/gtdb.bac120.user_msa.fasta'         % tmp_dir
    msa_ar53_gz                     = '%s/align/gtdb.ar53.user_msa.fasta.gz'        % tmp_dir
    msa_ar53                        = '%s/align/gtdb.ar53.user_msa.fasta'           % tmp_dir
    ...
    if gnm_tax_txt is None:
        ...
        cmd_classify_wd = 'gtdbtk classify_wf --cpus %s --pplacer_cpus 1 --skip_ani_screen --genome_dir %s --extension %s --out_dir %s --prefix gtdb > %s' % (num_threads, input_gnm_dir, file_extension, tmp_dir, gtdbtk_classify_wf_stdout_txt)
        ...
        cmd_identify = 'gtdbtk identify --genome_dir %s -x %s --out_dir %s --cpus %s > %s' % (input_gnm_dir, file_extension, tmp_dir, num_threads, gtdbtk_identify_stdout_txt)
        with open(log_txt, 'a') as log_txt_handle:
        ...
        cmd_align = 'gtdbtk align --identify_dir %s --out_dir %s --cpus %s > %s' % (tmp_dir, tmp_dir, num_threads, gtdbtk_align_stdout_txt)
        with open(log_txt, 'a') as log_txt_handle:
    ...
    if os.path.isfile(msa_bac120_gz) is True:
        cmd_infer_bac120 = 'gunzip %s; gtdbtk infer --msa_file %s --out_dir %s --cpus %s --prefix bac120 > %s' % (msa_bac120_gz, msa_bac120, tmp_dir, num_threads, gtdbtk_infer_bac_stdout_txt)
        with open(log_txt, 'a') as log_txt_handle:
    ...
    if os.path.isfile(msa_ar53_gz) is True:
        cmd_infer_ar53 = 'gunzip %s; gtdbtk infer --msa_file %s --out_dir %s --cpus %s --prefix ar53 > %s' % (msa_ar53_gz, msa_ar53, tmp_dir, num_threads, gtdbtk_infer_ar_stdout_txt)
        with open(log_txt, 'a') as log_txt_handle:
