...
def iothink_check_std_curve2(df,model):
	d1 = iothink_shift_corr(df,model["power"])
	d12 = max(pearsonr(df,model["power"])[0],0.01) #负数计算置信度会有问题，需要加上最小值限制
	d2 = max(df)
	d3 = sum(df)
	d4 = sum(iothink_crossline(df,model["power"]))/sum(model["power"])
	hmean = (model["d21"]+model["d22"])/2
	amean = (model["d31"]+model["d32"])/2							 
	c = round(iothink_corr_confidence(d12),5)
	return c,d1,d12,d2,d3,d4,hmean,amean
    
def iothink_check_std_curve3(df,model):
	d1 = iothink_shift_corr(df,model["power"])
	d12 = max(pearsonr(df,model["power"])[0],0.01)
	d2 = max(df)
	d3 = sum(df)
	d4 = sum(iothink_crossline(df,model["power"]))/sum(model["power"])
	hmean = (model["d21"]+model["d22"])/2
	grate = max(1-abs(d2-hmean)/hmean,0)
	amean = (model["d31"]+model["d32"])/2
	arate = max(1-abs(d3-amean)/amean,0)
	sim = round((d12+grate+arate)/3,4)
	c = round(iothink_corr_confidence(d12),5)
	return sim,c

def iothink_check_std_curve4(df,model):
	d1 = iothink_shift_corr(df,model["power"])
	d12 = max(pearsonr(df,model["power"])[0],0.01)
	d2 = max(df)
	d3 = sum(df)
