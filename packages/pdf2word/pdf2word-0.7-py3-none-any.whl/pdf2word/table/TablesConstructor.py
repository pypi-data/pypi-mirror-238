...
class TablesConstructor:
    ...
    def lattice_tables(self, 
        ...
        grouped_strokes = self._shapes.table_strokes \
            .group_by_connectivity(dx=connected_border_tolerance*1.5, dy=connected_border_tolerance*1.5)
        ...
        for strokes in grouped_strokes:
            ...
            # group_fills = fills.contained_in_bbox(strokes.bbox.inflate(connected_border_tolerance)) #strokes.bbox)
            # bbox = strokes.bbox
            # expanded_bbox = Rect(bbox.x0 - connected_border_tolerance,
            #                      bbox.y0 - connected_border_tolerance,
            #                      bbox.x1 + connected_border_tolerance,
            #                      bbox.y1 + connected_border_tolerance)
            # group_fills = fills.contained_in_bbox(expanded_bbox)
    ...
    def stream_tables(self, 
        ...
        tables_lines = self._blocks.collect_stream_lines(table_fillings, line_separate_threshold*1.5)            
