...
class Cell(Element, Layout):
    def __init__(self, raw: dict = None):
        ...
        merged_cells = raw.get('merged_cells', (1, 1))  # type: tuple [int]
        if len(merged_cells) < 4:
            merged_cells = tuple(list(merged_cells) + [1] * (4 - len(merged_cells)))
    ...
    def make_docx(self, table, indexes):
        ...
        # Add the following code for merged cells logic
        if self.merged_cells:
            print(f"merged_cells: {self.merged_cells}")  # Debugging statement
            start_row, start_col, end_row, end_col = self.merged_cells
            print(f"start_row: {start_row}, start_col: {start_col}, end_row: {end_row}, end_col: {end_col}")  # Debugging statement
            cell = table.cell(start_row, start_col)
            for r in range(start_row, end_row + 1):
                for c in range(start_col, end_col + 1):
                    table.cell(r, c).merge(cell)
    ...
    def _set_style(self, table, indexes):
        ...
        # # Add the following code for border logic
        # if self.border_width and self.border_color:
        #     docx_cell.borders.top.width = Pt(self.border_width[0])
        #     docx_cell.borders.top.color.rgb = RGBColor(*self.border_color[0])

        #     docx_cell.borders.bottom.width = Pt(self.border_width[2])
        #     docx_cell.borders.bottom.color.rgb = RGBColor(*self.border_color[2])

        #     docx_cell.borders.left.width = Pt(self.border_width[3])
        #     docx_cell.borders.left.color.rgb = RGBColor(*self.border_color[3])

        #     docx_cell.borders.right.width = Pt(self.border_width[1])
        #     docx_cell.borders.right.color.rgb = RGBColor(*self.border_color[1])

        # # Add the following code for shading logic
        # if self.bg_color:
        #     shading_elm = parse_xml(r'<w:shd {} w:fill="{}"/>'.format(nsdecls('w'), self.bg_color))
