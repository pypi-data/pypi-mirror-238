...
URLS["typesense-server_Linux_x86_64"] = "https://dl.typesense.org/releases/0.25.1/typesense-server-0.25.1-linux-amd64.tar.gz"
...
def SearchFilename(sdir: pathlib.Path, name: str, recursive: bool = True) -> pathlib.Path:
...
def SearchExecutable(bindir: pathlib.Path, binname: str, recursive: bool = True) -> pathlib.Path:
    if recursive:
        localexe = SearchExecutable(bindir.parent, binname, recursive=False)
        if localexe.is_file():
            return localexe
    if sys.platform == "win32":
        path = SearchFilename(bindir, binname + ".exe", recursive=recursive)
        path = SearchFilename(bindir, binname + ".bat", recursive=recursive) if not path.is_file() else path
        path = SearchFilename(bindir, binname + ".cmd", recursive=recursive) if not path.is_file() else path
    elif sys.platform == "linux":
        path = SearchFilename(bindir, binname, recursive=recursive)
        path = SearchFilename(bindir, binname + ".sh", recursive=recursive) if not path.is_file() else path
        if path.is_file():
            try:
...
def _GetBinary(  # noqa: PLR0912
    ...
    bindir = (binpath and pathlib.Path(binpath)) or GetBinPath()
    localexe = SearchExecutable(bindir, binname, recursive=False)
    if localexe.is_file():
    ...
    if (not bindir.exists()) or (not SearchExecutable(bindir, binname).is_file()):
        ...
        while len(subdir) <= 2 and pathlib.Path(subdir[0].path).is_dir():  # noqa: PLR2004
            MoveUp(pathlib.Path(subdir[0].path))
            subdir = list(os.scandir(bindir))
        if len(list(pathlib.Path(bindir).glob("*"))) <= 2:  # noqa: PLR2004
            MoveUp(bindir)
...
def GetTypesenseServer() -> pathlib.Path:
    return GetBinary("typesense-server")
...
def GetNPM():
    if sys.platform == "win32":
        ...
        npm = pathlib.Path(shutil.which("npm", path=GetNodeJS().parent))  # type: ignore
    AddToPath([npm.parent])
    return npm


def AddToPath(paths: list[pathlib.Path], environ: dict[str, str] | _Environ[str] | None = None):
    environ = environ or os.environ
    alreadypaths = set([pathlib.Path(onepath or ".").absolute() for onepath in environ["PATH"].split(os.pathsep)])
