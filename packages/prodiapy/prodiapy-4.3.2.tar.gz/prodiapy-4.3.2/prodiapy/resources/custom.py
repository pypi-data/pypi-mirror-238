...
class AsyncCustom(Engine):
    ...
    async def create(self, endpoint='/sd/generate', **params):
        return await super()._apost(url=f"{self.base}{endpoint}", body=params, api_key=self.api_key)

    async def constant(self, endpoint="/sd/models"):
        return await super()._aget(url=f"{self.base}{endpoint}", api_key=self.api_key)
