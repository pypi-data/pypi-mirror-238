...
def openAutoBackup_oneHour(aligo=None, access_token=None, time_sleep=0):
    ...
    for i in range(0, len(device_info['items'])):
        ...
        if autoStatus is True:
            data = requests.post(url='https://member.alipan.com/v2/activity/sign_in_info', json={},
                                 headers={
                                     'Authorization': f'Bearer {access_token}',
                                     'x-device-id': device_id}).json()

            return True, "获取开启自动备份的设备ID成功", device_id

    return False, "所有设备均为开启自动备份", None
