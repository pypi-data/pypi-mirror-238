...
_levels = ['base', 'terrain', 'objects', 'units', 'zones', 'effects', 'fog_of_war']
...
class BaseGridBlueprint(_AbstractGridBlueprint):
    ...
    def __init__(
        ...
        self._array = _np.array(
                [0 for _ in list(range((self._col_count * self._row_count) * len(_levels)))],
                dtype=type(any),
        ).reshape((self._col_count, self._row_count, len(_levels)))
        self._init()
    ...
    def _init_layers(self):
        for i, level in enumerate(_levels):
            for cell, information in self.dictGrid.items():
                ...
                self.array[information['col_index']][information['row_index']][i] = {f'{attr}': None for attr in _layer_attributes[level]}
            setattr(self, f'dict{level.capitalize()}', self.array[:, :, i])
