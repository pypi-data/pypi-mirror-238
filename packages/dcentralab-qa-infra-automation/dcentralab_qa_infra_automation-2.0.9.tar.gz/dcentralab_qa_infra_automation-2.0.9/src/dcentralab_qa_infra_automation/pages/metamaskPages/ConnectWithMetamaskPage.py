...
class ConnectWithMetamaskPage(BasePage):
    ...
    def is_page_loaded(self):
        ...
        return "connect" in url and self.is_element_exist("CONNECT_WALLET_PERMISSIONS_CONTAINER",
                                                          METAMASK_CONNECT_WALLET_PERMISSIONS_CONTAINER) and "MetaMask" in self.get_text(
            "CONTAINER_TITLE", METAMASK_CONNECT_WALLET_PERMISSIONS_CONTAINER_TITLE)
    ...
    def click_on_connect_button(self):
        ...
        """
        url = self.driver.current_url
        if "confirm-permissions" in url and "Connect" in self.get_text("CONNECT_BUTTON)",
                                                                       METAMASK_CONNECT_WALLET_PERMISSIONS_CONTAINER_CONNECT_BUTTON):
