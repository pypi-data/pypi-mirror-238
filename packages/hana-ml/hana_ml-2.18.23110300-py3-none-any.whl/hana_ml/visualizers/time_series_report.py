...
The following classes are available:
    ...
    * :class:`AdditiveModelForecastExplainer`
    * :class:`BSTSExplainer`
...
from hana_ml.visualizers.report_builder import ReportBuilder, ChartItem, TableItem, ForcePlotItem, DigraphItem
...
class TimeSeriesExplainer(object):
    ...
    def get_items_from_best_pipeline(best_pipeline: dataframe.DataFrame, highlighted_metric_name):
        def convert_to_digraph(pipeline_data) -> MultiDigraph:
        ...
        best_pipeline_pd_df = best_pipeline.collect()
        scores_dict = {}
        id_data = list(best_pipeline_pd_df['ID'].astype(str))
        for json_str_cell in list(best_pipeline_pd_df['SCORES']):
            cell_dict = json.loads(json_str_cell)
            for cell_key in cell_dict:
                if scores_dict.get(cell_key) is None:
                    scores_dict[cell_key] = []
                scores_dict[cell_key].append(cell_dict[cell_key])

        min_max_dict = {}
        x_y_list = []
        for key in scores_dict:
            current_data = scores_dict[key]
            mean = sum(current_data) / len(current_data)
            sum_mean = 0
            for d in current_data:
                sum_mean = sum_mean + abs(float(d) - mean)
            mean = sum_mean / len(current_data) + 0.1
            min_max_dict[key] = [min(current_data) - mean, max(current_data) + mean]
            if key == highlighted_metric_name:
                pass
            else:
                x_y_list.append([key, highlighted_metric_name])
        ...
        option = {
            ...
            'xAxis': {
                'type': 'value',
                'name': highlighted_metric_name,
                'boundaryGap': 'true',
                'min': min_max_dict[highlighted_metric_name][0],
                'max': min_max_dict[highlighted_metric_name][1],
                'axisTick': {
                    'alignWithLabel': 'true',
                    'show': 'false'
                },
            ...
            'series': [
                {
                    'type': 'bar',
                    'colorBy': 'data',
                    'data': scores_dict[highlighted_metric_name],
                    'symbol': 'none',
                    'smooth': 'true',
                    'emphasis': {
                        'focus': 'series'
        ...
        items.append(ChartItem('Metric:{}-{}'.format(highlighted_metric_name, 'ID'), option))

        for x_y in x_y_list:
            option_series = []
            index = 0
            for x in scores_dict[x_y[0]]:
                y = scores_dict[x_y[1]][index]
                z = id_data[index]
                index = index + 1
        ...
        logs_item = TableItem('Logs')
        logs_item.addColumn('ID', id_data)
        for name in ['PIPELINE', 'SCORES']:
            logs_item.addColumn(name, list(best_pipeline_pd_df[name]))
        items.append(logs_item)

        metrics_item = TableItem('Metrics')
        metrics_item.addColumn('ID', id_data)
        for name in scores_dict:
            metrics_item.addColumn(name, scores_dict[name])
        items.append(metrics_item)

        pipeline_item = DigraphItem('Pipelines', convert_to_digraph(best_pipeline_pd_df['PIPELINE']))
...
class DatasetAnalysis(object):
    ...
    def seasonal_item(self):
        ...
        """
        col = self.endog
        df = self.dataset.select([self.key, col]).generate_feature(targets=[self.key], trans_func="YEAR")
        df_columns = df.columns
        year_column_name = df_columns[len(df_columns) - 1]
        ordered_year_names = list(set(df.collect()[year_column_name]))
        ...
        for year_name in ordered_year_names:
            ...
            temp_df = df.filter('"{}"={}'.format(year_column_name, year_name))
            temp_df = temp_df.generate_feature(targets=[self.key], trans_func="MONTH")
            temp_df_columns = temp_df.columns
            month_column_name = temp_df_columns[len(temp_df_columns) - 1]
            temp_df = temp_df.agg([('avg', col, 'MONTH_AVG')], group_by=month_column_name).sort(cols=[month_column_name]).collect()

            x = []
            y = []
            for xd in list(temp_df.iloc[:, 0]):
                xd = convert_value_to_target_type(xd)
                push_valid_value(x, xd)
            x.insert(0, 'MONTH')
            for yd in list(temp_df.iloc[:, 1].astype(float)):
                yd = convert_value_to_target_type(yd)
                push_valid_value(y, yd)
