...
__execute_desc = {
	"name": "execute",
	"description": "executes a command on the user's linux computer and returns its output",
	"parameters": {
		"type": "object",
		"properties": {
			"command": {
...
def __get_page(*args, **kwargs):
	...
	rdict["page"] = ""
	try:
		page = bs4.BeautifulSoup(requests.get(url).text).text
		while '\n\n' in page:
			page = page.replace('\n\n', '\n')
		while '\t\t' in page:
			page = page.replace('\t\t', '\t')
...
__get_page_desc = {
	"name": "get_page",
	"description": "gets the requested page as an HTML string",
	"parameters": {
		"type": "object",
		"properties": {
			"url": {
