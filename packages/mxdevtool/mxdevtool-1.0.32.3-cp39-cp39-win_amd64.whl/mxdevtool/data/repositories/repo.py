...
class FolderRepository(Repository):
    ...
    def _action_holiday(self, calendar: mx.Calendar, line: str):
        date_act = line.strip('\n').split(',') 
        date = date_act[0] 
        act = date_act[1]
        
        if act == 'add':
            calendar.addHoliday(mx.DateParser.parseISO(date))
        elif act == 'remove':
            calendar.removeHoliday(mx.DateParser.parseISO(date))
        else:
