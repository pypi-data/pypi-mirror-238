...
class SSH(object):
    def __init__(self, hostname, username, password=None, port=22, **kwargs):
        ...
        Example:
            ...
            ssh.run_command_list(['enable', 'show run'])
            ssh.run_command_list(['enable', 'show run'], True) # return last command output
            ssh.run_command_list(['show version', 'show clock', 'show running-config'], prompt=ssh.get_prompt()) # reuse prompt
    ...
    def channel(self):
        if not hasattr(self, '_channel'):
            self._channel = self.ssh.invoke_shell(width=1000, height=100000)  # @2023-10-31 add width
            if self._timeout > 0:
                self._channel.settimeout(self._timeout)
        return self._channel

    def run_command(self, command, recv=True, clean=True, prompt=None):
        """
        Run the command.
        ...
        .. versionupdated::
            ...
            1.6.4   - @2023-10-30 add prompt parameter and logic
        ...
        if self._is_secondary_login(command, output):
            if self._secondary_login(output) is False:
                return output
            else:
        ...
        if clean is not False:
            output = _clean_output_info(output, command, prompt)

        return output

        # --------------------------------------------------------------

    def run_command_list(self, command_list, last_result=False, prompt=None):
        """
        Run a command list.
        ...
        Example:
         ...
         :param prompt: the prompt of the command line(for reuse)
        ...
        for command in command_list:
            re_list.append(self.run_command(command, prompt=prompt))
    ...
    def _is_secondary_login(self, command, output):
        ...
        return command.lower().startswith(enable_commands)
    ...
    def _recv_data(self, prompt=None):
        ...
        while True:
            ...
            if len(info) < 1024:  # read speed > write speed
                if prompt is not None and info.endswith(prompt):
                    break
...
def _clean_output_info(txt, command, prompt):
    """
    ...
    cmd_start = txt.rfind(cmd_prompt)
    if cmd_start > -1:
        txt = txt[cmd_start + len(cmd_prompt):]

    prompt_start = txt.rfind(prompt)
    if prompt_start > -1:
        txt = txt[:prompt_start]
    return txt.strip()
