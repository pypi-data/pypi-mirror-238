...
COLOR_DICT = {1: (1,0,0), 2: (0,1,0), 3:(0,0,1)}
...
def draw_boxes_on_patch(patch, boxes, labels, scores, threshold=0.5, thickness=1):
    """
    ...
    for box, label, score in zip(boxes, labels, scores):
        if score >= threshold:
            min_y, min_x, max_y, max_x = min(box[0], box[2]), min(box[1], box[3]), max(box[0], box[2]), max(box[1], box[3])
            cv2.rectangle(np.ascontiguousarray(patch), (min_x, min_y), (max_x, max_y), COLOR_DICT[label], thickness)
