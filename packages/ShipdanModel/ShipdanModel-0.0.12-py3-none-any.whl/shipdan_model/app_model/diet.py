...
class DayDietDeadlineModification(models.Model):
    """
    [3.9.2] 배송 불가능한 상황 대응
    - DayDiet의 order_deadline이 앞당겨지는 경우 발생
        ex) 월요일 DayDiet에 대해서 기존에는 금요일 12시 마감 -> 목요일 12시 마감 (Order의 deadline이 목요일 12시로 변경됨에 따라)
    - diet_started_at ~ diet_ended_at애 포함되는 DayDiet에 대해서, order_deadline을 입력받은 deadline으로 지정함
    """
    UNDEFINED = 0
    DELIVERY = 1
    STORE = 2
...
class UserMealTagLog(models.Model):
    UNDEFINED = 0
    SET = 1
    UNSET = -1
    ...
    user = models.ForeignKey(User, related_name='meal_tag_logs', on_delete=models.CASCADE)
    category = models.ForeignKey(UserMealTag, related_name='user_logs', on_delete=models.CASCADE)
