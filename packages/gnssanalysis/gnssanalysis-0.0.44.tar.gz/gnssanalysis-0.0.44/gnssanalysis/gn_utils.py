...
def clkq(input_clk_paths, norm, input_bia_paths, reject_re, plot):
    """
    ...
    if reject_re is not None:
        _logging.info(msg=f"Excluding satellites based on regex expression: '{reject_re}'")
        ...
        _logging.info(msg=f"Removed the following satellites from first file: '{sats_to_remove_a}'")
        clk_b = clk_b[~reject_mask_b]
        _logging.info(msg=f"Removed the following satellites from second file: '{sats_to_remove_b}'")
    ...
    if input_bia_paths is not None:
        ...
        _logging.info(msg="applying IF biases")
        diff_clk -= biasIF_sum
    ...
    buf = []
    for lvl_name in ["CODE", "GNSS"]:
        df_grouped = gn_aux.df_groupby_statistics(diff_clk_series, lvl_name)
        buf.append(df_grouped.round(4).to_csv(sep="\t", index=True, header=True))

    divider = "=" * 30 + "\n"
    _logging.info(msg="clkq\n" + divider.join(buf))
