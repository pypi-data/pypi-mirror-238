...
def _series_str_degminsec2deg(a: _pd.Series) -> _pd.Series:
    ...
    return _pd.Series(_ndarr_float_degminsec2deg(degminsec_series.to_numpy()), index=degminsec_series.index)
...
def degminsec2deg(a: _Union[_pd.Series, _pd.DataFrame, list, str]) -> _Union[_pd.Series, _pd.DataFrame, float]:
...
def df_groupby_statistics(df: _Union[_pd.Series, _pd.DataFrame], lvl_name: _Union[list, str]):
...
def _get_trend(dataset, deg=1):
    ...
    return _pd.DataFrame(p[0] * x_original[:, _np.newaxis] + p[1], index=x_original, columns=dataset.columns)


def remove_outliers(
    dataframe: _pd.DataFrame, cutoff: _Union[int, float, None] = None, coeff_std: _Union[int, float] = 3
) -> _pd.DataFrame:
