...
from data_diff_customize.abcs.compiler import AbstractCompiler, Compilable
from data_diff_customize.queries.extras import ApplyFuncAndNormalizeAsString, Checksum, NormalizeAsString
from data_diff_customize.utils import ArithString, is_uuid, join_iter, safezip
from data_diff_customize.queries.api import Expr, table, Select, SKIP, Explain, Code, this
from data_diff_customize.queries.ast_classes import (
    Alias,
    BinOp,
    CaseWhen,
    Cast,
...
from data_diff_customize.abcs.database_types import (
    Array,
    ColType_UUID,
    FractionalType,
    Struct,
...
class BaseDialect(abc.ABC):
    ...
    def compile(self, compiler: Compiler, elem, params=None) -> str:
        ...
        if compiler.root and isinstance(elem, Compilable) and not isinstance(elem, Root):
            from data_diff_customize.queries.ast_classes import Select
...
class Database(abc.ABC):
    ...
    def query(self, sql_ast: Union[Expr, Generator], res_type: type = None):
        ...
        if "sum" in sql_code: sql_code = sql_code.replace("'", "")
