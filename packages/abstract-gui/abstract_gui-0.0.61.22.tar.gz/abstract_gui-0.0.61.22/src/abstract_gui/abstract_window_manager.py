...
class AbstractWindowManager:
    ...
    def add_window(self, window_name=None, title=None, default_name=True, 
        ...
        if len(self.global_windows) == 0 or set_current:
            current = True
        self.global_windows.append({"name":window_name,"method":make_component('Window', title=title, *args, **kwargs),"event":False,"values":False,"current":current})
    ...
    def get_window(self,window_name=None,window=None):
        window_enumeration = get_any_enumeration(self,window_name=window_name,window=window)
        return self.global_windows[window_enumeration]["window"]
    ...
    def update_value(self, key, value=None, args=None,window_name=None,window=None,):
        window_enumeration = get_any_enumeration(self,window_name=window,window=window)
        if args:
            self.global_windows[window_enumeration][key].update(**args)
        else:
            self.global_windows[window_enumeration][key].update(value=value)
### current_window
    def set_current_window(self, window):
        window_enumeration = get_any_enumeration(self,window_name=window,window=window,current_window=True)
...
### get window method
    def get_window_method(self,window):
        window_enumeration = get_any_enumeration(self,window_name=window,window=window,current_window=False)
        if window_enumeration:
            return self.global_windows[window_enumeration]["method"]
### window read
    def update_read_values(self,enumeration):
        ...
        return event,values
    def read_window(self,window_name=None,window=None):
        window_enumeration = self.get_any_enumeration(window_name=window_name,window=window)
        if window_enumeration != None:
           return self.update_read_values(window_enumeration)
        else:
...
### window event and values
    def get_window_info(self,key,window_name=None,window=None):
        ...
        if window_enumeration:
            ...
            if not values:
                read_window(self,window=self.global_windows[window_enumeration]["method"])
            return self.global_windows[window_enumeration][key]
    
    def get_event(self,window_name=None,window=None):
        event = get_window_info(self,"event",window_name=window_name,window=window)
        
    def get_values(self,window_name=None,window=None):
        return get_window_info(self,"values",window_name=window_name,window=window)
...
window = window_mgr.add_window(layout=[[]])
window_mgr.read_window()
while True:
    print('going')
    event,values=window_mgr.read_window()
    print(event,values)
    if len(window_mgr.global_windows)==0:
        break
