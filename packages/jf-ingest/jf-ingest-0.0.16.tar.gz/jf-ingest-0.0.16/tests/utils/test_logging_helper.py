...
def test_log_entry_exit_log_level(caplog):
    ...
    assert (
        f"{_test_function_for_wrapper.__name__}: Starting"
        not in record_message_counts.keys()
    )
    assert (
        f"{_test_function_for_wrapper.__name__}: Ending"
        not in record_message_counts.keys()
    )
    ...
    # Assert that the starting and ending log got called exactly once
    assert (
        record_message_counts[f"{_test_function_for_wrapper.__name__}: Starting"] == 1
    )
...
def test_log_standard_error_smoke_test(caplog):
    ...
    assert len(caplog.records) == 1
    print(caplog.records[0].message)
    print(logging_helper.ERROR_MESSAGES.get(error_code).format(*msg_args))
