    ...
    if args.command == "start":
        ...
        with ExitStack() as stack:
            if args.protocol == "asio" or args.protocol == "both":
                servers.append(
                    {
                        "server": stack.enter_context(Server(port, args.zoo, "asio")),
                        "port": port,
                        "protocol": "asio",
                    }
                )
            ...
            if args.protocol == "http" or args.protocol == "both":
                servers.append(
                    {
                        "server": stack.enter_context(Server(port, args.zoo, "http")),
                        "port": port,
                        "protocol": "http",
                    }
                )
            ...
            def keyboard_handler():
                try:
                    msg = ""
                    for s in servers:
                        msg += f"DeGirum {s['protocol']} server is started at TCP port {s['port']}\n"
...
def _server_args(parser):
    ...
    parser.add_argument(
        "--port", type=int, default=8778, help="[start] TCP port to bind to"
    )
    parser.add_argument(
        "--protocol",
