...
def adaux(  # pylint: disable=too-many-arguments
    ctx: click.Context,
    ...
    bootstrap = cls(target, True, fuzzy_finder=not here)  # type: ignore
    with convert_runtime_to_click_error(ctx):
        ctx.obj = bootstrap.type_wo_disabled()(target, silent=silent, show_stacktrace=show_stacktrace, fuzzy_finder=not here)  # type: ignore
...
@click.option("-d", "--disabled", multiple=True, help="Disable parts of adaux.")
@click.option(
    "--no-source",
    is_flag=True,
    default=False,
