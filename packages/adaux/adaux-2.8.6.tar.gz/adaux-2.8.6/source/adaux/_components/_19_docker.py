...
class DockerMixin(PayloadMixin, DependencyMixin, ProjectMixin, MetaMixin):
    ...
    def _bake_docker_build_file(self, payload: _ProtoNamespace) -> None:
        ...
        try:
            ...
            logger.info(
                "Dockerfile: %s is custom (potential overwrite)", opts.docker_name
            )
            return
        except FileNotFoundError:  # then it must be non-custom
            pass

        logger.info("Dockerfile: %s is default", opts.docker_name)
        self.bake_file(
