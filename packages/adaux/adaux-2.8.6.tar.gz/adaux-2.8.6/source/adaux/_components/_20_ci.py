...
class CiMixin(PayloadMixin, MetaMixin):
    ...
    def templated(self, negative_default: bool = False) -> None:
        ...
        if self.is_enabled("pytest"):
            data["+push"]["-tag"]["-openmr"]["-release"]["pytest"] = None
    ...
    def enriched(self) -> None:
        ...
        data.used_tag_rules = self._generate_used_tag_rules(rule_used)

    def _generate_used_tag_rules(
        self, rule_used: tp.Dict[str, bool]
    ) -> tp.Dict[str, tp.Tuple[tp.Tuple[str, ...], str]]:
        tags = []
        for x in self._get_trigger_combos():
            if x[0] == "+tag":
                tags.append(x)
        ...
        return {
            f"{it[0][1:]}-{idx}": (it[1:], create_rule(*it[1:]))
            for idx, it in enumerate(tags)
        }

    def run_ci(  # pylint: disable=too-many-branches
        self, trigger_str: str, dry: bool = False
    ) -> int:
    ...
    def _triggers_from_gitlab_ci(  # pylint: disable=too-many-branches
        self,
    ) -> tp.Sequence[str]:
        triggers = []
        ...
        elif source_trigger == "+tag":
            branch_or_tag = env("CI_COMMIT_TAG")
            for tag_triggers, _ in self.auxe.ci.used_tag_rules.values():
        ...
        elif source_trigger in ["+pipeline", "+web"]:
            branch_or_tag = env("CI_COMMIT_REF_NAME")
        elif source_trigger == "+schedule":
