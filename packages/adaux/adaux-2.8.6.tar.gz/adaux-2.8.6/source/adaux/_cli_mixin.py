...
class CliMixin(  # pylint: disable=too-many-public-methods
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        for cand in candidates:
            if cand.exists():
                self.auxcon_file = cand.resolve()
                workroot = self.auxcon_file.parent
                logger.info("cwd changed to %s", workroot)
    ...
    def demo(
        ...
        if filter_words:
            ...
            last_i = -1
            val = []
            longest_line = max(map(len, keep.values()))
    ...
    def show(self: "CliMixin", show_all: bool = False) -> None:
        out = io.StringIO()
    ...
    def init(  # pylint: disable=too-many-arguments
        ...
        self.auxcon.meta = _ProtoNamespace(disabled=disabled)
        self.target.mkdir(parents=True, exist_ok=True)
        reduced = self.type_wo_disabled()(self.target, greeting=False)  # type: ignore
