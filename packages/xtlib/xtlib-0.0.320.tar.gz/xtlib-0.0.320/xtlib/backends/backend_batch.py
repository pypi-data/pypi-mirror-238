...
class AzureBatch(BackendBase):
    ...
    def __init__(self, compute, compute_def, core, config, username=None, arg_dict=None):
        ...
        #console.print("blob_service_client=", blob_service_client)
    ...
    def get_outfiles_container_url(self, dest_container_name):
        ...
        container_client = self.blob_service_client.get_container_client(dest_container_name)
        ...
        sas_token = generate_container_sas(account_name=account_name, container_name=dest_container_name, account_key=account_key,
            permission=BlobSasPermissions(read=True, write=True, create=True), expiry=self.cert_expiration, start=start_time)    
    ...
    def convert_blobs_to_resource_files(self, container_name, blob_names, file_names, writable=False):
        ...
        for i, blob_name in enumerate(blob_names):
            ...
            sas_token = generate_blob_sas(account_name=account_name, container_name=container_name,
                blob_name=blob_name, account_key=account_key, permission=BlobSasPermissions(read=True),
    ...
    def read_log_file(self, service_node_info, log_name, start_offset=0, end_offset=None, 
        ...
        if log_source != "live":
            ...
            if not found_file:
                # try to read the first run log
                job_num = job_helper.job_num(job_id)
                job_path = "runs/run{}.0/after/service_logs/{}".format(job_num, file_path)
                if self.store.does_job_file_exist(workspace, job_id, job_path):
                    new_text = self.store.read_job_file(workspace, job_id, job_path)
                    batch_status = "completed"
                    simple_status = "completed"
    ...
    def add_service_log_copy_cmds(self, ca, cmds, dest_dir, args):
        # this used to be done by the batch service but stopped working with insufficient diagnostics
        ...
        # non-docker case
        ca.append(cmds, "cp $XT_ORIG_WORKDIR/../std*.txt {}".format(dest_dir))
