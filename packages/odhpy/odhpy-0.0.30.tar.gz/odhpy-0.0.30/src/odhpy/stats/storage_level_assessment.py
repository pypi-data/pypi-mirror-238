...
class StorageLevelAssessment:
    ...
    def __init__(self, df: pd.Series, triggers: list, wy_month=7, allow_part_years=False) -> None:
        ...
        if (self.allow_part_years):
            self.df = df
            self.start_date=df.index[0]
            self.end_date=df.index[-1]
        else:
            self.start_date=utils.get_wy_start_date(df,self.wy_month)
            end_date=utils.get_wy_end_date(df,self.wy_month) + timedelta(days=1) # Add day because last index in .loc is not included
            if (end_date > self.start_date):
                self.df=df.loc[utils.get_dates(self.start_date,end_date)]
            else:
                return np.nan
            self.end_date = end_date - timedelta(days=1) # Remove 1 day for printing output
