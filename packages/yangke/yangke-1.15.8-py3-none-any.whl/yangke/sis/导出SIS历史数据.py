...
def get_history(tags, des, months=12, days=0):
    """
    从RDBProxy代理服务器上导出历史数据，如果导出数据的时标是1970年，请检查tags标签是否正确

    tags: list类型，导出测点的标签，需要与DCS系统上测点的KKS码或标签名一致
    des: list类型，导出测点的描述，可以自定义，用于注释导出的测点，如果传入该参数，则长度必须与tags相同
    month: 到处历史数据的月份
    """
    ...
    if months != 0:
        for i in range(12):
            logger.debug(f"正在导出第{i + 1}个月的历史数据...")
            api = init_dbp_api(ip=ip, port=port, user=user, password=passwd)
            his_value = api.get_his_value(tags=tags, tags_description=des,
            ...
            his_value.to_csv(f"origin_{i + 1}.csv", index=None)  # his_value为原始数据，不删除任何时间的数据
    else:
        api = init_dbp_api(ip=ip, port=port, user=user, password=passwd)
        his_value = api.get_his_value(tags=tags, tags_description=des,
                                      start_time=now - timedelta(days=days, hours=0),
                                      end_time=now - timedelta(days=0, hours=0), time_interval=60)
        # his_value = his_value.set_index("DateTime").rolling(window=10).mean()
        # his_value.dropna(how="any", inplace=True)
        # his_value.drop(his_value.tail(10).index, inplace=True)  # 删掉最后十行数据

        his_value.to_csv(f"最近{days}天历史数据.csv", index=None)  # his_value为原始数据，不删除任何时间的数据
...
if __name__ == "__main__":
    ...
    tags1.extend(tags2)
    des1.extend(des2)
    # get_history(tags1, des1, months=12, days=0)
    # deal_data(work_folder=r"D:\User")
    df = load_history_file("历史数据库.xlsx")
    res = generate_data(df)
    res.to_csv("结果3.csv", index=False)
    # df = find_condition(df=df,
    #                     unit_condition=[("凝汽器热负荷", 360, "1%"), ("环境温度", 33, "±2"), ("环境湿度", 50, "±10")],
    #                     cold_condition={"循泵方式": 'B', "机力塔数量": 8},
    #                     auto_loose=True
    #                     )
    # logger.debug(df.mean(numeric_only=True)["当前背压"])
