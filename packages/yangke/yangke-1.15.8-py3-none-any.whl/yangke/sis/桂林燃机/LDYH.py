...
from yangke.sis.导出SIS历史数据 import find_condition

rho_gas = 0.68  # 天然气密度
ncv_m = 44112  # 天然气热值 kJ/Nm3
ncv_v = ncv_m * rho_gas  # 天然气热值 kJ/Nm3
power_pump_定频泵 = 290  # kW
power_pump_工频 = 290  # kW
功率因数_pump_变频 = 0.81 / 1.732
功率因数_fan = 1 / 1.732
...
def read_data(data_folder=r"C:\Users\YangKe\Desktop\桂林历史数据"):
    ...
    for file in yield_all_file(folder=data_folder, filter_=[".csv"]):
        ...
        data["风机功率1"] = data.apply(lambda x: x["冷却塔1电流"] * 10 * 功率因数_fan if x["冷却塔1电流"] > 1 else 0, axis=1)
        data["风机功率2"] = data.apply(lambda x: x["冷却塔2电流"] * 10 * 功率因数_fan if x["冷却塔2电流"] > 1 else 0, axis=1)
        data["风机功率3"] = data.apply(lambda x: x["冷却塔3电流"] * 10 * 功率因数_fan if x["冷却塔3电流"] > 1 else 0, axis=1)
...
if __name__ == "__main__":
    generate_pickle()
    data = pd.read_pickle("default.pkl")
