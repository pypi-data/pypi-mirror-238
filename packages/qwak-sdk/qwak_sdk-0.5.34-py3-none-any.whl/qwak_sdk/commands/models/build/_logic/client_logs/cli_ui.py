...
from qwak.inner.build_logic.interface.time_source import SystemClockTimeSource, TimeSource, Stopwatch
...
def execute_build_pipeline(
    pipeline,
    build_runner: PhaseRunHandler,
):
    time_source = SystemClockTimeSource()
    for phase in pipeline.phases:
        with build_runner.handle_current_phase(phase):
            phase_run(phase, build_runner, time_source)
...
def phase_run(
    ...
    try:
        ...
        if not isinstance(e, QuietError):
            ...
            phase_run_handler.handle_pipeline_quiet_exception(
                build_id, current_step_phase, e, phase_duration
            )
