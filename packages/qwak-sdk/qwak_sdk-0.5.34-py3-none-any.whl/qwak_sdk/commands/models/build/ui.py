...
from qwak_sdk.commands.models.build._logic.client_logs.build_run_handlers import ProgrammaticPhaseRunHandler, \
    CLIPhaseRunHandler
    ...
    **kwargs,
    ...
    if out_conf:
        ...
        with get_build_logger(config=config, json_logs=json_logs) as (
            ...
            if programmatic:
                build_runner = ProgrammaticPhaseRunHandler(logger, config.verbose, json_logs)
            else:
                build_runner = CLIPhaseRunHandler(logger, log_path, config.verbose, json_logs)
