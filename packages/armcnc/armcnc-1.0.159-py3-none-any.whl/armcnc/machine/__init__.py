...
class Machine:
    ...
    def __init__(self, framework):
        ...
        self.is_alive = False
        self.info = None
        self.data = {"index": 0, "position": {}, "velocity": {}, "current_gcode": "", "g_offset": {}, "g5x_offset": {}, "g92_offset": {}, "dtg_offset": {}, "tool": {}, "options": []}
        self.machine_path = ""
    ...
    def set_data(self, index):
        ...
        if len(self.data["options"]) == 0:
            for key, val in enumerate(range(9)):
                if key > 5:
                    self.data["options"].append({"label": "P" + str(key + 1) + " G59." + str((key - 7) + 1), "p_name": "P" + str(key + 1), "value": key + 1, "name": "G59." + str((key - 7) + 1)})
                else:
