...
class Points2Dataset(Dataset):
    ...
    def normalize(self, set_std):
        if self.iscenter:
            self._g_mean = np.mean(self.data, axis=0, keepdims=True)
            self._g_std = np.std(self.data, axis=0, keepdims=True)
        self._g_set_std = set_std

        self.data = (self.data - self._g_mean) / self._g_std * set_std
    ...
    def restore(self, data):
        data = as_numpy(data)
        sample = data / self._g_set_std * self._g_std + self._g_mean
        return sample
