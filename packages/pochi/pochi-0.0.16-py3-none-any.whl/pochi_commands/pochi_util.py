            ...
            if options.default_connection is None:
                ...
                if self.__connection is None:
                    if (
                        ...
                        and (
                            "password" in options.default_connection
                            or (
                                "private_key_path" in options.default_connection
                                and "private_key_passphrase"
                                in options.default_connection
                            )
                        )
                    ):
                        if "private_key_path" in options.default_connection:
                            with open(
                                options.default_connection.private_key_path, "rb"
                            ) as key:
                                p_key = serialization.load_pem_private_key(
                                    key.read(),
                                    password=options.default_connection.private_key_passphrase.encode(),
