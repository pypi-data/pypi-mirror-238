...
def write(
    ...
    if use_HVG:
        ...
        min_mean = np.percentile(
            np.mean(adata.X.toarray(), axis=0)[np.mean(adata.X.toarray(), axis=0) > 0],
            90,
        )
...
def dynamical_clustering(
    ...
    ax1.legend(bbox_to_anchor=(1.08, 1), loc='upper left',frameon=False,fontsize=fs_lagened)
    if save:
        filename = (
...
def key_gene_dynamics(
    ...
    for i in range(len(target_clusters)):
        for j in range(i + 1, len(target_clusters)):
            columns_ = np.append(
                columns_, target_clusters[i] + "_vs_" + target_clusters[j]
            )
    ...
    for i in range(len(target_clusters)):
        for j in range(i + 1, len(target_clusters)):
            ...
            out_pd_[
                (target_clusters[i] + "_vs_" + target_clusters[j], "gene")
            ...
            out_pd_[
                (target_clusters[i] + "_vs_" + target_clusters[j], target_clusters[i])
            ] = [sign_dict_[str(s_)] for s_ in sign_]
            out_pd_[
                (target_clusters[i] + "_vs_" + target_clusters[j], target_clusters[j])
            ] = [sign_dict_[str(-s_)] for s_ in sign_]
