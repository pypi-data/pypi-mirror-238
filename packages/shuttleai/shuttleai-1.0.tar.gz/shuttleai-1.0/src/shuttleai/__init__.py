...
try:
    CURRENT_VERSION = get("https://pypi.org/pypi/shuttleai/json").json().get("info").get("version")
except:
    CURRENT_VERSION = __version__

if __version__ < CURRENT_VERSION:
    print(
        f"[shuttleai] Version Out-of-Date. Please upgrade by using: \"python.exe -m pip install -U shuttleai\""
    )
