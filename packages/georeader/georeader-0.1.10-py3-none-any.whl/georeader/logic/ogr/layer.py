...
class OGR_Layer:
    ...
    def _layer_features_valid(self):
        """
        The _is_valid_layer function is a helper function that checks to see if the layer has any features.
        If it does not, then an exception is raised. If it does, then the function will check to see if there are
        any null values in the sample features and raise an exception if there are.
        ...
        if feature_count < 1:
            ...
            sample = self.layer.GetNextFeature()
        if sample is None:
            raise Exception('Cannot read features from layer')
    ...
    def get_dataframe(self, schema: list = None) -> gpd.GeoDataFrame:
        ...
        try:
            ...
            raise Exception('failed to get dataframe, cannot read features') from e
