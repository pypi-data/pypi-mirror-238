...
async def _(event:Event) -> Result:
    ...
    if target_group_account.props.get(code,0):
        ...
        fee = int(gold * 0.02)
        Manager.pay_tax((target_user,target_group_account),Manager.locate_group(event.group_id),fee)
...
async def _(event:Event) -> Result:
    ...
    if group_account:
        ...
        return "你的账户\n" + "\n".join(f'{group_id} 金币：{x.gold}枚' for group_id,x in user.group_accounts.items())
...
async def _(event:Event) -> Result:
    ...
    user.avatar_url = event.extra_args.get("avatar")
    info.append(my_info_head(user.gold,user.win,user.lose,group_account.nickname,avatar))
    # 加载卡片
    ...
    # 加载股票信息
    msg = "\n".join(f"{Manager.locate_group(stock).company.company_name}[nowrap]\n[right][color][green]{i}" for stock,i in group_account.invest if i)
    if msg:
...
async def _(event:Event) -> Result:
        ...
        if stock:
            ...
            if (exchange := company.exchange.get(user.user_id)) and exchange.n:
                if exchange.group_id == group_account.group_id:
                    ...
                    account_name = Manager.locate_group(exchange.group_id).company.company_name
    ...
    if info:
        avatar = await event.avatar()
        user.avatar_url = event.extra_args.get("avatar")
        info.insert(0,my_exchange_head(group_account.gold,group_account.nickname,group_account.invest,avatar))
        return info_splicing(info,Manager.BG_path(event.user_id))
    else:
        return "你的股票信息为空。"
...
async def _(event:Event) -> Result:
    ...
    return info_splicing(info,Manager.BG_path(event.user_id),spacing = 5) if info else "您的仓库空空如也。"
...
async def _(event:Event) -> Result:
    ...
    if not user:
        return
    return info_splicing(alchemy_info(user.alchemy,user.nickname,await event.avatar()).BG_path(user.user_id),5)
...
async def draw_rank(ranklist:list, title:str = "金币", top:int = 20, group_id = None, bg_id = None):
    """
    排名信息
    """
    first = ranklist[0][1]
...
async def _(event:Event):
    ...
    if not group_account:
        return "未关联账户，请发送【关联账户】关联群内账户。"
    title = re.search(r"^(总金币|总资产|金币|资产|财富|胜率|胜场|败场|路灯挂件)(排行|榜)",event.raw_command.strip()).group(1)
...
async def _(event:Event) -> Result:
    title = re.search(r"^(金币|资产|财富|胜率|胜场|败场|路灯挂件)(总排行|总榜)",event.raw_command.strip()).group(1)
