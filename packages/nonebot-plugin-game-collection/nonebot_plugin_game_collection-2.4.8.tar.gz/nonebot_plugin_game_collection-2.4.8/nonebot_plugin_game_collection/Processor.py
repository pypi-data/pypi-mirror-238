...
class Event:
    ...
    @classmethod
    def is_command(cls,command:str,user_id:str,group_id:str) -> List[PreEventData]:
        """
        判断是否为指令
        """
        ...
        if cmd == command_start:
            ...
            args = command_list
        return [(event_name,cmd,args,user_id,group_id,cls.NEED_EXTRA_ARGS.get(event_name)) for event_name in event_name_set]
    ...
    def is_regex(cls,command:str,user_id:str,group_id:str) -> List[PreEventData]:
        ...
        for pattern, event_name_set in cls.REGEX_DICT.items():
            ...
            return []
        return [(event_name,command,[],user_id,group_id,cls.NEED_EXTRA_ARGS.get(event_name)) for event_name in event_name_set]
    ...
    def is_got(cls,command:str,user_id:str,group_id:str) -> List[PreEventData]:
        ...
        if time.time() - event_time > 60:
            ...
            return []
        return [(event_name,command,command.strip().split(),user_id,group_id,cls.NEED_EXTRA_ARGS.get(event_name))]
    ...
    def got(self,need_extra_args:dict = {}):
        ...
        def decorator(function:Coroutine):
            ...
            Event.GOT_ARG.setdefault(self.user_id,{})[self.group_id] = (event_name,time.time())
            if need_extra_args:
    ...
    def whisper(self,need_extra_args:dict = {}):
        ...
        def decorator(function:Coroutine):
            ...
            Event.GOT_ARG.setdefault(self.user_id,{})["private"] = (event_name,time.time())
            if need_extra_args:
                Event.NEED_EXTRA_ARGS[event_name] = need_extra_args            
            async def wrapper(event:Event):
                result = await function(event)
...
async def build_event(data:PreEventData,Adapters:dict,bot,event) -> Event:
    ...
    extra_args = {k:await Adapters[k](bot,event) for k in need_extra_args} if need_extra_args else {}
    return Event(event_name,raw_command,args,user_id,group_id,await Adapters["nickname"](bot,event),extra_args)
...
async def run(data:PreEventData,Adapters:dict,bot,event):
    ...
    try:
        ...
        result = await e.function(**{k:await Adapters[k](bot,event,v) for k,v in e.data.items()})
    return result
...
def reg_auto_event(event_name:str,commands:Union[Set[str],str], need_extra_args:dict = {}):
    """
    自动注册事件
    """
