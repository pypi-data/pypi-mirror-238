...
class WASBase (_WASType):
    ...
    def __init__ (self):
        self.apps: wsgi_apps.ModuleManager
        self.excutors: executors.Executors
        self.async_excutors: executors.AsyncExecutor
        self.thread_excutors: executors.ThreadExecutor
        self.process_excutors: executors.ProcessExecutor
        self.httpserver: http_server.http_server
        self.queue: threadlib.request_queue2
        self.threads: threadlib.thread_pool
        self.logger: triple_logger.Logger
    ...
    # class only methods ---------------------------------
    @classmethod
    def register (cls, name, obj, logging = True):
        if hasattr (cls, name):
            raise AttributeError ("server object `%s` is already exists" % name)
        cls.objects [name] = obj
        setattr (cls, name, obj)
        logging and cls.logger ("server", "context.%s is registered" % (tc.grey (name)), "info")
    ...
    def _clone (self, disposable = False):
        ...
        # cloning
        for attr in ('request', 'app', 'apps', 'subapp', 'response', 'stream', 'mount_options'):
            try:
