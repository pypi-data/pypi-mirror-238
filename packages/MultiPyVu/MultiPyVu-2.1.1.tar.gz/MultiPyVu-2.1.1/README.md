...
```
We also support reading back the temperature of the rotator thermometer. This reads off the calibration table of the resistor on the horizontal rotator that is programed onto the SROM of your BRT module. The rotator requires an extra connector that routes the thermometer to channel 1, channel 2 to be the new channel 1 and channel 3 to be the new channel 2. As such this command will only give a meaningful result with the horizontal rotator installed:
