...
class CommandSdoBase(ICommand):
    ...
    def convert_result(self, response) -> Tuple:
        '''
    ...
    def convert_state_dictionary(self, statusNumber: str) -> str:
        ...
        return str(can_err(can_err_as_int, abort_err_as_int))

    def state_code_dict(self):
        state_dict = {}
        for can_num, abort_num in zip(can_err_enum, abort_err_enum):
            state_dict[can_num, abort_num] = str(can_err(can_num, abort_num))
        return state_dict

    @abstractmethod
    def get_state_server(self, statusCode: str, stateValue: str, params: str = ''):
...
class CommandSdo(CommandSdoBase):
    '''
    ...
    def _binary_to_value(self,
                         sdo: SdoObject,
                         len: win32.VARIANT,
                         val: win32.VARIANT) -> Union[str, int, float]:
        '''
    ...
    def readSdo(self, sdo: SdoObject,) -> Tuple:
        '''
        ...
        if (can_error != can_err_enum.R_OK) or (error_variant.value != 0):
            ...
            # Note that the bug in win32com.client that swapped results
            # does not happened if the VARIANT type is not specified. So
            # this method should be compatible with all versions.
    ...
    def get_state_server(self, value_variant, state_variant, params) -> Tuple:
        ...
        sdo_tuple = self.readSdo(sdo_input)
        return sdo_tuple
...
class CommandSdoSim(CommandSdoBase):
    ...
    def __init__(self):
        ...
        self._int_val = 0
        self._float_val = 0.0
        self._str_val = 'simulated response'
