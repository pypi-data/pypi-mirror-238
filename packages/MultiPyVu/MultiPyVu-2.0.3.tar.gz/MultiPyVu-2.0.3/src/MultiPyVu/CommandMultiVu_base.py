...
class CommandMultiVuBase():
    ...
    def _check_command_name(self, input_command):
        if input_command not in self.cmd_dict:
            raise MultiPyVuException(f'Unknown command: "{input_command}".')
    ...
    def get_state(self, command: str, params: str = '') -> str:
        ...
        try:
            ...
            raise MultiPyVuException(f'Potential threading error:  {e}')
        try:
    ...
    def set_state(self, command, arg_string):
        ...
        try:
            ...
            if err == 0:
                ...
                if command == 'SDO':
                    msg += mv_command.convert_state_dictionary(err)
                else:
                    msg += f'error = {err}'
                return msg
