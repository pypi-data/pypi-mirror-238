...
# # Boolean for last date in month: when trades happen
# year_month = df_weights_ending.index.to_period("M")
# s_last_date_in_month_present = ~year_month.duplicated(keep="last")

# Add scaling factor to weights ending
# s_scaling_factor = (
#     1 + df_attr_returns_pivoted.sum(axis=1)
# ).cumprod() * s_last_date_in_month_present
s_scaling_factor = (1 + df_attr_returns_pivoted.sum(axis=1)).cumprod()
s_scaling_factor.iloc[0] = 1
# s_scaling_factor = s_scaling_factor.replace(0, pd.NA).fillna(method="ffill")
