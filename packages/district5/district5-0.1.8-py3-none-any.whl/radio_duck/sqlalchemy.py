...
class RadioDuckDialect(default.DefaultDialect):
    ...
    def has_index(self, connection, table_name, index_name, schema=None):
        ...
        rows = self._execute_query(connection, has_index_query, schema, table_name, index_name)
        return len(rows) == 1
    ...
    def has_table(self, connection, table_name, schema=None, **kw) -> None:
        ...
        if schema is None or "" == schema.strip():
            schema = RadioDuckDialect.default_schema_name
        rows = self._execute_query(connection, has_table_query, schema, table_name)
        return len(rows) == 1
    ...
    def has_sequence(
        ...
        rows = self._execute_query(connection, has_sequence_query, schema, sequence_name)
        return len(rows) == 1
    ...
    def get_table_names(
        ...
        rows = self._execute_query(connection, get_tables, schema)  # list of list
    ...
    def get_view_names(self, connection, schema=None, **kw):
        if schema is None or "" == schema.strip():
            schema = RadioDuckDialect.default_schema_name
        rows = self._execute_query(connection, get_views, schema)  # list of list
    ...
    def get_unique_constraints(
        ...
        rows = self._execute_query(connection, get_constraints, schema, table_name, "UNIQUE")  # list of list
        list_of_maps = [
    ...
    def get_temp_table_names(self, connection, schema=None, **kw):
        ...
        rows = self._execute_query(connection, get_temp_tables, schema)  # list of list
    ...
    def get_sequence_names(self, connection, schema=None, **kw) -> list[Any]:
        ...
        if schema is None or "" == schema.strip():
            schema = RadioDuckDialect.default_schema_name
        rows = self._execute_query(connection, get_sequences, schema)  # list of list
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        rows = self._execute_query(connection, get_constraints, schema, table_name, "PRIMARY KEY")  # list of list
        if len(rows) == 0:
            return []
        row = rows[0]
        list_of_maps = (
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        rows = self._execute_query(connection, get_indexes, schema, table_name)  # list of list
        list_of_maps = [
    ...
    def get_foreign_keys(self, connection, table_name, schema=None, **kw):
        ...
        rows = self._execute_query(connection, get_constraints, schema, table_name, "FOREIGN KEY")  # list of list
        list_of_maps = [
            {"name": row[0], "column_names": row[1]} for row in rows
        ]
        return list_of_maps
    ...
    def get_columns(self, connection, table_name, schema=None, **kw):
        ...
        rows = self._execute_query(connection, get_columns, schema, table_name)  # list of list
        list_of_maps = [
            {
                "name": row[0],
                "type": db_types.get_alchemy_type(row[1]),
                "nullable": row[2],
                "default": row[3],
                "autoincrement": "",
                "primary_key": None, #todo <-fill this pk
            }
            for row in rows
        ]
    ...
    def get_check_constraints(
        ...
        rows = self._execute_query(connection, get_constraints, schema, table_name, 'CHECK')  # list of list
        list_of_maps = [
    ...
    def _execute_query(
            self, connection: sqlalchemy.engine.base.Connection, query: str, *params, **kw
    ):
        if not params:
            result = connection.execute(query)
        else:
            result = connection.execute(query, params)  # use qmark ? style for params
