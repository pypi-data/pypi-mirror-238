...
class RadioDuckDialect(default.DefaultDialect):
    ...
    def get_schema_names(self, connection, **kw):
        """
        the superset ui asks the dialect to list schemas too
        :param connection:
        :param kw:
        :return list of schema names
        where catalog_name is neither temp not system:
        """
        cursor = None
        try:
            print(type(connection))
            logging.info("connection type is " + type(connection))
            cursor = connection.cursor()
            cursor.execute(get_schemas, parameters=[])
            rows = cursor.fetchall()  # list of list
            schema_names = [col_val for row in rows for col_val in row]
            return schema_names
        finally:
            if cursor is not None:
                cursor.close()
