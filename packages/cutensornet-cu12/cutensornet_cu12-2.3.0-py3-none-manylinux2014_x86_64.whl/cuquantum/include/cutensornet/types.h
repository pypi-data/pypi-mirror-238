...
/**
 * \brief This enum lists attributes for configuring computation of the tensor network state amplitudes.
 */
typedef enum {
    CUTENSORNET_ACCESSOR_OPT_NUM_HYPER_SAMPLES ///< int32_t: Number of hyper-samples used by the tensor network contraction path finder.
...
/**
 * \brief This enum lists attributes for configuring computation of the tensor network state expectation value.
 */
typedef enum {
    CUTENSORNET_EXPECTATION_OPT_NUM_HYPER_SAMPLES ///< int32_t: Number of hyper-samples used by the tensor network contraction path finder.
...
/**
 * \brief This enum lists attributes for configuring computation of the tensor network state marginal tensor. 
 */
typedef enum {
    CUTENSORNET_MARGINAL_OPT_NUM_HYPER_SAMPLES ///< int32_t: Number of hyper-samples used by the tensor network contraction path finder.
...
/**
 * \brief This enum lists attributes for configuring the tensor network state sampler.
 */
typedef enum{
    CUTENSORNET_SAMPLER_OPT_NUM_HYPER_SAMPLES ///< int32_t: Number of hyper-samples used by the tensor network contraction path finder.
...
/**
 * \brief This enum lists all attributes of a ::cutensornetState_t that can be modified.
 */
typedef enum
{
    CUTENSORNET_STATE_MPS_CANONICAL_CENTER = 0, ///< int32_t: The site where canonical center of the target MPS should be placed at. If less than 0 (default -1), no canonical center will be enforced.
    CUTENSORNET_STATE_MPS_SVD_CONFIG_ABS_CUTOFF = 1, ///< double: The absolute cutoff value for SVD truncation (default is 0).
    CUTENSORNET_STATE_MPS_SVD_CONFIG_REL_CUTOFF = 2, ///< double: The cutoff value for SVD truncation relative to the largest singular value (default is 0).
    CUTENSORNET_STATE_MPS_SVD_CONFIG_S_NORMALIZATION = 3, ///< cutensornetTensorSVDNormalization_t: How to normalize singular values after potential truncation. Default is no normalization.
    CUTENSORNET_STATE_MPS_SVD_CONFIG_ALGO = 4, ///< cutensornetTensorSVDAlgo_t: The SVD algorithm (default is `gesvd`).
    CUTENSORNET_STATE_MPS_SVD_CONFIG_ALGO_PARAMS = 5, /// Optional, the parameters specific to the SVD algorithm `cutensornetTensorSVDAlgo_t`, currently supporting cutensornetGesvdjParams_t for CUTENSORNET_TENSOR_SVD_ALGO_GESVDJ and cutensornetGesvdrParams_t for CUTENSORNET_TENSOR_SVD_ALGO_GESVDR.
    CUTENSORNET_STATE_MPS_SVD_CONFIG_DISCARDED_WEIGHT_CUTOFF = 6, ///< double: The maximal cumulative discarded weight (square sum of discarded singular values divided by the square sum of all singular values), defaults to 0. This option is not allowed when CUTENSORNET_TENSOR_SVD_ALGO_GESVDR is chosen.
    CUTENSORNET_STATE_NUM_HYPER_SAMPLES = 7 ///< int32_t: Number of hyper-samples used by the tensor network contraction path finder.
