...
def deploy(client, template_file, comment, release=False, force=False):
    ...
    :param template_file: path to template.yaml file
...
def deploy_build(
    ...
    log.debug("build archive: %s (%s)" % (archive_file, util.b2h(archive_file_size)))

    if archive_file_size > client.site().data().get("max_size", 1e7):
        raise Exception("build archive too large")

    file_data = util.read_zip_file(archive_file)
    if not file_data:
