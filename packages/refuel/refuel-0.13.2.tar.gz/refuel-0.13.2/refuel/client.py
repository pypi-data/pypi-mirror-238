...
class RefuelClient:
    ...
    def deploy_task(
        self, task: str, application_name: str, project: Optional[str] = None
    ):
        if is_valid_uuid(task):
            task_id = task
        else:
        ...
        response = self._post(
            url=url,
            params={"task_id": task_id, "application_name": application_name},
            headers=self._header,
        ).result()
        return response.json().get("data", [])
    ...
    def label(
        ...
        response = self._post(
            url=url,
            params={"application_id": application_id, "items": json.dumps(inputs)},
            headers=self._header,
        ).result()
        if response.status_code != 200:
            logger.error(
