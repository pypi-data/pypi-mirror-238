...
from spatialsorter.utils import lengths_from_xyz, moving
...
class SpatialSorter:
    ...
    def lap_indices(self) -> np.ndarray:
        ...
        a new lap begins
        """
        return detect_laps_by_length(
            np.array(self.sort(self.mls_arc_lengths, inverse=True))
        )
    ...
    def reconstruct(self) -> list:
        ...
        """
        # sort
        self._idx_sort, idx_missing = reconstruct_spatial_sorting(
            xyz=self._mls_unsorted
        )
        ...
        if len(idx_missing) > 0 and not self.discard_missing:
            ...
            for m_i, p in zip(idx_missing, missing_points):
                self._idx_sort, mls_sorted = _insert_point(
                    m_i, p, self._idx_sort, mls_sorted
                )

        # Set start position of lap as the position with the maximum z-value
        self._idx_sort = begin_with(
            idx=self._idx_sort, begin=int(np.argmax(self._mls_unsorted[:, 2]))
        )
    ...
    def sort(self, array: np.ndarray, inverse: bool = False) -> np.ndarray:
        ...
        """
        idx_sort = np.array(
            np.argsort(self.idx_sort) if inverse else self.idx_sort, dtype=int
        )
...
def reconstruct_spatial_sorting(xyz: np.ndarray) -> Tuple[list, list]:
    ...
    shortest_path_lengths = nx.shortest_path_length(
        mst, end_nodes[0], end_nodes[1], weight="weight"
    )
...
def compute_mst(xyz: np.ndarray) -> Tuple[nx.Graph, list]:
    ...
    logging.info(
        "%i points were discarded during delaunay triangulation!", len(missing)
    )
...
def infer_sort_index(sorting: Sorting, tstamps: np.ndarray):
    ...
    return (
        np.argsort(np.argsort(tstamps))
        if sorting == Sorting.SPATIAL
        else np.argsort(tstamps)
    )
