...
def re_encode(df,le_dict):
    """
    Encode categorical columns in a DataFrame using the created label encoders.
    ...
    Examples:
    --------
    >>> encoded_df = re_encode(df,le_dict)
    """
...
def numbers_to_text(df, le_dict): 
    ...
    Examples:
    --------
    >>> decoded_df = numbers_to_text(df,le_dict)
    """
    for i in le_dict.keys(): 
