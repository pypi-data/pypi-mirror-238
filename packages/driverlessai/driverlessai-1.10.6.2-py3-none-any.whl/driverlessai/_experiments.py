...
class Experiment(_commons.ServerJob):
    ...
    @_model_ready
    @_utils.min_supported_dai_version("1.10.0")
    def export_dai_file(
        self,
        dst_dir: str = ".",
    ...
    def fit_and_transform(
        ...
        Args:
            training_dataset: dataset to be used for refitting the
                data transformation pipeline,
            validation_split_fraction: fraction of data used for validation,
            seed: random seed to use to start a random generator,
            fold_column: column to create a stratified validation split,
            test_dataset: dataset to be used for final testing,
            validation_dataset: dataset to be used for tune parameters of models
        """
    ...
    def fit_and_transform_async(
        ...
        Args:
            training_dataset: dataset to be used for refitting
    ...
    def predict(
        ...
        Args:
            dataset: a Dataset object corresponding to a dataset on the
                Driverless AI server or a Pandas DataFrame
    ...
    @_utils.deprecated(
        version="1.10.6.2",
        new_api="",
        custom_message="The `summary()` method is deprecated. `summary` property "
        "will be introduced in 1.10.7 release.",
    )
    def summary(self) -> None:
        """Print experiment summary."""
        if not _commons.is_server_job_complete(self._status()):
            _logging.logger.warning(
                f"Experiment is not complete: {self.status(verbose=2)}"
            )
            return
        summary = (
        ...
        _logging.logger.info(summary.strip())
    ...
    def variable_importance(
        ...
        Args:
            iteration: which iteration of the experiment
        ...
        try:
            iterations = self._client._backend.list_model_iteration_data(
                key=self.key, limit=20, num_var_imp=14, offset=0
            )
            ...
            if iteration >= len(iterations) and model_index >= len(
                iterations[iteration].importances
            ):
                return None

            variable_importance = (
                iterations[iteration - 1].importances[model_index].dump()
            )
            return _utils.Table(
            ...
            _logging.logger.info("Variable importance not available.")
            return None
    ...
    def autodoc(self) -> "_autodoc.AutoDoc":
        ...
        if self._client._backend.get_autoreport_job(self.key).entity.report_path:
            return autodoc
    ...
    def create_async(
        ...
        **kwargs: Any,
        ...
        Keyword Args:
            ...
            drop_columns (List[str]): list of column names to be dropped
            enable_gpus (bool): allow GPU usage in experiment
            reproducible (bool): set experiment to be reproducible. If seed value set
                and reproducible was not set it will be marked as true by the client.
            time_period_in_seconds (int): the length of the time period in seconds,
              used in timeseries problems
            num_prediction_periods (int): timeseries forecast horizon in time
              period units
...
class FitAndTransformationJob(_commons.ServerJob):
    ...
    def _update(self) -> None:
        self._set_raw_info(self._client._backend.get_transformation_job(self.key))
