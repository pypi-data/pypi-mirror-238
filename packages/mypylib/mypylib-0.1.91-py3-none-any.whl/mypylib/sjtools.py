...
class SJ_wrapper:
    def __init__(self,
                 api_key='HKSHhxztmehZunSf83SP7V4zX9dL7dgVUAYs1XPWKwyZ',
                 secret_key='8p1EkVXHjtHY97y9pxNv6UpnxcKECBq3i8sJLsytCQQX',
    ...
    def save_all_contracts(self, path_to_save='all_stocks.pickle'):
        ...
        for c in self.api.Contracts.Stocks.OTC:
            stock = Stock(c.code, c.symbol, c.name, c.limit_up, c.limit_down, c.reference, c.day_trade)
        ...
        with open(f'{rows[0]}.txt', 'w') as fp:
            for stock in dict_all_stocks.values():
                fp.write(f'{stock.code}\t{stock.name}\t{stock.limit_up}\t{stock.limit_down}\t{stock.reference}\t{stock.day_trade}\n')
