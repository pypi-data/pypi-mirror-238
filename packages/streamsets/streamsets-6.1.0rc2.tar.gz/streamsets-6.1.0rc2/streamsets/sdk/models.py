...
class Configuration:
    ...
    This class enables easy access to and modification of data stored as a list of dictionaries. Configurations are
    stored in the form:
    ...
    def __init__(
        ...
        # Account for case where a single configuration is passed instead of a list of configurations
        ...
        self._configuration_index_map = {}
        for configuration_index_in_data, configuration in enumerate(self._data):
            for index, config in enumerate(configuration):
                self._configuration_index_map[config[self.property_key]] = (index, configuration_index_in_data)
    ...
    def items(self):
        ...
        for configuration in self._data:
            for config in configuration:
                configuration_dict[config[self.property_key]] = config[self.property_value]
