...
def get_mongodb_records_gen(database: str,
        ...
        -> Tuple[MongoDBEngine, Generator[pd.DataFrame, None, None]]:
    """
    ...
    if using_dist:
        ...
        return engine, engine.find_distinct_gen(distinct['group'], filter=distinct.get('filter'))
        ...
        return engine, engine.find_many_gen(filter_for_req, projection=projection)
    else:
...
def load_all_recs_with_distinct(database: str,
    ...
    engine, df_gen = get_mongodb_records_gen(database, collection, db_config, distinct=distinct_)
