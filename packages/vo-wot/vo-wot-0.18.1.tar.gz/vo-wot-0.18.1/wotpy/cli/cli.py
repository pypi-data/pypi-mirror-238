...
async def subscribe_remote_events(consumed_vos, consumed_vos_dict, wot, module):
    """Consumes VOs from the given URLs and maps remote events/property changes to
    the corresponding user-defined functions."""
    ...
    for vo_name, vo_dict in consumed_vos_dict.items():
        ...
        for retry in range(max_retries):
            try:
                consumed_vos[vo_name] = await wot.consume_from_url(url)
...
async def run_script(thing_description_path, script_path, config_path):
    ...
    await subscribe_remote_events(consumed_vos, consumed_vos_dict, wot, module)
