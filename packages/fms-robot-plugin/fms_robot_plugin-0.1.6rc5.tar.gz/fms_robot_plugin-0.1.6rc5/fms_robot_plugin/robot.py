...
from fms_robot_plugin.typings import ConnectionStatus, Status, LaserScan, Twist, Pose, Map, RobotInfo, Task
...
class Robot:
    ...
    def on_set_priority(self, cb: Callable[[dict], None]):
        topic = f"/robots/{self.robot_key}/priority"
        self.consumer(topic).consume(lambda data: cb(data["priority"]))
    ...
    def set_connection_status(self, status: ConnectionStatus):
        self.mqtt.publish(
            f"robots/{self.robot_key}/connection",
            {"status": status.value, "sent_at": datetime.datetime.utcnow().isoformat()},
        )
