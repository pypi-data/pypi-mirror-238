...
def dbtpurview(env,dwhcid,azpurview):
   ...
   files = os.listdir("/opt/airflow/git/dbtcoredemo1.git/dags/dbt/airlinedemo")
   files = [f for f in files if os.path.isfile(PROJECT_ROOT_PATH+'/'+f)] #Filtering only the files.
   print(*files, sep="\n")
