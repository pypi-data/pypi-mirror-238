    ...
    if torch.cuda.is_available():
        ...
        if device_capability >= 7:
            ...
            for config_file in ["32k.json", "40k.json", "48k.json"]:
                with open(os.path.join(now_dir, f"configs/{config_file}"), "r") as d:
                    data = json.load(d)

                if "train" in data and "fp16_run" in data["train"]:
                    data["train"]["fp16_run"] = True

                with open(os.path.join(now_dir, f"configs/{config_file}"), "w") as d:
            ...
            for config_file in ["32k.json", "40k.json", "48k.json"]:
                with open(os.path.join(now_dir, f"configs/{config_file}"), "r") as f:
                    data = json.load(f)
