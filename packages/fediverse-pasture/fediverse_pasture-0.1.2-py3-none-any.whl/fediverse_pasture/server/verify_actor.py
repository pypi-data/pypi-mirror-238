...
class ActorVerifier:
    ...
    @property
    def main_actor(self) -> ActorData:
        for actor in self.actor_list:
            if actor.actor_name == "actor":
                return actor

    async def check_webfinger_result(self, session):
        my_actor = self.main_actor
        actor_id = f"http://{self.domain}/{my_actor.actor_name}"
        bovine_actor, _ = bovine_actor_for_actor_data(actor_id, my_actor)
        ...
        if object_id == self.remote_actor_uri:
