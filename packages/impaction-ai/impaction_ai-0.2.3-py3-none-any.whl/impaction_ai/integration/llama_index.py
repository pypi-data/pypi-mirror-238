...
try:
    ...
    if pkg_resources.parse_version(llama_index_version) < pkg_resources.parse_version("0.8.4"):
        raise ImportError("You must install llama_index>=0.8.4 to use ImpactionChatMemory.")
    ...
    raise ImportError("You must install 'llama_index' to use ImpactionChatMemory.")
...
class ImpactionChatMemory(BaseMemory):
    ...
    class Config:  # for custom type ImpactionAI against pydantic validation.
    ...
    def __init__(
        ...
        """Initialize ImpactionChatMemory. When initialized, open_session event is emitted to ImpactionAI. If user_info is provided, identify_user event is also emitted.
        ...
        Args:
            ...
            impaction_client (ImpactionAI): impaction.ai SDK client.
        ...
        super().__init__(
            ...
            impaction_client=impaction_client,
            session_id=session_id,
            user_id=user_id,
            assistant_id=assistant_id,
            user_info=user_info,
        )
        self.impaction_client.open_session(
            session_id=self.session_id, user_id=self.user_id, assistant_id=self.assistant_id
        )
        if self.user_info:
            self.identify_user(
    ...
    def from_defaults(cls, chat_history: list[ChatMessage] | None = None, llm: LLM | None = None):
        """ImpactionChatMemory does not support 'from_defaults' constructor. Please use the constructor directly."""
        raise NotImplementedError(
            "from_defaults is not intended to be used with ImpactionChatMemory. Please use the constructor directly."
        )
    ...
    def put(self, message: ChatMessage) -> None:
        """Put chat history. If the message is user or assistant message, create_message event will be emitted to ImpactionAI.
        ...
        if message.role in [MessageRole.USER, MessageRole.ASSISTANT] and message.content is not None:
            self.impaction_client.create_message(
    ...
    def identify_user(
        ...
        """  # noqa: E501
        self.impaction_client.identify_user(
    ...
    def close(self) -> None:
        ...
        self.impaction_client.close_session(session_id=self.session_id)
