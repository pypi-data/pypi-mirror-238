...
#   ('--member-specs', 'list')
#   ('-f', '')
#   ('-o', 'tests/recursive_generator_method2_sup.py')
#   ('-s', 'tests/recursive_generator_method2_sub.py')
#   ('--export', 'generator')
#   ('--super', 'recursive_generator_method2_sup')
#
# Command line arguments:
#   tests/recursive_generator_method.xsd
#
# Command line:
#   generateDS.py --no-dates --no-versions --silence --member-specs="list" -f -o "tests/recursive_generator_method2_sup.py" -s "tests/recursive_generator_method2_sub.py" --export="generator" --super="recursive_generator_method2_sup" tests/recursive_generator_method.xsd
#
...
class people(GeneratedsSuper):
    ...
    def generateRecursively_(self, level=0):
        ...
        if self.person:
            for o in self.person:
                yield from o.generateRecursively_(level)
        if self.programmer:
            for o in self.programmer:
                yield from o.generateRecursively_(level)
        if self.python_programmer:
            for o in self.python_programmer:
                yield from o.generateRecursively_(level)
        if self.java_programmer:
            for o in self.java_programmer:
                yield from o.generateRecursively_(level)
...
class programmer(person):
    ...
    def generateRecursively_(self, level=0):
        ...
        level += 1
        if self.elparam:
            yield from self.elparam.generateRecursively_(level)
    def build(self, node, gds_collector_=None):
        self.gds_collector_ = gds_collector_
        if SaveElementTreeNode:
            self.gds_elementtree_node_ = node
...
class agent(GeneratedsSuper):
    ...
    def generateRecursively_(self, level=0):
        ...
        level += 1
        if self.info:
            yield from self.info.generateRecursively_(level)
    def build(self, node, gds_collector_=None):
        self.gds_collector_ = gds_collector_
        if SaveElementTreeNode:
            self.gds_elementtree_node_ = node
...
def parseLiteral(inFileName, silence=False, print_warnings=True):
    ...
    if not SaveElementTreeNode:
        ...
        rootNode = None
##     if not silence:
##         sys.stdout.write('#from recursive_generator_method2_sup import *\n\n')
##         sys.stdout.write('import recursive_generator_method2_sup as model_\n\n')
