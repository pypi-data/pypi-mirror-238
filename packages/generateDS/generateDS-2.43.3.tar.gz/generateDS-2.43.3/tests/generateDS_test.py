...
"""
    ...
    def test_048_recursive_generator_method(self):
        cmdTempl = (
            'python3 generateDS.py --no-dates --no-versions '
            '--silence --member-specs=list -f '
            '-o tests/%s2_sup.py -s tests/%s2_sub.py '
    ...
    def test_049_episodic_compilation(self):
        cmdTempl = (
            'python3 generateDS.py --no-dates --no-versions '
            '--silence --member-specs dict -f '
            '--one-file-per-xsd '
            '--use-source-file-as-module-name '
            '--namespace-to-import-path-configuration-file '
            'tests/episodic.json '
        ...
        cmd = cmdTempl % (t_, )
        self.executeClean(cmd)
        self.compareFiles('{}.py'.format(t_), '{}2.py'.format(t_))
        # cleanup generated files
        #self.remove('{}.py'.format(t_))

    def test_050_graphql_server(self):
        cmdTempl = (
            'python3 generateDS.py --no-dates --no-versions '
            '--silence --member-specs=dict -f '
            '--graphql="people:people" '
            '-o tests/%s_graphql2.py '
            'tests/%s.xsd'
        )
        t_ = 'people'
        cmd = cmdTempl % (t_, t_, )
        self.executeClean(cmd)
        self.compareFiles(
            '{}_graphql1.py'.format(t_),
            '{}_graphql2.py'.format(t_),
            ('sys.stdout.write',))
        self.compareFiles('{}_graphql1.py'.format(t_), '{}_graphql2.py'.format(t_))
        # cleanup generated files
        #self.remove('{}2_sup.py'.format(t_))
        #self.remove('{}2_sub.py'.format(t_))

    def test_051_split_join(self):
        cmdTempl = (
            'python3 generateDS.py --no-dates --no-versions '
            '--silence --member-specs=dict -f '
            '-o tests/%s2.py '
        ...
        self.executeClean(cmd)
        cmdTempl = (
            'python3 utils/gds_split_one_to_many.py '
            '--force '
            'tests/%s2.py tests/%s_dir'
        )
        cmd = cmdTempl % (t_, t_, )
        # print('2. debug:', cmd)
        self.executeClean(cmd)
        cmdTempl = (
            'python3 utils/gds_join_many_to_one.py '
            '--force '
            'tests/%s_dir tests/%s2a.py'
        )
        cmd = cmdTempl % (t_, t_, )
        # print('3. debug:', cmd)
        self.executeClean(cmd)
        self.compareFiles('{}1.py'.format(t_), '{}2.py'.format(t_))
        self.compareFiles('{}2.py'.format(t_), '{}2a.py'.format(t_))
        # cleanup generated files
        #self.remove('{}2_sup.py'.format(t_))
        #self.remove('{}2_sub.py'.format(t_))

    def test_052_type_substitution(self):
        cmdTempl = (
            'python3 generateDS.py --no-dates --no-versions '
            '--member-specs=list -f '
            '-o tests/%s2_sup.py -s tests/%s2_sub.py '
