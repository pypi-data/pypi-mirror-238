...
class OnlineFeatureGetter(object):
    def __init__(self, credential, initial_feature_uris=None):
        ...
        self.output_converters = {
            ...
            ColumnType.DATETIME.name: pandas.to_datetime,  # TODO: parse with customized format from customer featureset spec
        }
    ...
    def _populate_feature_map(self, feature_uris: List[str], credential):
        ...
        for feature_uri in feature_uris:
            ...
            if not feature.feature_set_reference.online_store.target:
                raise Exception(
                    f'Feature "{feature_uri}" belongs to a feature store that does not specify an online store connection.'
                )
            ...
            if feature.feature_set_reference.online_store.type == OnlineStoreType.REDIS:
                ...
                raise Exception(
                    f'Feature "{feature_uri}" specifies an online store connection of type "{feature.feature_set_reference.online_store.type}". Only "redis" online stores are currently supported.'
                )
    ...
    def _get_client_online_features(self, redis_client, feature_set, feature_pairs, observation_df):
        ...
        for _, row in observation_df.iterrows():
            ...
            results = redis_client.hmget(redis_key, [feature_name for feature_name, _ in feature_pairs])
