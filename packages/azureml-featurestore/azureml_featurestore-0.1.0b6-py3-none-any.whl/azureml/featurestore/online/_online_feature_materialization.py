...
def materialize_online(feature_set, dataframe_to_store, materialization_version=None):
    ...
    if not feature_set.materialization_settings.online_enabled:
        raise Exception(ONLINE_MATERIALIZATION_DISABLED.format(feature_set.name, feature_set.version))
    ...
    time_to_live_formatted_lua = "'" + str(bool(feature_set.temporal_join_lookback)).lower() + "'"
