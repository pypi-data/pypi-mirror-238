...
if _is_private_preview_enabled():
    ...
    class TransformationExpressionSchema(metaclass=PatchedSchemaMeta):
        ...
        source_column = fields.Str(required=True, allow_none=False)
        aggregation = StringTransformedEnum(
            allowed_values=[e.value for e in AggregationFunction], required=True, allow_none=False
        )
        window = NestedField(DateTimeOffsetSchema, required=True, allow_none=False)
...
class FeatureTransformationSchema(metaclass=PatchedSchemaMeta):
    ...
    def validate_transformation(self, data, **kwargs):
        if data.get("transformation_code") and data.get("transformation_expressions"):
            raise ValidationException(
                ...
                no_personal_data_message="Feature transformation can't have both transformation_code and transformation_expressions",
                error_type=ValidationErrorType.INVALID_VALUE,
                error_category=ErrorCategory.USER_ERROR,
                target=ErrorTarget.FEATURE_SET,
            )
    ...
    def make(self, data, **kwargs):
        ...
        if _is_private_preview_enabled() and "transformation_expressions" in data:
    ...
    def dump(self, data, **kwargs):
        ...
        if isinstance(data, TransformationCode):
            return {"transformation_code": TransformationCodeSchema().dump(data)}
        ...
        if _is_private_preview_enabled():
            ...
            if isinstance(data, TransformationExpressionCollection):
                return {
                    "transformation_expressions": TransformationExpressionSchema(many=True).dump(
