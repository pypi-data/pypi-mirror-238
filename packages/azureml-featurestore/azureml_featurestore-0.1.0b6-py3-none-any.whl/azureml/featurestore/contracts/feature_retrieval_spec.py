...
class FeatureRetrievalSpec(object):
    ...
    def from_config(spec_path: Union[str, PathLike]):
        ...
        try:
            ...
            package_logger.error(f"{PACKAGE_NAME}->FeatureRetrievalSpec.FromConfig, {type(ex).__name__}: {ex}")
    ...
    def serialize_to_file(cls, feature_store_client: "FeatureStoreClient", path: str, features: List[Feature]):
        ...
        credentials = feature_store_client._credential
        ...
        for feature_store_uri, feature_refs in feature_stores.items():
            ...
            feature_store_ref = FeatureStoreReference(
                uri=feature_store_uri, location=fs.location, workspace_id=fs._workspace_id, features=feature_refs
            )
    ...
    def dump(self, dest: Union[str, PathLike], **kwargs) -> None:
        ...
        Destination path must be a folder path, the spec file name is assumed as feature_retrieval_spec.yaml, and an exception is raised if the file exists.
