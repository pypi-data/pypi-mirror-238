...
class FeatureSetSpec:
        ...
        **kwargs,
        ...
        with log_activity(_get_logger(), f"{PACKAGE_NAME}->FeatureSetSpec.Init", ActivityType.PUBLICAPI):
            ...
            self.feature_transformation = (
                feature_transformation if feature_transformation else feature_transformation_code
            )
            ...
            if _is_private_preview_enabled():
                from azureml.featurestore.transformation.transformation_expression_collection import (
                    TransformationExpressionCollection,
                )

                if isinstance(self.feature_transformation, TransformationExpressionCollection):
                    if self.source_lookback and self.source_lookback.to_timedelta() != timedelta(0):
                        msg = "Dsl implicitly handles source lookback period. Source lookback setting is not allowed."
                    ...
                    if self.temporal_join_lookback and self.temporal_join_lookback.to_timedelta() != timedelta(0):
                        msg = "Dsl does not support temporal join lookback settings. Point-in-time join lookback does not have upper time bound."
                        raise ValidationException(
                            message=msg,
                            no_personal_data_message=msg,
                            error_type=ValidationErrorType.INVALID_VALUE,
                    ...
                    if self.source.source_delay and self.source.source_delay.to_timedelta() != timedelta(0):
                        msg = "Dsl does not support source delay settings."
                        raise ValidationException(
                            message=msg,
                            no_personal_data_message=msg,
                            error_type=ValidationErrorType.INVALID_VALUE,
    ...
    @monitor_with_activity(_get_logger(), f"{PACKAGE_NAME}->FeatureSetSpec.Dump", ActivityType.PUBLICAPI)
    ...
    def to_spark_dataframe(
        ...
        **kwargs,
        ...
        try:
            ...
            if _is_private_preview_enabled() and isinstance(
                self.feature_transformation, TransformationExpressionCollection
            ):
    ...
    """
            ...
            feathr_ops_string = feathr_ops_tm.render(feature_names=feature_names, end_time=str(end_time))  # noqa: F841
    ...
    try:
        ...
        if infer_schema:
            if _is_private_preview_enabled():
                ...
                # TODO: Add schema inference logic for DSL
