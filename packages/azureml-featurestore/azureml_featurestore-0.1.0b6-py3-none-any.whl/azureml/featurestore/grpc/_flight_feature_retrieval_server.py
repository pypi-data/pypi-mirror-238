...
def _get_logger():
    '''Get the logger
    Calling this function anywhere in the file will get the same logger 
    '''
...
class FlightFeatureRetrievalServer(pyarrow.flight.FlightServerBase):
    ...
    def do_exchange(self, context, descriptor, reader, writer):
        ...
        try: 
            ...
            self.log_queue.put(
                (features_df.num_rows, features_df.num_columns, features_df.nbytes)
            )
            

def _log_success(logger, num_rows, num_cols, num_bytes):
...
def sentinel(server):
    ...
    with log_activity(
            _get_logger(),
            f"{PACKAGE_NAME}->FlightFeatureRetrievalServer->Shutdown",
            ActivityType.INTERNALCALL
        ):
        pass
...
def log_worker(server):
    while True:
        log = server.log_queue.get() # Wait here until an item is available; Since it is a blocking operation, no try-catch block is needed.
        if log is None:
            break
        num_rows, num_cols, num_bytes = log
        _log_success(server.logger, num_rows, num_cols, num_bytes)
