...
from llama.program.util.run_ai import get_url_and_key, get_ui_url, get_model_config
...
class Lamini:
    ...
    def upload_file(self, filename):
        url = self.api_prefix + "upload_file"
        # TODO[large_file]: upload file to azure here.
        # get path from azure
        self.azure_file_path = "random_path"
        req_data = {"azure_file_path": self.azure_file_path}
        return self.make_web_request(url, "post", req_data)
    ...
    def train_async(
        ...
        if data is not None:
            req_data["data"] = data
        if self.azure_file_path is not None:
            req_data["azure_file_path"] = self.azure_file_path
