...
from .parallel_processor_utils import api_endpoint_from_url, num_tokens_consumed_from_request, append_to_jsonl, task_id_generator_function
...
async def process_api_requests_from_session(
    ...
    requests = session._api.openai[request_type][job_id]["requests"].__iter__()
    ...
    while True:
        ...
        if next_request is None:
            if not queue_of_requests_to_retry.empty():
                ...
                try:
                    # get new request
                    request_json = OmegaConf.to_container(next(requests), resolve=True)
