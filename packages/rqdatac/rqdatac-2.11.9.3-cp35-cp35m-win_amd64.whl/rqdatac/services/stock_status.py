...
def get_shares(order_book_ids, start_date=None, end_date=None, fields=None, expect_df=True, market="cn"):
    ...
    """
    def _is_listing(one_row):
        date, order_book_id = one_row.name
        de_listed_date = instruments(order_book_id).de_listed_date
        return de_listed_date == '0000-00-00' or str(date.date()) < de_listed_date
    ...
    if not is_panel_removed and not expect_df:
        ...
        df = df[df.apply(_is_listing, axis=1)]
        if df.empty:
            return None
