...
def get_indicator(order_book_ids, fiscal_year, fields=None, market='cn'):
    """
    获取一致预期数据
    ...
    data = get_client().execute('consensus.get_indicator', order_book_ids, fiscal_year, fields, market=market)
    if not data:
        return None
...
@export_as_api(namespace='consensus')
def get_comp_indicators(order_book_ids, start_date=None, end_date=None, fields=None, market='cn'):
    """
    获取个股一致预期表
    ...
    data = get_client().execute('consensus.get_comp_indicators', order_book_ids, start_date, end_date, fields, market=market)
    if not data:
        return None
    df = pd.DataFrame(data)
    df.set_index(['order_book_id', 'date'], inplace=True)
...
def get_expect_appr_exceed(
    ...
    :param start_date: 开始日期， date-like object, 默认三月前那天
    :param end_date: 结束日期， date-like object， 默认当天
    :param report_year: int, 报告年度，默认当年
