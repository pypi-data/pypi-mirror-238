...
async def on_message(message):
    ...
    if message.content.startswith('!lollms'):
        ...
        docs = '\n'.join(docs)
        context = f'!#>instruction:{lollms_app.personality.personality_conditioning}!#>Documentation:{docs}!#>{message.author.id}:{prompt}\n:!#>{lollms_app.personality.ai_message_prefix}:'
        out = ['']
        ...
        lollms_app.safe_generate(context, callback=callback)
