...
from mfire.localisation.core_spatial import LocalisationError, get_n_area
...
class SpatialIngredient:
    ...
    def load(cls, basename: Path):
        ...
        geodict: dict = JsonFile(str(basename.with_suffix(".json"))).load()
    ...
    def auto_save(self, basename: Path):
        ...
        JsonFile(str(basename.with_suffix(".json"))).dump(geodict)
...
class SpatialLocalisation:
    ...
    def localise(self, risk_level: int, risk_period: set) -> SpatialIngredient:
        """
        This functionality enable to localise a risk.
