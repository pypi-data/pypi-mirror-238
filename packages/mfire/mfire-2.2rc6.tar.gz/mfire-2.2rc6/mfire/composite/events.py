...
from mfire.utils.xr_utils import ArrayLoader, Loader, mask_set_up, sum_with_nan
...
class EventComposite(BaseComposite):
    ...
    @property
    def mask_da(self) -> Optional[xr.DataArray]:
        return self._mask_da

    @property
    def _cached_attrs(self) -> dict:
        return {"data": ArrayLoader, "values_ds": Loader, "mask_da": ArrayLoader}
    ...
    def init_fields(self) -> Tuple[xr.DataArray, xr.DataArray]:
        ...
        self._representative_field_da = field_da
    ...
    def compute_plain_and_mountain(
        ...
        if mountain is not None:
            ...
            ).where(mountain_field.notnull())
            risk_field = sum_with_nan(plain_risk, mountain_risk)
        ...
        if "representative" in self.compute_list:
            ...
            if mountain is not None:
                ...
                rep_value = self.get_representative_values(
                    field,
                    comparison_op=mountain.comparison_op,
                    original_unit=original_unit,
                    threshold=mountain.threshold,
                )
                if rep_value is not None:
        ...
        if aggregation is not None and not aggregation.method.is_after_threshold:
            ...
            if mountain is not None:
                ...
                risk = sum_with_nan(risk, mountain_risk)
        else:
