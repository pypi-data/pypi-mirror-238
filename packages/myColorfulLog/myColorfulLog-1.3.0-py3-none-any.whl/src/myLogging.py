...
class Logger:
    ...
    def __init__(self, log_path=None, time_type='hh'):
        if log_path is None:
        ...
        err_path = os.path.join(log_path, 'errors')
        if not os.path.exists(err_path): os.mkdir(err_path)
        if time_type == 'dd':
            self.__now_time = datetime.now().strftime('%Y-%m-%d')  # 当前日期格式化
        elif time_type == 'hh':
            self.__now_time = datetime.now().strftime('%Y-%m-%d-%H:00')  # 当前日期格式化
        elif time_type == 'mm':
            self.__now_time = datetime.now().strftime('%Y-%m-%d-%H:%M')  # 当前日期格式化
        else:
            raise('undefined Logging time type!')
        self.__all_log_path = os.path.join(log_path, f"{self.__now_time}-all.log")
        self.__error_log_path = os.path.join(err_path, f"{self.__now_time}-error.log")
    ...
    def __init_logger_handler(log_path):
        ...
        return RotatingFileHandler(filename=log_path, maxBytes=1 * 1024 * 1024, backupCount=3, encoding='utf-8')
