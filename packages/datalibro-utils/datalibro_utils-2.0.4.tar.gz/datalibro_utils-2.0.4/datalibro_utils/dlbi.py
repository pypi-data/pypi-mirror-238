...
def get_time_size_selection():
    return st.selectbox('**Time Size**',['Day', 'Week', 'Month','Season', 'Year'])
...
def add_time_cols(df, date_col):
    ...
    df['Week'] = df['Day'].dt.strftime('%Y').astype(str) + 'W' + (df['Day'].dt.strftime('%W').astype(int)+1).astype(str).apply(lambda x: x.zfill(2))
    df['Season'] = df['Day'].apply(get_quarter_string)
...
def side_control(df, date_col, view_size_cols):
    with st.sidebar:
        data_labels = get_data_labels()
        view_size_selection = get_view_size_selection(view_size_cols)
        time_size_selection = get_time_size_selection()
        st.divider()
        date_range = time_range_filter(df[date_col])
...
def stacked_bar(df, x_col, y_col, color, data_labels=True, title=False):
    ...
    df_plot = df.groupby([color, x_col]).sum()[y_col].reset_index()
    ...
    bar_chart.update_layout(title=title, xaxis_title=x_title, yaxis_title=y_title, width=1000, height=460)
    ...
    if data_labels:
        bar_chart.update_layout(annotations=[
            ...
            for x, total in df_label.values
        ])
    # Stacked plot
