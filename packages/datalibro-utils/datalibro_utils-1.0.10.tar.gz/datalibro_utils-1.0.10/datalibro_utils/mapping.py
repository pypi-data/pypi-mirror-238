...
def amz_merge(raw_df, merge_col=['all'], date_col=None, country_col=None, store_id_col=None, asin_col=None, msku_col=None, sku_col=None, scu_col=None, end_date=None):
    df = raw_df.copy()
    # 读取飞书映射表
    feishu_sheet = ('NPeWsHetvhIV7ft6QfBcEEq5nqb', 'WCwfoy!A:R')
    max_retries = 5
    delay = 3
    ...
    if country_col:
        ...
        if datalibro_amazon_mapping[country_col].apply(is_contains_chinese).any():
    ...
    datalibro_amazon_mapping.drop_duplicates(subset=key, keep='last')
    if date_col:
    ...
    # 创建信息映射键
    if not date_col:
        print(f"date column is missing for mapping")
    ...
    for i, arg in enumerate(args):
        if arg is None:
            print(f"{args_names[i]} column is missing for mapping")
        else:
            df['left_key'] += df[arg]
            ...
            for row1 in range(len(datalibro_amazon_mapping_changed)):
                for row2 in range(len(df)):
                    if (datalibro_amazon_mapping_changed.loc[row1, 'right_key'] == df.loc[row2, 'left_key']) & (df.loc[row2, date_col] > datalibro_amazon_mapping_changed.loc[row1,"change_date"]):
                        df.loc[row2, "sku"] = datalibro_amazon_mapping_changed.loc[row1,"sku"]
    else:
    ...
    # 检验空缺映射列
    def check_empty(df, map_col, key_col):
        ...
        if len(null_data) != 0:
            # 输出包含空值的行数
            print(f'存在空值行数: {len(null_data)}')
            if key_col:
                null_data = null_data[key_col+map_col].drop_duplicates()
            # 输出具有空值的行和其他列的内容
            return(null_data) 
    if check_empty(df, merge_col, key_col = key) is not None:
        print(check_empty(df, merge_col, key_col = key))
    else:
